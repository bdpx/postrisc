...
void EmulatorConfiguration::SetDefaults(void)
{
    ...
    memory_devices.back().mappings.back().address = U64C(0x0000065432100000);
    memory_devices.back().mappings.back().size    = 0x400000;
    memory_devices.back().mappings.back().offset  = 0x0;

    rom_devices.emplace_back();
    rom_devices.back().phys_address = U64C(0x0000000fcc000000);
    rom_devices.back().virt_address = U64C(0xffffffffcc000000);
    rom_devices.back().data = {{ 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18 }};

    video_device.phys_address = U64C(0x0000000fb0000000);
    video_device.virt_address = U64C(0xffffffffb0000000);
    video_device.width = 240;
    video_device.height = 160;

    device_array_config.phys_address = U64C(0x0000000fee000000);
    device_array_config.virt_address = U64C(0xffffffffee000000);
...
#undef DEFINE_PERFORMANCE_MONITOR_DEF
    ...
    assembler.RegisterDefine("DEVICE_CONFIG_VIRT_BASE", device_array_config.virt_address);
...
bool EmulatorConfiguration::parse(char const *filename)
{
    ...
    for (const auto& device : config.get_child("devices")) {
        ...
        if (device.first == "core") {
            ...
            u64 const memory_size = device.second.get<util::hexadecimal<u64>>("size");
            ...
            for (const auto& param : device.second) {
                if (param.first == "mapping") {
                    ...
                    u64 const mapping_address = mapping.get<util::hexadecimal<u64>>("address");
                    ...
                    u64 const mapping_size = mapping.get<util::hexadecimal<u64>>("size");
                    LOG_DEBUG(LOG_PLATFORM) << "mapping_size=" << fmt::hex(mapping_size);
                    u64 const mapping_offset = mapping.get<util::hexadecimal<u64>>("offset");
                    LOG_DEBUG(LOG_PLATFORM) << "mapping_offset=" << fmt::hex(mapping_offset);
