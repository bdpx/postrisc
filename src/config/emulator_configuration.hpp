...
public:
    ...
    unsigned get_config_flags(void) const { return config_flags; }

private:
    void Clear(void);

public:
    unsigned   bits_per_physical_address = 44;
    unsigned   config_flags = 0;
    unsigned   bitsPerPageOffset = 12;    // vmem page size 2^bitsPerPageOffset
    unsigned   numberOfPagingLevels = 3;  // top-down page table levels
    u64        base_address = 0;
