...
#define DECLARE_COMMAND_LINE_PARAMETERS_TABLE(X) \
    ...
    X("html",                0, html,                 "generate ISA html description: >gen.html") \
    X("llvm",                0, llvm,                 "generate LLVM TableGen ISA definition: >PostriscInstrEncoding.td") \
    ...
    X("timing_info",         0, timing_info,          "report timing info") \
...
void Configuration::SetDefaults(void)
{
    ...
    m_memory_devices.emplace_back(CPhysAddress(U64C(0x0000005432100000)), 0x400000);

    m_rom_devices.emplace_back();
    m_rom_devices.back().phys_address = CPhysAddress(U64C(0x0000000fcc000000));
    m_rom_devices.back().virt_address = CVirtAddress(U64C(0xffffffffcc000000));
    ...
    m_video_device.phys_address = CPhysAddress(U64C(0x0000000fb0000000));
    m_video_device.virt_address = CVirtAddress(U64C(0xffffffffb0000000));
    m_video_device.width = 240;
    m_video_device.height = 160;

    m_device_array_config.phys_address = CPhysAddress(U64C(0x0000000fee000000));
    m_device_array_config.virt_address = CVirtAddress(U64C(0xffffffffee000000));
...
void
Configuration::ExportDefinitions(CAssembler& assembler) const
{
    ...
    for (int i = 1; i < argc && argv[i] != nullptr; /*nothing here*/) {
        ...
        switch (ConfigurationCommands[k].command) {
            ...
            case command_device_array: {
                ...
                m_device_array_config.phys_address = CPhysAddress(phys_addr);
                m_device_array_config.virt_address = CVirtAddress(virt_addr);
                m_device_array_config.max_config_space_size = item_size;
                } break;
            case command_generate:
                m_command = ConfigurationCommands[k].command;
                result = extract(argv[i+1], m_codesize)
                      && extract(argv[i+2], m_rodatasize)
                      && extract(argv[i+3], m_datasize);
                break;
            ...
            case command_memory:
                ...
                if (result) {
                    m_memory_devices.emplace_back(CPhysAddress(phys_addr), size);
                }
                break;
            case command_exeapp:
                m_command = ConfigurationCommands[k].command;
            ...
            case command_rom:
                ...
                if (result) {
                    ...
                    rom_device.phys_address = CPhysAddress(phys_addr);
                    rom_device.virt_address = CVirtAddress(virt_addr);
                    rom_device.data.swap(data);
                }
                break;
            case command_timing_info:
                LOG_DEBUG(LOG_PLATFORM) << "timing_info";
            ...
            case command_video:
                ...
                m_video_device.phys_address = CPhysAddress(phys_addr);
                m_video_device.virt_address = CVirtAddress(virt_addr);
                break;
...
int Configuration::do_generate(void)
{
    ...
    return generator.AssembleGenerate(std::cout,
            U64C(1024) * m_codesize,
            U64C(1024) * m_rodatasize,
            U64C(1024) * m_datasize);
}
...
int Configuration::do_execute(void)
{
    ...
    CStatus status = targetSystem.StartEmulation(*this, ifs, std::cout);
...
int Configuration::do_execute_app(IDoomInterface& doom, int argc, char**argv)
{
    Decoder decoder;
...
int Configuration::do_run(IDoomInterface& doom, int argc, char **argv)
{
    int ret = 0;
    switch (m_command) {
...
bool Configuration::check(void)
{
    ...
    TranslationUnit mmu;
    mmu.SetPagingParameters(CPhysAddress(0), m_bitsPerPageOffset, m_numberOfPagingLevels);
    CHECK_CANONICAL_ADDRESS(m_base_address);
    CHECK_ADDRESS_ALIGNMENT(m_base_address, page_size);

    if (m_memory_devices.empty()) {
        m_memory_devices.emplace_back(CPhysAddress(U64C(0x0000001234500000)), 0x30000);
