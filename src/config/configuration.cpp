...
int Configuration::do_execute_app(IDoomInterface& doom, int argc, char**argv, char**env)
{
    Decoder decoder;
    TargetSingleApp target(decoder, doom);
    emulator_config.base_address = 0; // 0x02000000;
...
int Configuration::do_run(IDoomInterface& doom, int argc, char **argv, char **env)
{
    switch (command) {
        case command_version:            return do_version();
        case command_scan:               return do_tokenize();
        case command_scan_html:          return do_tokenize_html();
        case command_assemble:           return do_assemble();
        case command_disasm:             return do_disassemble();
        case command_dumpbin:            return do_dumpbin();
        case command_generate:           return do_generate();
        case command_execute:            return do_execute();
        case command_exeapp:             return do_execute_app(doom, argc, argv, env);
        case command_html:               return do_export_html();
        case command_llvm:               return do_export_llvm();
        case command_export_definitions: return do_export_definitions();
        case command_help:               return usage(std::cout);
        default:
