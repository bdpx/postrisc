...
X512(unsigned sign, int exponent)
{
     return ( uint64_t(sign) << 63 )
        | ( ( uint64_t(exponent) + f512::BIAS ) << f512::BITS_PER_HIGH_SIGNIFICAND );
}

static const uint64_t f512_nonfraction[] =
{
    X512(0, - (1 << 22) + 1),
...
u512
CSequence::u512_p1(uint16_t index)
{
    u512 value;
    const uint32_t nbits = 512 - (f512::BITS_PER_EXPONENT + 1);

    int bitnum = (index >> 2) % (nbits + 1);

           if (bitnum < 1*64) {   value = u512(0,0,0,0,0,0,0, uint64_t(1) << (bitnum - 0*64) );
    } else if (bitnum < 2*64) {   value = u512(0,0,0,0,0,0, uint64_t(1) << (bitnum - 1*64), 0);
    } else if (bitnum < 3*64) {   value = u512(0,0,0,0,0, uint64_t(1) << (bitnum - 2*64), 0,0);
    } else if (bitnum < 4*64) {   value = u512(0,0,0,0, uint64_t(1) << (bitnum - 3*64), 0,0,0);
    } else if (bitnum < 5*64) {   value = u512(0,0,0, uint64_t(1) << (bitnum - 4*64), 0,0,0,0);
    } else if (bitnum < 6*64) {   value = u512(0,0, uint64_t(1) << (bitnum - 5*64), 0,0,0,0,0);
    } else if (bitnum < 7*64) {   value = u512(0, uint64_t(1) << (bitnum - 6*64), 0,0,0,0,0,0);
    } else {                      value = u512( uint64_t(1) << (bitnum - 7*64), 0,0,0,0,0,0,0);
    }
...
CSequence::NextP1(f512 & z)
{
    ...
    z.m0 = float_p1[ sigNum ].m0 | f512_nonfraction[ expNum ];
    z.m1 = float_p1[ sigNum ].m1;
    z.m2 = float_p1[ sigNum ].m2;
    z.m3 = float_p1[ sigNum ].m3;
    z.m4 = float_p1[ sigNum ].m4;
    z.m5 = float_p1[ sigNum ].m5;
    z.m6 = float_p1[ sigNum ].m6;
    z.m7 = float_p1[ sigNum ].m7;
...
template<>
uint64_t
CSequence::WeightExponent<f512>(void)
{
    static const struct {
        uint32_t  mask;
        uint32_t  offset;
...
CSequence::Random(f512 & z)
{
    ...
    switch ( random8() & 7 ) {
        ...
        case 6:
            ...
            fraction += u512_p1();
            z = fraction;
            z.m0 = make_sign<uint64_t>() | WeightExponent<f512>() | lobits( z.m0, z.BITS_PER_HIGH_SIGNIFICAND);
            break;
        case 7:
            z.m7 = random64();
            z.m6 = random64();
            ...
            z.m2 = random64();
            z.m1 = random64();
            z.m0 = make_sign<uint64_t>() | WeightExponent<f512>() | lobits( random64(), z.BITS_PER_HIGH_SIGNIFICAND);
            break;
    }
}

size_t
CSequence::GetTotal(int level, f512 &)
{
    return level == 1 ? 3 * f512_num_nonfractionP1 : 2 * f512_num_nonfractionP2;
}

size_t
CSequence::GetTotal(int level, f512 &, f512 &)
{
     return level == 1 ? UINT64_C(6) * f512_num_nonfractionP1 * f512_num_nonfractionP1 :
                         UINT64_C(2) * f512_num_nonfractionP2 * f512_num_nonfractionP2;
}
