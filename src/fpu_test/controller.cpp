...
static const struct {
    ...
    uint8_t   numInputs                      : 2;
    uint8_t   supported                      : 2;
    uint8_t   roundingPrecision              : 1;
    uint8_t   roundingMode                   : 1;
    uint8_t   tininessMode                   : 1;
    uint8_t   tininessModeAtReducedPrecision : 1;
    uint16_t  types;
...
uint8_t CTestController::stop = 0;

void
CTestController::catchSIGINT(int UNUSED(signalCode))
{
...
CTestController::CTestController(void)
{
    set_defaults();
}
...
tininess_mode_t
CTestController::GetHardwareTininessMode(void)
{
...
void
CTestController::set_defaults(void)
{
    level         = 1;
    errorStop     = false;
    forever       = false;
...
void
CTestController::setLevel( int levelIn )
{
    if (    (levelIn != 1)
         && (levelIn != 2)
       ) {
...
void
CTestController::init_sequencer(void)
{
    sequenceA.init(1);
    sequenceB.init(2);
    sequenceC.init(3);
...
void
CTestController::writeTestsTotal( void )
{
    if ( forever ) {
...
void
CTestController::writeFunctionName(std::ostream & out)
{
    out << functionName;
...
void
CTestController::writeTestsPerformed(uint64_t count)
{
    if ( is_cout_tty )
       std::cout << '\r';
...
void
CTestController::checkEarlyExit(uint64_t& count)
{
...
template<>
void
CTestController::next<int32_t>(int32_t & a)
{
...
template<>
void
CTestController::next<int64_t>(int64_t & a)
{
    switch(CASE(level, subcase)) {
        case CASE(1,0):
        case CASE(2,0):
...
void
CTestController::writeErrorFound(std::ostream & out)
{
    out << "\nErrors found in ";
...
void
CTestController::setup_names(CTestContext & context)
{
...
void
CTestController::testingFunction(
    int                  functionCode,
...
int
CTestController::write_help(std::ostream & out, const char progName[])
{
    out <<
...
int
CTestController::write_list(std::ostream & out)
{
...
void
CTestController::abort_testing(void)
{
...
int
CTestController::ParseCommandLine(int argc, char **argv)
{
    ...
    for ( ; argc; --argc,  ++argv ) {
     ...
     optionError:
        ...
        else if ( streq( arg, "int32_t"        ) ) {  functionArgument = true;  functionCode = NOT_SET;  typesTested = type_int32_t;  }
        else if ( streq( arg, "int64_t"        ) ) {  functionArgument = true;  functionCode = NOT_SET;  typesTested = type_int64_t;  }
...
#if 0
int
CTestController::run(int argc, char **argv)
{
