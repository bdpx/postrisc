...
X32(unsigned sign, int exponent)
{
    return ( uint32_t(sign) << 31 )
       | ( ( uint32_t(exponent) + f32::BIAS ) << f32::BITS_PER_SIGNIFICAND );
}

static const uint32_t f32_nonfraction[] =
{
    X32(0,-127), //subnormal
...
void
CSequence::NextP1(f32 & z)
{
    static const uint32_t f32P1[] = {
        0x00000000,
        0x00000001,
        0x007FFFFF,
        0x007FFFFE
    ...
    z = f32( f32_nonfraction[ expNum ] | f32P1[ sigNum ] );
    ++sigNum;
...
void
CSequence::NextP2(f32 & z)
{
    unsigned sigNum = this->term1Num;
    z = f32( f32_nonfraction[ expNum ] | u32_p1(z.BITS_PER_SIGNIFICAND, sigNum) );
    ++sigNum;
    if ( f32NumP2 <= sigNum ) {
        sigNum = 0;
        ++expNum;
...
template<>
uint64_t
CSequence::WeightExponent<f32>(void)
{
    static const struct {
        uint32_t  mask;
        uint32_t  offset;
    ...
    uint32_t exponent =  uint32_t( random16() & 0x1FF ) << f32::BITS_PER_SIGNIFICAND;
...
void
CSequence::Random(f32 & z)
{
    uint32_t nonfraction, sign, fraction, exponent;
    ...
    switch ( random8() & 7 ) {
        ...
        case 5:
        case 6:
            sign     =  make_sign<uint32_t>();
            fraction =  lobits( u32_p1(z.BITS_PER_SIGNIFICAND) + u32_p1(z.BITS_PER_SIGNIFICAND), z.BITS_PER_SIGNIFICAND);
            exponent =  WeightExponent<f32>();
            z = f32( sign | exponent | fraction );
            break;
        case 7:
            sign     =  make_sign<uint32_t>();
            fraction =  lobits(random32(), z.BITS_PER_SIGNIFICAND);
...
size_t
CSequence::GetTotal(int level, f32 &)
{
    return level == 1 ? 3 * f32_num_nonfractionP1 : 2 * f32_num_nonfractionP2;
}

size_t
CSequence::GetTotal(int level, f32 &, f32 &)
{
