...
{
#define X(name, code, extname, format, descr) { descr },
...
const char *ChapterTitle(Chapter chapter)
{
    switch (chapter) {
        case chap_registers:    return "Register files";
        case chap_fields:       return "Instruction fields/arguments";
        case chap_formats:      return "Instruction formats";
        case chap_syntax:       return "Instruction mnemonics and syntax";
        case chap_instructions: return "Instruction encoding";
        case chap_opcodes:      return "Opcode groups";
        case chap_statistic:    return "Instruction set statistic";
        case chap_last: break;
    }
...
static void WriteColorMap(std::ostream& out)
{
    ...
    for (size_t i = 0; i < hues.size(); i++) {
        hues[i] = ((i * 360) / hues.size()) % 360;
    }

    out << "<style type=\"text/css\">\n";
    for (size_t i = 0; i < sizearray(instr_subset_name); i++) {
        out << "td." << instr_subset_name[i]
            << " {text-align:center; color:rgb(40,10,40); background-color:hsl(" << hues[i] << ",98%,80%); }\n";
    }
    out << "</style>\n";
}

static void chaptertitle(std::ostream& out, Chapter chapter_id)
{
    out << "<h2 id=\"chapter_" << static_cast<u32>(chapter_id) << "\">" << ChapterTitle(chapter_id) << "</h2>\n";
}
...
static void special_registerfile(std::ostream& out, int sc,
    const Decoder::regname_array &names, const char descr[])
{
    const int size = 1 << bits_per_register;
...
size_t codetable(std::ostream& out, const TABLE& names, const char *anchor, const char *descr)
{
    size_t imp = 0, maxint = 0;
    const size_t row_size = 8;
    // const int nbits = util::ilog2(names.size());

    const size_t mask = std::min(names.size(), row_size) - 1;
    ...
    out <<  "<div id=\"" << anchor << "_table\"></div>\n"
            "<table width=\"100%\">\n"
            "<caption>" << descr << " (" << imp << " from " << names.size() << ")</caption>\n";
    ...
    for (size_t i = 0; i < names.size(); i++) {
        ...
        if (mnem != insn_illegal) {
           const EInstructionSubset subset = asm_info[mnem].subset;
           out << "<td class=\"" << instr_subset_name[subset] << "\">";
           out << "<a href=\"#" << mnem;
           if (subset == instr_subset_group)
              out << "_table";
           out << "\">" << mnem << "</a><br/>";
        } else {
           out << "<td class=\"res\">reserved<br/>";
        }
        out << i << "</td>\n"; //<< ' ' << fmt::bin(i, nbits, false)
        if ((i & mask) == mask)
           out << "</tr>\n";
    }
...
int Decoder::GenerateHtml(std::ostream& out)
{
    ...
    // content
    out << "<ul>\n";
    for (size_t i = 0; i < chap_last; i++) {
        out << "<li class=\"link\"><a href=\"#chapter_" << i << "\">" << ChapterTitle(static_cast<Chapter>(i)) << "</a></li>\n";
    }
    out << "</ul>\n";
    ...
    ****************************************************************************/
    ...
    chaptertitle(out, chap_registers);
    registerfile(out, inst_fieldtype_gpr, Decoder::gpr_names, "128-bit general purpose registers");
    special_registerfile(out, inst_fieldtype_spr, Decoder::spr_names, "64/128-bit special purpose registers");

    /****************************************************************************
    * instructions fields/arguments
    ****************************************************************************/
    chaptertitle(out, chap_fields);
    ...
    ****************************************************************************/
    ...
    const size_t name_width = 16; // percents

    chaptertitle(out, chap_formats);
    ...
    ****************************************************************************/
    ...
    chaptertitle(out, chap_instructions);
    out << "<table width=\"" << table_width << "%\">\n"
           "<tr>\n"
    ...
    for (const auto & insn : asm_names) {
        ...
        const EInstructionFormat hw_format = asm_info[inst].hw_format;
        const EInstructionOpcode opcode = InstructionSingle(cmd).GetOpcode();

        out << "<tr>\n"
               "<td><div id=\"" << inst << "\"></div>" << inst << "</td>\n";
        ...
        for (int pos = disp_opcode, i = 0; pos > 0; i++) {
            const EInstructionField     field      = inst_fmt_field[hw_format][i];
            const EInstructionSize      field_size = inst_field_data[field].size;
            const EInstructionFieldKind field_type = inst_field_data[field].type;
            if (field_size > 0) {
               pos -= field_size;
               ...
               if (field_type == inst_fieldtype_opx) {
                  ...
                  out << "<a href=\"#" << arch_info_opcode[opcode]
                      << "_table\">" << opx << "</a>";
               } else {
                  out << inst_field_name[field];
               }
    ...
    ****************************************************************************/
    ...
    chaptertitle(out, chap_opcodes);
    const size_t opcodes = codetable(out, arch_info_opcode, "opcode", "<strong>Primary opcodes</strong>");

#define OPC_TABLE(NAME) \
    { insn_ ## NAME,    \
      codetable(out, arch_info_ ## NAME, to_string(insn_ ## NAME), "The &laquo;<strong>" #NAME "</strong>&raquo; extended opcodes") \
    },
    ...
    const struct {
        ...
        DECLARE_SUBGOUP_OPCODES(OPC_TABLE)
    };
    ...
    for (int inst = 0; inst < insn_illegal; inst++) {
        const EInstructionSubset subset = asm_info[inst].subset;
        const EInstructionID realInsnId = Decode(InstructionSingle(asm_info[inst].tmplt));
    ...
    assert(sizearray(opc_data) == snum[instr_subset_group][instr_htype_hw]);

    /*****************************************************************************
    * instruction subset encoding statistic
    *****************************************************************************/

    chaptertitle(out, chap_statistic);
    out <<
        "<table width=\"60%\">\n"
    ...
    *****************************************************************************/
    int allcodes = opcodes;
    for (const auto& data : opc_data) {
        allcodes += data.num;
    }
    ...
    out <<
        ...
        << (allcodes - sizearray(opc_data)) << " codes, "
        << sizearray(opc_data) << " groups):</caption>\n"
        "<thead>\n"
        "<tr>\n"
        "<th>subgroup</th>\n"
        "<th>num</th>\n"
    ...
    for (const auto& data : opc_data) {
        out << "<tr>\n"
               "<td class=\"cnt\">" << data.op << "</td>\n"
               "<td class=\"cnt\">" << data.num << "</td>\n"
               "</tr>\n";
    }
    out << "</tbody>\n"
           "</table>\n";
