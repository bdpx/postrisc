...
{
#define X(mnem, name, extname, format, descr) { descr },
    ...
    const char *descr)
{
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        if (mnem != insn_illegal) {
           ...
           out << "<a href=\"#" << to_attribute(asm_name[mnem]);
           if (mnem >= insn_last)
              out << "_table";
           out << "\">";
...
int Decoder::GenerateHtml(std::ostream& out)
{
    ...
    ****************************************************************************/
    ...
    for (const auto & insn : asm_names) {
        ...
        out << "<tr><td>" << instr_subset_name[subset] << "</td>"
               "<td><pre><a href=\"#" << to_attribute(asm_name[inst]) << "\">" << asm_name[inst] << "</a>";
    ...
    ****************************************************************************/
    ...
    for (const auto & insn : asm_names) {
        ...
        out << "<tr>\n"
               "<td><div id=\"" << to_attribute(asm_name[inst]) << "\"></div>" << asm_name[inst] << "</td>\n";
        ...
        for (int pos = disp_opcode, i = 0; pos > 0; i++) {
            ...
            if (field_size > 0) {
               ...
               if (field_type == inst_fieldtype_opx) {
                  ...
                  out << "<a href=\"#" << to_attribute(asm_name[arch_info_opcode[opcode]])
                      << "_table\">" << opx << "</a>";
               } else {
                  out << inst_field_name[field];
               }
    ...
    ****************************************************************************/
    ...
    chaptertitle(out, chapter++);
    const size_t opcodes = codetable(out, arch_info_opcode, "opcode", "primary opcodes");

    int num_opcodes_with_extcode = 0;
    EInstructionID opc_name[16];
    size_t opx_num[16];
...
#define OPC_TABLE(NAME)                                               \
    ...
    OPC_TABLE(bfsd)
    OPC_TABLE(bfsq)
    OPC_TABLE(indexed_ds)
    OPC_TABLE(indexed_ws)
    OPC_TABLE(indexed_wz)
    OPC_TABLE(misc)
