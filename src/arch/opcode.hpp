...
#define OPCODE_TABLE_ATOMIC_FULL_OPS(X, ITYPE, UTYPE, N) \
    X(amo_ld_    ## UTYPE, N + 0) \
    X(amo_st_    ## UTYPE, N + 1) \
    X(amo_swap_  ## UTYPE, N + 2) \
    X(amo_add_   ## UTYPE, N + 3) \
    X(amo_and_   ## UTYPE, N + 4) \
    X(amo_or_    ## UTYPE, N + 5) \
    X(amo_xor_   ## UTYPE, N + 6) \
    X(amo_min_   ## ITYPE, N + 7) \
    X(amo_max_   ## ITYPE, N + 8) \
    X(amo_min_   ## UTYPE, N + 9) \
    X(amo_max_   ## UTYPE, N + 10) \
    X(amo_sub_   ## UTYPE, N + 11) \
...
/**************************************************************************************
* opcode_fma extended opcodes (4 bit)
**************************************************************************************/
...
#define DECLARE_FMA_F16_EXTENDED_OPCODES(X) \
    FMA_SCALAR_OPS(X, f16,  0) \
    FMA_VECTOR_OPS(X, vf16, 4) \

enum fma_f16_opx {
    DECLARE_FMA_F16_EXTENDED_OPCODES(OPCODE_X)
    fma_f16_opx_last
};

inline constexpr u64 insn_tmplt(const fma_f16_opx opx)
{
    return insn_tmplt(EInstructionOpcode::opcode_fma_f16)
         | makepart(static_cast<u64>(opx), disp_indexed_opx);
}

#define DECLARE_FMA_F32_EXTENDED_OPCODES(X) \
    FMA_SCALAR_OPS(X, f32,  0) \
    FMA_VECTOR_OPS(X, vf32, 4) \

enum fma_f32_opx {
    DECLARE_FMA_F32_EXTENDED_OPCODES(OPCODE_X)
    fma_f32_opx_last
};

inline constexpr u64 insn_tmplt(const fma_f32_opx opx)
{
    return insn_tmplt(EInstructionOpcode::opcode_fma_f32)
         | makepart(static_cast<u64>(opx), disp_indexed_opx);
}

#define DECLARE_FMA_F64_EXTENDED_OPCODES(X) \
    FMA_SCALAR_OPS(X, f64,  0) \
    FMA_VECTOR_OPS(X, vf64, 4) \

enum fma_f64_opx {
    DECLARE_FMA_F64_EXTENDED_OPCODES(OPCODE_X)
    fma_f64_opx_last
};

inline constexpr u64 insn_tmplt(const fma_f64_opx opx)
{
    return insn_tmplt(EInstructionOpcode::opcode_fma_f64)
         | makepart(static_cast<u64>(opx), disp_indexed_opx);
}

#define DECLARE_FMA_F128_EXTENDED_OPCODES(X) \
    FMA_SCALAR_OPS(X, f128,  0) \
    X(amo_cas_u8,    11) \
    X(amo_cas_u16,   12) \
    X(amo_cas_u32,   13) \
    X(amo_cas_u64,   14) \
...
enum fma_f128_opx {
    DECLARE_FMA_F128_EXTENDED_OPCODES(OPCODE_X)
    fma_f128_opx_last
};

inline constexpr u64 insn_tmplt(const fma_f128_opx opx)
{
    return insn_tmplt(EInstructionOpcode::opcode_fma_f128)
         | makepart(static_cast<u64>(opx), disp_indexed_opx);
}
...
#define OPCODE_TABLE_FP_SCALAR_OPS(X, TYPE, N) \
    ...
    X(round_        ## TYPE,          N + 20) \
    X(roundnx_      ## TYPE,          N + 21) \
    DECLARE_FP_CMP_OPS(X, cmp, TYPE,  N + 24) /* 10 ops */ \
    X(cvt_          ## TYPE ## _i32,  N + 40) \
    X(cvt_          ## TYPE ## _u32,  N + 41) \
    X(cvt_i32_      ## TYPE,          N + 42) \
    X(cvt_u32_      ## TYPE,          N + 43) \
    X(cvt_          ## TYPE ## _i64,  N + 44) \
    X(cvt_          ## TYPE ## _u64,  N + 45) \
    X(cvt_i64_      ## TYPE,          N + 46) \
    X(cvt_u64_      ## TYPE,          N + 47) \
    X(cvt_          ## TYPE ## _i128, N + 48) \
    X(cvt_          ## TYPE ## _u128, N + 49) \
...
#define OPCODE_TABLE_FP_PACKED_OPS(X, TYPE, ITYPE, N) \
    ...
    X(pack_         ## TYPE,        N + 50) \
    X(cvt_ ## TYPE  ## _vi ## ITYPE, N + 51) \
    X(cvt_ ## TYPE  ## _vu ## ITYPE, N + 52) \
