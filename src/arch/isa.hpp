...
#define DECLARE_OPCODES_WITH_EXTENDED_CODES(X) \
    ...
    X(br_f32) X(br_f64) X(br_f128) X(br_i128) \
    X(nul_f32) X(nul_f64) X(nul_f128) \
    X(nul_rr) X(nul_i128) \
    X(mem_xi64) X(mem_xi32) X(mem_xu32)
...
#define DECLARE_INSTRUCTION_FIELDS(X) \
  ...
  X (rip_label,                bits_per_call,                     disp_call,                imm,    "disp28",     "forward ip-relative displacement") \
  X (rip_string,               bits_per_call,                     disp_call,                imm,    "disp28",     "label in rodata as string") \
...
#define DECLARE_ASM_FORMATS(X) \
  ...
  X (bin_simm_plus,  VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_bin_simm_plus)) \
  X (bin_simm_minus, VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_bin_simm_minus)) \
  X (bin_uimm,       VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_bin_uimm)) \
  X (bin_uimm_plus,  VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_bin_uimm_plus)) \
  X (bin_uimm_minus, VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_bin_uimm_minus)) \
  ...
  X (br_rr_swapped,  VARRAY(cf_lprefix, cf_argsdelimiter, cf_gprb, cf_comma, cf_gpra, cf_comma, cf_branch))  /* pseudo for RabB20 */ \
  ...
  X (br_simm_plus,   VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_brc_simm_plus, cf_comma, cf_branch)) \
  X (br_uimm_plus,   VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_brc_uimm_plus, cf_comma, cf_branch)) \
  ...
  X (r3_swapped,     VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprc, cf_comma, cf_gprb)) /* pseudo for r3 */ \
...
#define DECLARE_INSTRUCTION_FORMATS(X) \
  ...
  X (RaxbSI21, bin_imm,   "oooooooxxxxxxxbbbbbbbiiiiiiiiiiiiiiiiiiiii", VARRAY(cf_raopx, cf_gprb, cf_bin_simm), "Rb with signed imm") \
...
#define INSTRUCTION_FORMAT_ENUM_X(name, extname, descr, format, txt) inst_fmt_##name,
    ...
    inst_fmt_r3_swapped     = inst_fmt_r3,
    inst_fmt_br_rr_swapped  = inst_fmt_br_rr,    // inverted reg args
    inst_fmt_bin_simm_plus  = inst_fmt_bin_simm,  // immediate + 1
    inst_fmt_bin_simm_minus = inst_fmt_bin_simm,  // immediate - 1
    inst_fmt_br_simm_plus   = inst_fmt_br_simm,  // immediate + 1
    inst_fmt_bin_uimm_plus  = inst_fmt_bin_uimm,  // immediate + 1
    inst_fmt_bin_uimm_minus = inst_fmt_bin_uimm,  // immediate - 1
    inst_fmt_br_uimm_plus   = inst_fmt_br_uimm,  // immediate + 1
