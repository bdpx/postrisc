...
#define DECLARE_INSTRUCTION_FIELDS(X) \
  ...
  X (fldid,                    bits_per_call,                     disp_call,                imm,    "fp64",       "f64 inline immediate") \
  X (fldis,                    bits_per_call,                     disp_call,                imm,    "fp32",       "f32 inline immediate") \
  ...
  X (rounding_mode,            bits_per_rounding_mode,            disp_rounding_mode,       mod,    "rm",         "fp rounding mode") \
  X (optional_memory_ordering, bits_per_memory_ordering,          disp_memory_ordering,     mod,    "mo",         "memory ordering") \
  X (memory_ordering,          bits_per_memory_ordering,          disp_memory_ordering,     mod,    "mo",         "memory ordering") \
  ...
  X (ldarc,                    bits_per_call,                     disp_call,                imm,    "simm28x16",  "long label text+rodata") \
  ...
  X (fpclass,                  bits_per_update_stride_high,       disp_update_stride_high,  imm,    "uimm10",     "fp classify mask") \
  X (update_stride,            bits_per_update_stride_high,       disp_update_stride_high,  imm,    "simm10",     "signed immediate stride for base update") \
  X (update_stride_low,        bits_per_update_stride_low,        disp_update_stride_low,   imm,    "imm",        "low bits") \
  X (stride_simm,              bits_per_update_stride_high,       disp_update_stride_high,  imm,    "simm10",     "signed immediate for atomic load/store") \
  X (stride_simm_low,          bits_per_update_stride_low,        disp_update_stride_low,   imm,    "imm",        "low bits") \
...
#define DECLARE_ASM_FORMATS(X) \
  ...
  X(fldis,        VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_fldis)) \
  X(fldid,        VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_fldid)) \
  X(nop,          VARRAY(cf_lprefix, cf_argsdelimiter, cf_ldi)) \
  ...
  X(ldarc,        VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_ldarc)) \
  X(memrel,       VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_rip_label)) \
  X(write,        VARRAY(cf_lprefix, cf_argsdelimiter,                    cf_rip_string)) \
  X(fldsr,        VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_rip_float32)) \
  X(flddr,        VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_rip_float64)) \
  X(fldqr,        VARRAY(cf_lprefix, cf_argsdelimiter, cf_gpra, cf_comma, cf_rip_float128)) \
  ...
  X(r3scale,      VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_gprc, cf_comma, cf_indexed_scale)) \
  X(RbcScale,     VARRAY(            cf_argsdelimiter,                    cf_gprb, cf_comma, cf_gprc, cf_comma, cf_indexed_scale)) \
  X(fpclass,      VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_fpclass)) \
  ...
  X(r2rm,         VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_rounding_mode)) \
  X(r3rm,         VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_gprc, cf_rounding_mode)) \
  X(r4rm,         VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_gprc, cf_comma, cf_gprd, cf_rounding_mode)) \
  X(fence,        VARRAY(            cf_argsdelimiter, cf_memory_ordering)) \
  X(r2mo,         VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_optional_memory_ordering)) \
  X(r3mo,         VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_gprc, cf_optional_memory_ordering)) \
  X(r4mo,         VARRAY(            cf_argsdelimiter, cf_gpra, cf_comma, cf_gprb, cf_comma, cf_gprc, cf_comma, cf_gprd, cf_optional_memory_ordering)) \
  X(alloc,        VARRAY(            cf_argsdelimiter, cf_framesize)) \
  X(allocsp,      VARRAY(cf_lprefix, cf_argsdelimiter, cf_framesize, cf_comma, cf_bin_uimm)) \
...
#define DECLARE_INSTRUCTION_FORMATS(X) \
  ...
  X(memrel,       memrel,    "oooooooaaaaaaaiiiiiiiiiiiiiiiiiiiiiiiiiiii", VARRAY(cf_gpra, cf_rip_label), "load/store ip-relative") \
  X(write,        memrel,    "oooooooxxxxxxxiiiiiiiiiiiiiiiiiiiiiiiiiiii", VARRAY(cf_raopx, cf_rip_label), "write") \
  X(jmp,          call,      "oooooooxxxxxxxiiiiiiiiiiiiiiiiiiiiiiiiiiii", VARRAY(cf_raopx, cf_call), "jump ip-relative") \
  X(nop,          ldi,       "oooooooxxxxxxxiiiiiiiiiiiiiiiiiiiiiiiiiiii", VARRAY(cf_raopx, cf_ldi), "br/write ip-relative") \
  X(alloc,        none,      "oooooooxxxxxxxfffffff---------------------", VARRAY(cf_raopx, cf_framesize, cf_res21), "alloc") \
  X(allocsp,      bin_imm,   "oooooooxxxxxxxfffffffiiiiiiiiiiiiiiiiiiiii", VARRAY(cf_raopx, cf_framesize, cf_bin_uimm), "allocsp") \
  ...
  X(br_eh,        br_rr,     "oooooooxxxxxxxbbbbbbb----ppppppppppppppppp", VARRAY(cf_raopx, cf_gprb, cf_branch_opx_res, cf_branch), "ehadj") \
  ...
  X(r4rm,         none,      "oooooooaaaaaaabbbbbbbcccccccdddddddxxxxmmm", VARRAY(cf_gpra, cf_gprb, cf_gprc, cf_gprd, cf_indexed_opx, cf_rounding_mode), "fma") \
  X(r4mo,         none,      "oooooooaaaaaaabbbbbbbcccccccdddddddxxxxmmm", VARRAY(cf_gpra, cf_gprb, cf_gprc, cf_gprd, cf_indexed_opx, cf_memory_ordering), "cas") \
  X(gmemx,        gmemx,     "oooooooaaaaaaabbbbbbbccccccciiiiiiixxxxsss", VARRAY(cf_gpra, cf_gprb, cf_gprc, cf_indexed_disp, cf_indexed_opx, cf_indexed_scale), "scaled index load/store") \
  X(r3scale,      none,      "oooooooaaaaaaabbbbbbbcccccccxxxxxxxxxxxsss", VARRAY(cf_gpra, cf_gprb, cf_gprc, cf_misc_opx, cf_indexed_scale), "r3+scale") \
  X(RbcScale,     none,      "ooooooo-------bbbbbbbcccccccxxxxxxxxxxxsss", VARRAY(cf_unused_reg, cf_gprb, cf_gprc,  cf_misc_opx, cf_indexed_scale), "scaled indexed jump") \
  X(Rbc,          none,      "ooooooo-------bbbbbbbcccccccxxxxxxxxxxx---", VARRAY(cf_unused_reg, cf_gprb, cf_gprc,  cf_misc_opx, cf_misc_opx_null), "two args, no result") \
  X(mspr,         none,      "oooooooaaaaaaa-------kkkkkkkxxxxxxxxxxx---", VARRAY(cf_gpra, cf_unused_reg, cf_spr,   cf_misc_opx, cf_misc_opx_null), "get/set special-purpose register") \
  X(r2,           none,      "oooooooaaaaaaabbbbbbb-------xxxxxxxxxxx---", VARRAY(cf_gpra, cf_gprb, cf_unused_reg,  cf_misc_opx, cf_misc_opx_null), "unary") \
  X(r2s1,         none,      "oooooooaaaaaaabbbbbbbsssssssxxxxxxxxxxx---", VARRAY(cf_gpra, cf_gprb, cf_shamt_c,     cf_misc_opx, cf_misc_opx_null), "binary with shift amount") \
  X(r3,           none,      "oooooooaaaaaaabbbbbbbcccccccxxxxxxxxxxx---", VARRAY(cf_gpra, cf_gprb, cf_gprc,        cf_misc_opx, cf_misc_opx_null), "binary register-register") \
  X(r2rm,         none,      "oooooooaaaaaaabbbbbbb-------xxxxxxxxxxxmmm", VARRAY(cf_gpra, cf_gprb, cf_unused_reg,  cf_misc_opx, cf_rounding_mode), "unary (rm)") \
  X(r3rm,         none,      "oooooooaaaaaaabbbbbbbcccccccxxxxxxxxxxxmmm", VARRAY(cf_gpra, cf_gprb, cf_gprc,        cf_misc_opx, cf_rounding_mode), "binary register-register (rm)") \
  X(r2mo,         none,      "oooooooaaaaaaabbbbbbb-------xxxxxxxxxxxmmm", VARRAY(cf_gpra, cf_gprb, cf_unused_reg,  cf_misc_opx, cf_memory_ordering), "unary (mo)") \
  X(r3mo,         none,      "oooooooaaaaaaabbbbbbbcccccccxxxxxxxxxxxmmm", VARRAY(cf_gpra, cf_gprb, cf_gprc,        cf_misc_opx, cf_memory_ordering), "binary register-register (mo)") \
  X(fpclass,      none,      "oooooooaaaaaaabbbbbbbiiiiiiixxxxxxxxxxxiii", VARRAY(cf_gpra, cf_gprb, cf_fpclass,     cf_misc_opx, cf_stride_simm_low), "general [base with update] load/store") \
  X(gmemu,        none,      "oooooooaaaaaaabbbbbbbiiiiiiixxxxxxxxxxxiii", VARRAY(cf_gpra, cf_gprb, cf_stride_simm, cf_misc_opx, cf_stride_simm_low), "general [base with update] load/store") \
  X(int,          none,      "ooooooo-------bbbbbbbiiiiiiixxxxxxxxxxxiii", VARRAY(cf_unused_reg, cf_gprb, cf_stride_simm, cf_misc_opx, cf_stride_simm_low), "interrupt") \
  X(fence,        none,      "ooooooo---------------------xxxxxxxxxxxmmm", VARRAY(cf_misc_noarg, cf_misc_opx, cf_memory_ordering), "fence") \
  X(noargs,       none,      "ooooooo---------------------xxxxxxxxxxx---", VARRAY(cf_misc_noarg, cf_misc_opx, cf_misc_opx_null), "instruction without parameters") \
...
#define INSTRUCTION_FORMAT_ENUM_X(name, extname, descr, format, txt) inst_fmt_##name,
    ...
    inst_fmt_ldarc  = inst_fmt_call,
    inst_fmt_RabS10 = inst_fmt_gmemu,
    inst_fmt_fldis  = inst_fmt_ldi,
    inst_fmt_fldid  = inst_fmt_ldi,
    inst_fmt_fldsr  = inst_fmt_memrel,
    inst_fmt_flddr  = inst_fmt_memrel,
    inst_fmt_fldqr  = inst_fmt_memrel,
};
...
#define DECLARE_INSTRUCTION_EXTENSION_FORMATS(X) \
  ...
  X(memrel,   "------pppppppppppppppppppppppppppppppppppp",  "64(28+36) bit relative forward offset") \
...
extern const std::array<InstructionInfo, INSTRUCTION_LIST_SIZE> asm_info;
