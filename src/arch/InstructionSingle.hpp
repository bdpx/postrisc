...
public:
    ...
    ALWAYS_INLINE EInstructionOpcode GetOpcode(void) const {
        return static_cast<EInstructionOpcode>(get_u(disp_opcode, bits_per_opcode));
    }
    ...
    ALWAYS_INLINE RoundingMode GetRoundingMode(void) const {
        return static_cast<RoundingMode>(get_u(disp_rounding_mode, bits_per_rounding_mode));
    }
    void SetRoundingMode(RoundingMode rm) {
        return set_u(static_cast<u64>(rm), disp_rounding_mode, bits_per_rounding_mode);
    }

    ALWAYS_INLINE MemoryOrdering GetMemoryOrdering(void) const {
        return static_cast<MemoryOrdering>(get_u(disp_memory_ordering, bits_per_memory_ordering));
    }
    void SetMemoryOrdering(MemoryOrdering ordering) {
        return set_u(static_cast<u64>( ordering), disp_memory_ordering, bits_per_memory_ordering);
    }
    ...
    ALWAYS_INLINE i64 GetUpdateStride(void) const {
        return (get_s(disp_update_stride_high, bits_per_update_stride_high) << bits_per_update_stride_low)
               | get_u(disp_update_stride_low, bits_per_update_stride_low);
    }
    void SetUpdateStride(i64 stride) {
        set_u(stride, disp_update_stride_low, bits_per_update_stride_low);
        set_s(stride >> bits_per_update_stride_low, disp_update_stride_high, bits_per_update_stride_high);
    }

    ALWAYS_INLINE i64 GetFpClassifyMask(void) const {
        return (get_u(disp_update_stride_high, bits_per_update_stride_high) << bits_per_update_stride_low)
               | get_u(disp_update_stride_low, bits_per_update_stride_low);
    }
    void SetFpClassifyMask(u64 stride) {
        set_u(stride, disp_update_stride_low, bits_per_update_stride_low);
        set_u(stride >> bits_per_update_stride_low, disp_update_stride_high, bits_per_update_stride_high);
    }
