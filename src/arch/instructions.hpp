...
public:
    ...
    ALWAYS_INLINE EMemoryOrder GetMemoryOrder(void) const {
        return static_cast<EMemoryOrder>(get_u(disp_memory_order, bits_per_memory_order));
    }
    void SetMemoryOrder(EMemoryOrder scale) {
        return set_u(scale, disp_memory_order, bits_per_memory_order);
    }
...
public:
    static constexpr const int s_num_slots = 1;
...
public:
    ...
    u64 ComputeJumpRelative(u64 ip) const {
        return ip + bytes_per_bundle * GetJumpDisplacement();
    }
    ...
    u64 ComputeBranchRelative(u64 ip) const {
        return ip + bytes_per_bundle * GetBranchDisplacement();
    }
...
public:
    static constexpr const int s_num_slots = 2;
