...
// atomics
//===------------------------------------------------------===//
#define INSN_TABLE_ATOMIC_X(X, ITYPE, UTYPE) \
    X(amo_ld_   ## UTYPE, atomic, r2mo, "atomic load " #UTYPE) \
    X(amo_st_   ## UTYPE, atomic, r2mo, "atomic store " #UTYPE) \
    X(amo_swap_ ## UTYPE, atomic, r3mo, "atomic swap " #UTYPE) \
    X(amo_cas_  ## UTYPE, atomic, r4mo, "atomic compare and swap " #UTYPE) \
    X(amo_add_  ## UTYPE, atomic, r3mo, "atomic load-add " #UTYPE) \
    X(amo_sub_  ## UTYPE, atomic, r3mo, "atomic load-sub " #UTYPE) \
    X(amo_and_  ## UTYPE, atomic, r3mo, "atomic load-and " #UTYPE) \
    X(amo_or_   ## UTYPE, atomic, r3mo, "atomic load-or " #UTYPE) \
    X(amo_xor_  ## UTYPE, atomic, r3mo, "atomic load-xor " #UTYPE) \
    X(amo_min_  ## ITYPE, atomic, r3mo, "atomic load-minimum " #ITYPE) \
    X(amo_max_  ## ITYPE, atomic, r3mo, "atomic load-maximum " #ITYPE) \
    X(amo_min_  ## UTYPE, atomic, r3mo, "atomic load-minimum " #UTYPE) \
    X(amo_max_  ## UTYPE, atomic, r3mo, "atomic load-maximum " #UTYPE) \

INSN_TABLE_ATOMIC_X(X, i8,   u8)
INSN_TABLE_ATOMIC_X(X, i16,  u16)
INSN_TABLE_ATOMIC_X(X, i32,  u32)
INSN_TABLE_ATOMIC_X(X, i64,  u64)
INSN_TABLE_ATOMIC_X(X, i128, u128)

X(fence,  atomic, fence, "atomic fence")
...
X(clmul,    cipher, r3scale, "carry-less multiply")
...
#define INSN_FPU_SCALAR_COMMON(TYPE, GROUP) \
    ...
    X(add_          ## TYPE,          GROUP, r3rm,   "add " #TYPE) \
    X(sub_          ## TYPE,          GROUP, r3rm,   "subtract " #TYPE) \
    X(nadd_         ## TYPE,          GROUP, r3rm,   "negate add " #TYPE) \
    X(abs_diff_     ## TYPE,          GROUP, r3rm,   "absolute difference " #TYPE) \
    X(mul_          ## TYPE,          GROUP, r3rm,   "multiply " #TYPE) \
    X(nmul_         ## TYPE,          GROUP, r3rm,   "negate multiply " #TYPE) \
    X(nabs_diff_    ## TYPE,          GROUP, r3rm,   "negate absolute difference " #TYPE) \
    X(class_        ## TYPE,          GROUP, fpclass,"classify " #TYPE) \
    X(rsqrt_        ## TYPE,          GROUP, r2rm,   "reciprocal square root " #TYPE) \
    X(sqrt_         ## TYPE,          GROUP, r2rm,   "square root " #TYPE) \
    X(div_          ## TYPE,          GROUP, r3rm,   "divide " #TYPE) \
    ...
    X(madd_         ## TYPE,          GROUP, r4rm,   "multiply-add " #TYPE) \
    X(msub_         ## TYPE,          GROUP, r4rm,   "multiply-subtract " #TYPE) \
    X(nmadd_        ## TYPE,          GROUP, r4rm,   "negate multiply-add " #TYPE) \
    X(nmsub_        ## TYPE,          GROUP, r4rm,   "negate multiply-subtract " #TYPE) \
    X(merge_        ## TYPE,          GROUP, r4,     "merge " #TYPE) \
    X(round_        ## TYPE,          GROUP, r2rm,   "round " #TYPE) \
    X(roundnx_      ## TYPE,          GROUP, r2rm,   "round, detect inexact " #TYPE) \
    ...
    X(cvt_          ## TYPE ## _i32,  GROUP, r2rm,   "convert " #TYPE " to i32") \
    X(cvt_          ## TYPE ## _u32,  GROUP, r2rm,   "convert " #TYPE " to u32") \
    X(cvt_          ## TYPE ## _i64,  GROUP, r2rm,   "convert " #TYPE " to i64") \
    X(cvt_          ## TYPE ## _u64,  GROUP, r2rm,   "convert " #TYPE " to u64") \
    X(cvt_          ## TYPE ## _i128, GROUP, r2rm,   "convert " #TYPE " to i128") \
    X(cvt_          ## TYPE ## _u128, GROUP, r2rm,   "convert " #TYPE " to u128") \
    X(cvt_i32_      ## TYPE,          GROUP, r2rm,   "convert i32 to " #TYPE) \
    X(cvt_u32_      ## TYPE,          GROUP, r2rm,   "convert u32 to " #TYPE) \
    X(cvt_i64_      ## TYPE,          GROUP, r2rm,   "convert i64 to " #TYPE) \
    X(cvt_u64_      ## TYPE,          GROUP, r2rm,   "convert u64 to " #TYPE) \
    X(cvt_i128_     ## TYPE,          GROUP, r2rm,   "convert i128 to " #TYPE) \
    X(cvt_u128_     ## TYPE,          GROUP, r2rm,   "convert u128 to " #TYPE) \
...
#define INSN_FPU_VECTOR_COMMON(TYPE, ITYPE, GROUP) \
    ...
    X(abs_max_      ## TYPE,   GROUP,  r3,    "absolute maximum " #TYPE) \
    X(abs_min_      ## TYPE,   GROUP,  r3,    "absolute minimum " #TYPE) \
    X(dot_          ## TYPE,   GROUP,  r3rm,  "dot-product " #TYPE) \
    X(round_        ## TYPE,   GROUP,  r2rm,  "round " #TYPE) \
    X(roundnx_      ## TYPE,   GROUP,  r2rm,  "round, detect inexact " #TYPE) \
    X(madd_         ## TYPE,   GROUP,  r4rm,  "multiply-add " #TYPE) \
    X(msub_         ## TYPE,   GROUP,  r4rm,  "multiply-subtract " #TYPE) \
    X(nmadd_        ## TYPE,   GROUP,  r4rm,  "negate multiply-add " #TYPE) \
    X(nmsub_        ## TYPE,   GROUP,  r4rm,  "negate multiply-subtract " #TYPE) \
    X(madd_alt_     ## TYPE,   GROUP,  r4rm,  "multiply-alternating add-subtract " #TYPE) \
    X(msub_alt_     ## TYPE,   GROUP,  r4rm,  "multiply-alternating subtract-add " #TYPE) \
    X(merge_        ## TYPE,   GROUP,  r4,    "merge " #TYPE) \
    X(merge_high_   ## TYPE,   GROUP,  r3rm,  "merge high parts " #TYPE) \
    X(merge_low_    ## TYPE,   GROUP,  r3rm,  "merge low parts " #TYPE) \
    X(add_alt_      ## TYPE,   GROUP,  r3rm,  "add alternating " #TYPE) \
    X(sub_alt_      ## TYPE,   GROUP,  r3rm,  "subtract alternating " #TYPE) \
    X(add_horiz_    ## TYPE,   GROUP,  r3rm,  "add horizontal " #TYPE) \
    X(sub_horiz_    ## TYPE,   GROUP,  r3rm,  "subtract horizontal " #TYPE) \
    X(mul_horiz_    ## TYPE,   GROUP,  r3rm,  "multiply horizontal " #TYPE) \
    ...
    X(cmp_u_        ## TYPE,   GROUP,  r3,    "compare unordered " #TYPE) \
    X(cvt_vi ## ITYPE ## _   ## TYPE,  GROUP,  r2rm, "convert vi" #ITYPE " to " #TYPE) \
    X(cvt_vu ## ITYPE ## _   ## TYPE,  GROUP,  r2rm, "convert vu" #ITYPE " to " #TYPE) \
    X(cvt_ ## TYPE  ## _vi ## ITYPE, GROUP,  r2rm, "convert " #TYPE " to vi" #ITYPE) \
    X(cvt_ ## TYPE  ## _vu ## ITYPE, GROUP,  r2rm, "convert " #TYPE " to vu" #ITYPE) \
...
X(cvt_f32_f16,  f16,  r2rm, "convert f32 to f16")
X(cvt_f64_f16,  f16,  r2rm, "convert f64 to f16")
X(cvt_f64_f32,  f32,  r2rm, "convert f64 to f32")
X(cvt_f128_f64, f128, r2rm, "convert f128 to f64")
X(cvt_f128_f32, f128, r2rm, "convert f128 to f32")
X(cvt_f128_f16, f128, r2rm, "convert f128 to f16")
