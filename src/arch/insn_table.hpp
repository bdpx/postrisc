...
X(dep_r,    base,  r4,   "deposit register")
...
#define INSN_SCALAR_COMMON(STYPE, UTYPE) \
    ...
    X(cmp_ge_      ## STYPE,  base,        r3,              "compare greater-or-equal " #STYPE) \
    X(cmp_ge_      ## UTYPE,  base,        r3,              "compare greater-or-equal " #UTYPE) \
    ...
    X(cmp_ge_imm_  ## STYPE,  base,        bin_simm,        "compare greater or equal immediate " #STYPE) \
    X(cmp_ge_imm_  ## UTYPE,  base,        bin_uimm,        "compare greater or equal immediate " #UTYPE) \
...
//===------------------------------------------------------===//
X(call_mi,  jump, RabS10,    "call memory indirect")
X(call,     jump, call,      "call relative")
X(call_ri,  jump, r3,        "call register indirect")
X(call_rvt, jump, RabS10,    "call relative vtable")
...
#define INSN_TABLE_ATOMIC_FULL_X(X, ITYPE, UTYPE) \
    ...
    INSN_TABLE_ATOMIC_CAS  (X, cas,     ITYPE, "compare and swap") \
