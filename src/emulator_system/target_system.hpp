...
public:
    ...
    CStatus StartEmulation(const Configuration& configuration, std::istream& input, std::ostream& out);
    void WaitFinishEmulation(std::ostream& out);
    void Clear();
    CStatus Setup(const Configuration& configuration);
...
private:
    ...
    CStatus VirtualMap(const TranslationUnit &mmu, const CVirtAddress va_base, const CPhysAddress pa, u64 size, u64 attributes);
    CStatus VirtualAllocate(const TranslationUnit &mmu, u64 base, u64 size, u64 attributes);
    CStatus VirtualProtect(const TranslationUnit &mmu, u64 base, u64 size, u64 attributes);
    CStatus VirtualFree(const TranslationUnit &mmu, u64 base, u64 size);
    CStatus LoadSegment(Core& core, u64 base, size_t size, const u8 *data, u64 final_rights);
    CStatus LoadModule(Core& core, const u64 vbase, std::istream& input, CExeFileHeader& header);
    CStatus PrepareThread(Core& core, const u64 module_base, const CExeFileHeader &header);
    CStatus dump_virt_map_sub_dir(const TranslationUnit &mmu, const DumpFormatter& out,
                  const CPhysAddress pta, u64 base, u64& used, u64& extra_used, int level);
...
#define FIND_DEVICE(pa, len)                                                   \
    ...
    const CDeviceOffset device_offset(address - it->first);                    \
    auto& mapping = it->second;


#define SYSTEM_READ_TYPED_HANDLER(TYPE)                                        \
ALWAYS_INLINE CStatus                                                          \
TargetSystem::bus_read_##TYPE(const CPhysAddress pa, TYPE& data) const         \
{                                                                              \
...
ALWAYS_INLINE CStatus
TargetSystem::bus_read(const CPhysAddress pa, size_t len, void *bytes) const
{
    FIND_DEVICE(pa, len)
    mapping.m_device->m_reads_count++;
