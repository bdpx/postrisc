...
TargetSystem::Setup(EmulatorConfiguration const & configuration)
{
    ...
    unsigned const bytes_per_page = U32C(1) << configuration.bitsPerPageOffset;
    set_bits_per_page_offset(configuration.bitsPerPageOffset);

    // prepare special device: config space for all other devices
    LOG_DEBUG(LOG_LOADER) << "create DeviceArray";
    m_DeviceArray = boost::make_intrusive<DeviceArray>("DeviceArray",
    ...
    for (const auto& device_info : configuration.memory_devices) {
        u64 const size = device_info.size;
        const std::string name = std::string("memory") + std::to_string(id++);
        boost::intrusive_ptr<device_memory> memory =
            boost::make_intrusive<device_memory>(name, size);
        ...
        for (const auto& mapping_info : device_info.mappings) {
            PhysMemoryMapping mapping(
                ...
                bytes_per_page,
                memory);
            CHECK_STATUS(add_memory_mapping(CPhysAddress(mapping_info.address), mapping, is_memory));
        }
    }
    ...
    for (const auto& core_info : configuration.core_devices) {
        boost::intrusive_ptr<Core> core =
            boost::make_intrusive<Core>(core_info.name, core_id, m_configuration_flags);
        core->reset(core_id);
    ...
    m_video = boost::make_intrusive<device_video>("video",
        configuration.video_device.width, configuration.video_device.height);
    u64 const v_phys_address = configuration.video_device.phys_address;
    u64 const v_virt_address = configuration.video_device.virt_address;
    PhysMemoryMapping video_mapping(v_phys_address, bytes_per_page, m_video);
    CHECK_STATUS(add_memory_mapping(CPhysAddress(v_phys_address), video_mapping, false));
    CHECK_STATUS(VirtualMap(core->GetMMU(), v_virt_address, CPhysAddress(v_phys_address),
        m_video->size(), VM_PAGE_RIGHT_READ | VM_PAGE_RIGHT_WRITE));
    ...
    for (const auto& device_info : configuration.rom_devices) {
        ...
        boost::intrusive_ptr<Device> rom =
            boost::make_intrusive<device_rom>("rom", device_info.data);
        CHECK_STATUS(m_DeviceArray->add_device(rom));
        CPhysAddress const pa(device_info.phys_address);
        PhysMemoryMapping mapping(pa.get_address(), bytes_per_page, rom);
        CHECK_STATUS(add_memory_mapping(pa, mapping, false));
        // additionally map ROM to virtual memory
        CHECK_STATUS(VirtualMap(core->GetMMU(), device_info.virt_address,
                                pa, rom->size(), VM_PAGE_RIGHT_READ));
    }

    // map device array last, after all changes of its size
    {
        u64 const phys_address = configuration.device_array_config.phys_address;
        u64 const virt_address = configuration.device_array_config.virt_address;
        m_DeviceArray->config.device_array_address = phys_address;
        m_DeviceArray->config.device_array_len = m_DeviceArray->size();
        PhysMemoryMapping mapping(phys_address, bytes_per_page, m_DeviceArray);
        CHECK_STATUS(add_memory_mapping(CPhysAddress(phys_address), mapping, false));
        CHECK_STATUS(VirtualMap(core->GetMMU(), virt_address, CPhysAddress(phys_address),
...
TargetSystem::PrepareThread(Core& core, const u64 module_base, const CExeFileHeader &header)
{
    ...
    CHECK_STATUS(core.PrepareThread(stack_hi, bp, tp, eip));
...
TargetSystem::VirtualMap(const TranslationUnit &mmu,
    u64 base, const CPhysAddress pa, u64 size, u64 attributes)
{
    u64 const pagesize = mmu.GetPageSize();
...
CStatus
TargetSystem::LoadModule(Core& core, const u64 vbase,
