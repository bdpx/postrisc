...
CStatus TargetSystem::Setup(const Configuration& conf)
{
    Clear();
...

    {
        ...
        m_video = boost::make_intrusive<device_video>("video", video_device.width, video_device.height);
        CPhysAddress const pa = video_device.phys_address;
        CVirtAddress const va = video_device.virt_address;
    ...
    // map device array last, after all changes of its size
    {
        ...
        CPhysAddress const pa = device_array_config.phys_address;
        CVirtAddress const va = device_array_config.virt_address;
