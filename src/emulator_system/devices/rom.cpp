...
device_rom::device_rom(const std::string& name_, const std::vector<std::byte>& _data)
    : Device(name_), m_data(_data)
{
    m_data.resize(util::align_up(m_data.size(), Device::minimal_size_alignment), std::byte{0xff});
