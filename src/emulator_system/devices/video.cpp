...
#define READ_TYPED_HANDLER(TYPE)                                                       \
    CStatus device_video::read_##TYPE(const CDeviceOffset device_offset, TYPE& data) { \
        ...
        TYPE const * host_address =                                                    \
            reinterpret_cast<TYPE const *>(&m_Data[offset]);                           \
        data = *host_address;                                                          \
...
CStatus
device_video::read(const CDeviceOffset device_offset, size_t len, void *bytes)
{
    u64 const offset = device_offset.get_offset();
...
CStatus
device_video::read_config(const CDeviceOffset device_offset, size_t len, void *bytes)
{
    u64 const offset = device_offset.get_offset();
    switch (offset) {
