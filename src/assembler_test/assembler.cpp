...
DEFINE_TEST_HANDLER(RabBBS)
{
    ...
    for (const auto & test : test_cases) {
        ...
        const i64 ip = test.ip * bytes_per_bundle;
...
DEFINE_TEST_HANDLER(ldar)
{
    ...
    for (const auto & test : test_cases) {
        ...
        const i64 offset = test.offset * bytes_per_bundle;
        const i64 ip = test.ip * bytes_per_bundle;
...
DEFINE_TEST_HANDLER(RabcMo)
{
    ...
    static const struct {
      ...
      EMemoryOrder mo;
        ...
        { 0,  100, 12, EMemoryOrder::relaxed },
        { 0,  12,  15, EMemoryOrder::acquire },
        { 10, 12, 120, EMemoryOrder::release },
        { 20, 13, 127, EMemoryOrder::acq_rel },
    };
    ...
    for (const auto & test : test_cases) {
        ...
        BOOST_TEST(insn.rb() == test.rb);
        BOOST_TEST(insn.rc() == test.rc);
        BOOST_TEST(insn.GetMemoryOrder() == test.mo);
        BOOST_TEST(m_decoder.Decode(insn) == realInsnId);
    }
}

DEFINE_TEST_HANDLER(RabMo)
{
    // mnem ra, rb, mo
    static const struct {
      unsigned ra, rb;
      EMemoryOrder mo;
        ...
        { 0,  100, EMemoryOrder::relaxed },
        { 0,  12,  EMemoryOrder::acquire },
        { 10, 12,  EMemoryOrder::release },
        { 20, 13,  EMemoryOrder::acq_rel },
    };
...
DEFINE_TEST_HANDLER(fence)
{
    ...
    static const struct {
      EMemoryOrder mo;
        ...
        { EMemoryOrder::acq_rel },
        { EMemoryOrder::seq_cst },
    };
