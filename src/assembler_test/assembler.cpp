...
#define DECLARE_TEST_HANDLER_X(NAME, fields) void Test_##NAME(EInstructionID name, EInstructionID realInsnId);
    DECLARE_ASM_FORMATS(DECLARE_TEST_HANDLER_X)

    // definition of test handler
#define DEFINE_TEST_HANDLER(NAME) void AssemblerTester::Test_##NAME(EInstructionID name, EInstructionID realInsnId)
};
...
DEFINE_TEST_HANDLER(r2rm)
{
    ...
    static const struct {
      ...
      RoundingMode rm;
        ...
        { 30, 112, RoundingMode::DYN },
        { 10, 120, RoundingMode::RNE },
        { 20, 127, RoundingMode::RDN },
        { 20, 127, RoundingMode::RUP },
        { 20, 127, RoundingMode::RTZ },
        { 20, 127, RoundingMode::RMM },
    };
    ...
    for (const auto& test : test_cases) {
        ...
        BOOST_TEST(insn.ra() == test.ra);
        BOOST_TEST(insn.rb() == test.rb);
        BOOST_TEST(insn.GetRoundingMode() == test.rm);
        BOOST_TEST(m_decoder.Decode(insn) == realInsnId, name);
    }
}

DEFINE_TEST_HANDLER(r3rm)
{
    // mnem ra, rb, rc, rm
    static const struct {
      unsigned ra, rb, rc;
      RoundingMode rm;
        ...
        { 30, 112, 45, RoundingMode::DYN },
        { 10, 120, 45, RoundingMode::RNE },
        { 20, 127, 45, RoundingMode::RDN },
        { 20, 127, 45, RoundingMode::RUP },
        { 20, 127, 45, RoundingMode::RTZ },
        { 20, 127, 45, RoundingMode::RMM },
    };
...
DEFINE_TEST_HANDLER(r4rm)
{
    ...
    static const struct {
      ...
      RoundingMode rm;
        ...
        { 30, 112, 45, 23, RoundingMode::DYN },
        { 10, 120, 45, 23, RoundingMode::RNE },
        { 20, 127, 45, 23, RoundingMode::RDN },
        { 20, 127, 45, 23, RoundingMode::RUP },
        { 20, 127, 45, 23, RoundingMode::RTZ },
        { 20, 127, 45, 23, RoundingMode::RMM },
    };
...
DEFINE_TEST_HANDLER(fence)
{
    ...
    for (const auto& test : test_cases) {
        ...
        ss << name
           << " "
           << MemoryOrdering2Str(test.mo)
           << "\n";

        const InstructionDouble insn = Assemble(ss.str());
        BOOST_TEST(GetInstructionLength() == cmd_short);

        BOOST_TEST(insn.GetMemoryOrdering() == test.mo);
        BOOST_TEST(m_decoder.Decode(insn) == realInsnId, name);
    }
}

DEFINE_TEST_HANDLER(r2mo)
{
    // mnem ra, rb, rc, mo
    static const struct {
      unsigned ra, rb;
      MemoryOrdering mo;
        ...
        { 30, 112, MemoryOrdering::RELAXED },
        { 10, 120, MemoryOrdering::ACQUIRE },
        { 20, 127, MemoryOrdering::RELEASE },
        { 20, 127, MemoryOrdering::ACQ_REL },
    };
...
DEFINE_TEST_HANDLER(r3mo)
{
    ...
    static const struct {
      ...
      MemoryOrdering mo;
        ...
        { 30, 112, 33, MemoryOrdering::RELAXED },
        { 10, 120, 33, MemoryOrdering::ACQUIRE },
        { 20, 127, 33, MemoryOrdering::RELEASE },
        { 20, 127, 33, MemoryOrdering::ACQ_REL },
    };
...
DEFINE_TEST_HANDLER(r4mo)
{
    ...
    static const struct {
      ...
      MemoryOrdering mo;
        ...
        { 30, 112, 33, 45, MemoryOrdering::RELAXED },
        { 10, 120, 33, 45, MemoryOrdering::ACQUIRE },
        { 20, 127, 33, 45, MemoryOrdering::RELEASE },
        { 20, 127, 33, 45, MemoryOrdering::ACQ_REL },
    };
...
DEFINE_TEST_HANDLER(ldi)
{
    ...
    for (const auto& test : test_cases) {
        ...
        BOOST_TEST(insn.ra() == test.ra);
        BOOST_TEST(insn.InstructionSingle::get_ldi() == test.imm);
        BOOST_TEST(m_decoder.Decode(insn) == realInsnId);
    }
}
...
DEFINE_TEST_HANDLER(fpclass)
{
    // mnem ra, rb, uimm10
    static const struct {
      unsigned ra, rb;
      unsigned classify_mask;
    } test_cases[] = {
        { 20,  1, 512 },
        { 10, 12, 1 },
        { 20, 13, 511 },
    ...
    for (const auto& test : test_cases) {
        ...
        BOOST_TEST(insn.ra() == test.ra);
        BOOST_TEST(insn.rb() == test.rb);
        BOOST_TEST(insn.GetFpClassifyMask() == test. classify_mask);
        BOOST_TEST(m_decoder.Decode(insn) == realInsnId);
    }
}
...
// TODO
DEFINE_TEST_HANDLER(fldis) { UNREFERENCED_PARAMETER(name); UNREFERENCED_PARAMETER(realInsnId); }
DEFINE_TEST_HANDLER(fldid) { UNREFERENCED_PARAMETER(name); UNREFERENCED_PARAMETER(realInsnId); }

DEFINE_TEST_HANDLER(fldsr) { UNREFERENCED_PARAMETER(name); UNREFERENCED_PARAMETER(realInsnId); }
DEFINE_TEST_HANDLER(flddr) { UNREFERENCED_PARAMETER(name); UNREFERENCED_PARAMETER(realInsnId); }
DEFINE_TEST_HANDLER(fldqr) { UNREFERENCED_PARAMETER(name); UNREFERENCED_PARAMETER(realInsnId); }
...
{
    static const struct {
        void (AssemblerTester::*ptr)(EInstructionID name, EInstructionID realInsnId);
        EInstructionID origInsnId;
        u64       Tmplt;
    } test_cases[] = {
#define X(name, code, subset, format, descr) \
...
#undef X
    ...
    Decoder decoder;
    for (const auto& test : test_cases) {
        if (asm_info[test.origInsnId].subset != instr_subset_group) {
            AssemblerTester tester(decoder);
            (tester.*test.ptr)(test.origInsnId, Decoder::Decode(InstructionSingle(test.Tmplt)));
        }
    }
}
