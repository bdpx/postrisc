...
void Generator::MinMaxImm(std::ostream& out, const std::string& type, const imm_op& op, const imm_data<T>& imm, bool cmp_reg_first, bool choose_reg_first)
{
    ...
    if (imm.value == 0x7FFFFFFFFFFFFFFF && (
        name.str() == "maxmin_imm_ge_reg_then_reg_else_imm_u64_9223372036854775807" ||
        name.str() == "maxmin_imm_ge_reg_then_imm_else_reg_u64_9223372036854775807" ||
        name.str() == "minmax_reg_le_imm_then_imm_else_reg_u64_9223372036854775807" ||
        name.str() == "minmax_reg_le_imm_then_reg_else_imm_u64_9223372036854775807"
       )) {
        out << CHECK << "ld_imm.l %r2, 9223372036854775807" << std::endl;
        if (name.str() == "maxmin_imm_ge_reg_then_reg_else_imm_u64_9223372036854775807" ||
            name.str() == "minmax_reg_le_imm_then_reg_else_imm_u64_9223372036854775807")
           out << CHECK << "cmov_lt_i64 %r1, %r1, %r2, %r1" << std::endl;
        else
           out << CHECK << "cmov_lt_i64 %r1, %r1, %r1, %r2" << std::endl;
    } else {
        // NORMAL:
        ...
        if (imm.mode == LONG) {
            ...
            out << CHECK_NEXT << "retf 0" << std::endl;
        } else {
            out << CHECK_NEXT << "retf 0" << std::endl;
        }
    }
...
void Generator::MinMaxRegImm(std::ostream& out, int size)
{
    out << "//===----------------------------------------------------------------------===//" << std::endl;
