...
CODEGEN_HANDLER(StoreBase)
{
    ...
    for (const auto& test : LoadStoreTests) {
        ...
        out << CHECK_LABEL << label << '\n';
        out << CHECK << insn_name(insn_alloc) << " 3\n";
        out << CHECK_NEXT << InsnCounter(this, test.base.store) << " %r1, %r2, " << (test.size * 400) << '\n';
        out << CHECK_NEXT << InsnCounter(this, test.base.store) << " %gz, %r2, " << test.size * 100 << '\n';
        out << CHECK_EMPTY << '\n';
        out << CHECK_NEXT << "! BUNDLE\n";
        out << CHECK_NEXT << insn_name(insn_retf) << " 0\n";
        ...
        out << CHECK_LABEL << label << '\n';
        out << CHECK << insn_name(insn_alloc) << " 4\n";
        out << CHECK_NEXT << InsnCounter(this, test.xu32.store) << " %r1, %r2, %r3, " << test.scale << ", "<< disp  << '\n';
        out << CHECK_NEXT << insn_name(insn_retf) << " 0\n";
        ...
        out << CHECK_LABEL << label << '\n';
        out << CHECK << insn_name(insn_alloc) << " 3\n";
        out << CHECK_NEXT << InsnCounter(this, test.xu32.store) << " %gz, %r1, %r2, " << test.scale << ", " << disp << '\n';
        out << CHECK_NEXT << insn_name(insn_retf) << " 0\n";
...
void Generator::LoadGlobal(std::ostream& out, CodeModel cm)
{
...

    {
        ...
        out << "// function pointer materialize: " << insn_name(insn_lda_r) << '\n';
        ...
        out << "// function call: " << insn_name(insn_call) << '\n';
        ...
        out << "// function ptr call: " << insn_name(insn_call_plt) << '\n';
    ...
    for (const auto& test : LoadStoreTests) {
        ...
        switch (tm) {
            case LocalExec:
                ...
                out << CHECK << insn_name(insn_add_imm_i64) << data_suffix << " %r1, %tp, " << tpoff << "(" << object << ")\n";
                break;
            case LocalDynamic:
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tld << "(" << object << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << insn_name(insn_add_imm_i64) << data_suffix << " %r1, %r3, " << dtpoff << "(" << object << ")\n";
                break;
            case GlobalDynamic:
            case DefaultTLS:
                ...
                // call %r2, %jpc(__tls_get_addr)
                // mov %r1, %r3
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tgd << "(" << object << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << insn_name(insn_mov) << " %r1, %r3\n";
                break;
            case InitialExec:
                // ld_iprel_u64.l %r1, %gottpoff_ext(tls_large_InitialExec_i8)
                // add_i64 %r1, %tp, %r1
                out << CHECK << insn_name(insn_ld_iprel_u64) << data_suffix << " %r1, " << gottpoff << "(" << object << ")\n";
                out << CHECK << insn_name(insn_add_i64) << " %r1, %tp, %r1\n";
                break;
        }
        ...
        switch (tm) {
            case LocalExec:
                ...
                out << CHECK << insn_name(insn_ld_imm) << data_suffix << " %r1, " << tpoff << "("  << object_array << ")\n";
                out << CHECK << insn_name(insn_lda_xi64) << ".l %r1, %tp, %r1, 0, " << (100 * test.size) << '\n';
                break;
            case LocalDynamic:
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tld << "(" << object_array << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << insn_name(insn_ld_imm) << data_suffix << " %r1, " << dtpoff << "("  << object_array << ")\n";
                out << CHECK << insn_name(insn_lda_xi64) << ".l %r1, %r3, %r1, 0, " << (100 * test.size) << '\n';
                break;
            case GlobalDynamic:
            case DefaultTLS:
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tgd << "(" << object_array << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << insn_name(insn_add_imm_i64) << " %r1, %r3, " << (100 * test.size) << '\n';
                break;
            case InitialExec:
                // ld_iprel_u64.l %r1, %gottpoff_ext(tls_array_large_InitialExec_bool) 
                // lda_xi64.l %r1, %tp, %r1, 0, 100 
                out << CHECK << InsnCounter(this, insn_ld_iprel_u64) << data_suffix << " %r1, " << gottpoff << "(" << object_array << ")\n";
                out << CHECK << InsnCounter(this, insn_lda_xi64) << ".l %r1, %tp, %r1, 0, " << (100 * test.size) << '\n';
                break;
        }
        ...
        switch (tm) {
            case LocalExec:
                ...
                out << CHECK << insn_name(test.base.load) << data_suffix << " %r1, %tp, " << tpoff << "("  << object << ")\n";
                break;
            case LocalDynamic:
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tld << "("  << object << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << insn_name(test.base.load) << data_suffix << " %r1, %r3, " << dtpoff << "("  << object << ")\n";
                break;
            case GlobalDynamic:
            case DefaultTLS:
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tgd << "("  << object << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << insn_name(test.base.load) << " %r1, %r3, 0\n";
                break;
            case InitialExec:
                // ld_iprel_u64.l %r1, %gottpoff_ext(x_tls_extern)
                // ld_xi64_u64 %r1, %tp, %r1, 0, 0
                out << CHECK << insn_name(insn_ld_iprel_u64) << data_suffix << " %r1, " << gottpoff << "("  << object << ")\n";
                out << CHECK << insn_name(test.xi64.load) << " %r1, %tp, %r1, 0, 0\n";
                break;
        }
        ...
        switch (tm) {
            case LocalExec:
                ...
                out << CHECK << insn_name(insn_ld_imm) << data_suffix << " %r1, " << tpoff << "("  << object_array << ")\n";
                out << CHECK << insn_name(test.xi64.load) << ".l %r1, %tp, %r1, 0, " << (100 * test.size) << '\n';
                break;
            case LocalDynamic:
                ...
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << insn_name(insn_ld_imm) << data_suffix << " %r1, " << dtpoff << "("  << object_array << ")\n";
                out << CHECK << InsnCounter(this, test.xi64.load) << ".l %r1, %r3, %r1, 0, " << (100 * test.size) << '\n';
                break;
            case GlobalDynamic:
            case DefaultTLS:
                // lda_iprel.l %r3, %tgd_ext(x_tls_extern)
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tgd << "("  << object_array << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << InsnCounter(this, test.base.load) << " %r1, %r3, " << (100 * test.size) << '\n';
                break;
            case InitialExec:
                // ld_iprel_u64.l %r1, %gottpoff_ext(x_tls_extern)
                // ld_xi64_u64 %r1, %tp, %r1, 0, 0
                out << CHECK << insn_name(insn_ld_iprel_u64) << data_suffix << " %r1, " << gottpoff << "("  << object_array << ")\n";
                out << CHECK << InsnCounter(this, test.xi64.load) << ".l %r1, %tp, %r1, 0, " << (100 * test.size) << '\n';
                break;
        }
        ...
        switch (tm) {
            case LocalExec:
                ...
                out << CHECK << insn_name(test.base.store) << data_suffix << " %r1, %tp, " << tpoff << "("  << object << ")\n";
                break;
            case LocalDynamic:
                // lda_iprel.l %r3, %tld_ext(x_tls_extern)
                // call %r2, %jpc(__tls_get_addr)
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tld << "("  << object << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << InsnCounter(this, test.base.store) << data_suffix << " %r1, %r3, " << dtpoff << "("  << object << ")\n";
                break;
            case GlobalDynamic:
            case DefaultTLS:
                // lda_iprel.l %r3, %tgd_ext(x_tls_extern)
                ...
                out << CHECK << insn_name(insn_lda_iprel) << data_suffix << " %r3, " << tgd << "("  << object << ")\n";
                out << CHECK << insn_name(insn_call) << code_suffix << " %r2, " << code_reloc << "(__tls_get_addr)\n";
                out << CHECK << InsnCounter(this, test.base.store) << " %r1, %r3, 0\n";
                break;
            case InitialExec:
                // ld_iprel_u64.l %r2, %gottpoff_ext(x_tls_extern)
                // st_xi64_i64 %r1, %tp, %r2, 0, 0
                out << CHECK << insn_name(insn_ld_iprel_u64) << data_suffix << " %r2, " << gottpoff << "("  << object << ")\n";
                out << CHECK << InsnCounter(this, test.xi64.store) << " %r1, %tp, %r2, 0, 0\n";
                break;
        }
