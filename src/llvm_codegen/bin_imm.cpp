...
void Generator::BinaryRegImmCase(std::ostream& out, const func_info& frag, const imm_data<ITYPE>& imm, const std::string& name)
{
    ...
    if (frag.insn == insn_min_imm_u64 || frag.insn == insn_max_imm_u64) {
        if (imm.value == 0x7FFFFFFFFFFFFFFF && (
            ...
            out << CHECK << insn_name(insn_ld_imm) << ".l %r2, " << 0x7FFFFFFFFFFFFFFF << '\n';
            out << CHECK_EMPTY << '\n';
            out << CHECK_NEXT << "! BUNDLE\n";
            if (frag.insn == insn_min_imm_u64)
                out << CHECK_NEXT << "cmov_lt_i64 %r1, %r1, %r2, %r1\n";
            else
                out << CHECK_NEXT << "cmov_lt_i64 %r1, %r1, %r1, %r2\n";
            return;
        }
    }
    ...
    if (frag.insn == insn_add_imm_i64) {
        ...
        if constexpr (std::is_same<i64, ITYPE>::value) {
            ...
            if ((imm.value & util::makemask<u64>(bits_per_lda_near_shift)) == 0 && ((imm.value << skip) >> skip) == imm.value) {
                out << CHECK << insn_name(insn_lda_n) << " %r1, %r1, " << (imm.value >> bits_per_lda_near_shift) << '\n';
                return;
            }
        }
    }
    ...
    if (frag.insn == insn_and_imm) {
        ...
        if (population == sizeof(ITYPE)*8-1) {
            ...
            out << CHECK << insn_name(insn_bit_clear) << " %r1, %r1, " << bitpos << '\n';
            return;
        }

        // check possible replacement: AND_IMM ===> DEPOSIT
        if (name == "and_imm_i64_p4611686018427387903") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %gz, %r1, 0, 62\n";
            return;
        }
        if (name == "and_imm_i64_m4611686018427387904") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 0, 62\n";
            return;
        }
        if (name == "and_imm_i32_m134217728") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 0, 27\n";
            return;
        }
        if (name == "and_imm_i64_p1048575") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %gz, %r1, 0, 20\n";
            return;
        }
        if (name == "and_imm_i32_m1048576" || name == "and_imm_i64_m1048576") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 0, 20\n";
            return;
        }
        ...
        if (name == "and_imm_i32_p134217727") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %gz, %r1, 0, 27\n";
            return;
        }
        if (name == "and_imm_i64_m4611686018427387901") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 2, 60\n";
            return;
        }
        if (name == "and_imm_i64_p4611686018427387903") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 62, 2\n";
            return;
        }
        if (name == "and_imm_i64_m1048573" || name == "and_imm_i32_m1048573") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 2, 18\n";
           return;
        }
        if (name == "and_imm_i64_p1048575") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 20, 44\n";
            return;
        }

        if (name == "and_imm_i32_m2147483645" || name == "and_imm_i64_m2147483645") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 2, 29\n";
            return;
        }

        if (name == "and_imm_i32_p1048575") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 20, 12\n";
            return;
        }

        if (name == "and_imm_i32_p134217727") {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, 27, 5\n";
            return;
        }
...
/*
        ...
        if (CheckBlock<ITYPE, zeroes_block>(imm.value, fit_size, fit_pos)) {
            out << CHECK << insn_name(insn_deposit) << " %r1, %r1, %gz, " << fit_pos << ", " << fit_size << '\n';
            return;
        }
        if (CheckBlock<ITYPE, ones_block>(imm.value, fit_size, fit_pos)) {
            out << CHECK << insn_name(insn_deposit) << " %r1, %gz, %r1, " << fit_pos << ", " << fit_size << '\n';
            return;
        }
*/
    }
    ...
    if (population == 1 && (
        ...
        switch (frag.insn) {
            case insn_div_imm_u32:
                out << CHECK << insn_name(insn_srl_imm_u32) << " %r1, %r1, " << bitpos << '\n';
                break;
            case insn_div_imm_u64:
                out << CHECK << insn_name(insn_srl_imm_u64) << " %r1, %r1, " << bitpos << '\n';
                break;
            case insn_rem_imm_u32:
            case insn_rem_imm_u64:
                // FIXME: .l prefix changed
                // out << CHECK << insn_name(insn_and_imm) << " %r1, %r1, " << (imm.value-1) << '\n';
                break;
            default:
                break;
        }
    ...
    if ((frag.insn == insn_mul_imm_i32 || frag.insn == insn_mul_imm_i64) &&
        ...
        switch (frag.insn) {
            case insn_mul_imm_i32:
                out << CHECK << insn_name(insn_sl_sub_i32) << " %r1, %gz, %r1, " << bitpos << '\n';
                break;
            case insn_mul_imm_i64:
                out << CHECK << insn_name(insn_sl_sub_i64) << " %r1, %gz, %r1, " << bitpos << '\n';
                break;
            default:
                break;
        }
