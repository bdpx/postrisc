...
constexpr bool useFences = false;
constexpr MemoryOrdering fencedMemoryOrdering = useFences ? MemoryOrdering::RELAXED : MemoryOrdering::SEQ_CST;
...
CODEGEN_HANDLER(AtomicLoads)
{
    static const struct {
        ...
        { fencedMemoryOrdering, ATOMIC_SEQ_CST },
    };
    ...
    for (const auto& info : test.infos) {
        ...
        if (useFences && isOrderingFenced(order.ordering)) {
...
CODEGEN_HANDLER(AtomicCompareSwaps)
{
    static const struct {
        ...
        { fencedMemoryOrdering, ATOMIC_SEQ_CST, ATOMIC_RELAXED },
        { fencedMemoryOrdering, ATOMIC_SEQ_CST, ATOMIC_ACQUIRE },
        { fencedMemoryOrdering, ATOMIC_SEQ_CST, ATOMIC_CONSUME },
        { fencedMemoryOrdering, ATOMIC_SEQ_CST, ATOMIC_SEQ_CST },
    };
    ...
    for (const auto& info : test.infos) {
        ...
        if (useFences && (isOrderingFenced(order.success) || isOrderingFenced(order.failure))) {
            out << CHECK << insn_fence << ' ' <<  MemoryOrdering2Str(MemoryOrdering::SEQ_CST) << '\n';
        }
