...
{
#define TEST_TABLE_ATOMIC_LOAD_X(CMD_MO, SIZE, ORDER, SEXT) \
    { insn_amo_ld_u ## SIZE, CMD_MO, ORDER, SEXT,       "i" #SIZE }, \
    { insn_amo_ld_u ## SIZE, CMD_MO, ORDER, insn_undef, "u" #SIZE }, \

#define DEFINE_ATOMIC_LOAD_SIZES(CMD_MO, ORDER) \
    TEST_TABLE_ATOMIC_LOAD_X(CMD_MO, 8,   ORDER, insn_sext_i8) \
    TEST_TABLE_ATOMIC_LOAD_X(CMD_MO, 16,  ORDER, insn_sext_i16) \
    TEST_TABLE_ATOMIC_LOAD_X(CMD_MO, 32,  ORDER, insn_undef) \
    TEST_TABLE_ATOMIC_LOAD_X(CMD_MO, 64,  ORDER, insn_undef) \
    TEST_TABLE_ATOMIC_LOAD_X(CMD_MO, 128, ORDER, insn_undef) \

    static const struct {
        EInstructionID  atomic_insn;
        MemoryOrdering  cmd_mo;
        Ordering        ordering;
        EInstructionID  sext_insn;
        char            type[8];
    } tests[] = {
        DEFINE_ATOMIC_LOAD_SIZES(MemoryOrdering::RELAXED, ATOMIC_RELAXED)
        DEFINE_ATOMIC_LOAD_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_ACQUIRE)
        DEFINE_ATOMIC_LOAD_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_CONSUME)
        DEFINE_ATOMIC_LOAD_SIZES(MemoryOrdering::RELAXED, ATOMIC_SEQ_CST)
    };
    ...
    for (const auto& test : tests) {
        ...
        if (isOrderingFenced(test.ordering)) {
            out << CHECK << insn_name(insn_fence) << ' ' <<  MemoryOrdering2Str( MemoryOrdering::SEQ_CST) << '\n';
        }
        out << CHECK << InsnCounter(this, test.atomic_insn) << " %r1, %r1, " << MemoryOrdering2Str(test.cmd_mo) << "\n";
        // atomic loads do zero-extend, add sext if needed
        if (test.sext_insn != insn_undef) {
            out << CHECK << insn_name(test.sext_insn) << " %r1, %r1\n";
        }
        out << test.type << " " << name.str() << "(" << test.type << " *ptr) {\n";
        out << "    return __atomic_load_n(ptr, " << GetOrderingName(test.ordering) << ");\n";
        out << "}\n";
        out << '\n';
    }
}
...
{
#define TEST_TABLE_ATOMIC_STORE_X(CMD_MO, SIZE, ORDER) \
    { insn_amo_st_u ## SIZE, CMD_MO, ORDER, "i" #SIZE }, \
    { insn_amo_st_u ## SIZE, CMD_MO, ORDER, "u" #SIZE }, \

#define DEFINE_ATOMIC_STORE_SIZES(CMD_MO, ORDER) \
    TEST_TABLE_ATOMIC_STORE_X(CMD_MO, 8,   ORDER) \
    TEST_TABLE_ATOMIC_STORE_X(CMD_MO, 16,  ORDER) \
    TEST_TABLE_ATOMIC_STORE_X(CMD_MO, 32,  ORDER) \
    TEST_TABLE_ATOMIC_STORE_X(CMD_MO, 64,  ORDER) \
    TEST_TABLE_ATOMIC_STORE_X(CMD_MO, 128, ORDER) \
    ...
    static const struct {
        ...
        DEFINE_ATOMIC_STORE_SIZES(MemoryOrdering::RELAXED, ATOMIC_RELAXED)
        DEFINE_ATOMIC_STORE_SIZES(MemoryOrdering::RELEASE, ATOMIC_RELEASE)
        DEFINE_ATOMIC_STORE_SIZES(MemoryOrdering::RELAXED, ATOMIC_SEQ_CST)
    };
    ...
    for (const auto& test : tests) {
        ...
        if (isOrderingFenced(test.ordering)) {
            out << CHECK_NEXT << insn_name(insn_fence) << ' ' <<  MemoryOrdering2Str(MemoryOrdering::SEQ_CST) << '\n';
        }
        out << CHECK_NEXT << InsnCounter(this, test.atomic_insn) << " %r1, %r2, " <<  MemoryOrdering2Str(test.cmd_mo) << "\n";
        ...
        out << "    return __atomic_store_n(ptr, value, " << GetOrderingName(test.ordering) << ");\n";
        out << "}\n";
        out << '\n';
    }
}
...
{
#define TEST_TABLE_SWAP_X(CMD_MO, SIZE, ORDER, SEXT) \
    { insn_amo_swap_u ## SIZE, CMD_MO, ORDER, SEXT,       "i" #SIZE }, \
    { insn_amo_swap_u ## SIZE, CMD_MO, ORDER, insn_undef, "u" #SIZE }, \

#define DEFINE_SWAP_SIZES(CMD_MO, ORDER) \
    TEST_TABLE_SWAP_X(CMD_MO, 8,   ORDER, insn_sext_i8) \
    TEST_TABLE_SWAP_X(CMD_MO, 16,  ORDER, insn_sext_i16) \
    TEST_TABLE_SWAP_X(CMD_MO, 32,  ORDER, insn_undef) \
    TEST_TABLE_SWAP_X(CMD_MO, 64,  ORDER, insn_undef) \
    TEST_TABLE_SWAP_X(CMD_MO, 128, ORDER, insn_undef) \
    ...
    static const struct {
        ...
        DEFINE_SWAP_SIZES(MemoryOrdering::RELAXED, ATOMIC_RELAXED)
        DEFINE_SWAP_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_ACQUIRE)
        DEFINE_SWAP_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_CONSUME)
        DEFINE_SWAP_SIZES(MemoryOrdering::RELEASE, ATOMIC_RELEASE)
        DEFINE_SWAP_SIZES(MemoryOrdering::ACQ_REL, ATOMIC_ACQ_REL)
        DEFINE_SWAP_SIZES(MemoryOrdering::RELAXED, ATOMIC_SEQ_CST)
    };
    ...
    for (const auto& test : tests) {
        ...
        if (isOrderingFenced(test.ordering)) {
            ...
            out << CHECK_NEXT << "! BUNDLE\n";
            if (test.sext_insn != insn_undef) {
                out << CHECK_NEXT << insn_name(test.sext_insn) << " %r1, %r1\n";
            }
            out << CHECK_NEXT << insn_name(insn_retf) << " 0\n";
        } else {
            // no fence
            out << CHECK_NEXT << InsnCounter(this, test.atomic_insn) << " %r1, %r1, %r2, " << MemoryOrdering2Str(test.cmd_mo) << "\n";
            // atomic loads do zero-extend, add sext if needed
            if (test.sext_insn != insn_undef) {
                out << CHECK_NEXT << insn_name(test.sext_insn) << " %r1, %r1\n";
                out << CHECK_EMPTY << '\n';
        ...
        out << test.type << " " << name.str() << "(" << test.type << " *ptr, " << test.type << " value) {\n";
        out << "    return __atomic_exchange_n(ptr, value, " << GetOrderingName(test.ordering) << ");\n";
        out << "}\n";
        out << '\n';
    }
}
...
{
#define TEST_TABLE_CAS_X(MO, SIZE, SUCCESS_ORDER, FAILURE_ORDER, SEXT) \
    { insn_amo_cas_u ## SIZE, MO, SUCCESS_ORDER, FAILURE_ORDER, SEXT,       "i" #SIZE }, \
    { insn_amo_cas_u ## SIZE, MO, SUCCESS_ORDER, FAILURE_ORDER, insn_undef, "u" #SIZE }, \

#define DEFINE_CAS_SIZES( MO, SUCCESS_ORDER, FAILURE_ORDER) \
    TEST_TABLE_CAS_X(MO, 8,   SUCCESS_ORDER, FAILURE_ORDER, insn_sext_i8) \
    TEST_TABLE_CAS_X(MO, 16,  SUCCESS_ORDER, FAILURE_ORDER, insn_sext_i16) \
    TEST_TABLE_CAS_X(MO, 32,  SUCCESS_ORDER, FAILURE_ORDER, insn_sext_i32) \
    TEST_TABLE_CAS_X(MO, 64,  SUCCESS_ORDER, FAILURE_ORDER, insn_sext_i64) \
    TEST_TABLE_CAS_X(MO, 128, SUCCESS_ORDER, FAILURE_ORDER, insn_undef) \
    ...
    static const struct {
        ...
        DEFINE_CAS_SIZES(MemoryOrdering::RELAXED, ATOMIC_RELAXED, ATOMIC_RELAXED)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_ACQUIRE, ATOMIC_RELAXED)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_ACQUIRE, ATOMIC_ACQUIRE)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_ACQUIRE, ATOMIC_CONSUME)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_CONSUME, ATOMIC_RELAXED)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_CONSUME, ATOMIC_ACQUIRE)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQUIRE, ATOMIC_CONSUME, ATOMIC_CONSUME)
        DEFINE_CAS_SIZES(MemoryOrdering::RELEASE, ATOMIC_RELEASE, ATOMIC_RELAXED)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQ_REL, ATOMIC_ACQ_REL, ATOMIC_RELAXED)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQ_REL, ATOMIC_ACQ_REL, ATOMIC_CONSUME)
        DEFINE_CAS_SIZES(MemoryOrdering::ACQ_REL, ATOMIC_ACQ_REL, ATOMIC_ACQUIRE)
        DEFINE_CAS_SIZES(MemoryOrdering::RELAXED, ATOMIC_SEQ_CST, ATOMIC_RELAXED)
        DEFINE_CAS_SIZES(MemoryOrdering::RELAXED, ATOMIC_SEQ_CST, ATOMIC_ACQUIRE)
        DEFINE_CAS_SIZES(MemoryOrdering::RELAXED, ATOMIC_SEQ_CST, ATOMIC_CONSUME)
        DEFINE_CAS_SIZES(MemoryOrdering::RELAXED, ATOMIC_SEQ_CST, ATOMIC_SEQ_CST)
    };
    ...
    for (const auto& test : tests) {
        ...
        out << CHECK << insn_name(insn_alloc) << " 5\n";
        if (isOrderingFenced(test.success) || isOrderingFenced(test.failure)) {
            out << CHECK << insn_name(insn_fence) << ' ' <<  MemoryOrdering2Str( MemoryOrdering::SEQ_CST) << '\n';
        }
        out << CHECK << InsnCounter(this, test.cas_insn) << " %r3, %r1, %r4, %r3, " << MemoryOrdering2Str(test.memory_ordering) << "\n";
        out << CHECK << insn_name(insn_retf) << " 0\n";
        ...
        out << test.type << " " << name.str()
                  ...
                  << ", " << test.type << " value) {\n";
        //if (test.sext_insn != insn_undef) {
        //    out << CHECK << insn_name(test.sext_insn) << " %r3, %r3\n";
        //}
        out << "    return __atomic_compare_exchange_n(ptr, exchange, value, true, "
                  << GetOrderingName(test.success) << ", " << GetOrderingName(test.failure) << ");\n";
        out << "}\n";
        out << '\n';
    }
}
...
{
#define TEST_TABLE_LOAD_OP_X(CMD_MO, OP, UOP, SIZE, ORDER, FUNC, SEXT) \
    {  OP ## SIZE, CMD_MO, ORDER, FUNC, SEXT,       "i" #SIZE }, \
    { UOP ## SIZE, CMD_MO, ORDER, FUNC, insn_undef, "u" #SIZE }, \

#define DEFINE_LOAD_OP_SIZES(CMD_MO, OP, UOP, ORDER, FUNC) \
    TEST_TABLE_LOAD_OP_X(CMD_MO, OP, UOP, 8,   ORDER, FUNC, insn_sext_i8) \
    TEST_TABLE_LOAD_OP_X(CMD_MO, OP, UOP, 16,  ORDER, FUNC, insn_sext_i16) \
    TEST_TABLE_LOAD_OP_X(CMD_MO, OP, UOP, 32,  ORDER, FUNC, insn_undef) \
    TEST_TABLE_LOAD_OP_X(CMD_MO, OP, UOP, 64,  ORDER, FUNC, insn_undef) \
    TEST_TABLE_LOAD_OP_X(CMD_MO, OP, UOP, 128, ORDER, FUNC, insn_undef) \

#define DEFINE_LOAD_OP_ORDERS(OP, UOP, FUNC) \
        DEFINE_LOAD_OP_SIZES(MemoryOrdering::RELAXED, OP, UOP, ATOMIC_RELAXED, FUNC) \
        DEFINE_LOAD_OP_SIZES(MemoryOrdering::ACQUIRE, OP, UOP, ATOMIC_ACQUIRE, FUNC) \
        DEFINE_LOAD_OP_SIZES(MemoryOrdering::ACQUIRE, OP, UOP, ATOMIC_CONSUME, FUNC) \
        DEFINE_LOAD_OP_SIZES(MemoryOrdering::RELEASE, OP, UOP, ATOMIC_RELEASE, FUNC) \
        DEFINE_LOAD_OP_SIZES(MemoryOrdering::ACQ_REL, OP, UOP, ATOMIC_ACQ_REL, FUNC) \
        DEFINE_LOAD_OP_SIZES(MemoryOrdering::RELAXED, OP, UOP, ATOMIC_SEQ_CST, FUNC) \
    ...
    static const struct {
        ...
        DEFINE_LOAD_OP_ORDERS(insn_amo_add_u, insn_amo_add_u, "atomic_fetch_add")
        DEFINE_LOAD_OP_ORDERS(insn_amo_sub_u, insn_amo_sub_u, "atomic_fetch_sub")
        DEFINE_LOAD_OP_ORDERS(insn_amo_and_u, insn_amo_and_u, "atomic_fetch_and")
        DEFINE_LOAD_OP_ORDERS(insn_amo_or_u,  insn_amo_or_u,  "atomic_fetch_or")
        DEFINE_LOAD_OP_ORDERS(insn_amo_xor_u, insn_amo_xor_u, "atomic_fetch_xor")
        DEFINE_LOAD_OP_ORDERS(insn_amo_min_i, insn_amo_min_u, "atomic_fetch_min")
        DEFINE_LOAD_OP_ORDERS(insn_amo_max_i, insn_amo_max_u, "atomic_fetch_max")
    };
    ...
    for (const auto& test : tests) {
        ...
        if (isOrderingFenced(test.ordering)) {
            out << CHECK_NEXT << insn_name(insn_fence) << ' ' <<  MemoryOrdering2Str( MemoryOrdering::SEQ_CST) << '\n';
            out << CHECK_NEXT << InsnCounter(this, test.atomic_insn) << " %r1, %r1, %r3, " <<  MemoryOrdering2Str(test.cmd_mo) << "\n";
            out << CHECK_EMPTY << '\n';
            out << CHECK_NEXT << "! BUNDLE\n";
            // atomic loads do zero-extend, add sext if needed
            if (test.sext_insn != insn_undef) {
        ...
        out << test.type << " " << name.str()
                  ...
                  << ", " << test.type << " *exchange"
                  << ", " << test.type << " value) {\n";
        out << "    return __" << test.func << "(ptr, value, " << GetOrderingName(test.ordering) << ");\n";
        out << "}\n";
        out << '\n';
    }
}
...
{
#define TEST_TABLE_STORE_OP_X(CMD_MO, OP, UOP, SIZE, ORDER, FUNC) \
    {  OP ## SIZE, CMD_MO, ORDER, FUNC, "i" #SIZE }, \
    { UOP ## SIZE, CMD_MO, ORDER, FUNC, "u" #SIZE }, \

#define DEFINE_STORE_OP_SIZES(CMD_MO, OP, UOP, ORDER, FUNC) \
    TEST_TABLE_STORE_OP_X(CMD_MO, OP, UOP, 8,   ORDER, FUNC) \
    TEST_TABLE_STORE_OP_X(CMD_MO, OP, UOP, 16,  ORDER, FUNC) \
    TEST_TABLE_STORE_OP_X(CMD_MO, OP, UOP, 32,  ORDER, FUNC) \
    TEST_TABLE_STORE_OP_X(CMD_MO, OP, UOP, 64,  ORDER, FUNC) \
    TEST_TABLE_STORE_OP_X(CMD_MO, OP, UOP, 128, ORDER, FUNC) \

#define DEFINE_STORE_OP_ORDERS(OP, UOP, FUNC) \
        DEFINE_STORE_OP_SIZES(MemoryOrdering::RELAXED, OP, UOP, ATOMIC_RELAXED, FUNC) \
        DEFINE_STORE_OP_SIZES(MemoryOrdering::RELEASE, OP, UOP, ATOMIC_RELEASE, FUNC) \
        DEFINE_STORE_OP_SIZES(MemoryOrdering::RELAXED, OP, UOP, ATOMIC_SEQ_CST, FUNC) \
    ...
    static const struct {
        ...
        DEFINE_STORE_OP_ORDERS(insn_amo_add_u, insn_amo_add_u, "atomic_fetch_add")
        DEFINE_STORE_OP_ORDERS(insn_amo_sub_u, insn_amo_sub_u, "atomic_fetch_sub")
        DEFINE_STORE_OP_ORDERS(insn_amo_and_u, insn_amo_and_u, "atomic_fetch_and")
        DEFINE_STORE_OP_ORDERS(insn_amo_or_u,  insn_amo_or_u,  "atomic_fetch_or")
        DEFINE_STORE_OP_ORDERS(insn_amo_xor_u, insn_amo_xor_u, "atomic_fetch_xor")
        DEFINE_STORE_OP_ORDERS(insn_amo_min_i, insn_amo_min_u, "atomic_fetch_min")
        DEFINE_STORE_OP_ORDERS(insn_amo_max_i, insn_amo_max_u, "atomic_fetch_max")
    };
    ...
    for (const auto& test : tests) {
        ...
        out << "void " << name.str()
                  ...
                  << ", " << test.type << " *exchange"
                  << ", " << test.type << " value) {\n";
        out << "    __" << test.func << "(ptr, value, " << GetOrderingName(test.ordering) << ");\n";
        out << "}\n\n";
    }
}
