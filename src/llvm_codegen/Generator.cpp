...
const char *ValueType2Str(ValueType type)
{
    switch (type) {
      ...
      case FQ: return "f128";
      case FX: return "F128";
      case VIB: return "v16i8";
      case VIH: return "v8i16";
      case VIW: return "v4i32";
      case VID: return "v2i64";
      case VUB: return "v16u8";
      case VUH: return "v8u16";
      case VUW: return "v4u32";
      case VUD: return "v2u64";
      case VFH: return "v8f16";
      case VFW: return "v4f32";
      case VFD: return "v2f64";
    }
    assert(!"unknown ValueType");
    return nullptr;
}
...
{
    out << "// RUN: clang %cflags " << params << " %s | FileCheck %s\n"
...
postrisc_flags = f'-mllvm --postrisc-enable-block-merging=1 -mllvm --postrisc-enable-dense-calls=1 -mllvm --postrisc-enable-move-merging=1'

cflags = f'--target={target} -mcpu={cpu} -std=c23 -fPIC -ffast-math -S -O2 -ffreestanding -fomit-frame-pointer -mllvm -verify-analysis-invalidation=0 {postrisc_flags} {MUSL_PATHS} -w -o -'
cxxflags = f'%cflags'
...
public:
    ...
    bool operator>(const InstructionSorter& r) const {
        return std::strcmp(to_string(id), to_string(r.id)) < 0;
    }
public:
    EInstructionID id;
};
...
#undef X_TABLE
    ...
    for (size_t i = 0; i < nontested.size(); i++) {
        std::cout << std::setw(20) << nontested[i].id;
        if (i % 8 == 7) std::cout << '\n';
    }
