...
CODEGEN_HANDLER(IntAlignUpU64)
{
    for(int align_size = 1; align_size < 64; align_size+=2) {
        ...
        for(int shift_size = 1; shift_size < align_size; shift_size+=2) {
            ...
            out << CHECK_LABEL << fname.str() << '\n';
            out << CHECK << insn_name(insn_alloc) << " 2\n";
            out << CHECK_NEXT << InsnCounter(this, insn_alignup_u64) << " %r1, %r1, " << shift_size << ", " << align_size << '\n';
            out << CHECK_NEXT << insn_name(insn_retf) << " 0\n";
...
CODEGEN_HANDLER(StackArray)
{
    ...
    for(size_t size = 1; size < 256; size++) {
        ...
        out << CHECK_LABEL << fname.str() << '\n';
        out << CHECK << InsnCounter(this, insn_alloc_sp) << " 5, " << util::roundup_pow2<size_t>(size, 16) << '\n';
        out << CHECK << InsnCounter(this, insn_call) << " %r2, %jpc(array_consumer)\n";
        out << CHECK << insn_name(insn_retf) << " " << util::roundup_pow2<size_t>(size, 16) << '\n';
...
CODEGEN_HANDLER(FunctionPointer)
{
    ...
    out << CHECK << InsnCounter(this, insn_call_ri) << " %r3, %r1, %gz\n";
    ...
    out << CHECK_LABEL << fname.str() << '\n';
    out << CHECK << InsnCounter(this, insn_alloc) << " 5\n";
    out << CHECK << InsnCounter(this, insn_call_mi) << " %r3, %r1, " << (offset * sizeof(u64)) << '\n';
    out << CHECK << insn_name(insn_retf) << " 0\n";
