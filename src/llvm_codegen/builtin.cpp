...
CODEGEN_HANDLER(BuiltinNoArgs)
{
    for (const auto& func : builtin_noargs_functions) {
        ...
        // out << "// CHECK-NEXT: " << insn_name(insn_retf) << " 0" << std::endl;
...
#undef SPECIAL_REGISTER_ENUM_X
    ...
    //{ "dcbf3",      insn_dcbf,    "void", "PVOID", "%r1, 1000000000",   "__builtin_postrisc_dcbf(x+1000000000)" },
    ...
    //{ "dcbt3",      insn_dcbt,    "void", "PVOID", "%r1, 1000000000",   "__builtin_postrisc_dcbt(x+1000000000)" },
    ...
    //{ "dcbi3",      insn_dcbi,    "void", "PVOID", "%r1, 1000000000",   "__builtin_postrisc_dcbi(x+1000000000)" },
    ...
    //{ "icbi3",      insn_icbi,    "void", "PVOID", "%r1, 1000000000",   "__builtin_postrisc_icbi(x+1000000000)" },
...
CODEGEN_HANDLER(BuiltinUnary)
{
    for (const auto& func : builtin_unary_functions) {
        ...
        out << "// CHECK-LABEL: @" << fname.str() << std::endl;
        out << "// CHECK: " << insn_name(insn_alloc) << " 2" << std::endl;
        out << "// CHECK-NEXT: " << insn_name(func.insn) << " " << func.args << std::endl;
        out << "// CHECK-NEXT: " << insn_name(insn_retf) << " 0" << std::endl;
