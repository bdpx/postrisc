...
#define DECLARE_CODEGEN_HANDLERS(X) \
    ...
    X(BranchOnBit, "") \
    X(SwitchTable, "") \
    X(SwitchTableNonzero, "") \
    X(BuiltinNoArgs, "") \
    X(BuiltinSpecialRegs, "") \
    X(BuiltinUnary, "") \
    X(BuiltinBinary, "") \
    X(BuiltinTernary, "") \
    X(DividePow2, "") \
    ...
    X(BitSelect, "") \
    X(ComputedGoto, "") \
    X(FpBranchCompare, "-mllvm=--postrisc-enable-nullification=0") \
    X(FpBranchCompareGoto, "-mllvm=--postrisc-enable-nullification=0") \
    X(FpBranchCompareStrict, "-ffp-model=strict -mllvm=--postrisc-enable-nullification=0") \
    X(FpUnary, "-Xclang -fnative-half-type") \
    X(FpUnaryRm, "-Xclang -fnative-half-type") \
    X(FpBinary, "-Xclang -fnative-half-type") \
    X(FpBinaryRm, "-Xclang -fnative-half-type") \
    X(FpTernary, "-Xclang -fnative-half-type") \
    X(FpTernaryRm, "-Xclang -fnative-half-type") \
    X(FpStrict, "-ffp-model=strict") \
    X(FpClassifyF16, "-ffp-model=strict -Xclang -fnative-half-type") \
    X(FpClassifyF32, "-ffp-model=strict -Xclang -fnative-half-type") \
    X(FpClassifyF64, "-ffp-model=strict -Xclang -fnative-half-type") \
    X(FpClassifyF128, "-ffp-model=strict -Xclang -fnative-half-type") \
    X(FunctionPointer, "") \
    X(IntUnary, "") \
    X(IntBinary, "") \
    X(IntAlignUpU64, "") \
    ...
    X(IntDeposit64, "") \
    X(IntTernary, "") \
    X(NullificationOnBit, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareRegReg, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareImmI32, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareImmI64, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareImmI128, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareImmU32, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareImmU64, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareImmU128, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyCompareZero, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyFpCompare, "-mllvm=--postrisc-enable-nullification=1") \
    X(NullifyFpCompareStrict, "-ffp-model=strict -mllvm=--postrisc-enable-nullification=1") \
    X(StackArray, "") \
    X(LoadBase, "") \
    X(StoreBase, "") \
    X(LoadStorePostInc, "-mllvm=--postrisc-enable-pre-post-update=1") \
...
enum ValueType {
     BOOL, VOID, PVOID, IB, UB, IH, UH, IW, UW, ID, UD, IQ, UQ, FH, FS, FD, FQ, FX,
     VIB, VIH, VIW, VUB, VUH, VUW, VID, VUD, VFH, VFW, VFD,
};
const char *ValueType2Str(ValueType type);

inline std::ostream& operator<<(std::ostream& out, ValueType type)
{
    return out << ValueType2Str(type);
}
...
private:
    ...
    void FpClassify(std::ostream& out, EInstructionID insn, ValueType type);
...
#undef XXX
    ...
    std::array<bool, INSTRUCTION_LIST_SIZE> m_used{};
