...
static void LIntUnary(std::ostream& out, const std::string& type, const std::string& op, const std::string& c_op)
{
    std::ostringstream name;
    name << "unary_" << op << "_" << type;

    out << "// CHECK-LABEL: @" << name.str() << std::endl;
    out << "// CHECK: " << op << "_" << type << " %r1, %r1" << std::endl;
...
template<typename ITYPE, typename UTYPE> static void IntRegImm(std::ostream& out, int size)
{
    static const struct {
         char asm_name[16];
         char c_fragment[16];
