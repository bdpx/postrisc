...
public:
    CRandomProvider() : distribution(0, std::numeric_limits<u64>::max())
    {}
...
CBinaryRandomGenerator::CompileRandomCode(CRandomProvider & random, CProgramSection & section)
{
    ...
    const u64 ip = section.size() + sizeof(CBundle);
    ...
    for (const EInstructionField field : asm_fmt_field[fmt]) {
        ...
        switch (field) {
            ...
            case cf_bin_simm:          cmd.set_bin_simm(r);                 break;
            case cf_bin_uimm:          cmd.set_bin_uimm(r);                 break;
            ...
            case cf_rip_label: {
                ...
                u64 rr = util::roundup(GetSection(ESectionId_code).global_size, bits_per_segment_offset) - ip;
...
i64
CBinaryRandomGenerator::GetRandomBranchDisplacement(u64 ip, u64 random, EInstructionSize size1, EInstructionFullSize size2)
{
    unsigned const nbits = GetImmediateSize(size1, size2);

    u64 const mod = GetSection(ESectionId_code).global_size - sizeof(CBundle);
    ...
    rr /= static_cast<i64>(sizeof(CBundle));
    LOG_DEBUG(LOG_PARSER) << " rr=" << rr << std::endl;
