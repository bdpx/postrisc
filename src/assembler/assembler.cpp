...
void
CAssembler::RegisterDefine(const char *name, u64 value)
{
...
CAssembler::make_nop()
{
    //unsigned const linenum = GetLineNum() & util::makemask(bits_per_call);
...
i64
CAssembler::ComputeBundledOffset(i64 offset) const
{
...
{
    if (GetToken() != CParserToken(operator_percent)) {
...
void
CAssembler::EmitInstruction(CProgramSection & section, u64 cmd)
{
...
void
CAssembler::EmitInstruction(CProgramSection & section, u64 cmd, u64 ext)
{
    LOG_DEBUG(LOG_PARSER)
...
**********************************************************************/
void
CAssembler::EmitLabel(CProgramSection & section, u64 cmd)
{
    switch (m_CurrentSlotNum) {
        case 0:
            break;
...
********************************************************************************/
i64
CAssembler::GetLabel(i64 ip, ESectionKind requiredSectionType)
{
...
void
CAssembler::SetForwardBranch(u64 uimm, CInsn2 & cmd)
{
    ...
    if (m_PassNumber == EAssemblerPass::link &&
        ...
        REPORT_ERROR(this)
            << "too long distance for forward branch: " << uimm;
    }

    if (m_PassNumber == EAssemblerPass::link &&
        uimm % forward_offset_alignment != 0) {
        REPORT_ERROR(this)
            << "forward branch distance " << uimm
            << " isn't aligned to " << forward_offset_alignment
            << " bytes";
    }
...
void
CAssembler::ReadonlyFloatForRelative(i64 ip, CInsn2 & cmd)
{
...
#define FIELD_HANDLER_IMPLEMENTATION(FIELD_NAME) \
    void CAssembler::FieldHandler_##FIELD_NAME(i64 ip, CInsn2 & cmd)
...
FIELD_HANDLER_IMPLEMENTATION(framesize)
{
    ...
    if (imm > max_stacked_framesize) {
        REPORT_ERROR(this) << "framesize=" << imm
                           << " exceed limit=" << max_stacked_framesize;
    }
...
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const shift_amout = GetVectorShiftAmount();
    LOG_DEBUG(LOG_PARSER) << "shift_amout(b)=" << fmt::dec(shift_amout);
...
FIELD_HANDLER_IMPLEMENTATION(memory_order)
{
    // mnem.xd, mnem.xw, mnem.xuw
    UNREFERENCED_PARAMETER(ip);
    if (GetCurrentToken() != CParserToken(operator_point)) {
        cmd.SetMemoryOrder(EMemoryOrder::relaxed);
        return;
    }
    PopToken();
    ...
    const u32 tdata = token.GetData();
    if (ttype != token_identifier) {
        REPORT_ERROR(this) << "invalid memory order, token = (" << ttype << "," << tdata << ")";
    }
    const std::string & cmd_name = GetIdentifierInfo(tdata).GetName();

           if (cmd_name == "relaxed") { cmd.SetMemoryOrder(EMemoryOrder::relaxed);
    } else if (cmd_name == "acquire") { cmd.SetMemoryOrder(EMemoryOrder::acquire);
    } else if (cmd_name == "release") { cmd.SetMemoryOrder(EMemoryOrder::release);
    } else if (cmd_name == "acq_rel") { cmd.SetMemoryOrder(EMemoryOrder::acq_rel);
    } else if (cmd_name == "seq_cst") { cmd.SetMemoryOrder(EMemoryOrder::seq_cst);
    } else {
...
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const nbits = GetImmediateSize(bits_per_call, bits_per_ldi_long);
    const i64 imm = EvaluateInt(nbits, eval_signed, 0);
...
FIELD_HANDLER_IMPLEMENTATION(brc_simm_plus)
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const nbits = GetImmediateSize(bits_per_brci_imm, bits_per_brci_imm_long);
    i64 imm = EvaluateInt(nbits, eval_signed, 1);
    imm = util::sext64(imm, nbits);
    cmd.set_brc_simm(imm);
}
...
FIELD_HANDLER_IMPLEMENTATION(brc_uimm_plus)
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const nbits = GetImmediateSize(bits_per_brci_imm, bits_per_brci_imm_long);
...
FIELD_HANDLER_IMPLEMENTATION(bin_simm_plus)
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const nbits = GetImmediateSize(bits_per_binimm, bits_per_binimm_long);
    i64 imm = EvaluateInt(nbits, eval_signed, 1);
...
FIELD_HANDLER_IMPLEMENTATION(bin_simm_minus)
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const nbits = GetImmediateSize(bits_per_binimm, bits_per_binimm_long);
    i64 imm = EvaluateInt(nbits, eval_signed, -1);
...
FIELD_HANDLER_IMPLEMENTATION(bin_uimm_plus)
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const nbits = GetImmediateSize(bits_per_binimm, bits_per_binimm_long);
    u64 imm = EvaluateInt(nbits, eval_unsigned, 1);
...
FIELD_HANDLER_IMPLEMENTATION(bin_uimm_minus)
{
    UNREFERENCED_PARAMETER(ip);
    unsigned const nbits = GetImmediateSize(bits_per_binimm, bits_per_binimm_long);
    u64 imm = EvaluateInt(nbits, eval_unsigned, -1);
...
FIELD_HANDLER_IMPLEMENTATION(indexed_scale_mode)
{
    // mnem.xd, mnem.xw, mnem.xuw
    UNREFERENCED_PARAMETER(ip);
    if (GetCurrentToken() != CParserToken(operator_point)) {
        cmd.SetIndexedScaleMode(scale_mode_dword);
        return;
    }
    PopToken();
    ...
    if (cmd_name == "xd") {
        ...
        cmd.SetIndexedScaleMode(scale_mode_sext_word);
    } else if (cmd_name == "xuw") {
        cmd.SetIndexedScaleMode(scale_mode_zext_word);
    } else if (cmd_name == "l") {
        // there is no indexed mode
        // set default `xd`
        cmd.SetIndexedScaleMode(scale_mode_dword);
        // and rollback comma and long instructioon pefix
        PutBackToken();
        PutBackToken();
    } else {
        REPORT_ERROR(this) << "invalid indexed scale mode '" << cmd_name << "'";
    }
}
...
#define FIELD_HANDLER_ADDRESS(field, size, disp, fieldtype, inst_name, descr) \
    &CAssembler::FieldHandler_##field,
...
***************************************************************************/
void
CAssembler::CompileCode(i64 ip, CInsn2 & cmd)
{
...
CAssembler::CompileData(CProgramSection & section)
{
    ...
    switch (mid) {
        ...
        case asm_float:   EvaluateFloat<float,  u32>(section);       break;
        case asm_double:  EvaluateFloat<double, u64>(section);       break;
        //case asm_extend:  EvaluateFloat<extended, u128>(section);      break;
        case asm_quad:    EvaluateFloat<quadruple, u128>(section);   break;
...
CAssembler::AssembleForCodeSection(CProgramSection & section)
{
    while (IsNextRowExist()) {
        ...
        CInsn2 cmd(0,0);
        i64 const ip = section.ComputeNextIp();
        // find possible label
