...
class CParserToken {
public:
    explicit constexpr CParserToken(void) : value(0) {}
    explicit constexpr CParserToken(ETokenKind type, size_t data) : value(static_cast<u32>(data) | (static_cast<u32>(type) << data_bits)) {}
    explicit constexpr CParserToken(EOperatorKind op) : value(op | (static_cast<u32>(token_operator) << data_bits)) {}
...
public:
    explicit CAssemblerIdentifier(const std::string& _name) : name(_name) {}
...
public:
    ...
    CAssemblerIdentifier& GetIdentifierInfo(u32 ident) { return m_Idents[ident]; }
    const CAssemblerIdentifier& GetIdentifierInfo(u32 ident) const { return m_Idents[ident]; }
...
class CErrorHandler {
public:
    explicit CErrorHandler(const CTokenizer *p) : ctrl(p) {}
...
public:
    explicit CWarningHandler(const CTokenizer *p) : ctrl(p) {}
    void operator+=(Formatter& formatter) {
...
class CTokenPrinter {
public:
    explicit CTokenPrinter(const CTokenizer& _assembler, CParserToken _token)
