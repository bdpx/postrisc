...
***************************************************************************/
i64
CAssembler::EvaluateInt(unsigned int nbits, eval_mode_type evalmode, i64 addval)
{
    int  level = 0;
    ...
    for (;;) {
        ...
        LOG_DEBUG(LOG_EVALUATE) << "next token: " << CTokenPrinter(*this, token);
...
i64
CAssembler::EvaluatePostfixInt(const std::vector<CParserToken>& postfix, eval_mode_type evalmode) const
{
