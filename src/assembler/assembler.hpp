...
public:
    explicit CAssembler(const Decoder& decoder);
    ~CAssembler(void);
...
private:
    ...
    i64 EvaluatePostfixInt(const std::vector<CParserToken>& postfix, eval_mode_type evalmode) const;
    ...
    template<class SIGNED> void EvaluateSigned(CProgramSection& section);
