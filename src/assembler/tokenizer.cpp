...
public:
    ...
    void echo_operator(EOperatorKind name) override {
        output_string(CParserToken::operator_names[name].c_str());
    }
...
u64
CTokenizer::GetHexadecimal(u32 tokendata) const
{
    u64 imm  = 0;
...
u64
CTokenizer::GetChar(u32 tokendata) const
{
    u64 result = 0;
...
u64
CTokenizer::GetDecimal(u32 tokendata) const
{
     u64 imm = 0;
...
u64
CTokenizer::GetOctal(u32 tokendata) const
{
    u64 imm = 0;
    const char * buf = m_TextStorage.GetData(tokendata);
...
u64
CTokenizer::GetBinary(u32 tokendata) const
{
    u64 imm = 0;
...
quad_type
CTokenizer::GetFloat(u32 tokendata) const
{
    const char * buf = m_TextStorage.GetData(tokendata);
