...
STATIC_ASSERT(std::numeric_limits<std::streamoff>::max() == std::numeric_limits<int64_t>::max());

namespace postrisc {

class IFormatterInterface {
public:
    virtual ~IFormatterInterface() {};
...
*************************************************************************/
class CFormatter : public IFormatterInterface {
public:
    CFormatter(void) {}
    ~CFormatter(void) override {}
...
const unsigned int end_of_chain = ~0;

/***************************************************************************
* this function load data from any stream (don't used file-specific)
***************************************************************************/
void
CScanner::read(std::istream & input)
{
    m_Source.clear();
...
CParserToken
CTokenizer::GetCurrentToken(unsigned offset) const
{
    return m_TokenStorage[m_token_pos+offset];
}

void
CTokenizer::PopToken(void)
{
    ++m_token_pos;
}

void
CTokenizer::PutBackToken(void)
{
    --m_token_pos;
}

CParserToken
CTokenizer::GetToken(void)
{
    CParserToken token = m_TokenStorage[m_token_pos];
    LOG_DEBUG(LOG_PARSER) << "pop token, type=" << fmt::dec(token.GetType());
    ++m_token_pos;
    return token;
}
...
***************************************************************************/
bool
CTokenizer::IsNextRowExist(void)
{
    ...
    if (m_TokenStorage[m_token_pos] == CParserToken(token_eof, 0))
       return false;
...
***********************************************************************/
void
CTokenizer::report_error(std::ostringstream & out) const
{
    ...
    LOG_DEBUG(LOG_EVALUATE)
        << "tokennum=" << tokennum
        << CTokenPrinter(this, m_TokenStorage[tokennum]);
...
void
CTokenizer::report_warning(std::ostringstream & out) const
{
...
CParserToken
CTokenizer::AddIdentifier(const std::string & name)
{
    const auto it = m_MapNameToIdent.find(name);
    if (it != m_MapNameToIdent.end()) {
        return CParserToken(token_identifier, it->second);
    }

    // new identifier
    const uint32_t id = m_Idents.size();
    m_MapNameToIdent[name] = id;
    m_Idents.emplace_back(name);
    return CParserToken(token_identifier, id);
}

CParserToken
CTokenizer::TokenizeIdentifier(IFormatterInterface & formatter, int chr)
{
    std::string name;
    ...
    CParserToken token = AddIdentifier(name);
    formatter.echo_ident_old(name);
    return token;
}

CParserToken
CTokenizer::TokenizeMantissa(IFormatterInterface & UNUSED(formatter), size_t txtpos)
{
    return CParserToken(token_real, txtpos);
}

CParserToken
CTokenizer::TokenizeDecimal(IFormatterInterface & formatter, size_t txtpos)
{
    ...
    return CParserToken(token_type, txtpos);
}

CParserToken
CTokenizer::TokenizeConstant(IFormatterInterface & formatter, int chr)
{
    CParserToken token;
    ...
    if (chr == '0') {
        ...
        if (chr == 'x' || chr == 'X') {
            ...
            m_Scanner.Unget(chr);
            m_TextStorage.Store(0);
            token = CParserToken(token_hexadecimal, txtpos);
            ...
            m_Scanner.Unget(chr);
            m_TextStorage.Store(0);
            token = CParserToken(token_binary, txtpos);
            formatter.echo_hexadecimal(m_TextStorage.GetData(txtpos));
            ...
            token = CParserToken(token_octal, txtpos);
        } else {
            m_Scanner.Unget(chr);
...
CParserToken
CTokenizer::TokenizeOperator(IFormatterInterface & formatter, EOperatorKind op)
{
    formatter.echo_operator(op);
    return CParserToken(op);
}

CParserToken
CTokenizer::TokenizeOperator(IFormatterInterface & formatter, int chr)
{
    switch (chr) {
    ...
    return CParserToken(operator_here);
}

CParserToken
CTokenizer::TokenizeString(IFormatterInterface & formatter)
{
    const size_t txtpos = m_TextStorage.GetCurrentPosition();
    ...
    return CParserToken(token_string, txtpos);
}

CParserToken
CTokenizer::TokenizeChar(IFormatterInterface & formatter)
{
    const size_t txtpos = m_TextStorage.GetCurrentPosition();
    ...
    return CParserToken(token_character, txtpos);
}

int
CTokenizer::TokenizeSpaces(IFormatterInterface & formatter)
{
    int chr;
        ...
        if (chr == '/') {
            ...
            if (chr == '*') {
                ...
                while (chr = m_Scanner.Get(), chr >= 0) {
                    formatter.echo_char(chr);
                    if (chr == '\n') {
                        m_TokenStorage.push_back(CParserToken(token_eol, m_Scanner.GetPosition()));
                        m_linenum++;
                    }
                    if (chr == '/' && oldchr == '*') {
                        break;
...
int
CTokenizer::TokenizeInput(std::istream & input)
{
...
int
CTokenizer::TokenizeInput(IFormatterInterface & formatter, std::istream & input)
{
    ...
    m_TokenStorage.push_back(CParserToken(token_eol, 0));

    int chr;
    while (chr = TokenizeSpaces(formatter), chr >= 0) {
        CParserToken token(token_eof, 0);
    ...
    m_TokenStorage.push_back( CParserToken( token_eol, m_Scanner.GetPosition() ) );
    m_TokenStorage.push_back( CParserToken( token_eof, 0 ) );
...
************************************************************************/
void
CTokenizer::PrintToken(std::ostream & out, CParserToken token) const
{
    const ETokenKind    type = token.GetType();
    const uint32_t      data = token.GetData();

    out << CParserToken::token_names[type]
        << ' ' << std::hex << std::setfill('0') << std::setw(7) << data << std::dec;
    ...
    switch (type) {
        ...
        case token_operator:
        case token_unary:
            out << ' ' << CParserToken::operator_names[data] << '\n';
            break;
...
void
CTokenizer::PrintTokens(std::ostream& out, const std::vector<CParserToken> & tokens, int flags) const
{
    ...
    if (flags) {
       ...
       if (ntokens > 0) {
          ...
          for (size_t i = 0; i<counters.size(); i++) {
              out << CParserToken::token_names[i]
...
const std::array<std::string, token_last> CParserToken::token_names =
{
#define TOKEN_TEXT_X(enumeration, text) text,
    DECLARE_TOKEN_TABLE(TOKEN_TEXT_X)
};

const std::array<std::string, operator_last> CParserToken::operator_names =
{
...
*************************************************************************/
int
CTokenizer::Scan(std::istream& input, std::ostream& out)
{
    CFormatter formatter;
...
*************************************************************************/
class CHtmlFormatter : public IFormatterInterface {
public:
    ~CHtmlFormatter() override {}
    CHtmlFormatter(std::ostream& o) : out(o) {}

    void echo_char(char chr) override                 { out << chr; }
    void echo_comment_start(void) override            { out << "<span class=\"comment\">"; }
    void echo_comment_stop(void) override             { out << "</span>"; }
    void echo_keyword(const char *name) override      { out << "<span class=\"keyword\">" << (name) << "</span>"; }
    void echo_ident_new(const std::string & name) override    { out << "<span class=\"ident\">" << (name) << "</span>"; }
    void echo_ident_old(const std::string & name) override    { out << "<span class=\"ident\">" << (name) << "</span>"; }
    void echo_hexadecimal(const char *name) override  { out << "<span class=\"hex\">" << (name) << "</span>"; }
    void echo_octal(const char *name) override        { out << "<span class=\"oct\">" << (name) << "</span>"; }
    void echo_decimal(const char *name) override      { out << "<span class=\"dec\">" << (name) << "</span>"; }
    void echo_real(const char *name) override         { out << "<span class=\"flt\">" << (name) << "</span>"; }
    void echo_string(const char *name) override       { out << "<span class=\"str\">&quot;" << (name) << "&quot;</span>"; }
    void echo_operator(EOperatorKind name) override   { out << CParserToken::operator_names[name]; }
    void echo_character(char name) override           { out << "<span class=\"chr\">" << (name) << "</span>"; }
    void echo_character(const char *name) override    { out << "<span class=\"chr\">" << (name) << "</span>"; }
...
int
CTokenizer::ScanHtml(std::istream& input, std::ostream& out)
{
    CHtmlFormatter formatter(out);

    xhtml_header(out, "html-program");
    out << "<pre>";
...
***************************************************************************/
void
CTokenizer::GetString(CProgramSection & seg)
{
...
uint64_t
CTokenizer::GetHexadecimal(uint32_t tokendata) const
{
    uint64_t imm  = 0;
...
uint64_t
CTokenizer::GetChar(uint32_t tokendata) const
{
    uint64_t result = 0;
...
uint64_t
CTokenizer::GetDecimal(uint32_t tokendata) const
{
     uint64_t imm = 0;
...
uint64_t
CTokenizer::GetOctal(uint32_t tokendata) const
{
    uint64_t imm = 0;
    const char * buf = m_TextStorage.GetData(tokendata);
...
uint64_t
CTokenizer::GetBinary(uint32_t tokendata) const
{
    uint64_t imm = 0;
...
mpfr::mpreal
CTokenizer::GetFloat(uint32_t tokendata) const
{
    const char * buf = m_TextStorage.GetData(tokendata);
