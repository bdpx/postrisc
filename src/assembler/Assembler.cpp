...
FIELD_HANDLER_IMPLEMENTATION(memory_ordering)
{
    ...
    CompileMemoryOrdering(cmd);
}

FIELD_HANDLER_IMPLEMENTATION(optional_memory_ordering)
{
    UNREFERENCED_PARAMETER(ip);

    // optional argument
    if (GetCurrentToken() != ParserToken(operator_comma)) {
        cmd.SetMemoryOrdering(MemoryOrdering::Default);
        return;
    }
    PopToken(); // skip comma
...
FIELD_HANDLER_IMPLEMENTATION(rounding_mode)
{
    ...
    if (GetCurrentToken() != ParserToken(operator_comma)) {
        cmd.SetRoundingMode(RoundingMode::Default);
        return;
    }
    ...
    const u32 ident = GetIdentifier();
    const std::string& name = GetIdentifierInfo(ident).GetName();

    static const RoundingMode rms[] = {
        RoundingMode::DYN,
        RoundingMode::RNE,
        RoundingMode::RDN,
        RoundingMode::RUP,
    ...
    for (const RoundingMode rm : rms) {
        if (RoundingMode2Str(rm) == name) {
            LOG_DEBUG(LOG_PARSER) << "rm=" << rm;
            cmd.SetRoundingMode(rm);
            return;
        }
    }

    REPORT_ERROR(this) << "unknown rounding mode" << name;
    return;
}
...
FIELD_HANDLER_IMPLEMENTATION(fpclass)
{
    UNREFERENCED_PARAMETER(ip);
    u64 const imm = EvaluateInt(bits_per_update_stride, eval_unsigned, 0);
    LOG_DEBUG(LOG_PARSER) << "uimm10=" << imm;
    cmd.SetFpClassifyMask(imm);
}
...
FIELD_HANDLER_IMPLEMENTATION(stride_simm_low)
{
    UNREFERENCED_PARAMETER(ip);
    UNREFERENCED_PARAMETER(cmd);
}
...
FIELD_HANDLER_IMPLEMENTATION(update_stride_low)
{
    UNREFERENCED_PARAMETER(ip);
    UNREFERENCED_PARAMETER(cmd);
}
...
FIELD_HANDLER_IMPLEMENTATION(fldis)
{
    UNREFERENCED_PARAMETER(ip);
    const float f = static_cast<float>(EvaluateFloat());
...
FIELD_HANDLER_IMPLEMENTATION(fldid)
{
    UNREFERENCED_PARAMETER(ip);
...
// branch distance for branch/loadaddress instructions
FIELD_HANDLER_IMPLEMENTATION(ldarc)
{
    const i64 imm = ComputeBundledOffset(
...
void Assembler::CompileMemoryOrdering(InstructionDouble& cmd)
{
    const u32 ident = GetIdentifier();
    const std::string& name = GetIdentifierInfo(ident).GetName();

    static const MemoryOrdering rms[] = {
    ...
    for (const MemoryOrdering rm : rms) {
        if (MemoryOrdering2Str(rm) == name) {
            LOG_DEBUG(LOG_PARSER) << "rm=" << rm;
            cmd.SetMemoryOrdering(rm);
            return;
        }
    }

    REPORT_ERROR(this) << "unknown memory ordering" << name;
    return;
}
...
Assembler::CompileCode(i64 ip, InstructionDouble& cmd)
{
    ...
    auto instr_length = cmd_short;
    //if (ident.ends_with(".l")) {
    //    instr_length = cmd_long;
    ...
    SetInstructionLength(instr_length);
