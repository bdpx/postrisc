...
public:
    ...
    template<typename FLOAT> bool compare_ordered_le  (FLOAT a, FLOAT b) { return !__builtin_isunordered(a.hard(), b.hard()) && a.hard() <= b.hard(); }
    ...
    template<typename FLOAT> bool compare_unordered_le(FLOAT a, FLOAT b) { return  __builtin_isunordered(a.hard(), b.hard()) || a.hard() <= b.hard(); }
...
template<typename FLOAT> FLOAT CHardFPU::abs(FLOAT a)
{
    const typename FLOAT::F_UINT tmp = a.v & ~(typename FLOAT::F_UINT{1} << (FLOAT::F_SIZE - 1));
    return FLOAT{tmp};
}

template<typename FLOAT> FLOAT CHardFPU::chs(FLOAT a)
{
    const typename FLOAT::F_UINT tmp = a.v ^ (typename FLOAT::F_UINT{1} << (FLOAT::F_SIZE - 1));
    return FLOAT{tmp};
}

template<typename FLOAT> FLOAT CHardFPU::nabs(FLOAT a)
{
    const typename FLOAT::F_UINT tmp = a.v | (typename FLOAT::F_UINT{1} << (FLOAT::F_SIZE - 1));
    return FLOAT{tmp};
}
