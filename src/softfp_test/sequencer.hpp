...
public:
    ...
    template<class T> void start(T& a);
    template<class T> void start(T& a, T& b);
    template<class T> void start(T& a, T& b, T& c);

    // get next values in sequence
    template<class T> void next(T& a);
    template<class T> void next(T& a, T& b);
    template<class T> void next(T& a, T& b, T& c);

public:
    template<typename T> bool is_ok(T trueZ, fp_flags trueFlags, T testZ, fp_flags testFlags);

    template<class RESULT, class T, class OP, class TRUE_FPU, class TEST_FPU> void test_1 (TRUE_FPU& true_fpu, TEST_FPU& test_fpu, char const *name);
    template<class RESULT, class T, class OP, class TRUE_FPU, class TEST_FPU> void test_1a(TRUE_FPU& true_fpu, TEST_FPU& test_fpu, char const *name);
    template<class RESULT, class T, class OP, class TRUE_FPU, class TEST_FPU> void test_2 (TRUE_FPU& true_fpu, TEST_FPU& test_fpu, char const *name);
    template<class RESULT, class T, class OP, class TRUE_FPU, class TEST_FPU> void test_3 (TRUE_FPU& true_fpu, TEST_FPU& test_fpu, char const *name);
...
template<class T> void TestSequencer::next(T& a)
{
    sequenceA.next(a, 1);
}

template<class T> void TestSequencer::next(T& a, T& b)
{
    sequenceA.next(a, 1);
    sequenceB.next(b, 5);
}

template<class T> void TestSequencer::next(T& a, T& b, T& c)
{
    sequenceA.next(a, 1);
    sequenceB.next(b, 5);
    sequenceC.next(c, 7);
