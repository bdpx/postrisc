...
* template op helpers
**************************************************************************************/
class add    { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a, SRC b) { return fpu.add(a, b, fpu.get_rounding_mode()); }};
class sub    { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a, SRC b) { return fpu.sub(a, b, fpu.get_rounding_mode()); }};
class mul    { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a, SRC b) { return fpu.mul(a, b, fpu.get_rounding_mode()); }};
class div    { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a, SRC b) { return fpu.div(a, b, fpu.get_rounding_mode()); }};
...
class muladd { public: template<typename DST, typename SRC, class FPU>
    static DST func(FPU& fpu, SRC a, SRC b, SRC c) { return fpu.muladd(a, b, c,  fpu.get_rounding_mode(), fpu::FLOAT_MULADD_NEGATE_NONE); }};

class exp     { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a) { return fpu.exp(a,  fpu.get_rounding_mode()); }};
class sqrt    { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a) { return fpu.sqrt(a,  fpu.get_rounding_mode()); }};
class log2    { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a) { return fpu.log2(a,  fpu.get_rounding_mode()); }};
