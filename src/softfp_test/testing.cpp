...
static void writeValue(std::ostream& out, const f16 f)
{
    out << std::setfill('0') << std::hex
        << std::setw(2) << (f.val() >> f.MANT_SIZE)
        << '.'
        << std::setw(3) << f.get_fraction()
        << ' '
...
{
    out << std::setfill('0') << std::hex
        << std::setw(3) << (f.val() >> f.MANT_SIZE)
        << '.'
        << std::setw(6) << f.get_fraction()
        << ' '
...
static void writeValue(std::ostream& out, const f128 f)
{
    ...
    out << std::setfill('0') << std::hex
        << std::setw(4) << (u32)(f.val() >> f.MANT_SIZE)
        << '.'
        << std::setw(12) << (u64)(f.get_fraction() >> 64)
        << std::setw(16) << (u64)(f.get_fraction())
        << ' '
...
class compare_ordered_le   { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a, SRC b) { return fpu.compare_ordered_le  (a, b); }};
...
class compare_unordered_le { public: template<typename DST, typename SRC, class FPU> static DST func(FPU& fpu, SRC a, SRC b) { return fpu.compare_unordered_le(a, b); }};
...
template<typename T> std::ostream& operator<<(std::ostream& out, const CResult<T>& r);

template<typename T> class CResult {
public:
    CResult(T _v, fp_flags _flags) : v(_v), flags(_flags) {}
    friend std::ostream& operator<< <> (std::ostream& out, const CResult& r);
private:
    T        v;
    fp_flags flags;
};
...
template<typename T>
std::ostream& operator<<(std::ostream& out, const CResult<T>& r)
{
    out << r.flags << ' ';
    writeValue(out, r.v);
    out << fmt::endl;
...
template<class T> class CInputData_1 {
public:
    CInputData_1(T _a) : a(_a) {}
    friend std::ostream& operator<<(std::ostream& out, const CInputData_1& r) {
...
template<class T> class CInputData_2 {
public:
    CInputData_2(T _a, T _b) : a(_a), b(_b) {}
    friend std::ostream& operator<<(std::ostream& out, const CInputData_2& r) {
        out << "1: ";  writeValue(out, r.a);  out << fmt::endl;
...
template<class T> class CInputData_3 {
public:
    CInputData_3(T _a, T _b, T _c) : a(_a), b(_b), c(_c) {}
    friend std::ostream& operator<<(std::ostream& out, const CInputData_3& r) {
        out << "1: ";  writeValue(out, r.a);  out << fmt::endl;
        out << "2: ";  writeValue(out, r.b);  out << fmt::endl;
...
template<class RESULT, class T, class OP, class TRUE_FPU, class TEST_FPU>
void
    ...
    while ((count < total) || TestController::forever) {
        ...
        if (!ok) {
            std::cout << "error in " << name << fmt::endl
                      << CInputData_1(a)
                      << CResult(trueZ, trueFlags)
                      << CResult(testZ, testFlags)
                      << fmt::endl;
            ++errorCount;
...
#define DEFINE_TEST_RM(Templt, DST, SRC, OP) \
    FPU_RM_TESTING_TEST_CASE(CHardFPU, CSoftFPU, Templt, DST, SRC, OP)
...
#define DEFINE_TEST(Templt, DST, SRC, OP) \
    FPU_TESTING_TEST_CASE(CHardFPU, CSoftFPU, Templt, DST, SRC, OP)
...
DEFINE_TEST(2,  bool,  f16,   compare_ordered_le)
...
DEFINE_TEST(2,  bool,  f16,   compare_unordered_le)
...
DEFINE_TEST(2,  bool,  f32,   compare_ordered_le)
...
DEFINE_TEST(2,  bool,  f32,   compare_unordered_le)
...
DEFINE_TEST(2,  bool,  f64,   compare_ordered_le)
...
DEFINE_TEST(2,  bool,  f64,   compare_unordered_le)
...
DEFINE_TEST(2,  bool,  f128,  compare_ordered_le)
...
DEFINE_TEST(2,  bool,  f128,  compare_unordered_le)
