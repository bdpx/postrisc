...
template<> f16  HardFPU::muladd< f16>( f16  a, f16  b, f16  c, RoundingModeEnum, muladd_negate_t ) { return  f16::soft( (f16::F_HARD)fmaf( a.hard(), b.hard(), c.hard() )); }
template<> f32  HardFPU::muladd< f32>( f32  a, f32  b, f32  c, RoundingModeEnum, muladd_negate_t ) { return  f32::soft( fmaf( a.hard(), b.hard(), c.hard() )); }
template<> f64  HardFPU::muladd< f64>( f64  a, f64  b, f64  c, RoundingModeEnum, muladd_negate_t ) { return  f64::soft( fma ( a.hard(), b.hard(), c.hard() )); }
template<> f128 HardFPU::muladd<f128>( f128 a, f128 b, f128 c, RoundingModeEnum, muladd_negate_t ) { return f128::soft( (native::f128)FMAQ( a.hard(), b.hard(), c.hard() )); }

template<> f16  HardFPU::sqrt<f16> ( f16  a, RoundingModeEnum) { return f16 ::soft( (f16::F_HARD)::sqrtf( a.hard() ) ); }
template<> f32  HardFPU::sqrt<f32> ( f32  a, RoundingModeEnum) { return f32 ::soft( ::sqrtf( a.hard() ) ); }
template<> f64  HardFPU::sqrt<f64> ( f64  a, RoundingModeEnum) { return f64 ::soft( ::sqrt ( a.hard() ) ); }
template<> f128 HardFPU::sqrt<f128>( f128 a, RoundingModeEnum) { return f128::soft( (native::f128)SQRTQ( a.hard() ) ); }

template<> f32 HardFPU::exp<f32>( f32 a, RoundingModeEnum) { return f32::soft( ::expf( a.hard() ) ); }
template<> f64 HardFPU::exp<f64>( f64 a, RoundingModeEnum) { return f64::soft( ::exp ( a.hard() ) ); }
//    f128 exp2( f128 a ) { f128 z;  z.hard() = ::exp2q( a.hard() );   return z; }

template<> f32  HardFPU::log2<f32>( f32  a, RoundingModeEnum) { return f32::soft( ::log2f( a.hard() ) ); }
template<> f64  HardFPU::log2<f64>( f64  a, RoundingModeEnum) { return f64::soft( ::log2 ( a.hard() ) ); }
template<> f128 HardFPU::log2<f128>( f128 a, RoundingModeEnum) { return f128::soft( (native::f128)LOG2Q( a.hard() ) ); }
