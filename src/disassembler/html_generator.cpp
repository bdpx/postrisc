...
static void WriteColorMap(std::ostream& out)
{
    ...
    for (size_t i = 0; i < sizearray(instr_subset_name); i++) {
        unsigned const base = 175;
        unsigned const diff =  75;
        out << "td." << instr_subset_name[i]
            << " {text-align:center; color:rgb(40,10,40); background-color:rgb(";
        unsigned red   = base + (offset * 57) % diff;
        unsigned green = base + (offset * 31) % diff;
        unsigned blue  = base + (offset * 47) % diff;
        out << (red   & 0xFF) << ',';
        out << (green & 0xFF) << ',';
        out << (blue  & 0xFF) << "); }\n";
        offset += 111;
...
void xhtml_header(std::ostream& out, const char *title, const char *description, bool colormap)
{
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (mnem != insn_illegal) {
           ...
           if (mnem >= insn_last)
              out << "_table";
           out << "\">";
           size_t const len = strlen(asm_name[mnem]);
           if (len > max_name_len) {
             out << std::string_view(asm_name[mnem], max_name_len) << "&hellip;";
           } else {
             out << asm_name[mnem];
           }
           out << "</a><br/>" << i << "</td>\n";
...
int Decoder::GenerateHtml(std::ostream& out)
{
    ...
    xhtml_header(out, ARCH_NAME " virtual processor instruction set",
                      ARCH_NAME " virtual processor instruction set generated manual", true);
    ...
    ****************************************************************************/
    ...
    for (const auto & insn : asm_names) {
        ...
        for (size_t i = 0; i < asm_fmt_field[format].size(); i++) {
            const EInstructionField field = asm_fmt_field[format][i];
            if (field == cf_none)
                break;
            switch (field) {
            case cf_lprefix:
            case cf_indexed_scale_mode:
            case cf_memory_order:
                out << '.';
                break;
            case cf_argsdelimiter:
                if (asm_fmt_field[format][i] != cf_none) out << ' ';
                break;
            default:
                break;
            }
        ...
        EInstructionID realInsnId = Decode(CInsn1(asm_info[inst].tmplt));
        if (inst != realInsnId) {
            out << "*";
        }
        ...
        for (int pos = disp_opcode, i = 0; pos > 0; i++) {
            ...
            if (field_size > 0) {
               ...
               if (field_type == inst_fieldtype_opx) {
                  ...
                  out << "<a href=\"#" << asm_name[arch_info_opcode[opcode]]
                      << "_table\">" << opx << "</a>";
               } else {
                  out << inst_field_name[field];
               }
    ...
    ****************************************************************************/
    ...
    unsigned int opcodes = codetable<1 << bits_per_opcode>(out,
        arch_info_opcode, "opcode", "primary opcodes");

    int num_opcodes_with_extcode = 0;
    unsigned int opc_name[16], opx_num[16];

#define OPC_TABLE(NAME, NBITS)                                        \
    opc_name[num_opcodes_with_extcode] = insn_##NAME;                 \
    opx_num[num_opcodes_with_extcode++] = codetable<1 << NBITS>       \
          (out, arch_info_ ## NAME, asm_name[insn_##NAME],   \
                "The &laquo;" #NAME "&raquo; extended opcodes");

    OPC_TABLE(raopx,    bits_per_register)
    OPC_TABLE(fused,    bits_per_fused_opx)
    OPC_TABLE(nullrr,   bits_per_branch_opx)
    OPC_TABLE(nulfss,   bits_per_branch_opx)
    OPC_TABLE(nulfsd,   bits_per_branch_opx)
    OPC_TABLE(nulfsq,   bits_per_branch_opx)
    OPC_TABLE(brr,      bits_per_branch_opx)
    OPC_TABLE(loop,     bits_per_branch_opx)
    OPC_TABLE(bfss,     bits_per_branch_opx)
    OPC_TABLE(bfsd,     bits_per_branch_opx)
    OPC_TABLE(bfsq,     bits_per_branch_opx)
    OPC_TABLE(misc,     bits_per_misc_opx)
    ...
    *****************************************************************************/
    ...
    out << "<p></p>\n"
           "<hr/>\n"
           "<p class=\"copyright\">" ARCH_NAME " 64-bit virtual processor</p>\n"
           ...
           "<p class=\"copyright\">"
               "<a href=\"https://validator.w3.org/check?uri=https://bdpx.github.io/postrisc/gen.html\">\n"
                   "<img src=\"http://www.w3.org/Icons/valid-xhtml11.png\" height=\"31\" width=\"88\" alt=\"Valid XHTML 1.1!\"/>\n"
               "</a>\n"
           "</p>\n"
           "</body>\n"
           "</html>\n";
