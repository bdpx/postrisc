...
void Decoder::tablegen_general_registerfile(std::ostream& out) const
{
    ...
    out << "\n\n";
    out << "class gpr_reg<bits<" << NBITS << "> val> {\n"
...
void Decoder::tablegen_special_registerfile(std::ostream& out) const
{
    ...
    out << "\n\n";
    out << "class spr_reg<bits<" << NBITS << "> val> {\n"
           "    bits<" << NBITS << "> value = val;\n"
           "}\n\n";
...
{
    out << "\n\n";
    out << "class memory_order_opx<bits<" << (unsigned)bits_per_memory_order << "> val> {\n"
           "    bits<" << (unsigned)bits_per_memory_order << "> value = val;\n"
           "}\n\n";

    out << "def memory_order_relaxed : memory_order_opx<" << (unsigned)EMemoryOrder::relaxed << ">;\n";
    out << "def memory_order_acquire : memory_order_opx<" << (unsigned)EMemoryOrder::acquire << ">;\n";
    out << "def memory_order_release : memory_order_opx<" << (unsigned)EMemoryOrder::release << ">;\n";
    out << "def memory_order_acq_rel : memory_order_opx<" << (unsigned)EMemoryOrder::acq_rel << ">;\n";
    out << "def memory_order_seq_cst : memory_order_opx<" << (unsigned)EMemoryOrder::seq_cst << ">;\n";
}
