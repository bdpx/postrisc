...
void Decoder::print_section_info(u64 address, std::ostream& out) const
{
...
void Decoder::print_address_info(u64 address, std::ostream& out) const
{
    out << fmt::hex(address);

    auto it = m_symbols.lower_bound(address);
    if (unlikely(it == m_symbols.end())) {
        print_section_info(address, out);
        return;
    }
    /* address should be after device base address */
    assert(it->first <= address);
...
template<class T> constexpr int size();
template<> constexpr int size<CInsn1>() { return 1; }
template<> constexpr int size<CInsn2>() { return 2; }
    ...
    DecoderContext& context) const
{
    ...
    case EBundleTemplate::bundle_lls:
        ...
        PrintInstruction(bundle.s2(), out, ip, 1, context);
        break;
