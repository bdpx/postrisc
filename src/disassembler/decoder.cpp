...
public:
    ...
    friend std::ostream & operator << (std::ostream & out, const as_label& r)
    {
        ...
        out << 'L';
        r.decoder.print_address_info(r.v, out);
        // << std::hex << a.v << std::dec;
        if (r.mode & disasm_mode_html)
            out << "</span>";
        return out;
...
#define INSN_X(NAME, VALUE) \
   assert(arr[opcode_ ## NAME] == insn_illegal); \
...
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_brr    = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_BRR_EXTENDED_OPCODES);
...
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_bfss   = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_BFSS_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_bfsd   = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_BFSD_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_bfsq   = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_BFSQ_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_nullrr = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_NULLRR_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_nulfss = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_NULFSS_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_nulfsd = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_NULFSD_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<(1 << bits_per_branch_opx)>   Decoder::arch_info_nulfsq = LAMBDA_INITIALIZER(bits_per_branch_opx, DECLARE_NULFSQ_EXTENDED_OPCODES);
#undef INSN_X
#undef LAMBDA_INITIALIZER

Decoder::regname_array Decoder::gpr_names = {};
Decoder::regname_array Decoder::spr_names = {};
...
Decoder::Decoder()
{
    for (int i = 0; i < max_stacked_framesize; i++) {
       ...
       strcpy(&gpr_names[real_index][0], rname.c_str());
    ...
    strcpy(&gpr_names[reg_fp][0], "fp");
    strcpy(&gpr_names[reg_sp][0], "sp");
    strcpy(&gpr_names[reg_tp][0], "tp");
    strcpy(&gpr_names[reg_gz][0], "gz");
...
#define ADD_SPECIAL_REGISTER(value, name, regclass, descr) \
...
void
Decoder::add_symbol(u64 address, u64 size, char const *name)
{
...
void
Decoder::print_address_info(u64 address, std::ostream& out) const
{
    out << fmt::hex(address);
    ...
    out << ' ' << &m_symbol_names[it->second.name_offset];
    if (address > it->first) {
...
int Decoder::GetGprIndex(char const *name)
{
    for (int i = 0; i < (int)gpr_names.size(); i++) {
        if (strcmp(name, gpr_names[i]) == 0) return i;
    }
...
int Decoder::GetSprIndex(char const *name)
{
    for (int i = 0; i < (int)spr_names.size(); i++) {
        if (strcmp(name, spr_names[i]) == 0) return i;
    }
...
void
Decoder::WriteRegister(std::ostream & out, unsigned regnum, disassembler_mode mode)
{
...
***************************************************************************/
static size_t PrintIndexedScaleMode(std::ostream& out, EScaleMode scale_mode)
{
    switch (scale_mode) {
        case scale_mode_dword:
            // .xd is also default
            out << ".xd";
            return 3;
        case scale_mode_zext_word:
            out << ".xuw";
            return 4;
        case scale_mode_sext_word:
            out << ".xw";
            return 3;
        default:
            LOG_ABORT << "unknown scale mode " << fmt::dec(scale_mode);
            return 0;
    }
}

static void PrintMemoryOrder(std::ostream& out, EMemoryOrder memory_order)
{
    switch (memory_order) {
        case EMemoryOrder::relaxed: out << "relaxed"; break;
        case EMemoryOrder::acquire: out << "acquire"; break;
        case EMemoryOrder::release: out << "release"; break;
        case EMemoryOrder::acq_rel: out << "acq_rel"; break;
        case EMemoryOrder::seq_cst: out << "seq_cst"; break;
        default:
            LOG_ABORT << "unknown memory order " << fmt::dec(memory_order);
    }
}

template<class CMD>
void
Decoder::DisassembleInstructionField(std::ostream& out, bool last,
    CMD cmd, u64 ip,
    const EInstructionField field, disassembler_mode mode) const
{
    switch (field) {
        case cf_lprefix:
            if constexpr (CMD::s_num_slots == 2) {
                out << ".l";
            }
            break;
        ...
        case cf_indexed_scale_mode:
            PrintIndexedScaleMode(out, cmd.GetIndexedScaleMode());
            break;
        ...
        case cf_memory_order:    out << '.'; PrintMemoryOrder(out, cmd.GetMemoryOrder());   break;

        case cf_flddi:           out << as_imm(std::bit_cast<double>(cmd.get_ldi()), mode); break;
...
Decoder::Disassemble(std::ostream& out, const CMD cmd, u64 ip, disassembler_mode mode) const
{
    ...
    for (size_t i = 0; i < asm_fmt_field[fmt].size(); i++) {
        const EInstructionField field = asm_fmt_field[fmt][i];
        if (field == cf_none)
           break;
        assert(i + 1 < asm_fmt_field[fmt].size());
        const EInstructionField next_field = asm_fmt_field[fmt][i+1];
        bool const last = (next_field == cf_none);
        DisassembleInstructionField(out, last, cmd, ip, field, mode);
    }
...
template EInstructionID Decoder::Disassemble<CInsn1>(std::ostream& out, const CInsn1 cmd, u64 ip, disassembler_mode mode) const;
template EInstructionID Decoder::Disassemble<CInsn2>(std::ostream& out, const CInsn2 cmd, u64 ip, disassembler_mode mode) const;
...
template<class CMD>
void
Decoder::PrintInstruction(const CMD cmd, std::ostream &out, u64 ip,
    unsigned insnNumber, DecoderContext& context) const
{
    // nullification may be before current instruction
    if (insnNumber == context.future_ri) {
        context.insn_number = 0;
    }

    if (context.mode & disasm_mode_html) {
        bool const isCurrent = insnNumber == context.ri;
        if (isCurrent) {
            out << "<span class=\"current\">";
        }
        out << fmt::hex(ip - sizeof(CBundle));
        out << ':' << static_cast<char>('A' + insnNumber - 1);
        if (isCurrent) {
            out << "</span>";
        }
    }
...
void
Decoder::DisassembleBundle(
    const CBundle bundle, u64 ip, std::ostream& out,
    DecoderContext& context) const
{
    const EBundleTemplate tmplt = bundle.GetTemplate();

    // ip is next_ip, bundle address is 16 bytes before
    u64 const real_ip = ip - sizeof(CBundle);

    if (context.mode & disasm_mode_dump) {
        out << "\n;ip=";
        if (context.mode & disasm_mode_names) {
            print_address_info(real_ip, out);
        } else {
            out << fmt::hex(real_ip);
        }
        out << " bundle: " << bundle << '\n';
    ...
    case EBundleTemplate::bundle_sss:
        PrintInstruction(bundle.s0(), out, ip, 1, context);
        PrintInstruction(bundle.s1(), out, ip, 2, context);
        PrintInstruction(bundle.s2(), out, ip, 3, context);
        break;

    case EBundleTemplate::bundle_sll:
        PrintInstruction(bundle.s0(), out, ip, 1, context);
        PrintInstruction(bundle.l1(), out, ip, 2, context);
        break;

    case EBundleTemplate::bundle_lls:
        PrintInstruction(bundle.l0(), out, ip, 1, context);
...
Decoder::Disassemble(std::istream &in, std::ostream &out, disassembler_mode mode) const
{
    ...
    for (address=0; address < header.size_text; address += sizeof(CBundle)) {
        u64 const current_address = start_address + address;
        if (labels.is_label(address)) {
            out << as_label(*this, current_address, mode) << ":\n";
        }

        CBundle const bundle = *reinterpret_cast<const CBundle*>(filedata.data() + address);
        u64 const ip = current_address + sizeof(CBundle);
