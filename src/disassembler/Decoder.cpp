...
#define LAMBDA_INITIALIZER(NBITS, XMACRO)                       \
    []() constexpr -> auto {                                    \
        TInstructionMnemonicTable<NBITS> arr = {};              \
        for (auto& insn : arr) {                                \
            insn = insn_illegal;                                \
        }                                                       \
        XMACRO(INSN_X)                                          \
...
const Decoder::TInstructionMnemonicTable<bits_per_opcode>       Decoder::arch_info_opcode   = LAMBDA_INITIALIZER(bits_per_opcode,       DECLARE_PRIMARY_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_misc_opx>     Decoder::arch_info_misc     = LAMBDA_INITIALIZER(bits_per_misc_opx,     DECLARE_MISC_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_fused_opx>    Decoder::arch_info_fused    = LAMBDA_INITIALIZER(bits_per_fused_opx,    DECLARE_FUSED_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_register>     Decoder::arch_info_raopx    = LAMBDA_INITIALIZER(bits_per_register,     DECLARE_RAOPX_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_loop     = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_LOOP_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_br_misc  = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_BR_MISC_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_br_32    = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_BR_32_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_br_64    = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_BR_64_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_br_128   = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_BR_128_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_nul_misc = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_NUL_MISC_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_nul_32   = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_NUL_32_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_nul_64   = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_NUL_64_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_branch_opx>   Decoder::arch_info_nul_128  = LAMBDA_INITIALIZER(bits_per_branch_opx,   DECLARE_NUL_128_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_mem_xi64 = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_MEM_XI64_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_mem_xu64 = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_MEM_XU64_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_mem_xi32 = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_MEM_XI32_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_mem_xu32 = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_MEM_XU32_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_fma_f16  = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_FMA_F16_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_fma_f32  = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_FMA_F32_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_fma_f64  = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_FMA_F64_EXTENDED_OPCODES);
const Decoder::TInstructionMnemonicTable<bits_per_indexed_opx>  Decoder::arch_info_fma_f128 = LAMBDA_INITIALIZER(bits_per_indexed_opx,  DECLARE_FMA_F128_EXTENDED_OPCODES);
    ...
    CMD cmd, u64 ip, const EInstructionField field, disassembler_mode mode) const
{
    switch (field) {
        ...
        case cf_rounding_mode:
            if (cmd.GetRoundingMode() != RoundingMode::Default) {
                out << ',' << RoundingMode2Str(cmd.GetRoundingMode());
            }
            break;

        case cf_memory_ordering:
            out << MemoryOrdering2Str(cmd.GetMemoryOrdering());
            break;

        case cf_optional_memory_ordering:
            if (cmd.GetMemoryOrdering() != MemoryOrdering::Default) {
                out << ',' << MemoryOrdering2Str(cmd.GetMemoryOrdering());
            }
            break;

        case cf_fpclass:
            out << as_imm(cmd.GetFpClassifyMask(), mode);
            break;

        case cf_stride_simm:
        case cf_update_stride:
            out << as_imm(cmd.GetUpdateStride(), mode);
            break;
