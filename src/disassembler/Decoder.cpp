...
***************************************************************************/
#define INSN_X(NAME, VALUE, SUBSET, FORMAT, DESCR) \
   assert(arr[opcode_ ## NAME] == insn_illegal && #NAME); \
   arr[opcode_ ## NAME] = insn_ ## NAME;


#define DEFINE_OPCODE_TABLE(NAME, NBITS, XMACRO)                      \
    const Decoder::TInstructionMnemonicTable<NBITS> Decoder::NAME =   \
    []() constexpr -> auto {                                          \
...
DEFINE_OPCODE_TABLE(arch_info_opcode,  bits_per_opcode,       DECLARE_PRIMARY_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_misc,    bits_per_misc_opx,     DECLARE_MISC_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_fused,   bits_per_fused_opx,    DECLARE_FUSED_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_raopx,   bits_per_register,     DECLARE_RAOPX_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_loop,    bits_per_branch_opx,   DECLARE_LOOP_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_brh,     bits_per_branch_opx,   DECLARE_BRH_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_brw,     bits_per_branch_opx,   DECLARE_BRW_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_brd,     bits_per_branch_opx,   DECLARE_BRD_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_brq,     bits_per_branch_opx,   DECLARE_BRQ_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_nulh,    bits_per_branch_opx,   DECLARE_NULH_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_nulw,    bits_per_branch_opx,   DECLARE_NULW_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_nuld,    bits_per_branch_opx,   DECLARE_NULD_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_nulq,    bits_per_branch_opx,   DECLARE_NULQ_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_memxsd,  bits_per_indexed_opx,  DECLARE_MEMXSD_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_memxud,  bits_per_indexed_opx,  DECLARE_MEMXUD_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_memxsw,  bits_per_indexed_opx,  DECLARE_MEMXSW_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_memxuw,  bits_per_indexed_opx,  DECLARE_MEMXUW_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_fmah,    bits_per_indexed_opx,  DECLARE_FMAH_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_fmas,    bits_per_indexed_opx,  DECLARE_FMAS_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_fmad,    bits_per_indexed_opx,  DECLARE_FMAD_EXTENDED_OPCODES)
DEFINE_OPCODE_TABLE(arch_info_fmaq,    bits_per_indexed_opx,  DECLARE_FMAQ_EXTENDED_OPCODES)

#undef INSN_X
#undef DEFINE_OPCODE_TABLE
...
#undef ADD_SPECIAL_REGISTER
    ...
    for (int i = 0; i < insn_illegal; i++) {
        if (asm_info[i].subset == instr_subset_group)
            continue;
        std::string cand(to_string(static_cast<EInstructionID>(i)));
        if (asm_names.find(cand) != asm_names.end()) {
            std::cerr << "duplicate:: " << cand << std::endl;
        }
        asm_names[cand] = static_cast<EInstructionID>(i);
    }
    //if (asm_names.size() != insn_illegal) {
    //    std::cerr << "asm_names: " << asm_names.size() << ", expected: " << insn_illegal << std::endl;
    //}
    //assert(asm_names.size() == insn_illegal);
}
    ...
    CMD cmd, u64 ip, const EInstructionField field, disassembler_mode mode) const
{
    switch (field) {
        ...
        case cf_fldis:      out << as_imm(std::bit_cast<float>(static_cast<u32>(cmd.get_ldi())), mode); break;
        case cf_fldid:      out << as_imm(std::bit_cast<double>(cmd.get_ldi()), mode); break;
...
Decoder::PrintInstruction(const CMD cmd, std::ostream& out, u64 ip, unsigned slotNumber, DecoderContext& context) const
{
    ...
    if (context.instr_number >= 0 && context.instr_number < bits_per_nullify_future) {
        if (context.mode & disasm_mode_html) {
        ...
        out << ':' << context.instr_number;
        context.future_predication >>= 1;
    }

    context.instr_number++;
