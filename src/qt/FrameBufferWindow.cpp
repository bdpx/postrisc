    ...
    , m_TargetSystem(targetSystem)
{
    ...
    m_Label = new QLabel();
    m_Label->setBaseSize(QSize(320, 200));
    m_Label->resize(m_Label->baseSize());

    _scene->addWidget(m_Label);
...
void FrameBufferWindow::applyImage(const QImage& image)
{
    ...
    //_scene->clear();
    //_scene->addPixmap(p);
    m_Label->setBaseSize(QSize(static_cast<int>(factor*image.width()), static_cast<int>(factor*image.height())));
    m_Label->resize(m_Label->baseSize());
    m_Label->setPixmap(p);
    _scene->update();
    ++m_frame;
