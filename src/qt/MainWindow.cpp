...
void MainWindow::Action_Step()
{
    try {
        ...
        if (core) {
            ...
            status = core->execute_until_interruption(m_TargetSystem);
            m_DisassemblerWidget->SetProgram();
            m_CoreWidget->SetInfo(core);
        }
    } catch(...) {
...
int QtDoomInterface::Init(unsigned resx, unsigned resy) const
{
    LOG_DEBUG(LOG_DOOM) << "QT: Init: w=" << resx << " h=" << resy;
    return 0;
}

int QtDoomInterface::DrawFrame(u32 const *screenbuffer, unsigned resx, unsigned resy) const
{
    assert(m_updater);
    if (m_updater->stopped)
        return -1;
    LOG_DEBUG(LOG_DOOM) << "QT: Draw: w=" << resx << " h=" << resy;
    QImage image(reinterpret_cast<u8 const*>(screenbuffer), resx, resy, QImage::Format_RGB32);
    m_updater->SendApplyImage(image);
    return 0;
}

int QtDoomInterface::SetWindowTitle(char const *title) const
{
...
int QtDoomInterface::GetKey(int *pressed, u8 *doomKey) const
{
    bool ispressed;
    Qt::Key keyCode;
...
int ConfRunner::run(IDoomInterface& doom)
{
    return conf.do_run(doom, argc, argv, env);
}
