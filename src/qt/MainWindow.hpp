...
class ConfRunner {
public:
    ConfRunner(Configuration& conf_, int argc_, char **argv_, char **env_)
        : conf(conf_), argc(argc_), argv(argv_), env(env_) {}

    int run(IDoomInterface& doom);
...
class QtDoomInterface : public IDoomInterface {
public:
    ...
    int GetKey(int* pressed, unsigned char* Key) const override;
