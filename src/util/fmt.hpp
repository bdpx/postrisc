...
private:
    ...
    bool delimeter;
public:
    explicit biner(T _v, int _nbits, bool _delimeter)
       : v(_v), nbits(_nbits), delimeter(_delimeter) {}

    friend std::ostream& operator<<(std::ostream& out, const biner& r) {

        for (int i = r.nbits - 1; i > 0; i--) {
            out << (char)(((r.v >> i) & 1) + '0');
            if (r. delimeter && (i & 7) == 0)
                out << '_';
        }
        out << (char)(((r.v) & 1) + '0');
        return out;
...
template<typename T> inline biner<T> bin(T v, int nbits = 8 * sizeof(T), bool delimeter = true) {
    return biner<T>(v, nbits, delimeter);
}
