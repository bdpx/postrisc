...
{
    return (static_cast<T>(1) << nbits) - static_cast<T>(1);
}
...
inline u64 rounddown(u64 val, unsigned int numbits) { return val & ~makemask<u64>(numbits); }
inline u64 roundup(u64 val, unsigned int numbits) { return (val + makemask<u64>(numbits)) & ~makemask<u64>(numbits); }
