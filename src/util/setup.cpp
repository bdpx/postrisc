...
static void signal_handler(int sig)
{
    ...
    out << "signal handler called: " << sig << std::endl;
#if defined(__unix__) && defined(__clang__) && defined(ASAN)
    __sanitizer_print_stack_trace();
#else
    boost::stacktrace::stacktrace st;
    out << "stack trace: " << st.size() << " frames" << std::endl
        << st << std::endl;
#endif
    exit(EXIT_FAILURE);
}
...
{
    std::ostream& out = dbgs();
    out << "terminate handler called" << std::endl;
#if defined(__unix__) && defined(__clang__) && defined(ASAN)
    __sanitizer_print_stack_trace();
#else
    boost::stacktrace::stacktrace st;
...
void setup(int UNUSED(argc), char** UNUSED(argv))
{
#if !defined(__unix__)
