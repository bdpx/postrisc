...
public:
    static u32 hash(const void* buf, size_t len, u32 initial = 0)
    {
    ...
    template<typename T> static u32 hash_intermediate(u32 c, T value)
    {
        if constexpr (std::is_same<u8, T>::value) {
            return table[(c ^ value) & 0xFF] ^ (c >> 8);
        } else {
            for (size_t i = 0; i < sizeof(T); ++i) {
                c = table[(c ^ (value & 0xFF)) & 0xFF] ^ (c >> 8);
                value >>= 8;
            }
            return c;
        }
