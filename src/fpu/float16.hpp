...
class f16 : public CBaseFloat<16,5,1> {
    uint16_t v;

public:
    static const uint16_t HIDDEN_BIT = UINT16_C(1) << BITS_PER_SIGNIFICAND; //0x0400

    uint16_t val(void) const { return v; }

    sign_type sign(void) const { return (sign_type)(v>>15); }
    fp_exponent_t exponent(void) const { return ( v >> BITS_PER_SIGNIFICAND ) & EINF; }
    uint16_t fraction(void) const { return lobits(v, BITS_PER_SIGNIFICAND); }
    ...
    constexpr explicit f16(uint16_t val) : v(val) {}
...
//    friend bool operator != (const f16 & a, const f16 & b) {  return ( a.v != b.v);  }
    ...
    static constexpr f16 inf(sign_type s)  { return f16((static_cast<uint16_t>(s) << 15) | (uint16_t(EINF) << BITS_PER_SIGNIFICAND)); }
    static constexpr f16 zero(sign_type s) { return f16(static_cast<uint16_t>(s) << 15); }

    f16  abs(void) const { return f16(v & ~(UINT16_C(1) << 15));  }
    f16  chs(void) const { return f16(v ^  (UINT16_C(1) << 15));  }
    f16 nabs(void) const { return f16(v |  (UINT16_C(1) << 15));  }
...
{
    return (uint16_t(val()<<1) < uint16_t(b.val()<<1)) ? false :
           (uint16_t(val()<<1) > uint16_t(b.val()<<1)) ? true  :
           (val() < b.val());
}
