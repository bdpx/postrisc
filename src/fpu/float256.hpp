...
* octuple precision
**************************************************************************************/
class f256 : public CBaseFloat<256,19,1>
{
public:
    uint64_t  m3, m2, m1, m0;
public:
    ...
    constexpr explicit f256(uint64_t x0, uint64_t x1, uint64_t x2, uint64_t x3)
       : m3(x3), m2(x2), m1(x1), m0(x0) {}
    constexpr explicit f256(sign_type sign, int power)
       : m3(0), m2(0), m1(0), m0( (uint64_t(sign) << 63) | (uint64_t(power + BIAS) << BITS_PER_HIGH_SIGNIFICAND) ) {}

    friend bool IsBothZero(const f256 & a, const f256 & b);
    void operator = (const u256 & u);

    static constexpr f256 default_nan(void) { return f256( UINT64_C( 0xFFFFF80000000000 ), 0, 0, 0); }
    static constexpr f256 inf(sign_type s)  { return f256( (uint64_t(s) << 63) | (uint64_t(EINF) << BITS_PER_HIGH_SIGNIFICAND), 0, 0, 0 ); }
    static constexpr f256 zero(sign_type s) { return f256( uint64_t(s) << 63, 0, 0, 0 ); }
...
{
    return ( UINT64_C( 0xFFFFE00000000000 ) <= uint64_t( m0<<1 ) )
...
{
    return lobits(m0, 63) == UINT64_C(0x7FFFF00000000000) && (m1 | m2 | m3) == 0;
}

inline bool f256::IsZeroOrDenormal(void) const
{
    return (m0 & UINT64_C(0x7FFFF00000000000)) == 0;
}
