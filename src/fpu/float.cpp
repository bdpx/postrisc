...
uint64_t
CBaseFPU::get_control_word(void) const
{
    uint64_t word = 0;
    word |= static_cast<uint64_t>(get_flags())                << disp_flags;
    word |= static_cast<uint64_t>(get_rounding_mode())        << disp_rounding_mode;
    word |= static_cast<uint64_t>(get_rounding_precision())   << disp_rounding_precision;
    word |= static_cast<uint64_t>(tininess_before_rounding()) << disp_tininess_mode;
    word |= static_cast<uint64_t>(flush_to_zero())            << disp_flush_to_zero;
    word |= static_cast<uint64_t>(flush_inputs_to_zero())     << disp_flush_input_to_zero;
    word |= static_cast<uint64_t>(default_nan_mode())         << disp_default_nan_mode;
    return word;
}


void
CBaseFPU::set_control_word(uint64_t word)
{
...
void
CBaseFPU::change_control_word(uint64_t and_mask, uint64_t or_mask)
{
    const uint64_t mask = makemask64(7);
...
std::ostream &
operator << (std::ostream & out, float_flag flags )
{
    return out
...
std::ostream &
operator << (std::ostream & out, const CBaseFPU &fpu )
{
    return out
...
******************************************************************************/
void
writeHex(std::ostream& out, const f32& f)
{
    out << std::setfill('0') << std::hex
        << std::setw(3) << (f.val() >> f.BITS_PER_SIGNIFICAND)
        << '.'
        << std::setw(6) << f.fraction()
        << ' ' << f.hard();
}

void
writeHex(std::ostream& out, const f64& f)
{
    out << std::setfill('0') << std::hex
        << std::setw(3) << (f.val() >> f.BITS_PER_SIGNIFICAND)
        << '.'
        << std::setw(13) << f.fraction()
        << ' ' << f.hard();
}

void
writeHex(std::ostream& out, const f80& f)
{
    out << std::setfill('0') << std::hex
        << std::setw(4) << (f.high)
        << '.'
        << std::setw(16) << f.fraction()
        << ' ' << f.hard();
}

void
writeHex(std::ostream& out, const f128& f)
{
    using postrisc::operator<<;

    out << std::setfill('0') << std::hex
        << std::setw(4) << (f.high >> f.BITS_PER_HIGH_SIGNIFICAND)
        << '.'
        << std::setw(12) << lobits(f.high, f.BITS_PER_HIGH_SIGNIFICAND)
        << std::setw(16) << f.low
        << ' ' << f.hard();
}
...
*----------------------------------------------------------------------------*/
int32_t
CSoftFPU::round_pack_to_i32(int_nan_mode_t bothSignedNAN, sign_type zSign, uint64_t absZ)
{
    ...
    absZ &= ~ uint64_t( ( ( roundBits ^ HIGHEST_GUARD_BIT ) == 0 ) & roundNearestEven );  //0x40
    int32_t z = absZ;
    if ( zSign ) z = - z;
...
int64_t
CSoftFPU::round_pack_to_i64(int_nan_mode_t bothSignedNAN, sign_type zSign, uint64_t absZ0, uint64_t absZ1)
{
    int64_t z;

    rounding_mode_t roundingMode = get_rounding_mode();
    bool roundNearestEven = ( roundingMode == FLOAT_ROUND_TONEAREST_EVEN );
    bool increment = ( int64_t(absZ1) < 0 );
    if ( ! roundNearestEven ) {
        if ( roundingMode == FLOAT_ROUND_TOWARDZERO ) {
            increment = 0;
        }
...
*----------------------------------------------------------------------------*/
commonNaNT CSoftFPU::ToCommonNaN(f16 a)
{
    commonNaNT z;
...
*----------------------------------------------------------------------------*/
f16 CSoftFPU::to_f16(commonNaNT a)
{
    if( default_nan_mode() ) {
       return f16::default_nan();
    }

    uint16_t mantissa = a.high >> ( 64 - f16::BITS_PER_SIGNIFICAND );
    if( mantissa ) {
       return f16( (uint16_t(a.sign) << 15) | (f16::EINF << f16::BITS_PER_SIGNIFICAND) | mantissa);
    } else {
       return f16::default_nan();
    }
}
...
f32 CSoftFPU::to_f32(f16 a, bool ieee)
{
    fp_exponent_t aExp;
    uint32_t  aSig;
...
f16 CSoftFPU::to_f16(f32 a, bool ieee)
{
...
uint64_t CSoftFPU::estimateDiv128To64( uint64_t a0, uint64_t a1, uint64_t b )
{
    uint64_t   b0, b1, rem0, rem1, term0, term1;

    assert( (b >> 63) != 0 );
    if ( b <= a0 ) return std::numeric_limits<uint64_t>::max();
    b0 = b>>32;
    uint64_t z = ( b0<<32 <= a0 ) ? makemask64(32) << 32 : ( a0 / b0 ) << 32;
...
uint32_t CSoftFPU::estimateSqrt32(int aExp, uint32_t a)
{
