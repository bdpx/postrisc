...
*----------------------------------------------------------------------------*/
static inline uint32_t shift32RightJamming(uint32_t a, int count)
{
    uint32_t z;
...
commonNaNT CSoftFPU::ToCommonNaN(f32 a)
{
    commonNaNT z;
    ...
    z.sign = a.sign();
    z.low  = 0;
    z.high = uint64_t( a.val() )<< (64 - a.BITS_PER_SIGNIFICAND);
    return z;
}


f32 CSoftFPU::to_f32(commonNaNT a)
{
    if (default_nan_mode()) {
    ...
    uint32_t mantissa = a.high >> offset;

    if ( mantissa )
        return f32( ( uint32_t(a.sign)<<31 ) | 0x7F800000 | 0x00400000 | mantissa );
    else
        return f32::default_nan();
}
...
f32 CSoftFPU::RoundPackTo_f32(sign_type zSign, fp_exponent_t zExp, f32::fraction_t zSig)
{
    const int NUM_ROUNDING_BITS = 7;
    const int HIGHEST_GUARD_BIT = 1 << (NUM_ROUNDING_BITS - 1); //0x40
    const int32_t NNNN = f32::EINF - 2; //0xFD
    ...
    if ( static_cast<uint32_t>(NNNN) <= static_cast<uint32_t>(zExp) ) {
        if (    ( NNNN < zExp )
             || (    ( zExp == NNNN )
                  && ( static_cast<int32_t>( zSig + roundIncrement ) < 0 ) )
           ) {
    ...
    zSig &= ~ ( ( ( roundBits ^ HIGHEST_GUARD_BIT ) == 0 ) & roundNearestEven );
    if ( zSig == 0 ) zExp = 0;
...
f32 CSoftFPU::NormalizeRoundPackTo_f32(sign_type zSign, fp_exponent_t zExp, uint32_t zSig)
{
    assert( zSig );
...
f32 CSoftFPU::to_f32( int32_t a )
{
    ...
    uint64_t absA = a < 0 ? -int64_t(a) : int64_t(a);
...
f32 CSoftFPU::to_f32( uint64_t a )
{
    if ( a == 0 ) return f32_zero;
...
int32_t CSoftFPU::to_i32_round_to_zero( f32 a )
{
    f32::fraction_t aSig = a.fraction();
    fp_exponent_t aExp = a.exponent();
    sign_type a_sign = a.sign();
...
int16_t CSoftFPU::to_i16_round_to_zero(f32 a)
{
    f32::fraction_t  aSig = a.fraction();
    fp_exponent_t aExp = a.exponent();
    sign_type  a_sign = a.sign();
...
int64_t CSoftFPU::to_i64_round_to_zero( f32 a )
{
    f32::fraction_t aSig = a.fraction();
    fp_exponent_t  aExp = a.exponent();
    sign_type  a_sign = a.sign();
    ...
    uint64_t aSig64 = aSig | a.HIDDEN_BIT;
    aSig64 <<= 40;
    int64_t z = aSig64 >> ( - shiftCount );
...
f32 CSoftFPU::round_to_int( f32 a)
{
    fp_exponent_t aExp = a.exponent();
    ...
    if ( aExp < a.BIAS ) { //0x7E
        if ( uint32_t ( a.val()<<1 ) == 0 ) return a;
           raise(FLOAT_FLAG_INEXACT);
        sign_type a_sign = a.sign();
        switch ( get_rounding_mode() ) {
         ...
         case FLOAT_ROUND_DOWNWARD:
            return f32(a_sign ? 0xBF800000 : 0);
         case FLOAT_ROUND_UPWARD:
            return f32(a_sign ? 0x80000000 : 0x3F800000);
         default:
    ...
    if ( get_rounding_mode() == FLOAT_ROUND_TONEAREST_EVEN ) {
        ...
        if ( f32(z).sign() ^ ( get_rounding_mode() == FLOAT_ROUND_UPWARD ) ) {
            z += roundBitsMask;
        }
    }
    z &= ~ roundBitsMask;
...
f32 CSoftFPU::round_to_int(f32 a, rounding_mode_t rmode)
{
...
f32 CSoftFPU::exp2( f32 a )
{

    static const uint64_t f32_exp2_coefficients[15] =
    {
        UINT64_C( 0x3ff0000000000000 ), //  1
        UINT64_C( 0x3fe0000000000000 ), //  2
        UINT64_C( 0x3fc5555555555555 ), //  3
        UINT64_C( 0x3fa5555555555555 ), //  4
        UINT64_C( 0x3f81111111111111 ), //  5
        UINT64_C( 0x3f56c16c16c16c17 ), //  6
        UINT64_C( 0x3f2a01a01a01a01a ), //  7
        UINT64_C( 0x3efa01a01a01a01a ), //  8
        UINT64_C( 0x3ec71de3a556c734 ), //  9
        UINT64_C( 0x3e927e4fb7789f5c ), // 10
        UINT64_C( 0x3e5ae64567f544e4 ), // 11
        UINT64_C( 0x3e21eed8eff8d898 ), // 12
        UINT64_C( 0x3de6124613a86d09 ), // 13
        UINT64_C( 0x3da93974a8c07c9d ), // 14
        UINT64_C( 0x3d6ae7f3e733b81f ), // 15
    };
    ...
    f64 r = f64_one;
    for (int i = 0 ; i < 15 ; i++) {
        f64 f = mul(xn, f64(f32_exp2_coefficients[i]));
        r = add(r, f);
        xn = mul(xn, x);
    }
...
f32 CSoftFPU::log2( f32 a )
{
    ...
    for (i = uint32_t(1) << (a.BITS_PER_SIGNIFICAND - 1); i > 0; i >>= 1) {
        aSig = ( uint64_t(aSig) * aSig ) >> a.BITS_PER_SIGNIFICAND;
        if ( aSig & 0x01000000 ) {
            aSig >>= 1;
            zSig |= i;
        }
...
bool CSoftFPU::eq_signaling( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::le_signaling( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::lt_signaling( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::unordered( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return true;
...
bool CSoftFPU::eq_quiet( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
...
bool CSoftFPU::le_quiet( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
bool CSoftFPU::lt_quiet( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
bool CSoftFPU::unordered_quiet( f32 a, f32 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
f32 CSoftFPU::to_f32( uint32_t a )
{
    return to_f32(int64_t(a));
}

uint32_t CSoftFPU::to_u32( f32 a )
{
    uint32_t res;
...
uint32_t CSoftFPU::to_u32_round_to_zero( f32 a )
{
    uint32_t res;

    int64_t v = to_i64_round_to_zero(a);
    if (v < 0) {
        res = 0;
        raise(FLOAT_FLAG_INVALID);
    } else if (v > std::numeric_limits<uint32_t>::max()) {
...
uint16_t CSoftFPU::to_u16_round_to_zero(f32 a)
{
    uint16_t res;
...
f32 CSoftFPU::scalbn( f32 a, int n )
{
    f32::fraction_t aSig = a.fraction();
    fp_exponent_t aExp = a.exponent();
    sign_type a_sign = a.sign();
...
***********************************************************************/
f32 CSoftFPU::merge(f32 sign, f32 exponent, f32 mantissa)
{
    return f32(
...
float_relation CSoftFPU::compare(f32 a, f32 b) {  return compare_internal(a, b, false);  }
float_relation CSoftFPU::compare_quiet(f32 a, f32 b) {  return compare_internal(a, b, true);  }

compare_mode_t CSoftFPU::compare(f32 a, f32 b, compare_mode_t mode)
{
    return compare_internal(a, b, mode);
}

f32 CSoftFPU::min(f32 a, f32 b, bool ignore_nans) {  return minmax<f32>(a, b, ignore_nans, true);  }
f32 CSoftFPU::max(f32 a, f32 b, bool ignore_nans) {  return minmax<f32>(a, b, ignore_nans, false); }
...
f32 CSoftFPU::addSigs( f32 a, f32 b, sign_type zSign)
{
    fp_exponent_t aExp, bExp, zExp, expDiff;
    f32::fraction_t aSig, bSig, zSig;

    const int FIXED_POINT = 6;
    const f32::fraction_t FIXED_HIDDEN_BIT = uint32_t(1) << (a.BITS_PER_SIGNIFICAND + FIXED_POINT);
...
f32 CSoftFPU::subSigs( f32 a, f32 b, sign_type zSign)
{
    ...
    const uint32_t FIXED_HIDDEN_BIT = uint32_t(1) << (a.BITS_PER_SIGNIFICAND + FIXED_POINT);
...
f32 CSoftFPU::mul( f32 a, f32 b )
{
    ...
    zSig = static_cast<uint32_t>(zSig64);
    if ( 0 <= int32_t( zSig<<1 ) ) {
        zSig <<= 1;
        --zExp;
    }
...
f32 CSoftFPU::muladd(f32 a, f32 b, f32 c, muladd_negate_t flags)
{
    ...
    if (cExp == 0) {
        if (!cSig) {
            ...
            pSig = static_cast<uint32_t>(pSig64);
            return RoundPackTo_f32(zSign, pExp - 1, pSig);
        }
        cExp = normalize_subnormal_f32( cSig );
    }

    cSig64 = uint64_t(cSig) << (62 - a.BITS_PER_SIGNIFICAND);
    cSig64 |= UINT64_C(0x4000000000000000);
    expDiff = pExp - cExp;
