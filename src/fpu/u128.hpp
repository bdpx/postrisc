...
public:
    uint64_t  m1, m0;   // little endian order!!! (low, high)
public:
    uint64_t & high() { return m0; }
    uint64_t & low() { return m1; }
    const uint64_t & high() const { return m0; }
    const uint64_t & low() const { return m1; }

    u128(void) {}
    constexpr explicit u128(uint64_t x1) : m1(x1), m0(0) {}
    constexpr explicit u128(uint64_t x0, uint64_t x1) : m1(x1), m0(x0) {}
    ...
    uint64_t jamming(int count, uint64_t a2);
