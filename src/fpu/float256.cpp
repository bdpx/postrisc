...
f256 CSoftFPU::to_f256(commonNaNT a)
{
    ...
    high |= ( uint64_t(a.sign)<<63 ) | UINT64_C( 0x7FFFF00000000000 ) | UINT64_C( 0x0000080000000000 );
...
f256 CSoftFPU::RoundPackTo_f256(
    sign_type zSign, fp_exponent_t zExp, u256 & zSig, uint64_t zSigExtra)
{
    const int32_t NNNN = f256::EINF - 2;
    ...
    if ( uint32_t(NNNN) <= uint32_t(zExp) ) {
        ...
        const u256 full_ones( UINT64_C( 0x00001FFFFFFFFFFF ),
    ...
    if ( increment ) {
        ++zSig;
        zSig.m3 &= ~ ( ( zSigExtra + zSigExtra == 0 ) & roundNearestEven );
    }
    else {
        if ( ! zSig ) zExp = 0;
    }
...
{
    int32_t count = zSig.normalize_subnormal() - 1;
...
f256 CSoftFPU::to_f256( int32_t a )
{
...
f256 CSoftFPU::to_f256( int64_t a )
{
    sign_type   zSign;
    uint64_t      absA;
    fp_exponent_t zExp;
...
f256 CSoftFPU::to_f256(uint64_t a)
{
    if (a == 0) {
...
int32_t CSoftFPU::to_i32( f256 a )
{
    sign_type   a_sign;
...
int32_t CSoftFPU::to_i32_round_to_zero( f256 a )
{
    sign_type  a_sign;
    fp_exponent_t aExp, shiftCount;
    u256    aSig;
...
int64_t CSoftFPU::to_i64( f256 a )
{
    sign_type   a_sign;
    fp_exponent_t aExp, shiftCount;
    u256     aSig;
...
int64_t CSoftFPU::to_i64_round_to_zero( f256 a )
{
    sign_type  a_sign;
    fp_exponent_t aExp, shiftCount;
    u256    aSig;
    ...
    if ( 0 < shiftCount ) {
        if ( a.BIAS + 63 <= aExp ) {
            ...
            if (    ( a.m0 == UINT64_C( 0xC003E00000000000 ) )
                 && ( aSig.m1 < UINT64_C( 0x0000200000000000 ) ) ) {
        ...
        if ( (aSig.m1 | aSig.m2 | aSig.m3) || ( shiftCount && uint64_t( aSig.m0<<( shiftCount & 63 ) ) ) ) {
            raise(FLOAT_FLAG_INEXACT);
        }
    }
    if ( a_sign ) z = - z;
...
f256 CSoftFPU::to_f256( f32 a )
{
    ...
    return pack_f256( a_sign, aExp + f256::BIAS - a.BIAS,
              u256( uint64_t(aSig) << (32 - (f256::BITS_PER_EXPONENT - f32::BITS_PER_EXPONENT)), 0, 0, 0 ) );
...
f64 CSoftFPU::to_f64( f256 a )
{
    ...
    aSig.m0 |= ( (aSig.m1 | aSig.m2 | aSig.m3) != 0 );
    if ( aExp || aSig.m0 ) {
        aSig.m0 |= ( uint64_t(1) << (a.BITS_PER_HIGH_SIGNIFICAND + 18) ); // set hidden bit
...
f256 CSoftFPU::round_to_int( f256 a )
{
    ...
    //lastBitMask = 1 << count;
    if (count < 64) {
       lastBitMask = u256(0, 0, 0, uint64_t(1) << count);
    } else if (count < 128) {
       lastBitMask = u256(0, 0, uint64_t(1) << (count - 64), 0);
    } else if (count < 192) {
       lastBitMask = u256(0, uint64_t(1) << (count - 128), 0, 0);
    } else if (count < 256) {
       lastBitMask = u256(uint64_t(1) << (count - 192), 0, 0, 0);
    } else {
       //!!!!!!!!!!!!
...
f256 CSoftFPU::subSigs( f256 a, f256 b, sign_type zSign)
{
    ...
    const uint64_t FIXED_HIDDEN_BIT = uint64_t(1) << (f256::BITS_PER_HIGH_SIGNIFICAND + FIXED_POINT);
