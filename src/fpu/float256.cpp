...
commonNaNT SoftFPU::ToCommonNaN(f256 a)
{
    commonNaNT z;
...
f256 SoftFPU::to_f256(commonNaNT a)
{
    u64 high, low;
...
f256 SoftFPU::NormalizeRoundPackTo_f256( sign_type zSign, fp_exponent_t zExp, u256 & zSig )
{
...
i32 SoftFPU::to_i32_round_to_zero( f256 a )
{
    sign_type  a_sign;
    fp_exponent_t aExp, shiftCount;
    u256    aSig;
...
i64 SoftFPU::to_i64_round_to_zero( f256 a )
{
    sign_type  a_sign;
    fp_exponent_t aExp, shiftCount;
    u256    aSig;
...
f256 SoftFPU::round_to_int( f256 a )
{
    fp_exponent_t aExp = a.exponent();
    u256 aSig = a.fraction();
...
f256 SoftFPU::round_to_int(f256 a, rounding_mode_t rmode)
{
...
bool SoftFPU::eq_signaling( f256 a, f256 b )
{
    if (a.IsNan() || b.IsNan()) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool SoftFPU::le_signaling( f256 a, f256 b )
{
...
bool SoftFPU::lt_signaling( f256 a, f256 b )
{
    sign_type a_sign, b_sign;
...
bool SoftFPU::eq_quiet( f256 a, f256 b )
{
...
bool SoftFPU::le_quiet( f256 a, f256 b )
{
    sign_type a_sign, b_sign;
...
bool SoftFPU::lt_quiet( f256 a, f256 b )
{
    if (a.IsNan() || b.IsNan()) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
           raise(FLOAT_FLAG_INVALID);
...
bool SoftFPU::unordered( f256 a, f256 b )
{
    if (a.IsNan() || b.IsNan()) {
        raise(FLOAT_FLAG_INVALID);
        return true;
...
bool SoftFPU::unordered_quiet( f256 a, f256 b )
{
    if (a.IsNan() || b.IsNan()) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
f256 SoftFPU::scalbn( f256 a, int n )
{
    u256 aSig = a.fraction();
    fp_exponent_t aExp = a.exponent();
    sign_type a_sign = a.sign();
...
f256 SoftFPU::merge(f256 sign, f256 exponent, f256 mantissa)
{
    u64 high =
...
float_relation SoftFPU::compare(f256 a, f256 b) {  return compare_internal(a, b, false);  }
float_relation SoftFPU::compare_quiet(f256 a, f256 b) {  return compare_internal(a, b, true);  }

f256 SoftFPU::min(f256 a, f256 b, bool ignore_nans) {  return minmax<f256>(a, b, ignore_nans, true);  }
f256 SoftFPU::max(f256 a, f256 b, bool ignore_nans) {  return minmax<f256>(a, b, ignore_nans, false); }
...
f256 SoftFPU::addSigs( f256 a, f256 b, sign_type zSign)
{
...
f256 SoftFPU::subSigs( f256 a, f256 b, sign_type zSign)
{
    fp_exponent_t aExp, bExp, zExp;
    u256   zSig;
    fp_exponent_t expDiff;
...
f256 SoftFPU::add( f256 a, f256 b )
{
    sign_type a_sign, b_sign;
...
f256 SoftFPU::sub( f256 a, f256 b )
{
    sign_type a_sign, b_sign;
...
f256 SoftFPU::mul( f256 a, f256 b )
{
...
f256 SoftFPU::div( f256 a, f256 b )
{
    sign_type   a_sign, b_sign, zSign;
    u256   zSig;
    u320     term, rem;
...
f256 SoftFPU::rem( f256 a, f256 b )
{
    u256   aSig  = a.fraction();
    fp_exponent_t  aExp  = a.exponent();
    sign_type a_sign = a.sign();
...
*----------------------------------------------------------------------------*/
f256 SoftFPU::muladd(f256 a, f256 b, f256 c, muladd_negate_t flags)
{
