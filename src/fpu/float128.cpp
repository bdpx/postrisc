...
uint64_t
u128::jamming(int count, uint64_t a2)
{
    if ( count >= 127 + 64 ) {
       a2 = ( (m0 | m1 | a2) != 0);
       m0 = m1 = 0;
...
void
u192::mul( const u128 & a, uint64_t b )
{
    uint64_t z1;
...
///////////////////////////////////////////////////////////
//
commonNaNT CSoftFPU::ToCommonNaN(f128 a)
{
    commonNaNT z;
...
f128 CSoftFPU::to_f128(commonNaNT a)
{
    uint64_t high, low;
    ...
    high |= ( uint64_t(a.sign)<<63 ) | UINT64_C( 0x7FFF000000000000 ) | UINT64_C( 0x0000800000000000 );
    return f128(high, low);
}
...
f128 CSoftFPU::RoundPackTo_f128(
     sign_type zSign, fp_exponent_t zExp, u128 zSig, uint64_t zSig2)
{
...
//    const int HIGHEST_GUARD_BIT = 1 << (NUM_ROUNDING_BITS - 1); //0x200
    ...
    bool increment = ( int64_t(zSig2) < 0 );
    if ( ! roundNearestEven ) {
    ...
    if ( uint32_t(NNNN) <= uint32_t(zExp) ) {
        if (    ( NNNN < zExp )
             || (    ( zExp == NNNN )
                  && u128( UINT64_C( 0x0001FFFFFFFFFFFF ), UINT64_C( 0xFFFFFFFFFFFFFFFF ) ) == zSig
                  && increment
                )
           ) {
            ...
            if (                 ( roundingMode == FLOAT_ROUND_TOWARDZERO )
                ...
                return pack_f128(zSign, NNNN + 1, u128(makemask64(f128::BITS_PER_HIGH_SIGNIFICAND), UINT64_C( 0xFFFFFFFFFFFFFFFF )) ); //0x7FFE
            }
        ...
        if ( zExp < 0 ) {
            ...
            bool isTiny = tininess_before_rounding()
                || ( zExp < -1 )
                || ! increment
                || zSig < u128( UINT64_C( 0x0001FFFFFFFFFFFF ),
    ...
    if ( increment ) {
        ++zSig;
        zSig.m1 &= ~ ( ( zSig2 + zSig2 == 0 ) & roundNearestEven );
    }
    else {
        if ( ! zSig ) zExp = 0;
    }
...
f128
CSoftFPU::NormalizeRoundPackTo_f128( sign_type zSign, fp_exponent_t zExp, u128 zSig)
{
    uint64_t zSig2 = 0;
...
f128 CSoftFPU::to_f128( int32_t a )
{
...
f128 CSoftFPU::to_f128( int64_t a )
{
    sign_type     zSign;
    uint64_t      absA;
    fp_exponent_t zExp;
...
f128 CSoftFPU::to_f128(uint64_t a)
{
    if (a == 0) {
        return f128_zero;
    }
...
int32_t CSoftFPU::to_i32( f128 a )
{
    int32_t       shiftCount;
    ...
    uint64_t zSig = aSig.m0 | ( aSig.m1 != 0 );
...
int32_t CSoftFPU::to_i32_round_to_zero( f128 a )
{
    f128::fraction_t aSig  = a.fraction();
    fp_exponent_t  aExp  = a.exponent();
    sign_type    a_sign = a.sign();
...
//        goto invalid;
    ...
    zSig |= a.HIDDEN_BIT;
    int32_t shiftCount = (a.BIAS + 112 - 64) - aExp; //0x402F
    uint64_t savedASig = zSig;
    zSig >>= shiftCount;
    int32_t z = zSig;
    if ( a_sign ) z = - z;
    if ( ( z < 0 ) ^ a_sign ) {
...
int64_t CSoftFPU::to_i64( f128 a )
{
    f128::fraction_t aSig  = a.fraction();
    fp_exponent_t  aExp  = a.exponent();
    sign_type    a_sign = a.sign();
    ...
    if ( aExp ) aSig.m0 |= a.HIDDEN_BIT;
    int32_t shiftCount = (a.BIAS + a.BITS_PER_SIGNIFICAND - 64) - aExp;  //0x402F
    if ( shiftCount <= 0 ) {
...
int64_t CSoftFPU::to_i64_round_to_zero( f128 a )
{
    int64_t      z;
    ...
    int32_t shiftCount = aExp - (a.BIAS + a.BITS_PER_SIGNIFICAND - 64); //0x402F;
    ...
    if ( 0 < shiftCount ) {
        if ( a.BIAS + 63 <= aExp ) { //0x403E
            ...
            if (    ( a.high == UINT64_C( 0xC03E000000000000 ) )
                 && ( aSig.m1 < UINT64_C( 0x0002000000000000 ) ) ) {
        ...
        if ( uint64_t( aSig.m1<<shiftCount ) ) {
            raise(FLOAT_FLAG_INEXACT);
        }
    }
    else {
        ...
        if ( aSig.m1 || ( shiftCount && uint64_t( aSig.m0<<( shiftCount & 63 ) ) ) ) {
            raise(FLOAT_FLAG_INEXACT);
        }
    }
    if ( a_sign ) z = - z;
...
f32 CSoftFPU::to_f32( f128 a )
{
    ...
    uint32_t zSig = shift64RightJamming( aSig.m0 | ( aSig.m1 != 0 ), 18);

    if ( aExp || zSig ) {
        zSig |= ( uint32_t(1) << (a.BITS_PER_HIGH_SIGNIFICAND - 18) ); // set hidden bit
...
f64 CSoftFPU::to_f64( f128 a )
{
    ...
    uint64_t zSig = aSig.m0 | ( aSig.m1 != 0 );
    if ( aExp || zSig ) {
        zSig |= ( uint64_t(1) << (a.BITS_PER_HIGH_SIGNIFICAND + 14) ); // set hidden bit
...
f128 CSoftFPU::round_to_int( f128 a )
{
    sign_type   a_sign;
    fp_exponent_t aExp;
...
f128 CSoftFPU::round_to_int(f128 a, rounding_mode_t rmode)
{
...
bool CSoftFPU::eq_signaling( f128 a, f128 b )
{
    if (a.IsNan() || b.IsNan()) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::le_signaling( f128 a, f128 b )
{
...
bool CSoftFPU::lt_signaling( f128 a, f128 b )
{
    sign_type a_sign, b_sign;
...
bool CSoftFPU::eq_quiet( f128 a, f128 b )
{
    ...
    return
           ( a.low == b.low )
        && (    ( a.high == b.high )
             || ( ( a.low == 0 ) && ( uint64_t( ( a.high | b.high )<<1 ) == 0 ) )
           );

}


bool CSoftFPU::le_quiet( f128 a, f128 b )
{
    sign_type a_sign, b_sign;
...
bool CSoftFPU::lt_quiet( f128 a, f128 b )
{
    if (a.IsNan() || b.IsNan()) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
           raise(FLOAT_FLAG_INVALID);
...
bool CSoftFPU::unordered( f128 a, f128 b )
{
    if (a.IsNan() || b.IsNan()) {
        raise(FLOAT_FLAG_INVALID);
        return true;
...
bool CSoftFPU::unordered_quiet( f128 a, f128 b )
{
    if (a.IsNan() || b.IsNan()) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
f128 CSoftFPU::to_f128( f80 a )
{
    ...
    aExp = a.exponent();
    sign_type a_sign = a.sign();
    if ( ( aExp == a.EINF ) && uint64_t( aSig<<1 ) ) {
...
f128 CSoftFPU::to_f128( f32 a )
{
    ...
    return pack_f128( a_sign, aExp + f128::BIAS - a.BIAS, u128( uint64_t(aSig) << (25), 0) ); //0x3F80

}

/* Multiply A by 2 raised to the power N.  */
f128 CSoftFPU::scalbn( f128 a, int n )
{
    f128::fraction_t aSig = a.fraction();
    fp_exponent_t  aExp   = a.exponent();
    sign_type a_sign = a.sign();
...
f128 CSoftFPU::merge(f128 sign, f128 exponent, f128 mantissa)
{
    uint64_t high =
    ...
    return f128(high, mantissa.low);
}


float_relation CSoftFPU::compare(f128 a, f128 b) {  return compare_internal(a, b, false);  }
float_relation CSoftFPU::compare_quiet(f128 a, f128 b) {  return compare_internal(a, b, true);  }

compare_mode_t CSoftFPU::compare(f128 a, f128 b, compare_mode_t mode)
{
    return compare_internal(a, b, mode);
}

f128 CSoftFPU::min(f128 a, f128 b, bool ignore_nans) {  return minmax<f128>(a, b, ignore_nans, true);  }
f128 CSoftFPU::max(f128 a, f128 b, bool ignore_nans) {  return minmax<f128>(a, b, ignore_nans, false); }
...
f128 CSoftFPU::addSigs( f128 a, f128 b, sign_type zSign)
{
    fp_exponent_t  zExp;
    f128::fraction_t zSig;
    uint64_t    zSigExtra;
...
f128 CSoftFPU::subSigs( f128 a, f128 b, sign_type zSign)
{
    fp_exponent_t  zExp;
    f128::fraction_t  zSig;

    const int FIXED_POINT = 14;
    const uint64_t FIXED_HIDDEN_BIT = uint64_t(1) << (f128::BITS_PER_HIGH_SIGNIFICAND + FIXED_POINT);
...
f128 CSoftFPU::add( f128 a, f128 b )
{
    sign_type a_sign, b_sign;
...
f128 CSoftFPU::sub( f128 a, f128 b )
{
    sign_type a_sign, b_sign;
...
f128 CSoftFPU::mul( f128 a, f128 b )
{
    f128::fraction_t aSig = a.fraction();
    fp_exponent_t  aExp = a.exponent();
    sign_type   a_sign = a.sign();
...
f128 CSoftFPU::div( f128 a, f128 b )
{
...
f128 CSoftFPU::rem( f128 a, f128 b )
{
    u192 term;
...
*----------------------------------------------------------------------------*/
extern "C" quadruple fmaq(quadruple a, quadruple b, quadruple c);

f128 CSoftFPU::muladd(f128 a, f128 b, f128 c, muladd_negate_t flags)
{
