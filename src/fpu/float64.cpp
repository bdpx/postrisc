...
commonNaNT CSoftFPU::ToCommonNaN(f64 a)
{
    commonNaNT z;
...
f64 CSoftFPU::to_f64(commonNaNT a)
{
    if ( default_nan_mode() ) {
    ...
    if ( mantissa )
        return f64( ( uint64_t(a.sign)<<63 ) | UINT64_C( 0x7FF0000000000000 ) | UINT64_C( 0x0008000000000000 ) | mantissa);
    else
        return f64::default_nan();
}
...
f64 CSoftFPU::RoundPackTo_f64(sign_type zSign, fp_exponent_t zExp, f64::fraction_t zSig)
{
...
f64 CSoftFPU::NormalizeRoundPackTo_f64(sign_type zSign, fp_exponent_t zExp, f64::fraction_t zSig)
{
    assert( zSig );
...
f64 CSoftFPU::to_f64( int32_t a)
{
    if ( a == 0 ) return f64_zero;
    sign_type zSign = get_sign(a);
    f64::fraction_t absA = zSign ? - int64_t(a) : int64_t(a);
...
f64 CSoftFPU::to_f64( int64_t a)
{
    if ( a == 0 ) return f64_zero;
    if ( a == std::numeric_limits<int64_t>::min() ) {
        return f64(SIGN_NEGATIVE, 63);
    }
    sign_type zSign = get_sign(a);
...
f64 CSoftFPU::to_f64(uint64_t a)
{
...
| returned.
*----------------------------------------------------------------------------*/
int64_t CSoftFPU::to_i64_round_to_zero( f64 a)
{
    sign_type  a_sign;
    fp_exponent_t aExp, shiftCount;
    f64::fraction_t  aSig;
    ...
    if ( 0 <= shiftCount ) {
        if ( (a.BIAS + 63) <= aExp ) { //0x43E
            if ( a != f64(SIGN_NEGATIVE, 63) ) { //UINT64_C( 0xC3E0000000000000 )
                raise(FLOAT_FLAG_INVALID);
                if ( ! a_sign || a.IsNan() ) {
                    return std::numeric_limits<int64_t>::max();
                }
...
f64 CSoftFPU::round_to_int( f64 a)
{
    ...
    if ( aExp < a.BIAS ) { //0x3FF
        if ( uint64_t( a.val()<<1 ) == 0 ) return a;
            raise(FLOAT_FLAG_INEXACT);
        ...
        switch ( get_rounding_mode() ) {
         ...
         case FLOAT_ROUND_DOWNWARD:
             return f64(a_sign ? UINT64_C( 0xBFF0000000000000 ) : 0);
         case FLOAT_ROUND_UPWARD:
             return f64(a_sign ? UINT64_C( 0x8000000000000000 ) : UINT64_C( 0x3FF0000000000000 ));
         default:
...
f64 CSoftFPU::round_to_int(f64 a, rounding_mode_t rmode)
{
...
f64 CSoftFPU::trunc_to_int( f64 a)
{
...
f64 CSoftFPU::log2( f64 a)
{
    ...
    aSig |= a.HIDDEN_BIT;
    zSign = get_sign(aExp);
    zSig = uint64_t(aExp) << a.BITS_PER_SIGNIFICAND;
    for (i = uint64_t(1) << 51; i > 0; i >>= 1) {
        mul64To128( aSig, aSig, &aSig0, &aSig1 );
        aSig = ( aSig0 << 12 ) | ( aSig1 >> 52 );
        if ( aSig & (uint64_t(1) << 53) ) {
            aSig >>= 1;
            zSig |= i;
        }
    }
...
bool CSoftFPU::eq_signaling( f64 a, f64 b)
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::le_signaling( f64 a, f64 b)
{
...
bool CSoftFPU::lt_signaling( f64 a, f64 b)
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::unordered( f64 a, f64 b)
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return true;
...
bool CSoftFPU::eq_quiet( f64 a, f64 b)
{
    if ( a.IsNan() || b.IsNan() ) {
...
bool CSoftFPU::le_quiet( f64 a, f64 b)
{
    sign_type   a_sign, b_sign;
    f64::fraction_t  av, bv;
...
bool CSoftFPU::lt_quiet( f64 a, f64 b)
{
    sign_type  a_sign, b_sign;
    f64::fraction_t av, bv;
...
bool CSoftFPU::unordered_quiet( f64 a, f64 b)
{
    if ( a.IsNan() || b.IsNan() ) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
int32_t CSoftFPU::to_i32_round_to_zero( f64 a)
{
    int32_t    z, shiftCount;
...
int16_t CSoftFPU::to_i16_round_to_zero(f64 a)
{
    fp_exponent_t aExp, shiftCount;
...
//        goto invalid;
    ...
    savedASig = aSig;
    aSig >>= shiftCount;
    int32_t z = static_cast<int32_t>(aSig);
    if ( a_sign ) {
        z = - z;
    }
...
f32 CSoftFPU::to_f32( f64 a)
{
    ...
    uint32_t zSig = static_cast<uint32_t>(aSig);
    if ( aExp || zSig ) {
        zSig |= ( uint32_t(1) << (f32::BITS_PER_SIGNIFICAND + 7) );
...
f64 CSoftFPU::to_f64( f32 a)
{
    ...
    return pack_f64( a_sign,
                         ...
                         uint64_t(aSig) << (f64::BITS_PER_SIGNIFICAND - a.BITS_PER_SIGNIFICAND) ); //0x380

}

f64 CSoftFPU::to_f64(uint32_t a)
{
    return to_f64(int64_t(a));
}

uint32_t CSoftFPU::to_u32( f64 a)
{
    uint32_t res;
    ...
    if (v < 0) {
        ...
        res = static_cast<uint32_t>(v);
    }
    return res;
}

uint32_t CSoftFPU::to_u32_round_to_zero( f64 a)
{
    uint32_t res;
...
uint16_t CSoftFPU::to_u16_round_to_zero(f64 a)
{
    uint16_t res;
...
uint64_t CSoftFPU::to_u64 (f64 a)
{
    int64_t v;

    v = to_f64( std::numeric_limits<int64_t>::min() ).val();
    v += a.val();
    v = to_i64( f64( uint64_t(v) ) );

    return v - std::numeric_limits<int64_t>::min();
}

uint64_t CSoftFPU::to_u64_round_to_zero (f64 a)
{
    int64_t v;

    v = to_f64(std::numeric_limits<int64_t>::min()).val();
    v += a.val();
    v = to_i64_round_to_zero( f64( uint64_t(v) ) );

    return v - std::numeric_limits<int64_t>::min();
}

/* Multiply A by 2 raised to the power N.  */
f64 CSoftFPU::scalbn( f64 a, int n)
{
    f64::fraction_t aSig = a.fraction();
    fp_exponent_t aExp = a.exponent();
    sign_type  a_sign = a.sign();
...
f64 CSoftFPU::merge(f64 sign, f64 exponent, f64 mantissa)
{
    return f64(
...
float_relation CSoftFPU::compare(f64 a, f64 b) {  return compare_internal(a, b, false);  }
float_relation CSoftFPU::compare_quiet(f64 a, f64 b) {  return compare_internal(a, b, true);  }

compare_mode_t CSoftFPU::compare(f64 a, f64 b, compare_mode_t mode)
{
    return compare_internal(a, b, mode);
}

f64 CSoftFPU::min(f64 a, f64 b, bool ignore_nans) {  return minmax<f64>(a, b, ignore_nans, true);  }
f64 CSoftFPU::max(f64 a, f64 b, bool ignore_nans) {  return minmax<f64>(a, b, ignore_nans, false); }
...
| Floating-Point Arithmetic.
*----------------------------------------------------------------------------*/
f64 CSoftFPU::addSigs( f64 a, f64 b, sign_type zSign)
{
    ...
    const uint64_t FIXED_HIDDEN_BIT = uint64_t(1) << (52 + FIXED_POINT);
...
*----------------------------------------------------------------------------*/
f64 CSoftFPU::subSigs( f64 a, f64 b, sign_type zSign)
{
    fp_exponent_t aExp, bExp, zExp;
    f64::fraction_t aSig, bSig, zSig;
    fp_exponent_t expDiff;
...
f64 CSoftFPU::muladd(f64 a, f64 b, f64 c, muladd_negate_t flags)
{
    ...
    cSig1 = 0;
    cSig0 |= UINT64_C( 0x4000000000000000 );
    expDiff = pExp - cExp;
