...
commonNaNT CSoftFPU::ToCommonNaN(f512 a)
{
    commonNaNT z;
...
f512 CSoftFPU::to_f512(commonNaNT a)
{
    uint64_t high, low;
    ...
    high |= ( uint64_t(a.sign)<<63 ) | UINT64_C( 0x7FFFFF0000000000 ) | UINT64_C( 0x0000008000000000 );
...
f512 CSoftFPU::RoundPackTo_f512(
    sign_type zSign, fp_exponent_t zExp, u512 & zSig, uint64_t zSigExtra)
{
    ...
    bool increment = ( int64_t(zSigExtra) < 0 );
    if ( ! roundNearestEven ) {
    ...
    if ( uint32_t(NNNN) <= uint32_t(zExp) ) {

        const u512 full_ones( UINT64_C( 0x000001FFFFFFFFFF ),
    ...
    if ( increment ) {
        ++zSig;
        zSig.m7 &= ~ ( ( zSigExtra + zSigExtra == 0 ) & roundNearestEven );
    }
    else {
        if ( ! zSig ) zExp = 0;
    }
...
f512 CSoftFPU::NormalizeRoundPackTo_f512( sign_type zSign, fp_exponent_t zExp, u512 & zSig )
{
    fp_exponent_t count = zSig.normalize_subnormal() - 1;

    uint64_t zSigExtra = 0;
...
f512 CSoftFPU::to_f512( int32_t a )
{
...
f512 CSoftFPU::to_f512( int64_t a )
{
    sign_type   zSign;
    uint64_t      absA;
    fp_exponent_t zExp;
...
f512 CSoftFPU::to_f512(uint64_t a)
{
    if (a == 0) {
...
int32_t CSoftFPU::to_i32( f512 a )
{
    sign_type   a_sign;
...
int32_t CSoftFPU::to_i32_round_to_zero( f512 a )
{
    sign_type  a_sign;
    fp_exponent_t aExp, shiftCount;
    u512    aSig;
 ...
 invalid:
        raise(FLOAT_FLAG_INVALID);
        return a_sign ? std::numeric_limits<int32_t>::min() : std::numeric_limits<int32_t>::max();
    }
...
int64_t CSoftFPU::to_i64( f512 a )
{
    sign_type   a_sign;
    fp_exponent_t  aExp, shiftCount;
    u512     aSig;
    ...
    if ( shiftCount <= 0 ) {
        if ( a.BIAS + 63 < aExp ) { //0x403E
            ...
            return a_sign ? std::numeric_limits<int64_t>::min() : std::numeric_limits<int64_t>::max();
        }
        aSig <<= -shiftCount;
    }
    else {
...
int64_t CSoftFPU::to_i64_round_to_zero( f512 a )
{
    sign_type  a_sign;
    fp_exponent_t aExp, shiftCount;
    u512    aSig;
    ...
    if ( 0 < shiftCount ) {
        if ( a.BIAS + 63 <= aExp ) {
            ...
            if (    ( a.m0 == UINT64_C( 0xC0003E0000000000 ) )
                 && ( aSig.m1 < UINT64_C( 0x0000020000000000 ) ) ) {
        ...
        if ( (aSig.m1 | aSig.m2 | aSig.m3 | aSig.m4 | aSig.m5 | aSig.m6 | aSig.m7)
              || ( shiftCount && uint64_t( aSig.m0<<( shiftCount & 63 ) ) ) ) {
            raise(FLOAT_FLAG_INEXACT);
        }
    }
    if ( a_sign ) z = - z;
...
f512 CSoftFPU::to_f512( f32 a )
{
    uint32_t aSig = a.fraction();
    ...
    return pack_f512( a_sign, aExp + f512::BIAS - a.BIAS,
              u512( uint64_t(aSig) << (32 - (f512::BITS_PER_EXPONENT - f32::BITS_PER_EXPONENT)), 0, 0, 0, 0, 0, 0, 0 ) );

}


f32 CSoftFPU::to_f32( f512 a )
{
    ...
    uint32_t zSig = aSig.m0;
    if ( aExp || zSig ) {
        zSig |= ( uint32_t(1) << (f32::BITS_PER_SIGNIFICAND + 7) ); // set hidden bit
...
f64 CSoftFPU::to_f64( f512 a )
{
    ...
    aSig.m0 |= ( (aSig.m1 | aSig.m2 | aSig.m3 | aSig.m4 | aSig.m5 | aSig.m6 | aSig.m7) != 0 );
    if ( aExp || aSig.m0 ) {
        aSig.m0 |= ( uint64_t(1) << (a.BITS_PER_HIGH_SIGNIFICAND + 22) ); // set hidden bit
...
f512 CSoftFPU::to_f512( f80 a )
{
    uint64_t    aSig  = a.fraction();
    fp_exponent_t aExp  = a.exponent();
    sign_type a_sign = a.sign();
...
f512 CSoftFPU::round_to_int( f512 a )
{
    fp_exponent_t aExp = a.exponent();
    u512 aSig = a.fraction();
    ...
    //lastBitMask = 1 << count;
    if (count < 64) {
       lastBitMask = u512( 0, 0, 0, 0, 0, 0, 0, uint64_t(1) << (count - 0*64) );
    } else if (count < 2*64) {
       lastBitMask = u512( 0, 0, 0, 0, 0, 0, uint64_t(1) << (count - 1*64), 0 );
    } else if (count < 3*64) {
       lastBitMask = u512( 0, 0, 0, 0, 0, uint64_t(1) << (count - 2*64), 0, 0 );
    } else if (count < 4*64) {
       lastBitMask = u512( 0, 0, 0, 0, uint64_t(1) << (count - 3*64), 0, 0, 0 );
    } else if (count < 5*64) {
       lastBitMask = u512( 0, 0, 0, uint64_t(1) << (count - 4*64), 0, 0, 0, 0 );
    } else if (count < 6*64) {
       lastBitMask = u512( 0, 0, uint64_t(1) << (count - 5*64), 0, 0, 0, 0, 0 );
    } else if (count < 7*64) {
       lastBitMask = u512( 0, uint64_t(1) << (count - 6*64), 0, 0, 0, 0, 0, 0 );
    } else if (count < 8*64) {
       lastBitMask = u512( uint64_t(1) << (count - 7*64), 0, 0, 0, 0, 0, 0, 0 );
    } else {
       //!!!!!
...
f512 CSoftFPU::round_to_int(f512 a, rounding_mode_t rmode)
{
...
bool CSoftFPU::eq_signaling( f512 a, f512 b )
{
    if (a.IsNan() || b.IsNan()) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::le_signaling( f512 a, f512 b )
{
...
bool CSoftFPU::lt_signaling( f512 a, f512 b )
{
    sign_type a_sign, b_sign;
...
bool CSoftFPU::eq_quiet( f512 a, f512 b )
{
...
bool CSoftFPU::le_quiet( f512 a, f512 b )
{
    sign_type a_sign, b_sign;
...
bool CSoftFPU::lt_quiet( f512 a, f512 b )
{
    if (a.IsNan() || b.IsNan()) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
           raise(FLOAT_FLAG_INVALID);
...
bool CSoftFPU::unordered( f512 a, f512 b )
{
    if (a.IsNan() || b.IsNan()) {
        raise(FLOAT_FLAG_INVALID);
        return true;
...
bool CSoftFPU::unordered_quiet( f512 a, f512 b )
{
    if (a.IsNan() || b.IsNan()) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
f512 CSoftFPU::scalbn( f512 a, int n )
{
    u512   aSig = a.fraction();
    fp_exponent_t aExp = a.exponent();
    sign_type a_sign = a.sign();
...
f512 CSoftFPU::merge(f512 sign, f512 exponent, f512 mantissa)
{
    uint64_t high =
...
float_relation CSoftFPU::compare(f512 a, f512 b) {  return compare_internal(a, b, false);  }
float_relation CSoftFPU::compare_quiet(f512 a, f512 b) {  return compare_internal(a, b, true);  }

f512 CSoftFPU::min(f512 a, f512 b, bool ignore_nans) {  return minmax<f512>(a, b, ignore_nans, true);  }
f512 CSoftFPU::max(f512 a, f512 b, bool ignore_nans) {  return minmax<f512>(a, b, ignore_nans, false); }
...
f512 CSoftFPU::addSigs( f512 a, f512 b, sign_type zSign)
{
    ...
    const uint64_t OVER_HIDDEN_BIT = a.HIDDEN_BIT << 1; //UINT64_C( 0x0002000000000000 );
...
f512 CSoftFPU::subSigs( f512 a, f512 b, sign_type zSign)
{
    fp_exponent_t aExp, bExp, zExp;
    u512   zSig;
    fp_exponent_t expDiff;
    ...
    const uint64_t FIXED_HIDDEN_BIT = uint64_t(1) << (f512::BITS_PER_HIGH_SIGNIFICAND + FIXED_POINT);
...
f512 CSoftFPU::add( f512 a, f512 b )
{
    sign_type a_sign, b_sign;
...
f512 CSoftFPU::sub( f512 a, f512 b )
{
    sign_type a_sign, b_sign;
...
f512 CSoftFPU::mul( f512 a, f512 b )
{
...
uint64_t CSoftFPU::divide_step( const u512 & bSig, u576 & rem )
{
    u576 term;
    uint64_t z = estimateDiv128To64( rem.m1, rem.m2, bSig.m0 );
    term.mul( bSig, z );
    rem = u576(rem.m1, rem.m2, rem.m3, rem.m4, rem.m5, rem.m6, rem.m7, rem.m8, 0) - term;
    while ( int64_t(rem.m0) < 0 ) {
        --z;
        rem += bSig;
    }
    return z;
...
f512 CSoftFPU::div( f512 a, f512 b )
{
    sign_type  a_sign, b_sign, zSign;
    u512    zSig;
    u576    term;
 ...
 invalid:
    ...
    uint64_t zSigExtra = zSig.jamming( a.BITS_PER_EXPONENT, 0 );
    return RoundPackTo_f512( zSign, zExp, zSig, zSigExtra );
}


f512 CSoftFPU::rem( f512 a, f512 b )
{
    u512   aSig  = a.fraction();
    fp_exponent_t aExp  = a.exponent();
    sign_type a_sign = a.sign();
 ...
 invalid:
    ...
    do {
        ...
        ++q;
        aSig -= bSig;
    } while ( int64_t(aSig.m0) >= 0 );

    u512 sigMean;
    sigMean.add( aSig, alternateASig );

    if ( ( int64_t(sigMean.m0) < 0 ) || ( !sigMean && ( q & 1 ) ) ) {
        aSig = alternateASig;
    }

    sign_type zSign = get_sign( int64_t(aSig.m0) );
...
CSoftFPU::sqrt( f512 a )
{
 ...
 invalid:
    ...
    uint64_t const zSigExtra = zSig.jamming( a.BITS_PER_EXPONENT - 1, 0 );
...
*----------------------------------------------------------------------------*/
f512
CSoftFPU::muladd(f512 a, f512 b, f512 c, muladd_negate_t flags)
{
    ...
    u1024 pSig;
    pSig.mul( aSig, bSig );
    if ( int64_t(pSig.m0 << 1) >= 0 ) {
        pSig <<= 1;
        pExp--;
    }
    assert ( int64_t(pSig.m0) >= 0 );
