...
commonNaNT CSoftFPU::ToCommonNaN(f80 a)
{
    commonNaNT z;
...
f80 CSoftFPU::to_f80(commonNaNT a)
{
    if (default_nan_mode()) {
    ...
    if (mantissa)
        return f80( ( uint16_t(a.sign)<<15 ) | f80::EINF, f80::UNHIDDEN_BIT | UINT64_C( 0x4000000000000000 ) | mantissa);
    else
        return f80::default_nan();
}
...
f80 CSoftFPU::RoundPackTo_f80(rounding_precision_t roundingPrecision,
        sign_type zSign, fp_exponent_t zExp, f80::fraction_t zSig0, uint64_t zSig1)
{
    bool    increment;
    ...
    if ( uint32_t(NNNN) <= uint32_t( zExp - 1 ) ) {
        if (    ( 0x7FFE < zExp )
 ...
 precision80:
    ...
    if ( 0x7FFD <= uint32_t( zExp - 1 ) ) {
        if (    ( 0x7FFE < zExp )
             || (    ( zExp == 0x7FFE )
                  && ( zSig0 == std::numeric_limits<uint64_t>::max() )
                  && increment
                )
           ) {
            roundMask = 0;
...
f80 CSoftFPU::NormalizeRoundPackTo_f80(
     rounding_precision_t roundingPrecision,
     sign_type zSign, fp_exponent_t zExp, uint64_t zSig0, uint64_t zSig1)
{
    if ( zSig0 == 0 ) {
        zSig0 = zSig1;
        zSig1 = 0;
...
int32_t CSoftFPU::to_i32_round_to_zero( f80 a )
{
    sign_type   a_sign;
    ...
    if ( (a.BIAS + 31) < aExp ) {  //0x401E
        if ( ( aExp == a.EINF ) && uint64_t( aSig<<1 ) ) a_sign = SIGN_POSITIVE;
        goto invalid;
    }
    else if ( aExp < a.BIAS ) {
...
int64_t CSoftFPU::to_i64_round_to_zero( f80 a )
{
    sign_type   a_sign;
    fp_exponent_t aExp, shiftCount;
    f80::fraction_t  aSig;
    ...
    if ( 0 <= shiftCount ) {
        if ( a != f80(SIGN_NEGATIVE, 63) ) { //0xC03E
            raise(FLOAT_FLAG_INVALID);
            if ( ! a_sign || a.IsNan() ) {
                return std::numeric_limits<int64_t>::max();
            }
...
f80 CSoftFPU::round_to_int( f80 a )
{
    sign_type        a_sign;
    ...
    if ( (a.BIAS + 63) <= aExp ) { //0x403E
        if ( ( aExp == a.EINF ) && uint64_t( a.fraction()<<1 ) ) {
            return propagate_nan(a);
        }
        return a;
    }
    if ( aExp < a.BIAS ) {
        if ( ( aExp == 0 ) && ( uint64_t( a.fraction()<<1 ) == 0 ) ) {
            return a;
        }
        raise(FLOAT_FLAG_INEXACT);
        a_sign = a.sign();
        switch ( get_rounding_mode() ) {
         case FLOAT_ROUND_TONEAREST_EVEN:
            if ( ( aExp == a.BIAS - 1 ) && uint64_t( a.fraction()<<1 ) ) {
...
f80 CSoftFPU::round_to_int(f80 a, rounding_mode_t rmode)
{
...
f80 CSoftFPU::log2(f80 a)
{
    ...
    for (i = uint64_t(1) << 51; i > 0; i >>= 1) {
        ...
        if ( aSig & ( uint64_t(1) << (f64::BITS_PER_SIGNIFICAND + 1) ) ) {
            aSig >>= 1;
            zSig |= i;
        }
    }
...
bool CSoftFPU::eq_signaling( f80 a, f80 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::le_signaling( f80 a, f80 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::lt_signaling( f80 a, f80 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return false;
...
bool CSoftFPU::unordered( f80 a, f80 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        raise(FLOAT_FLAG_INVALID);
        return true;
...
bool CSoftFPU::eq_quiet( f80 a, f80 b )
{
...
bool CSoftFPU::le_quiet( f80 a, f80 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
bool CSoftFPU::lt_quiet( f80 a, f80 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
bool CSoftFPU::unordered_quiet( f80 a, f80 b )
{
    if ( a.IsNan() || b.IsNan() ) {
        if ( a.IsSignalingNan() || b.IsSignalingNan() ) {
            raise(FLOAT_FLAG_INVALID);
...
f80 CSoftFPU::to_f80( f32 a )
{
    ...
    aSig |= a.HIDDEN_BIT;
    return pack_f80( a_sign, aExp - a.BIAS + f80::BIAS, uint64_t( aSig ) << (64 - 24) ); //0x3F80

}

// Multiply A by 2 raised to the power N.
f80 CSoftFPU::scalbn( f80 a, int n )
{
    f80::fraction_t aSig  = a.fraction();
    fp_exponent_t aExp  = a.exponent();
    sign_type  a_sign = a.sign();
...
f80 CSoftFPU::merge(f80 sign, f80 exponent, f80 mantissa)
{
    uint16_t high = (sign.high & (UINT16_C(1) << f80::BITS_PER_EXPONENT) ) |
                  lobits(exponent.high, f80::BITS_PER_EXPONENT);
    return f80(high, mantissa.low);
}


float_relation CSoftFPU::compare(f80 a, f80 b) {  return compare_internal(a, b, false);  }
float_relation CSoftFPU::compare_quiet(f80 a, f80 b) {  return compare_internal(a, b, true);  }

f80 CSoftFPU::min(f80 a, f80 b, bool ignore_nans) {  return minmax<f80>(a, b, ignore_nans, true);  }
f80 CSoftFPU::max(f80 a, f80 b, bool ignore_nans) {  return minmax<f80>(a, b, ignore_nans, false); }
...
f80
CSoftFPU::addSigs( f80 a, f80 b, sign_type zSign)
{
    ...
    if ( 0 < expDiff ) {
        if ( aExp == a.EINF ) {
            if ( uint64_t( aSig<<1 ) ) return propagate_nan(a, b);
            return a;
        }
        if ( bExp == 0 ) --expDiff;
        ...
        if ( bExp == b.EINF ) {
            if ( uint64_t( bSig<<1 ) ) return propagate_nan(a, b);
            return b.inf(zSign);
        }
        if ( aExp == 0 ) ++expDiff;
...
f80
CSoftFPU::subSigs( f80 a, f80 b, sign_type zSign )
{
    fp_exponent_t aExp, bExp, zExp;
    f80::fraction_t aSig, bSig, zSig0, zSig1;
    int     expDiff;
...
f80 CSoftFPU::rem( f80 a, f80 b )
{
    ...
    if ( bExp == b.EINF ) {
        if ( uint64_t( bSig<<1 ) ) return propagate_nan(b);
        return a;
    }
    if ( bExp == 0 ) {
        if ( bSig == 0 ) {
...
f80 CSoftFPU::sqrt( f80 a )
{
    ...
    a_sign = a.sign();
    if ( aExp == a.EINF ) {
        if ( uint64_t( aSig0<<1 ) ) return propagate_nan(a);
        if ( ! a_sign ) return a;
        goto invalid;
    }
    if ( a_sign ) {
...
f80 CSoftFPU::muladd(f80 a, f80 b, f80 c, muladd_negate_t flags)
{
    ...
    uint64_t      pSig0, pSig1, pSig2, cSig0, cSig1, cSig2, zSig0, zSig1, zSig2;
    ...
    if (pSign == cSign) {
        ...
        if (expDiff > 0) {
            shift192RightJamming(cSig0, cSig1, cSig2, expDiff, &cSig0, &cSig1, &cSig2);
            //if (expDiff >= 128) {
            //   if (pSig1 == UINT64_C(0x8000000000000001) && cSig1 == UINT64_C(1)) {
            //      cSig1 = 0;
            //   }
            //}
