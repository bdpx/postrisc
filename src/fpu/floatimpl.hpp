...
******************************************************************************************/
#define f32_zero       f32( UINT32_C( 0x00000000 ) )
#define f32_one        f32( UINT32_C( 0x3f800000 ) )
#define f32_ln2        f32( UINT32_C( 0x3f317218 ) )
#define f32_pi         f32( UINT32_C( 0x40490fdb ) )
#define f32_half       f32( UINT32_C( 0x3f000000 ) )
#define f32_infinity   f32( UINT32_C( 0x7f800000 ) )

#define f64_zero       f64( UINT64_C( 0x0000000000000000 ) )
#define f64_one        f64( UINT64_C( 0x3ff0000000000000 ) )
#define f64_ln2        f64( UINT64_C( 0x3fe62e42fefa39ef ) )
#define f64_pi         f64( UINT64_C( 0x400921fb54442d18 ) )
#define f64_half       f64( UINT64_C( 0x3fe0000000000000 ) )
#define f64_infinity   f64( UINT64_C( 0x7ff0000000000000 ) )

#define f80_zero      f80( 0x0000, UINT64_C( 0x0000000000000000 ) )
#define f80_one       f80( 0x3fff, UINT64_C( 0x8000000000000000 ) )
#define f80_ln2       f80( 0x3ffe, UINT64_C( 0xb17217f7d1cf79ac ) )
#define f80_pi        f80( 0x4000, UINT64_C( 0xc90fdaa22168c235 ) )
#define f80_half      f80( 0x3ffe, UINT64_C( 0x8000000000000000 ) )
#define f80_infinity  f80( 0x7fff, UINT64_C( 0x8000000000000000 ) )

#define f128_zero     f128( UINT64_C( 0x0000000000000000 ), UINT64_C( 0x0000000000000000 ) )
#define f128_one      f128( UINT64_C( 0x3fff000000000000 ), UINT64_C( 0x0000000000000000 ) )
#define f128_pi       f128( UINT64_C( 0x4000921fb54442d1 ), UINT64_C( 0x8469898cc51701b8 ) )
#define f128_half     f128( UINT64_C( 0x3ffe000000000000 ), UINT64_C( 0x0000000000000000 ) )
#define f128_infinity f128( UINT64_C( 0x7fff000000000000 ), UINT64_C( 0x0000000000000000 ) )
...
{
    if (a.IsSignalingNan()) {
       return f16(a.val() | (UINT16_C(1) << (a.BITS_PER_SIGNIFICAND-1)));
    }
    return a;
}
...
{
    if (a.IsSignalingNan()) {
        return f32(a.val() | (UINT32_C(1) << (a.BITS_PER_SIGNIFICAND-1)));
    }
    return a;
}
...
{
    if (a.IsSignalingNan()) {
        return f64(a.val() | (UINT64_C(1) << (a.BITS_PER_SIGNIFICAND-1)));
    }
    return a;
}
...
{
    if (a.IsSignalingNan()) {
        a.m0 |= UINT64_C( 0x0000080000000000 );
        return a;
    }
    return a;
}
...
{
    if (a.IsSignalingNan()) {
        a.m0 |= UINT64_C( 0x0000008000000000 );
        return a;
    }
    return a;
}
...
inline f16 pack_f16(sign_type zSign, fp_exponent_t zExp, uint16_t zSig)
{
    return f16( (uint16_t(zSign) << 15) + (uint16_t(zExp) << f16::BITS_PER_SIGNIFICAND) + zSig );
}

inline f32 pack_f32(sign_type zSign, fp_exponent_t zExp, uint32_t zSig)
{
    return f32( (uint32_t(zSign) << 31) + (uint32_t(zExp) << f32::BITS_PER_SIGNIFICAND ) + zSig );
}

inline f64 pack_f64(sign_type zSign, fp_exponent_t zExp, uint64_t zSig)
{
    return f64( (uint64_t(zSign) << 63) + (uint64_t(zExp) << f64::BITS_PER_SIGNIFICAND ) + zSig );
}

inline f80 pack_f80( sign_type zSign, fp_exponent_t zExp, uint64_t zSig )
{
    return f80( ( uint16_t(zSign) << f80::BITS_PER_EXPONENT ) + zExp, zSig);
}

inline f128 pack_f128( sign_type zSign, fp_exponent_t zExp, u128 zSig )
{
    return f128( ( uint64_t(zSign) << 63 ) + ( uint64_t(zExp) << f128::BITS_PER_HIGH_SIGNIFICAND ) + zSig.m0,
                     zSig.m1);
}

inline f256 pack_f256( sign_type zSign, fp_exponent_t zExp, u256 zSig )
{
    return f256( ( uint64_t(zSign) << 63 ) + ( uint64_t(zExp) << f256::BITS_PER_HIGH_SIGNIFICAND ) + zSig.m0,
                     zSig.m1, zSig.m2, zSig.m3 );
}

inline f512 pack_f512( sign_type zSign, fp_exponent_t zExp, u512 zSig )
{
    return f512( ( uint64_t(zSign) << 63 ) + ( uint64_t(zExp) << f512::BITS_PER_HIGH_SIGNIFICAND ) + zSig.m0,
...
{
    return f32(lobits(a.val(), 31) | (uint32_t(sign) << 31));
}
inline f64 set_sign(f64 a, sign_type sign)
{
    return f64(lobits(a.val(), 63) | (uint64_t(sign) << 63));
}
...
inline bool eq128(uint64_t a0, uint64_t a1, uint64_t b0, uint64_t b1) {  return ( a0 == b0 ) && ( a1 == b1 );  }
inline bool le128(uint64_t a0, uint64_t a1, uint64_t b0, uint64_t b1) {  return ( a0 <  b0 ) || ( ( a0 == b0 ) && ( a1 <= b1 ) ); }
inline bool lt128(uint64_t a0, uint64_t a1, uint64_t b0, uint64_t b1) {  return ( a0 <  b0 ) || ( ( a0 == b0 ) && ( a1 <  b1 ) );  }
...
*----------------------------------------------------------------------------*/
inline uint64_t shift64RightJamming(uint64_t a, int count)
{
    uint64_t z;
...
inline
void shift192RightJamming(uint64_t a0, uint64_t a1, uint64_t a2,
       int count, uint64_t *z0Ptr, uint64_t *z1Ptr, uint64_t *z2Ptr)
{
    uint64_t z0, z1, z2;
...
inline void add128(uint64_t a0, uint64_t a1, uint64_t b0, uint64_t b1, uint64_t *z0Ptr, uint64_t *z1Ptr )
{
    const uint64_t z1 = a1 + b1;
    *z1Ptr = z1;
    *z0Ptr = a0 + b0 + ( z1 < a1 );
}
...
ALWAYS_INLINE fp_exponent_t normalize_subnormal_f32(uint32_t& aSig)
{
    assert( aSig );
...
ALWAYS_INLINE fp_exponent_t normalize_subnormal_f64(uint64_t& aSig)
{
    assert( aSig );
...
ALWAYS_INLINE fp_exponent_t normalize_subnormal_f80(uint64_t& aSig)
{
    assert( aSig );
...
FLOAT CSoftFPU::propagate_nan( FLOAT a)
{
...
template<class FLOAT>
FLOAT CSoftFPU::propagate_nan( FLOAT a, FLOAT b)
{
    int which;
...
*----------------------------------------------------------------------------*/
template<class FLOAT>
FLOAT CSoftFPU::propagate_nan(FLOAT a, FLOAT b, FLOAT c, bool infzero)
{
    int which;
 ...
 */
template<class FLOAT>
FLOAT CSoftFPU::minmax(FLOAT a, FLOAT b, bool ignore_nans, bool ismin)
{
...
template<class FLOAT> float_relation
CSoftFPU::compare_internal( FLOAT a, FLOAT b, bool is_quiet)
{
    if (a.IsNan() || b.IsNan()) {
...
template<class FLOAT>
compare_mode_t
CSoftFPU::compare_internal(FLOAT a, FLOAT b, compare_mode_t mode)
{
    if (a.IsNan() || b.IsNan()) {
