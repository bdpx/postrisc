...
template<class OP, class FPU> static void timing(const OP& op, FPU& fpu, const std::string& name)
{
...
template<class OP, class FPU> static void timing_rm(const OP& op, FPU& fpu, const std::string& name)
{
    static const struct {
...
template<class T, class FPU, class OP> class op_f1 {
public:
...
template<class T, class FPU, class OP> class op_f2 {
public:
    void func(FPU& fpu, i64 count) const {
        u32 inputNumA = 0;
        u32 inputNumB = 0;
...
template<class T, class FPU, class OP> class op_f3 {
public:
    void func(FPU& fpu, i64 count) const {
        u32 inputNumA = 0;
        u32 inputNumB = 0;
...
class fadd    { public: template<class T, class FPU> static T func(FPU& fpu, T a, T b     ) { return fpu.add(a, b); }};
class fsub    { public: template<class T, class FPU> static T func(FPU& fpu, T a, T b     ) { return fpu.sub(a, b); }};
class fmul    { public: template<class T, class FPU> static T func(FPU& fpu, T a, T b     ) { return fpu.mul(a, b); }};
class fdiv    { public: template<class T, class FPU> static T func(FPU& fpu, T a, T b     ) { return fpu.div(a, b); }};
class fmuladd { public: template<class T, class FPU> static T func(FPU& fpu, T a, T b, T c) { return fpu.muladd(a, b, c, fpu::FLOAT_MULADD_NEGATE_NONE); }};

template<class T, class FPU, class OP> static void test_f2(const std::string& name)
{
    FPU fpu;
    op_f2<T, FPU, OP> op;
...
template<class T, class FPU, class OP> static void test_f3(const std::string& name)
{
    FPU fpu;
    op_f3<T, FPU, OP> op;
    timing_rm(op, fpu, name);
...
template<class FPU, class OP> static void test_f2_timing(const std::string& name)
{
...
template<class FPU, class OP> static void test_f3_timing(const std::string& name)
{
...
BOOST_AUTO_TEST_CASE(timing_add) { test_f2_timing<fpu::CSoftFPU, fadd>("add"); }
BOOST_AUTO_TEST_CASE(timing_sub) { test_f2_timing<fpu::CSoftFPU, fsub>("sub"); }
BOOST_AUTO_TEST_CASE(timing_mul) { test_f2_timing<fpu::CSoftFPU, fmul>("mul"); }
BOOST_AUTO_TEST_CASE(timing_div) { test_f2_timing<fpu::CSoftFPU, fdiv>("div"); }
BOOST_AUTO_TEST_CASE(timing_muladd) { test_f3_timing<fpu::CSoftFPU, fmuladd>("muladd"); }
