...
template<>
const std::array<f16, NUM_INPUTS> test_data<f16>::values =
{
    FLOAT16_C( 0x4EFA ),
    FLOAT16_C( 0xC1D0 ),
    FLOAT16_C( 0x8000 ),
...
template<typename OP, typename FPU>
static void timing(const OP& op, FPU& fpu, RoundingModeEnum rm, const std::string& name)
{
    TimePoint startClock = Time::now();
    i64 count = 0;
    const i64 MIN_ITERATIONS = 500 * 1000;
    do {
        op.func(fpu, rm, MIN_ITERATIONS);
        count += MIN_ITERATIONS;
    } while (duration_is_enough(startClock));

    startClock = Time::now();
    op.func(fpu, rm, count);
    reportTime(count, duration_in_seconds(startClock), name);
}

template<typename OP, typename FPU>
static void timing_rm(const OP& op, FPU& fpu, const std::string& name)
{
    for (auto const& test : rms) {
        fpu.set_rounding_mode(test.rm);
        timing(op, fpu, test.rm, name+test.name);
    }
}

template<typename T, typename FPU, typename OP> class op_f1 {
public:
    void func(FPU& fpu, RoundingModeEnum rm, i64 count) const {
        u32 inputNum = 0;
        for (i64 i = count; i != 0; --i) {
            volatile auto result = OP::template func<T,FPU>(fpu,
                test_data<T>::values[inputNum], rm);
...
public:
    void func(FPU& fpu, RoundingModeEnum rm, i64 count) const {
        ...
        for (i64 i = count; i != 0; --i) {
            volatile auto result = OP::template func<T,FPU>(fpu,
                test_data<T>::values[inputNumA],
                test_data<T>::values[inputNumB], rm);
            UNREFERENCED_PARAMETER(result);
    ...
    void func(FPU& fpu, RoundingModeEnum rm, i64 count) const {
        ...
        for (i64 i = count; i != 0; --i) {
            volatile auto result = OP::template func<T,FPU>(fpu,
                ...
                test_data<T>::values[inputNumC], rm);
            UNREFERENCED_PARAMETER(result);
            inputNumA = next_index(inputNumA);
            if (inputNumA == 0) {
                inputNumB = next_index(inputNumB);
...
class fsqrt   { public: template<typename T, typename FPU> static T func(FPU& fpu, T a, RoundingModeEnum rm) { return fpu.sqrt(a, rm); }};

class fadd    { public: template<typename T, typename FPU> static T func(FPU& fpu, T a, T b, RoundingModeEnum rm) { return fpu.add(a, b, rm); }};
class fsub    { public: template<typename T, typename FPU> static T func(FPU& fpu, T a, T b, RoundingModeEnum rm) { return fpu.sub(a, b, rm); }};
class fmul    { public: template<typename T, typename FPU> static T func(FPU& fpu, T a, T b, RoundingModeEnum rm) { return fpu.mul(a, b, rm); }};
class fdiv    { public: template<typename T, typename FPU> static T func(FPU& fpu, T a, T b, RoundingModeEnum rm) { return fpu.div(a, b, rm); }};

class fmuladd { public: template<typename T, typename FPU> static T func(FPU& fpu, T a, T b, T c, RoundingModeEnum rm) { return fpu.muladd(a, b, c, rm, FLOAT_MULADD_NEGATE_NONE); }};

template<typename T, typename FPU, typename OP> static void test_f1(const std::string& name)
{
    FPU fpu;
    op_f1<T, FPU, OP> op;
...
template<typename FPU, typename OP> static void test_f1_timing(const std::string& name)
{
    test_f1<f16,  FPU, OP>(name+":f16");
    test_f1<f32,  FPU, OP>(name+":f32");
    test_f1<f64,  FPU, OP>(name+":f64");
    test_f1<f128, FPU, OP>(name+":f128");
}

template<typename FPU, typename OP> static void test_f2_timing(const std::string& name)
{
    test_f2<f16,  FPU, OP>(name+":f16");
    test_f2<f32,  FPU, OP>(name+":f32");
    test_f2<f64,  FPU, OP>(name+":f64");
    test_f2<f128, FPU, OP>(name+":f128");
}

template<typename FPU, typename OP> static void test_f3_timing(const std::string& name)
{
    test_f3<f16,  FPU, OP>(name+":f16");
    test_f3<f32,  FPU, OP>(name+":f32");
    test_f3<f64,  FPU, OP>(name+":f64");
    test_f3<f128, FPU, OP>(name+":f128");
}

BOOST_AUTO_TEST_SUITE(test_timing)

BOOST_AUTO_TEST_CASE(timing_sqrt) { test_f1_timing<SoftFPU, fsqrt>("sqrt"); }
BOOST_AUTO_TEST_CASE(timing_add) { test_f2_timing<SoftFPU, fadd>("add"); }
BOOST_AUTO_TEST_CASE(timing_sub) { test_f2_timing<SoftFPU, fsub>("sub"); }
BOOST_AUTO_TEST_CASE(timing_mul) { test_f2_timing<SoftFPU, fmul>("mul"); }
BOOST_AUTO_TEST_CASE(timing_div) { test_f2_timing<SoftFPU, fdiv>("div"); }
BOOST_AUTO_TEST_CASE(timing_muladd) { test_f3_timing<SoftFPU, fmuladd>("muladd"); }
