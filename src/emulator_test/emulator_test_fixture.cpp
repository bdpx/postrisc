...
emulator_test_fixture::prepare(TargetSystem& targetSystem, const std::string& program)
{
    ...
    BOOST_TEST(targetSystem.Setup(configuration)
               == CStatus::continue_execution);
    Core *core = targetSystem.get_bootstrap_core();
    BOOST_TEST(core != nullptr);
    BOOST_TEST(targetSystem.LoadModule(*core, vbase, input_module, header)
               == CStatus::continue_execution);
    BOOST_TEST(targetSystem.PrepareThread(*core, vbase, header)
               == CStatus::continue_execution);
    return core;
}
