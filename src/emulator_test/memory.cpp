...
test(std::ostream & os)
{
    ...
    os << fmt::hex(std::bit_cast<u32>(2.33f)) << fmt::endl
       ...
       << fmt::hex(std::bit_cast<u128>((quadruple)2.33L)) << fmt::endl;

    os << "VM_PAGE_PHYS_ADDRESS_MASK=" << fmt::hex(VM_PAGE_PHYS_ADDRESS_MASK) << fmt::endl;
}

void
TargetSystem::RunMemoryTests(std::ostream & os)
{
    test(os);
    CStatus status;
    ...
    status = VirtualAllocate(mmu, vbase, vsize, VM_PAGE_RIGHT_READ | VM_PAGE_RIGHT_WRITE);
    ...
    for (int i=0; i<nrepeats; i++) {
        for (u64 address = vbase; address < vbase + vsize; address += sizeof(u64)) {
            ...
            status = mmu_write(*core, CVirtAddress(address), sizeof(u64), &value);
        }
    }
    ...
    BOOST_CHECK(!free_page(CPhysAddress(vbase), page_size).success());
...
#undef TEST_PAGE_SIZE
    ...
    u64 value = 4;
    BOOST_CHECK(target.mmu_write(*core, CVirtAddress((uintptr_t)data.data() + 8), sizeof(value), &value).success());
