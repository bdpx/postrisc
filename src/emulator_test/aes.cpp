...
#include "emulator/instructions/aes.hpp"
...
static void expand(CGeneralRegister& dst, char const summary[32])
{
   for (size_t i = 0; i < 16; i++) {
      char const x = summary[31-2*i];
      char const y = summary[31-2*i-1];
      uint8_t xx = fromhex (x);
      uint8_t yy = fromhex (y);
      assert(xx < 16);
      assert(yy < 16);
      xx = (yy << 4) | xx;
      assert(xx < 256);
      dst.vector<uint8_t>(i) = xx;
   }
}

static void reverse(CGeneralRegister& dst)
{
   for (size_t i = 0, j = 15; i < j; i++, j--) {
      std::swap(dst.vector<uint8_t>(i), dst.vector<uint8_t>(j));
   }
}
...
BOOST_AUTO_TEST_CASE(test_aeskeygenassist)
{
    ...
    for( auto const& test : tests) {
        CGeneralRegister key;
        expand(key, test.key);
        CGeneralRegister result;
        expand(result, test.result);
        CGeneralRegister ret = aes::keygen_assist(key, test.rcon);
        reverse(ret);
...
// aesimc 8dcab9dc035006bc8f57161e00cafd8d result 77575fc53bccc9eeae5e8b9267a635d6
    ...
    for (auto const& test : tests) {
        CGeneralRegister src1;  expand(src1, test.src1);
        CGeneralRegister src2;  expand(src2, test.src2);

        CGeneralRegister aesenc_result;
        expand(aesenc_result, test.aesenc_result);
        CGeneralRegister ret = aes::encrypt(src1, src2);
        reverse(ret);
        ...
        CGeneralRegister aesenclast_result;
        ...
        CGeneralRegister aesdec_result;
        ...
        CGeneralRegister aesdeclast_result;
