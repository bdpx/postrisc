...
BOOST_AUTO_TEST_CASE(instr_add)
{
    ...
    for (const auto& test : test_cases) {
         test_binop(insn_addd, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_addi)
{
    ...
    for (const auto& test : test_cases) {
         test_binimm(insn_addid, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_alloc)
{
    test_alloc();
}

BOOST_AUTO_TEST_CASE(instr_and)
{
    static const TestData_binop test_cases[] = {
    ...
    for (const auto& test : test_cases) {
        test_binop(insn_and, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_clmul)
{
...
BOOST_AUTO_TEST_CASE(instr_cntlz)
{
    ...
    for (const auto& test : test_cases) {
         test_bin_shamt(insn_cntlz, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_cntpop)
{
    static const TestData_RaAssignRbShamt test_cases[] = {
    ...
    for (const auto& test : test_cases) {
         test_bin_shamt(insn_cntpop, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_cnttz)
{
    static const TestData_RaAssignRbShamt test_cases[] = {
        { GeneralRegister(0, U64C(0x0)), 63, GeneralRegister(0, 64) },
    ...
    for (const auto& test : test_cases) {
         test_bin_shamt(insn_cnttz, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_faddss)
{
    static const TestData_binop test_cases[] = {
    ...
    for (const auto& test : test_cases) {
        test_binop(insn_fadds, test);
    }
}


BOOST_AUTO_TEST_CASE(instr_or)
{
    static const TestData_binop test_cases[] = {
        { GeneralRegister(U64C(0xffffffffffffffff), U64C(0xffffffffffffffff)),
          GeneralRegister(U64C(0x7fffffffffffffff), U64C(0xffffffffffffffff)),
    ...
    for (const auto& test : test_cases) {
        test_binop(insn_or, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_permb)
{
    ...
    for (const auto& test : test_cases) {
         test_bin_shamt(insn_permb, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_slli)
{
    const u64 val = U64C(0x1234567890abcdef);
    ...
    for (const auto& test : test_cases) {
         test_bin_shamt(insn_sllid, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_sub)
{
    static const TestData_binop test_cases[] = {
    ...
    for (const auto& test : test_cases) {
         test_binop(insn_subd, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_xnor)
{
    static const TestData_binop test_cases[] = {
    ...
    for (const auto& test : test_cases) {
        test_binop(insn_xnor, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_xor)
{
    static const TestData_binop test_cases[] = {
        { GeneralRegister(0, 1), GeneralRegister(0, 2), GeneralRegister(0, 3) },
        { GeneralRegister(0,-1), GeneralRegister(0,-2), GeneralRegister(0, 1) },
    ...
    for (const auto& test : test_cases) {
        test_binop(insn_xor, test);
    }
}

BOOST_AUTO_TEST_CASE(instr_xori)
{
    static const TestData_binimm test_cases[] = {
        { GeneralRegister(0, 1),  2, GeneralRegister(0, 3) },
