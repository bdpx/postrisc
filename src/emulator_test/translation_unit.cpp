...
public:
    void TestIndexing(uint64_t translated_address, const unsigned * PagingIndexes) const;
    void TestCanonical(uint64_t address, bool is_canonical) const;
...
void
TranslationUnit_fixture::TestCanonical(uint64_t address, bool is_canonical) const
{
...
void
TranslationUnit_fixture::TestIndexing(uint64_t translated_address, const unsigned * PagingIndexes) const
{
...
static constexpr unsigned GetPageIndex(uint64_t address, unsigned bitsPerPageOffset, unsigned level)
{
    ...
    return (address >> shift) & util::makemask(bitsPerPageIndex);
}

void
TranslationUnit_fixture::test()
{
    const uint64_t addr = UINT64_C(0x9876543223456789);
    ...
    for (const auto & config : configs) {
    	mmu.SetPagingParameters(0, config.BitsPerPageOffset, config.NumberOfPagingLevels);
        ...
        uint64_t const HighestUserAddress = (UINT64_C(1) << (VirtualAddressBits - 1)) - 1;
        ...
        uint64_t const LowestKernelAddress = ~HighestUserAddress;
