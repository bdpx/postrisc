...
void set_flags(RoundingModeEnum rm)
{
    ...
    if (0 != std::fesetround(rdir)) {
        std::cerr << "fesetround failed" << std::endl;
        exit(1);
    }
}
...
template<>
native::f128 muladd<native::f128>(native::f128 a, native::f128 b, native::f128 c, RoundingModeEnum rm, u8 & flags)
{
    set_flags(rm);
    native::f128 ret = F128_FUNC_NAME(fma)(a, b, c);
...
template<>
native::f128 rint(native::f128 a, RoundingModeEnum rm)
{
    UNREFERENCED_PARAMETER(rm);
    return F128_FUNC_NAME(rint)(a);
}
...
template<>
float my_sqrt(float a, RoundingModeEnum) {  return ::sqrtf(a); }

template<>
double my_sqrt(double a, RoundingModeEnum) {  return std::sqrt(a); }

template<>
native::f128 my_sqrt(native::f128 a, RoundingModeEnum rm)
{
    UNREFERENCED_PARAMETER(rm);
    return F128_FUNC_NAME(sqrt)(a);
}
...
template<>
native::f128 my_log2(native::f128 a, RoundingModeEnum rm)
{
    UNREFERENCED_PARAMETER(rm);
    return F128_FUNC_NAME(log2)(a);
}
...
template<>
native::f128 my_exp(native::f128 a, RoundingModeEnum rm)
{
    UNREFERENCED_PARAMETER(rm);
    return F128_FUNC_NAME(exp)(a);
}
...
template<typename INTEGER> INTEGER cvt_int(native::f128 a, RoundingModeEnum rm)
{
    UNREFERENCED_PARAMETER(rm);
    return F128_FUNC_NAME(rint)(a);
}
...
std::ostream& operator<< (std::ostream& out, print_sf<FLOAT> v)
{
    ...
    out << ' ';
    out << fmt::hex<typename FLOAT::F_UINT, (FLOAT::FRAC_SIZE + 3) / 4>(a_frac);
    return out;
}
...
std::ostream& operator<< (std::ostream& out, print_sf<f128> v)
{
    ...
    out << ' ' << fmt::hex<u64, ((f128::FRAC_SIZE - 64) + 3) / 4>((u64)(a_frac >> 64));
    out << '_' << fmt::hex((u64)(a_frac));
    return out;
}
...
FLOAT rrandom_sf(void)
{
    ...
    typename FLOAT::F_UINT a_frac = rrandom_u<FLOAT>(FLOAT::FRAC_SIZE);
...
{
    const FLOAT ref = exec_ref_op(op, a, b, rm);
    const u8 ref_fflags = get_flags();

    SoftFPU fpu;
    fpu.set_rounding_mode(rm);
    FLOAT r;
    switch (op) {
    case OP_FADD:    r = fpu.add(a, b, rm);       break;
    case OP_FSUB:    r = fpu.sub(a, b, rm);       break;
    case OP_FMUL:    r = fpu.mul(a, b, rm);       break;
    case OP_FDIV:    r = fpu.div(a, b, rm);       break;
    case OP_FSQRT:   r = fpu.sqrt(a, rm);         break;
    case OP_FLOG2:   r = fpu.log2(a, rm);         break;
    case OP_FMIN:    r = fpu.min(a, b);           break;
    case OP_FMAX:    r = fpu.max(a, b);           break;
    case OP_FEXP:    r = fpu.exp(a, rm);          break;
    default:
        std::cerr << "abort " << __FILE__ << __LINE__;
        exit(1);
    }
...
test_cvt_sf_sf(FLOAT_SRC a)
{
    ...
    if (r.v != ref.u.v || ref_fflags != fflags) {
        std::cout << "ERROR op=cvt_f_f (f" << FLOAT_SRC::F_SIZE << " => f"
                  << FLOAT_DST::F_SIZE << ") rm=" << rm << fmt::endl;
        std::cout << "a  = " << print_sf(a) << fmt::endl;
        std::cout << "ref= " << print_sf(ref.u) << " fflags=0x" << fmt::hex(ref_fflags) << fmt::endl;
        std::cout << "r  = " << print_sf(r) << " fflags=0x" << fmt::hex(fflags) << fmt::endl;
...
test_rint(FLOAT a, RoundingModeEnum rm)
{
    ...
    FLOAT const r = fpu.round<FLOAT>(a, rm, true);
    u8 const fflags = fpu.get_flags();
    ...
    if (r.v != ref.u.v /*|| ref_fflags != fflags*/) {
        std::cout << "ERROR op=round (f" << FLOAT::F_SIZE
                  << ") rm=" << rm << fmt::endl;
        std::cout << "a  = " << print_sf(a) << fmt::endl;
        std::cout << "ref= " << print_sf(ref.u) << " fflags=0x" << fmt::hex(ref_fflags) << fmt::endl;
        std::cout << "r  = " << print_sf(r) << " fflags=0x" << fmt::hex(fflags) << fmt::endl;
...
test_fma(FLOAT a, FLOAT b, FLOAT c, RoundingModeEnum rm)
{
    ...
    const FLOAT r = fpu.muladd(a, b, c, rm, FLOAT_MULADD_NEGATE_NONE);
    u8 fflags = fpu.get_flags();

    u8 ref_fflags = 0;
    ref.f = muladd<typename FLOAT::F_HARD>(a.hard(), b.hard(), c.hard(), rm, ref_fflags);
    if (is_nan(ref.u))
        ref.u.v = FLOAT::F_QNAN;
    ...
    if constexpr (FLOAT::F_SIZE == 128) {
        ...
        if (rm == RM_RDN &&
            r.get_sign() && is_inf(r) &&
            ref.u.v == SoftFPU::pack_sf<FLOAT>(1, FLOAT::EXP_MASK - 1, FLOAT::FRAC_MASK).v) {
           return;
        }
        if (rm == RM_RUP &&
            ref.u.get_sign() && is_inf(ref.u) &&
            r.v == SoftFPU::pack_sf<FLOAT>(1, FLOAT::EXP_MASK - 1, FLOAT::FRAC_MASK).v) {
           return;
        }
    }
...
static const RoundingModeEnum rms[4] = {
...
test_float_special(void)
{
    ...
    for (int i = 0; i < SPECIAL_COUNT; i++) {
        const FLOAT a = special_sf<FLOAT>(i);
        for (int j = 0; j < SPECIAL_COUNT; j++) {
            const FLOAT b = special_sf<FLOAT>(j);
            for (const RoundingModeEnum rm : rms) {
                test_op_all(a, b, rm);
                for (int k = 0; k < SPECIAL_COUNT; k++) {
                    const FLOAT c = special_sf<FLOAT>(k);
                    test_fma(a, b, c, rm);
                }
            }
        }
...
void test(void)
{
    static const struct {
     ...
     { RM_RDN,
       FLOAT32_C(0xc5dbffff),
       FLOAT32_C(0x7a340000),
       FLOAT32_C(0x534fffff) },
     { RM_RUP,
       FLOAT32_C(0x45dbffff),
       FLOAT32_C(0xfa340000),
       FLOAT32_C(0xd34fffff) },
     { RM_RNE,
       FLOAT32_C(0x3c1bffff),
       FLOAT32_C(0x1d35ffff),
       FLOAT32_C(0x1961ffff) },
    };

    for (auto const& test : tests_32) {
        SoftFPU fpu;
        fpu.set_rounding_mode(test.rm);
        f32 const r = fpu.muladd(test.a, test.b, test.c, test.rm, FLOAT_MULADD_NEGATE_NONE);
        u8 const fflags = fpu.get_flags();
    ...
    static const struct {
     ...
     { RM_RDN,
       FLOAT64_C(0xc5dbffffffffffff),
       FLOAT64_C(0x7a34000000000000),
       FLOAT64_C(0x534fffffffffffff) },
     { RM_RUP,
       FLOAT64_C(0x45dbffffffffffff),
       FLOAT64_C(0xfa34000000011111),
       FLOAT64_C(0xd34fffffffffff00) },
     { RM_RNE,
       FLOAT64_C(0x3c1bffffc0000000),
       FLOAT64_C(0x1d35fffffffff000),
       FLOAT64_C(0x1961ffffffff3fff) },
    };

    for (auto const& test : tests_64) {
        SoftFPU fpu;
        fpu.set_rounding_mode(test.rm);
        f64 const r = fpu.muladd(test.a, test.b, test.c, test.rm, FLOAT_MULADD_NEGATE_NONE);
        u8 const fflags = fpu.get_flags();
    ...
    for (auto const& test : tests_128) {
        SoftFPU fpu;
        fpu.set_rounding_mode(test.rm);
        f128 const r = fpu.muladd(test.a, test.b, test.c, test.rm, FLOAT_MULADD_NEGATE_NONE);
        u8 const fflags = fpu.get_flags();
