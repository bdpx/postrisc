...
FLOAT rrandom_sf(void)
{
    ...
    return CSoftFPU::pack_sf<FLOAT>(a_sign, a_exp, a_mant);
}
...
FLOAT special_sf(int i)
{
    ...
    case 0: /* zero */
    case 1:
        return CSoftFPU::pack_sf<FLOAT>(i & 1, 0, 0);
    case 2: /* infinity */
    case 3:
        return CSoftFPU::pack_sf<FLOAT>(i & 1, FLOAT::EXP_MASK, 0);
    case 4: /* 1.0 */
    case 5:
        return CSoftFPU::pack_sf<FLOAT>(i & 1, FLOAT::BIAS, 0);
    case 6: /* QNAN */
    case 7:
        return CSoftFPU::pack_sf<FLOAT>(i & 1, FLOAT::EXP_MASK, FLOAT::QNAN_MASK);
    case 8: /* SNAN */
    case 9:
        return CSoftFPU::pack_sf<FLOAT>(i & 1, FLOAT::EXP_MASK, 1);
    case 10: /* subnormal */
    case 11:
        return CSoftFPU::pack_sf<FLOAT>(i & 1, 0, 1);
    default:
...
test_cvt_sf_sf(FLOAT_SRC a)
{
    ...
    if (r.v != ref.u.v || ref_fflags != fflags) {
        ...
        std::cout << "ref= " << print_sf(ref.u) << " fflags=0x" << fmt::hex(ref_fflags) << fmt::endl;
        std::cout << "r  = " << print_sf(r) << " fflags=0x" << fmt::hex(fflags) << fmt::endl;
        //std::cout << "DUMP:\n" << CSoftFPU::debug_stream.str() << fmt::endl;
    }
}
...
test_fma(FLOAT a, FLOAT b, FLOAT c, RoundingModeEnum rm)
{
    ...
    if constexpr (FLOAT::F_SIZE == 128) {
        ...
        if (rm == RM_RDN &&
            r.get_sign() && is_inf(r) &&
            ref.u.v == CSoftFPU::pack_sf<FLOAT>(1, FLOAT::EXP_MASK - 1, FLOAT::MANT_MASK).v) {
           return;
        }
        if (rm == RM_RUP &&
            ref.u.get_sign() && is_inf(ref.u) &&
            r.v == CSoftFPU::pack_sf<FLOAT>(1, FLOAT::EXP_MASK - 1, FLOAT::MANT_MASK).v) {
           return;
        }
    }
