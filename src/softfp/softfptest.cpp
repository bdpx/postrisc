...
#include "test/timing.cpp"

//#define QUADMATH_USE 1

namespace postrisc {

void test_scalar_uint()
{
    scalar_uint<uint32_t> a;
    scalar_uint<uint32_t> b = 2u;
    scalar_uint<uint32_t> c = 2u;
    scalar_uint<uint32_t> d(1u);
    scalar_uint<uint32_t> e(1u);
    scalar_uint<uint32_t> f = a + b;
    d += c/e;
    a <<= 1;
    b >>= 1;
    f <<= 2;
...
void test_256()
{
    ...
    LOG_FATAL(LOG_EVALUATE) << "a  =" << a;
    LOG_FATAL(LOG_EVALUATE) << "b  =" << b;
    uint<512> c = umul(a, b);

    LOG_FATAL(LOG_EVALUATE) << "c  =" << c;
    LOG_FATAL(LOG_EVALUATE) << "chx=" << hex(c);
    LOG_FATAL(LOG_EVALUATE) << "c10=" << to_string(c, 10);
}
...
uint8_t get_flags()
{
    ...
    if (hw_flags & FE_INVALID)   flags |= FFLAG_INVALID_OP;
    if (hw_flags & FE_DIVBYZERO) flags |= FFLAG_DIVIDE_ZERO;
...
uint8_t get_mpft_flags()
{
   ...
   if (mf & MPFR_FLAGS_NAN) ret |= FFLAG_INVALID_OP;
   if (mf & MPFR_FLAGS_INEXACT) ret |= FFLAG_INEXACT;
   if (mf & MPFR_FLAGS_ERANGE) ret |= 0;
   if (mf & MPFR_FLAGS_DIVBY0) ret |= FFLAG_DIVIDE_ZERO;
   return ret;
}


template<typename FLOAT>
FLOAT muladd(FLOAT a, FLOAT b, FLOAT c, RoundingModeEnum rm, uint8_t & flags);

template<>
float muladd<float>(float a, float b, float c, RoundingModeEnum rm, uint8_t & flags)
{
    set_flags(rm);
...
template<>
double muladd<double>(double a, double b, double c, RoundingModeEnum rm, uint8_t & flags)
{
    set_flags(rm);
...
template<>
__float128 muladd<__float128>(__float128 a, __float128 b, __float128 c, RoundingModeEnum rm, uint8_t & flags)
{
#if defined(QUADMATH_USE)
    set_flags(rm);
    ...
    mpfr_rnd_t const rdir = setup_mpfr(rm);
    mpfr_set_emin(-float128::BIAS - float128::MANT_SIZE + 1);
    mpfr_set_emax(float128::BIAS + 1);
...
public:
    ...
    template<class T> friend std::ostream& operator<< (std::ostream& out, print_sf<T> v);
private:
    FLOAT v;
};
...
template<>
std::ostream& operator<< (std::ostream& out, print_sf<float128> v)
{
    auto a_sign = v.v.get_sign();
    auto a_exp = v.v.get_exp();
    auto a_mant = v.v.get_fraction();
    out << a_sign;
    out << ' ' << fmt::hex<uint64_t, (float128::EXP_SIZE + 3) / 4>(a_exp);
    out << ' ' << fmt::hex<uint64_t, ((float128::MANT_SIZE - 64) + 3) / 4>((uint64_t)(a_mant >> 64));
    out << '_' << fmt::hex((uint64_t)(a_mant));
    return out;
}
...
FLOAT rrandom_sf(void)
{
    ...
    return FPU::pack_sf<FLOAT>(a_sign, a_exp, a_mant);
}
...
FLOAT special_sf(int i)
{
    ...
    case 0: /* zero */
    case 1:
        return FPU::pack_sf<FLOAT>(i & 1, 0, 0);
    case 2: /* infinity */
    case 3:
        return FPU::pack_sf<FLOAT>(i & 1, FLOAT::EXP_MASK, 0);
    case 4: /* 1.0 */
    case 5:
        return FPU::pack_sf<FLOAT>(i & 1, FLOAT::BIAS, 0);
    case 6: /* QNAN */
    case 7:
        return FPU::pack_sf<FLOAT>(i & 1, FLOAT::EXP_MASK, FLOAT::QNAN_MASK);
    case 8: /* SNAN */
    case 9:
        return FPU::pack_sf<FLOAT>(i & 1, FLOAT::EXP_MASK, 1);
    case 10: /* subnormal */
    case 11:
        return FPU::pack_sf<FLOAT>(i & 1, 0, 1);
    default:
...
test_op(FloatOpEnum op, FLOAT a, FLOAT b, RoundingModeEnum rm)
{
    ...
    uint8_t const ref_fflags = get_flags();
    ...
    uint8_t const fflags = fpu.get_flags();
...
test_cvt_sf_sf(FLOAT_SRC a, RoundingModeEnum rm)
{
    ...
    FPU fpu(rm);
    FLOAT_DST const r = fpu.cvt_sf_sf<FLOAT_SRC, FLOAT_DST>(a, rm);
    uint8_t const fflags = fpu.get_flags();
...
test_cvt_sf_sf(FLOAT_SRC a)
{
    ...
    FPU fpu(rm);
    FLOAT_DST const r = fpu.cvt_sf_sf<FLOAT_SRC, FLOAT_DST>(a);
    uint8_t const fflags = fpu.get_flags();
    ...
    if (r.v != ref.u.v || ref_fflags != fflags) {
        std::cout << "ERROR op=cvt_sf_sf (f" << FLOAT_SRC::F_SIZE << " => f"
                  << FLOAT_DST::F_SIZE << ") rm=" << rm_to_str[rm] << fmt::endl;
        std::cout << "a  = " << print_sf(a) << fmt::endl;
        std::cout << "ref= " << print_sf(ref.u) << " fflags=0x" << fmt::hex(ref_fflags) << fmt::endl;
        std::cout << "r  = " << print_sf(r) << " fflags=0x" << fmt::hex(fflags) << fmt::endl;
        //std::cout << "DUMP:\n" << FPU::debug_stream.str() << fmt::endl;
    }
}
...
{
    FPU fpu(rm);
    INTEGER const r = fpu.cvt_sf_i<FLOAT, INTEGER>(a, rm);
    uint8_t const fflags = fpu.get_flags();
    ...
    if ((((ref_fflags | fflags) & FFLAG_INVALID_OP) == 0 && r != r_ref) ||
        (sizeof(FLOAT) < 16U && (ref_fflags & FFLAG_INVALID_OP) != (fflags & FFLAG_INVALID_OP))) {
        std::cout << "ERROR op=cvt_sf_i (f" << FLOAT::F_SIZE << " => i" << (8*sizeof(INTEGER))
                  << ") rm=" << rm_to_str[rm] << fmt::endl;
        std::cout << "a  = " << print_sf(a) << fmt::endl;
...
template<typename FLOAT>
void
report(std::ostream& out, FLOAT a, FLOAT b, FLOAT c, RoundingModeEnum rm, FLOAT ref, uint8_t ref_fflags, FLOAT r, uint8_t fflags)
{
    if (r.v != ref.v)
        out << "ERRVV";
...
test_fma(FLOAT a, FLOAT b, FLOAT c, RoundingModeEnum rm)
{
    ...
    uint8_t fflags = fpu.get_flags();

    uint8_t ref_fflags = 0;
    ref.f = muladd(a.hard(), b.hard(), c.hard(), rm, ref_fflags);
    if (isnan_sf(FLOAT{ref.u}))
        ref.u.v = FLOAT::F_QNAN;
    ...
    if constexpr (FLOAT::F_SIZE == 128) {
        ...
        if (rm == RM_RDN &&
            r.get_sign() && isinf_sf(r) &&
            ref.u.v == FPU::pack_sf<FLOAT>(1, FLOAT::EXP_MASK - 1, FLOAT::MANT_MASK).v) {
           return;
        }
        if (rm == RM_RUP &&
            ref.u.get_sign() && isinf_sf(ref.u) &&
            r.v == FPU::pack_sf<FLOAT>(1, FLOAT::EXP_MASK - 1, FLOAT::MANT_MASK).v) {
           return;
        }
    }
...
test_cmp(FLOAT a, FLOAT b)
{
    ...
    int r, ref;
    uint8_t fflags, ref_fflags;
    const char *op_str;

    FPU fpu(rm);
    r = fpu.eq_quiet_sf(a, b);
...
test_op_all(FLOAT a, FLOAT b, RoundingModeEnum rm)
{
    ...
    if constexpr (sizeof(FLOAT) > sizeof(float32 )) test_cvt_sf_sf<FLOAT, float32 >(a, rm);
    if constexpr (sizeof(FLOAT) > sizeof(float64 )) test_cvt_sf_sf<FLOAT, float64 >(a, rm);
    if constexpr (sizeof(FLOAT) > sizeof(float128)) test_cvt_sf_sf<FLOAT, float128>(a, rm);

    if constexpr (sizeof(FLOAT) < sizeof(float32 )) test_cvt_sf_sf<FLOAT, float32 >(a);
    if constexpr (sizeof(FLOAT) < sizeof(float64 )) test_cvt_sf_sf<FLOAT, float64 >(a);
    if constexpr (sizeof(FLOAT) < sizeof(float128)) test_cvt_sf_sf<FLOAT, float128>(a);

    test_rint(a, rm);

    test_cvt_int_sf<FLOAT, int32_t>(a, rm);
    test_cvt_int_sf<FLOAT, int64_t>(a, rm);
}
...
void test_fpu(size_t n)
{
    ...
    test_float_special<float32>();
    test_float_special<float64>();
    test_float_special<float128>();
    for (size_t i = 0; i <= 100; i++) {
        test_float<float32>(n);
        test_float<float64>(n);
        test_float<float128>(n);
...
void test(void)
{
    ...
    for (auto const& test : tests) {
        FPU fpu(test.rm);
        float128 const r = fpu.muladd(test.a, test.b, test.c, FLOAT_MULADD_NEGATE_NONE);
        uint8_t const fflags = fpu.get_flags();
        set_flags(test.rm);
        float_union<float128> ref;
        uint8_t ref_fflags = 0;
...
int main(int argc, char **argv)
{
    ...
    postrisc::util::setup();
    postrisc::test_256();
    postrisc::test_scalar_uint();

    //postrisc::fpu::test();
    postrisc::fpu::test_timing();
