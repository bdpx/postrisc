...
std::ostream& operator<< (std::ostream& out, print_sf<float128> v)
{
    ...
    auto a_mant = v.v.get_fraction();
    out << a_sign;
    out << ' ' << fmt::hex<u64, (float128::EXP_SIZE + 3) / 4>(a_exp);
    out << ' ' << fmt::hex<u64, ((float128::MANT_SIZE - 64) + 3) / 4>((u64)(a_mant >> 64));
...
exec_ref_op(FloatOpEnum op, FLOAT a1, FLOAT b1, RoundingModeEnum rm)
{
    ...
    case OP_FLOG2:
        ...
        break;
    case OP_FMIN:
        if (FPU::isnan_sf(a.u) || FPU::isnan_sf(b.u))
            r.u.v = FLOAT::F_QNAN;
        else
            r.f = std::min(a.f, b.f);
        break;
