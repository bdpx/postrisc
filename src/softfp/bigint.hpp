...
inline constexpr uint128 umul(uint64_t x, uint64_t y) noexcept
{
    ...
    uint64_t const xl = x & 0xffffffff;
    uint64_t const xh = x >> 32;
    uint64_t const yl = y & 0xffffffff;
    uint64_t const yh = y >> 32;

    uint64_t const t0 = xl * yl;
    uint64_t const t1 = xh * yl;
    uint64_t const t2 = xl * yh;
    uint64_t const t3 = xh * yh;

    uint64_t const u1 = t1 + (t0 >> 32);
    uint64_t const u2 = t2 + (u1 & 0xffffffff);

    uint64_t const lo = (u2 << 32) | (t0 & 0xffffffff);
    uint64_t const hi = t3 + (u2 >> 32) + (u1 >> 32);
    return {lo, hi};
}
...
template <size_t N>
inline void store(uint8_t (&dst)[N / 8], const postrisc::uint<N>& x) noexcept
{
...
template <size_t M, size_t N>
inline void trunc(uint8_t (&dst)[M], const postrisc::uint<N>& x) noexcept
{
...
template <size_t N>
inline void store(uint8_t* dst, const postrisc::uint<N>& x) noexcept
{
    const auto d = bswap(x);
    std::memcpy(dst, &d, sizeof(d));
}
