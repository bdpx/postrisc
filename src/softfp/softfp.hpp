...
#define FLOAT_BASE(FF_SIZE, TEMPLATE_EXP_SIZE, FF_UINT, FF_FLOAT)                                                    \
...
class float32 {
public:
    FLOAT_BASE(32, 8, u32, float)
    using F_ULONG = u64;
    static constexpr int clz(F_UINT a) { return (a == 0) ? 32 : __builtin_clz(a); }
};

#define FLOAT32_C(V) float32{ U32C(V) }

class float64 {
public:
    FLOAT_BASE(64, 11, u64, double)
    using F_ULONG = u128;
    static constexpr int clz(F_UINT a) { return (a == 0) ? 64 : __builtin_clzll(a); }
};

#define FLOAT64_C(V) float64{ U64C(V) }

class float128 {
public:
    FLOAT_BASE(128, 15, u128, __float128)
    static ALWAYS_INLINE constexpr int clz(F_UINT a) {
        if (a == 0) { return 128;
        } else {
            u64 const ah = a >> 64;
            u64 const al = a;
            return ah ? __builtin_clzll(ah) : __builtin_clzll(al) + 64;
        }
    }
};

#define FLOAT128_C(HI, LO) float128{ (u128(U64C(HI)) << 64) | u128(U64C(LO)) }
...
public:
    ...
    template<typename FLOAT> static bool isinf_sf(FLOAT a);
    template<typename FLOAT> static bool isnan_sf(FLOAT a);
    template<typename FLOAT> static bool issignan_sf(FLOAT a);
    template<typename FLOAT> static bool IsBothZero(FLOAT a, FLOAT b) { return ( !!( a.v | b.v ) << 1); }
    ...
    template<class FLOAT> bool compare_ordered(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_ordered_eq(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_ordered_ne(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_ordered_lt(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_ordered_le(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_unordered(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_unordered_eq(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_unordered_ne(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_unordered_lt(FLOAT a, FLOAT b);
    template<class FLOAT> bool compare_unordered_le(FLOAT a, FLOAT b);
...
private:
    ...
    template<typename FLOAT> FLOAT round_pack_sf(u32 a_sign, int a_exp,
        typename FLOAT::F_UINT a_mant, RoundingModeEnum rm);
    template<typename FLOAT> FLOAT normalize_sf(u32 a_sign, int a_exp,
        typename FLOAT::F_UINT a_mant, RoundingModeEnum rm);
    template<typename FLOAT> FLOAT normalize2_sf(u32 a_sign, int a_exp,
...
template<class Archive> void FPU::serialize(Archive & ar, const unsigned int UNUSED(version))
{
...
inline u64 FPU::get_control_word(void) const
{
    u64 word = 0;
...
inline void FPU::set_control_word(u64 word)
{
...
inline void FPU::change_control_word(u64 and_mask, u64 or_mask)
{
...
inline std::ostream& operator<<(std::ostream & out, const FPU& fpu)
{
    return out
...
template<typename FLOAT> inline FLOAT FPU::pack_sf(u32 a_sign, u32 a_exp, typename FLOAT::F_UINT a_mant)
{
...
template<typename FLOAT> inline FLOAT FPU::zero_sf(u32 sign)
{
...
template<typename FLOAT>
FLOAT FPU::round_pack_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rm)
{
    LOG_DEBUG(LOG_EVALUATE) << " exp=" << a_exp << " mant=" << fmt::hex(a_mant);
...
template<typename FLOAT>
FLOAT
FPU::normalize_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rm)
{
    LOG_DEBUG(LOG_EVALUATE) << " exp=" << a_exp << " mant=" << fmt::hex(a_mant);

    int const shift = FLOAT::clz(a_mant) - (FLOAT::F_SIZE - 1 - FLOAT::IMANT_SIZE);
    assert(shift >= 0);
    a_exp -= shift;
    a_mant <<= shift;
...
template<typename FLOAT>
FLOAT
FPU::normalize2_sf(u32 a_sign, int a_exp,
    typename FLOAT::F_UINT a_mant1, typename FLOAT::F_UINT a_mant0, RoundingModeEnum rm)
{
    ...
    int const l = a_mant1 ? FLOAT::clz(a_mant1) : FLOAT::F_SIZE + FLOAT::clz(a_mant0);
...
template<typename FLOAT> bool FPU::isinf_sf(FLOAT a)
{
...
template<typename FLOAT> bool FPU::isnan_sf(FLOAT a)
{
    auto const a_exp = a.get_exp();
    auto const a_mant = a.get_fraction();
    return (a_exp == FLOAT::EXP_MASK && !!a_mant);
...
template<typename FLOAT> bool FPU::issignan_sf(FLOAT a)
{
...
template<typename FLOAT> FLOAT FPU::add(FLOAT a, FLOAT b)
{
...
template<typename FLOAT> FLOAT FPU::sub(FLOAT a, FLOAT b)
{
    b.v ^= FLOAT::SIGN_MASK;
    return add(a, b);
}
...
{
    int const shift = FLOAT::MANT_SIZE - ((FLOAT::F_SIZE - 1 - FLOAT::clz(a_mant)));
    *pa_exp = 1 - shift;
    return a_mant << shift;
}

template<typename F_UINT> F_UINT mul_u(F_UINT *plow, F_UINT a, F_UINT b);

template<> u16 mul_u(u16 *plow, u16 a, u16 b)
{
    u32 r = (u32)a * (u32)b;
    *plow = r;
    return r >> 16;
...
template<> u32 mul_u(u32 *plow, u32 a, u32 b)
{
    u64 r = (u64)a * (u64)b;
    *plow = r;
    return r >> 32;
...
template<> u64 mul_u(u64 *plow, u64 a, u64 b)
{
...
template<> u128 mul_u(u128 *plow, u128 a, u128 b)
{
    using F_UINT = u128;
    using F_UHALF = u64;
...
template<typename FLOAT> FLOAT FPU::mul(FLOAT a, FLOAT b)
{
    auto a_sign = a.get_sign();
    auto b_sign = b.get_sign();
...
template<typename FLOAT> FLOAT FPU::muladd(FLOAT a, FLOAT b, FLOAT c, muladd_negate_t negate_mask)
{
    ...
    if (r_mant1 == 0) {
        ...
        shift = FLOAT::clz(r_mant1) - (F_SIZE - 1 - IMANT_SIZE);
        if (shift != 0) {
...
// FIXME: optimize u128
template<typename F_UINT> F_UINT divrem_u(F_UINT *pr, F_UINT a1, F_UINT a0, F_UINT b)
{
...
template<> u16 divrem_u(u16 *pr, u16 ah, u16 al, u16 b)
{
    u32 a = ((u32)ah << 16) | al;
    *pr = a % b;
    return a / b;
...
template<> u32 divrem_u(u32 *pr, u32 ah, u32 al, u32 b)
{
    u64 a = ((u64)ah << 32) | al;
    *pr = a % b;
    return a / b;
...
template<> u64 divrem_u(u64 *pr, u64 ah, u64 al, u64 b)
{
...
template<typename FLOAT> FLOAT FPU::div(FLOAT a, FLOAT b)
{
    auto a_sign = a.get_sign();
    auto b_sign = b.get_sign();
    auto r_sign = a_sign ^ b_sign;
...
static int sqrtrem_u(F_UINT *pr, F_UINT ah, F_UINT al)
{
    ...
    /* 2^l >= a */
    if (ah != 0) {
        l = 2 * F_SIZE - FLOAT::clz(ah - 1);
    } else {
        if (al == 0) {
            *pr = 0;
            return 0;
        }
        l = F_SIZE - FLOAT::clz(al - 1);
    }
...
sqrtrem_u(typename FLOAT::F_UINT *pr, typename FLOAT::F_UINT a1, typename FLOAT::F_UINT a0)
{
    ...
    /* 2^l >= a */
    if (a1 != 0) {
        l = 2 * F_SIZE - FLOAT::clz(a1 - 1);
    } else {
        if (a0 == 0) {
            *pr = 0;
            return 0;
        }
        l = F_SIZE - FLOAT::clz(a0 - 1);
    }
...
template<typename FLOAT> FLOAT FPU::sqrt(FLOAT a)
{
    auto a_sign = a.get_sign();
    auto a_exp = a.get_exp();
    auto a_mant = a.get_fraction();
...
template<typename FLOAT> FLOAT FPU::min(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> FLOAT FPU::max(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> int FPU::eq_quiet_sf(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> int FPU::le_sf(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        raise(FFLAG_INVALID_OP);
        return 0;
...
template<typename FLOAT> int FPU::lt_sf(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        raise(FFLAG_INVALID_OP);
        return 0;
...
compare_mode_t FPU::compare_internal(FLOAT a, FLOAT b, compare_mode_t mode)
{
    if (isnan_sf(a) || isnan_sf(b)) {
...
template<typename FLOAT> u32 FPU::classify(FLOAT a)
{
    auto a_sign = a.get_sign();
    auto a_exp = a.get_exp();
    auto a_mant = a.get_fraction();
...
FLOAT_DST FPU::cvt_sf_sf(FLOAT_SRC a)
{
...
template<typename FLOAT_SRC, typename FLOAT_DST>
FLOAT_DST FPU::cvt_sf_sf(FLOAT_SRC a, RoundingModeEnum rm)
{
    static_assert(sizeof(FLOAT_SRC) > sizeof(FLOAT_DST));
...
*----------------------------------------------------------------------------*/
template<typename FLOAT>
FLOAT FPU::round_to_int(FLOAT a, RoundingModeEnum rm)
{
    auto a_sign = a.get_sign();
    auto aExp   = a.get_exp();
...
INTEGER FPU::cvt_sf_i(FLOAT a, RoundingModeEnum rm)
{
...
template<typename FLOAT, typename INTEGER>
FLOAT FPU::cvt_i_sf(INTEGER a, RoundingModeEnum rm)
{
    using UNSIGNED_INTEGER = typename std::make_unsigned<INTEGER>::type;
    ...
    int const l = 8*sizeof(INTEGER) - FLOAT::clz(r) - (FLOAT::F_SIZE - 1);
    if (l > 0) {
...
template<typename FLOAT> bool FPU::compare_ordered(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_unordered(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_ordered_eq(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
    ...
    const auto a_sign = a.sign();
    const auto b_sign = b.sign();
...
template<typename FLOAT> bool FPU::compare_unordered_eq(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_ordered_ne(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_unordered_ne(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_ordered_lt(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_unordered_lt(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_ordered_le(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
template<typename FLOAT> bool FPU::compare_unordered_le(FLOAT a, FLOAT b)
{
    if (isnan_sf(a) || isnan_sf(b)) {
        if (issignan_sf(a) || issignan_sf(b)) {
            raise(FFLAG_INVALID_OP);
...
*----------------------------------------------------------------------------*/
template<typename FLOAT> FLOAT FPU::log2(FLOAT a)
{
    using F_UINT = typename FLOAT::F_UINT;
...
template<typename FLOAT> FLOAT FPU::exp(FLOAT a)
{
    auto aSig = a.get_fraction();
    auto aExp = a.get_exp();
    auto a_sign = a.get_sign();
