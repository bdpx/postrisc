...
using TestFunctionPtr = void (*)(CTestController & ctrl, CTestContext it);

class CTestController {
public:
    CTestController(void);
    ~CTestController(void);
...
public:
    ...
    void writeTestsPerformed(uint64_t count);
    void checkEarlyExit(uint64_t & count);
    ...
    void setup_names(CTestContext & context);

public:
    template<class FPU>
    static void setup_fpu(FPU & fpu, const CTestContext & context);
    ...
    void reportTime(int64_t count, double seconds);
...
template<class FPU>
void
CTestController::setup_fpu(FPU & fpu, const CTestContext & context)
{
...
template<class T>
void
CTestController::start(T & a)
{
    total = CSequence::GetTotal(level, a);
}

template<class T>
void
CTestController::start(T & a, T & b)
{
    total = CSequence::GetTotal(level,a,b);
...
template<class T>
void
CTestController::next(T & a, T & b, T & c)
{
    sequenceA.Random(a);
    sequenceB.Random(b);
    sequenceC.Random(c);
