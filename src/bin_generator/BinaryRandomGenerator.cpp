...
{
    int mnem = random() % INSTRUCTION_LIST_SIZE;
    ...
    for (const EInstructionField field : asm_fmt_field[fmt]) {
        ...
        switch (field) {
            ...
            case cf_fpclass:           cmd.SetFpClassifyMask(static_cast<unsigned>(r));        break;
            ...
            case cf_rounding_mode:     cmd.SetRoundingMode(static_cast<RoundingMode>(r%5));    break;

            case cf_optional_memory_ordering:
            case cf_memory_ordering:
                cmd.SetMemoryOrdering(static_cast<MemoryOrdering>(r%5));
                break;
