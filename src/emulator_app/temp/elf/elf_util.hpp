...
typedef Elf_Ehdr_template<uint64_t> Elf64_Ehdr_;
...
typedef Elf_Shdr_template<uint64_t> Elf64_Shdr_;
...
template <>
struct Elf_Phdr_template<uint64_t> {
    typedef uint64_t AddressOffsetT;
...
typedef Elf_Phdr_template<uint64_t> Elf64_Phdr;
...
template <>
struct Elf_Sym_template<uint64_t> {
  typedef uint64_t AddressOffsetT;
...
typedef Elf_Sym_template<uint64_t> Elf64_Sym_;
...
typedef elf_info<uint64_t> elf_info64;
...
static inline uint64_t GetProtection(u32 flags)
{
    uint64_t protection = VM_PAGE_RIGHT_FINAL;
...
template <class AddressOffsetT, class VECTOR>
int load_library(
     const std::vector<u8> &image,        // elf image
                         VECTOR &memory,       // aligned memory device
          std::vector<uint64_t> &PageData,     // per-page info
                  ...
                  elf_load_info &load_info)    // as-relative entry address
{
    ...
    for (size_t i = 0; i < eh.e_phnum; i++) {
        ...
        if (ph.p_type == PT_DYNAMIC) {
            ...
            //dso->relro_start = util::align_down(ph.p_vaddr, pagesize);
            //dso->relro_end = util::align_down(ph.p_vaddr + ph.p_memsz, pagesize);
    ...
    addr_min  = util::align_down(addr_min, pagesize);
    LOG_OUTPUT(LOG_LOADER) << "addr_min=" << fmt::hex(addr_min) << std::endl;
    addr_max  = util::align_up(addr_max, pagesize);
    LOG_OUTPUT(LOG_LOADER) << "addr_max=" << fmt::hex(addr_max) << std::endl;
    off_start = util::align_down(off_start, pagesize);
...
#endif
        size_t const this_min  = util::align_down(ph.p_vaddr, pagesize);
        size_t const this_max  = util::align_up  (ph.p_vaddr + ph.p_memsz, pagesize);
        off_start = util::align_down(ph.p_offset, pagesize);
        uint64_t const protection = GetProtection(ph.p_flags);
...
#if 0
        if (util::align_down(ph.p_vaddr, pagesize) != addr_min || DL_NOMMU_SUPPORT)
            if (mmap_fixed(base+this_min, this_max-this_min, prot, MAP_PRIVATE|MAP_FIXED, fd, off_start) == MAP_FAILED)
                goto error;
        if (ph.p_memsz > ph.p_filesz && (ph.p_flags & PF_W)) {
            size_t brk = (size_t)base + ph.p_vaddr + ph.p_filesz;
            size_t pgbrk = util::align_up(brk, pagesize);
