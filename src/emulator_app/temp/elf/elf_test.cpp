...
int test(char const *filename, size_t memory_size, size_t module_offset)
{
    ...
    std::vector<Elf_Phdr_template<uint64_t>> ps = elf.program_sections(fs);
    DumpProgramHeaders<uint64_t>(std::cout, ps.data(), ps.size());

    std::vector<u8> memory(memory_size, 0);
    load_library<uint64_t>(image, memory, module_offset);
