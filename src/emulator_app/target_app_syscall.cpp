...
{
    char const *file = (char const *)_file;
    char const *func = (char const *)_func;
...
{
    char const *pathname = (char const *)_pathname;
...
u64 TargetSingleApp::system_read(u64 fd, u64 addr, u64 size)
{
    ...
    u64 result = read((int)fd, (void*)addr, size);
...
u64 TargetSingleApp::system_readv(u64 fd, u64 piovec, u64 iovcnt)
{
    ...
    u64 result = readv((int)fd, (const struct iovec *)piovec, iovcnt);
    if ((intptr_t)result < 0) {
...
u64 TargetSingleApp::system_write(u64 fd, u64 addr, u64 size)
{
    ...
    u64 result = write((int)fd, (void*)addr, size);
    if ((intptr_t)result < 0) {
...
u64 TargetSingleApp::system_writev(u64 fd, u64 piovec, u64 iovcnt)
{
    ...
    u64 result = writev((int)fd, (const struct iovec *)piovec, (int)iovcnt);
    if ((intptr_t)result < 0) {
...
u64 TargetSingleApp::system_mmap(u64 addr, u64 length, u64 prot, u64 flags, u64 fd, u64 offset)
{
    ...
    void *base = mmap((void*)addr, (size_t)length, TargetProtectionToHost(prot), flags, (int)fd, offset);
    if (base == MAP_FAILED) {
...
u64 TargetSingleApp::system_munmap(u64 addr, u64 length)
{
    ...
    if (munmap((void*)addr, length) < 0) {
        ...
        RemoveHostMemory((void*)addr, length);
    }
    return 0;
}
...
u64 TargetSingleApp::system_mprotect(u64 addr, u64 len, u64 prot)
{
    ...
    if (mprotect((void*)addr, len, TargetProtectionToHost(prot)) < 0) {
        LOG_DEBUG(LOG_SYSCALL) << "SYS_mprotect failed " << errno;
        return -1LL;
    }
    AddHostMemory((void*)addr, len, TargetProtectionToEmulator(prot));
    return 0;
}
...
u64 TargetSingleApp::system_wait4(u64 pid, u64 stat_addr, u64 options, u64 rusage_addr)
{
    ...
    pid_t result = wait4((pid_t)pid, (int *)stat_addr, (int)options, (struct rusage *)rusage_addr);
    ...
    u64 r1, u64 r2, u64 r3, u64 r4, u64 r5, u64 r6)
{
    ...
    switch (code) {
        ...
        HANDLE_SYSCALL(3, doom_DrawFrame)
    ...
    if (code == SYS_exit || code == SYS_exit_group) {
        dbgs() << "exit detected " << code << " status=" << r1 << ", halt" << std::endl;
        return -1;
    }

    if (syscall_force_fail) {
        dbgs() << "syscall " << code << " forces halt" << std::endl;
        return -1;
    }
