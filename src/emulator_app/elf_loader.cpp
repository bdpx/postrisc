...
static void dump_shdrs(const std::span<const typename ELF::ELF_SHDR> sections, std::ostream& out, const std::span<const char> shstrtab)
{
    ...
    for (size_t i = 0; i < sections.size(); i++) {
        ...
        out << "<tr><td>" << i
            ...
            << "</td><td>0x" << std::hex << shdr.sh_addr << std::dec    // Section virtual addr at execution
    ...
    std::ostream& out)
{
    ...
    for (size_t i = 0; i < program_headers.size(); i++) {
        ...
        out << "<tr><td>" << i
            ...
            << "</td><td>0x" << std::hex << phdr.p_vaddr << std::dec   // Segment virtual address
            << "</td><td>0x" << std::hex << phdr.p_paddr << std::dec   // Segment physical address
...
TargetSingleApp::parse_code(const typename ELF::ELF_SHDR& section, const std::span<const std::byte> image, Decoder& tempDecoder)
{
    ...
    //WITH_DEBUG(LOG_LOADER) {
        ...
        dbgs() << "<div>\n";
        for (size_t k = 0; k < section.sh_size / sizeof(CBundle); k++) {
            u64 const ip = section.sh_addr + k * sizeof(CBundle); // reinterpret_cast<uintptr_t>(&bundle[k]);
            dbgs() << "<pre>";
    ...
    const std::span<const std::byte> original_file, const std::span<const std::byte> image, ElfInfo& elf_info)
{
    ...
    dbgs() << "<pre>_DYNAMIC=" << fmt::hex(elf_info.dynamic) << fmt::endl;
    ...
    Decoder& tempDecoder)
{
    ...
    dbgs() <<
        "<table><caption>Symtab: " << nsyms << " symbols</caption>"
        "<thead><tr>"
        "<th>value</th>"
        "<th>size</th>"
        "<th>bind</th>"
