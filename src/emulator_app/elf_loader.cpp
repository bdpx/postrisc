...
static void dump_ehdr(const typename ELF::ELF_EHDR& ehdr, std::ostream& out)
{
    out << "ELF header:"
        << "\n  class      "   << static_cast<unsigned>(ehdr.e_ident[EI_CLASS])
        << "\n  data       "   << static_cast<unsigned>(ehdr.e_ident[EI_DATA])
        << "\n  version    "   << static_cast<unsigned>(ehdr.e_ident[EI_VERSION])
        << "\n  osabi      "   << static_cast<unsigned>(ehdr.e_ident[EI_OSABI])
        << "\n  abiversion "   << static_cast<unsigned>(ehdr.e_ident[EI_ABIVERSION])
        << "\n  type       "   << ehdr.e_type               // Object file type
        << "\n  machine    0x" << fmt::hex(ehdr.e_machine)  // Architecture
        << "\n  version    "   << ehdr.e_version            // Object file version
        << "\n  entry      0x" << fmt::hex(ehdr.e_entry)    // Entry point virtual address
        << "\n  phoff      0x" << fmt::hex(ehdr.e_phoff)    // Program header table file offset
        << "\n  shoff      0x" << fmt::hex(ehdr.e_shoff)    // Section header table file offset
        << "\n  flags      0x" << fmt::hex(ehdr.e_flags)    // Processor-specific flags
        << "\n  ehsize     "   << ehdr.e_ehsize             // ELF header size in bytes
        << "\n  phentsize  "   << ehdr.e_phentsize          // Program header table entry size
        << "\n  phnum      "   << ehdr.e_phnum              // Program header table entry count
        << "\n  shentsize  "   << ehdr.e_shentsize          // Section header table entry size
        << "\n  shnum      "   << ehdr.e_shnum              // Section header table entry count
        << "\n  shstrndx   "   << ehdr.e_shstrndx           // Section header string table index
        << "\n";
}

template<typename ELF>
static int check_header(const typename ELF::ELF_EHDR *ehdr, size_t filesize, ElfInfo &elf_info)
{
    ENSURE(sizeof(*ehdr) < filesize, "Failed to read ELF file header");
    dump_ehdr<ELF>(*ehdr, dbgs());
...
static void dump_shdrs(const std::span<const typename ELF::ELF_SHDR> sections, std::ostream &out, const std::span<const char> shstrtab)
{
    ...
    size_t const id_len = 2;
    size_t const name_len = 16;
    out << std::left << std::setw(id_len) << "id"
        << ' ' << std::setw(2 * sizeof(sections[0].sh_offset)) << "offset"
        << ' ' << std::setw(2 * sizeof(sections[0].sh_addr)) << "addr"
        << ' ' << std::setw(2 * sizeof(sections[0].sh_size)) << "size"
        << ' ' << "align"
        << ' ' << "link    "
        << ' ' << "info"
        << ' ' << "entsize"
        << ' ' << "type    "
        << ' ' << std::setw(name_len) << "name"
        << ' ' << "flags"
        << fmt::endl;

    for (size_t i = 0; i < sections.size(); i++) {
        const typename ELF::ELF_SHDR& shdr = sections[i];
        out << std::left << std::setw(id_len) << i
            << ' ' << fmt::hex(shdr.sh_offset)            // Section file offset
            << ' ' << fmt::hex(shdr.sh_addr)              // Section virtual addr at execution
            << ' ' << fmt::hex(shdr.sh_size)              // Section size in bytes
            << std::left
            << ' ' << std::setw(5) << shdr.sh_addralign   // Section alignment
            << ' ' << std::setw(8) << shdr.sh_link        // Link to another section
            << ' ' << std::setw(4) << shdr.sh_info        // Additional section information
            << ' ' << std::setw(7) << shdr.sh_entsize;    // Entry size if section holds table
        ...
        out << ' ' << std::setw(name_len) << &shstrtab[shdr.sh_name];
        // out << " " << shdr.sh_name;
    ...
    std::ostream& out)
{
    ...
    size_t const id_len = 2;
    out << std::left << std::setw(id_len) << "id"
        << ' ' << std::setw(2 * sizeof(program_headers[0].p_offset)) << "offset"
        << ' ' << std::setw(2 * sizeof(program_headers[0].p_vaddr)) << "vaddr"
        << ' ' << std::setw(2 * sizeof(program_headers[0].p_paddr)) << "paddr"
        << ' ' << std::setw(2 * sizeof(program_headers[0].p_filesz)) << "filesz"
        << ' ' << std::setw(2 * sizeof(program_headers[0].p_memsz)) << "memsz"
        << ' ' << std::setw(2 * sizeof(program_headers[0].p_align)) << "align"
        << ' ' << "rwx"
        << ' ' << "type       sections"
        << fmt::endl;
    ...
    for (size_t i = 0; i < program_headers.size(); i++) {
        ...
        out << std::right << std::setw(id_len) << i
            << ' ' << fmt::hex(phdr.p_offset) // Segment file offset
            << ' ' << fmt::hex(phdr.p_vaddr)   // Segment virtual address
            << ' ' << fmt::hex(phdr.p_paddr)   // Segment physical address
            << ' ' << fmt::hex(phdr.p_filesz) // Segment size in file
            << ' ' << fmt::hex(phdr.p_memsz)   // Segment size in memory
            << ' ' << fmt::hex(phdr.p_align);  // Segment alignment
        ...
        if (type_name)
            out << ' ' << std::left << std::setw(10) << type_name;
        else
            out << ' ' << std::setw(10) << phdr.p_type;

        out << " sec:";
        for (const auto& section : sections) {
            if (phdr.p_vaddr <= section.sh_addr &&
                section.sh_size > 0 &&
                section.sh_addr != 0 &&
...
{
#define X(FLAG) if (val & (DF_1_##FLAG)) out <<  ' ' << #FLAG;
...
template<typename ELF>
int
TargetSingleApp::parse_code(const typename ELF::ELF_SHDR &section, const std::span<const std::byte> image)
{
    CBundle const *bundle = reinterpret_cast<CBundle const *>(&image[section.sh_addr]);
    LOG_DEBUG(LOG_LOADER) << "CODE=" << fmt::hex((uintptr_t)bundle);

    //WITH_DEBUG(LOG_LOADER) {
        std::array<u64, insn_reserved> instruction_stat = {{ 0 }};
        ...
        for (size_t k = 0; k < section.sh_size / sizeof(typename ELF::ELF_DYN); k++) {
            u64 const next_ip = reinterpret_cast<uintptr_t>(&bundle[k+1]);
            get_decoder().DisassembleBundle(bundle[k], next_ip, dbgs(), decoderContext);
            switch (bundle[k].GetTemplate()) {
                case EBundleTemplate::bundle_sss:
                    instruction_stat[Decoder::Decode(bundle[k].s0())]++;
                    instruction_stat[Decoder::Decode(bundle[k].s1())]++;
                    instruction_stat[Decoder::Decode(bundle[k].s2())]++;
                    break;
                case EBundleTemplate::bundle_sll:
                    instruction_stat[Decoder::Decode(bundle[k].s0())]++;
                    instruction_stat[Decoder::Decode(bundle[k].l1())]++;
                    break;
                case EBundleTemplate::bundle_lls:
                    instruction_stat[Decoder::Decode(bundle[k].l0())]++;
                    instruction_stat[Decoder::Decode(bundle[k].s2())]++;
                    break;
                default:
        ...
        Core::dump_insn_stat(dbgs(), instruction_stat, "Static instruction frequencies");
    //}
    return 0;
}

template<typename ELF>
static int parse_dynamic(unsigned index, const typename ELF::ELF_SHDR &section,
    const std::span<const std::byte> original_file, const std::span<const std::byte> image, ElfInfo &elf_info)
{
    LOG_DEBUG(LOG_LOADER) << "Dynamic found: " << index;
    ...
    elf_info.dynamic = (uintptr_t)&image[section.sh_addr];
    dbgs() << "_DYNAMIC=" << fmt::hex(elf_info.dynamic) << fmt::endl;

    typename ELF::ELF_DYN const *dynamic =
        reinterpret_cast<typename ELF::ELF_DYN const *>(&original_file[section.sh_offset]);

    for (size_t k = 0; k < section.sh_size / sizeof(typename ELF::ELF_DYN); k++) {
        dbgs() << "val=0x" << fmt::hex(dynamic[k].d_un.d_val); // d_ptr
        char const *name = get_dynamic_type_name(dynamic[k].d_tag);
        dbgs() << " type=";
        if (name)
            dbgs() << name;
        else
            dbgs() << "0x" << fmt::hex(dynamic[k].d_tag);
...
template<typename ELF>
static int parse_rel(unsigned index, const typename ELF::ELF_SHDR &section, const std::span<const std::byte> original_file, const std::span<std::byte> image)
{
    size_t const nrel = section.sh_size / sizeof(typename ELF::ELF_REL);
    dbgs() << "\nREL found: sndx=" << index << " nrel=" << nrel << fmt::endl;
    ...
    for (size_t k = 0; k < nrel; k++) {
        ...
        WITH_DEBUG(LOG_DUMP) {
            std::ostream& out = dbgs();
            out << "offset=0x" << fmt::hex(rel[k].r_offset)
                << " info=" << rtype;
            char const *reloc_name = postrisc_reloc_2_str((postrisc_reloc)rtype);
            if (reloc_name)
                out << " " << reloc_name;
            if (value != 0)
                out << " value=0x" << fmt::hex(value);
            out << std::endl;
        }
        switch(rtype) {
        case R_POSTRISC_RELATIVE:
            value += (uintptr_t)image.data();
            break;
...
template<typename ELF>
static int parse_rela(unsigned index, const typename ELF::ELF_SHDR &section, const std::span<const std::byte> original_file, const std::span<std::byte> image)
{
    size_t const nrel = section.sh_size / sizeof(typename ELF::ELF_RELA);
    dbgs() << "\nRELA found: shid=" << index << " nrel=" << nrel << fmt::endl;
    ...
    typename ELF::ELF_RELA const *rel =
        reinterpret_cast<typename ELF::ELF_RELA const *>(&original_file[section.sh_offset]);

    dbgs() << std::left
           << std::setw(2 * sizeof(rel->r_offset))      << "offset"
           << ' ' << std::setw(2 * sizeof(rel->r_addend)) << "addend"
           << ' ' << std::setw(2 * sizeof(intptr_t))  << "value"
           << ' ' << "info"
           << fmt::endl;
    ...
    for (size_t k = 0; k < nrel; k++) {
        ...
        dbgs() << fmt::hex(rel[k].r_offset)
               << ' ' << fmt::hex(rel[k].r_addend)
               << ' ' << fmt::hex(value)
               << ' ' << ELF::get_r_type(rel[k].r_info);
        if (old_value)
            dbgs() << " old=" << fmt::hex(old_value);
        dbgs() << fmt::endl;
    }
    return 0;
}
...
public:
    bool operator()(typename ELF::ELF_SYM const& a, typename ELF::ELF_SYM const& b) const { return a.st_value < b.st_value; }
};

template<typename ELF>
int
TargetSingleApp::parse_symtab(unsigned index,
                       const std::span<const typename ELF::ELF_SHDR> shdr,
                       ...
                       const std::span<const std::byte> original_file, const std::span<const std::byte> image)
{
    ...
    dbgs() << "\nSymtab " << index << " nsyms=" << nsyms << fmt::endl;
    ...
    std::vector<typename ELF::ELF_SYM> syms(nsyms);
    memcpy(syms.data(), symtab, nsyms * sizeof(typename ELF::ELF_SYM));

    std::sort(syms.begin(), syms.end(), symtab_comparator<ELF>());

    typename ELF::ELF_SYM dummy;
    dbgs() << std::left
           << std::setw(2 * sizeof(uintptr_t)) << "addr"
           << ' ' << std::setw(2 * sizeof(dummy.st_value)) << "offs"
           << ' ' << std::setw(2 * sizeof(dummy.st_size))  << "size"
           << ' ' << std::setw(6) << "bind"
           << ' ' << std::setw(7) << "type"
           << ' ' << std::setw(9) << "visiblty"
           << ' ' << "section:name"
    ...
    for (size_t k = 0; k < nsyms; k++) {
        ...
        dbgs();
        if (visibility_name)
            dbgs() << ' ' << std::left << std::setw(9) << visibility_name;
        else
            dbgs() << " vis=" << static_cast<unsigned>(sym.st_other);
        ...
        if (name)
            ...
            dbgs() << ' ' << &shstrtab[shdr[sym.st_shndx].sh_name];
        }

        dbgs() << ':' << &strtab[sym.st_name] << fmt::endl;

        if (address && sym.st_name > 0) {
    ...
    const std::span<const typename ELF::ELF_PHDR> program_headers)
{
    ...
    for (size_t i = 0; i < program_headers.size(); i++) {
        ...
        LOG_DEBUG(LOG_LOADER)
            << " map biased: " << fmt::hex((uintptr_t)seg_map)
            << ".." << fmt::hex((uintptr_t)seg_map + segment_size);
        ...
        */
        ...
        if (bss_start < bss_map_start) {
            ...
            LOG_DEBUG(LOG_LOADER) << "memset high:"
                ...
                << " map=" << fmt::hex((uintptr_t)&image[bss_start])
...
TargetSingleApp::load_program(const std::span<const std::byte> original_file, ElfInfo &elf_info, std::span<std::byte>& out_image)
{
    ...
    const typename ELF::ELF_EHDR *ehdr =
        reinterpret_cast<const typename ELF::ELF_EHDR*>(&original_file[0]);
    if (check_header<ELF>(ehdr, original_file.size(), elf_info) < 0) {
        return -1;
    }

    // locate phdr
    ENSURE(ehdr->e_phoff + ehdr->e_phnum * sizeof(typename ELF::ELF_PHDR) <= original_file.size(), "no place for phdrs");
    const typename ELF::ELF_PHDR *phdr =
        reinterpret_cast<const typename ELF::ELF_PHDR *>(&original_file[ehdr->e_phoff]);

    // locate section headers
    ENSURE(ehdr->e_shoff + ehdr->e_shnum * sizeof(typename ELF::ELF_SHDR) <= original_file.size(), "can't place shdrs");
    const typename ELF::ELF_SHDR *shdr =
        reinterpret_cast<const typename ELF::ELF_SHDR *>(&original_file[ehdr->e_shoff]);
    ...
    dbgs() << "mapped image: 0x" << fmt::hex((uintptr_t)image.data()) << fmt::endl;

    ENSURE(map_image<ELF>(image, original_file, program_headers) == 0, "map_image failed");

    elf_info.load_base = (uintptr_t)image.data();
    elf_info.entry     = (uintptr_t)image.data() + ehdr->e_entry;
    elf_info.ph_base   = (uintptr_t)image.data() + ehdr->e_phoff; // FIXME
    elf_info.dynamic   = 0;
    ...
    for (size_t i = 0; i < ehdr->e_shnum; i++) {
        ...
        dbgs() << "SECTION: " << i << " name=" << &shstrtab[sh.sh_name] << fmt::endl;
        ...
        switch (sh.sh_type) {
        case SHT_DYNAMIC:
            ENSURE(parse_dynamic<ELF>(i, sh, original_file, image, elf_info) == 0, "parse_dynamic failed");
            break;
        case SHT_SYMTAB:
        case SHT_DYNSYM:
            ENSURE(parse_symtab<ELF>(i, section_headers, sh, shdr[sh_link], shstrtab, original_file, image) == 0, "parse_symtab failed");
            break;
        case SHT_REL:
            ENSURE(parse_rel<ELF>(i, sh, original_file, image) == 0, "parse_rel failed");
            break;
        case SHT_RELA:
            ENSURE(parse_rela<ELF>(i, sh, original_file, image) == 0, "parse_rela failed");
            break;
        case SHT_GNU_HASH:
    ...
    for (size_t i = 0; i < ehdr->e_shnum; i++) {
        ...
        case SHT_PROGBITS:
            if (sh.sh_flags & SHF_EXECINSTR) {
                dbgs() << "SECTION: " << i << " name=" << &shstrtab[sh.sh_name] << fmt::endl;
                ENSURE(parse_code<ELF>(sh, image) == 0, "parse_code failed");
                elf_info.text_base = (uintptr_t)&image[sh.sh_addr];
...
TargetSingleApp::load_program(const std::span<const std::byte> original_file, ElfInfo &elf_info, std::span<std::byte>& image)
{
    ...
    for (unsigned i = 0; i < EI_NIDENT; i++) {
        dbgs() << ' ' << fmt::hex(static_cast<u8>(original_file[i]));
    }
    dbgs() << fmt::endl;
...
TargetSingleApp::load_elf_module(char const *file_name, ElfInfo &elf_info, std::span<std::byte>& image)
{
    ...
    dbgs() << "ELF file `" << file_name << "` size=" << original_file.size() << fmt::endl;
