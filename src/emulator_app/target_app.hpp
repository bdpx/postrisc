...
public:
    TargetSingleApp(Decoder& decoder, IDoomInterface& doom);
    TargetSingleApp(size_t pagesize, Decoder& decoder, IDoomInterface& doom);
    ~TargetSingleApp(void) override;

    STANDARD_ADDRESS_SPACE_INTERFACE

    int DoSyscall(unsigned code, u64 &result,
                  u64 r1, u64 r2, u64 r3,
                  u64 r4, u64 r5, u64 r6) override;

    int AddHostMemory(void const *p, size_t size, u64 protection);
    int RemoveHostMemory(void *p, size_t size);
    int StartEmulation(EmulatorConfiguration const & configuration,
...
private:
    ...
    template<typename ELF> int load_program(
                    const std::span<const std::byte> original_file,
                    ElfInfo &elf_info,
    ...
    template<typename ELF> int map_image(
                    const std::span<std::byte> mapped_image,
                    const std::span<const std::byte> original_file,
                    const std::span<const typename ELF::ELF_PHDR> program_headers);
    ...
    template<typename ELF> u8* prepare_stack(
                    ...
                    int argc, char **argv, char **env, ElfInfo &elf_info);
...
inline CStatus                                                               \
TargetSingleApp::bus_read_##TYPE(const CPhysAddress pa, TYPE& data) {        \
...
inline CStatus
TargetSingleApp::bus_read(const CPhysAddress pa, size_t len, void *bytes)
{
