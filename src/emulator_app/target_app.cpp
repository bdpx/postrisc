...
TargetSingleApp::prepare_stack(const std::span<std::byte> stack, int argc, char **argv, char **env, ElfInfo &elf_info)
{
    ...
    elf_info.stack_bottom = (uintptr_t)stack.data();
    elf_info.stack_top    = (uintptr_t)stack.data() + stack.size();
    ...
    LOG_DEBUG(LOG_DUMP)
        << "stack: " << fmt::hex((uintptr_t)stack.data())
        << ".." << fmt::hex((uintptr_t)top);
    ...
    // align auxv 16-byte
    while ((uintptr_t)(top - rest) % 16) { --top; --len; }
...
#define PUSH_AUXV(TYPE, VALUE)                                            \
    ...
    push_ptr(top, len, (char const*)(intptr_t)argc);
    ...
    LOG_DEBUG(LOG_DUMP) << "\n" << fmt::hex_data(dump_base, stack.size() - dumped_offset, (uintptr_t)dump_base);

    get_decoder().add_symbol((uintptr_t)stack.data(), stack.size(), "!STACK");
...
TargetSingleApp::prepare_backstore(const std::span<std::byte> backstore)
{
    ...
    get_decoder().add_symbol((uintptr_t)backstore.data(), backstore.size(), "!BACKSTORE");
}
    ...
    for (auto const & x : m_HostAccessMap) {
        ...
        for (auto const & y : x.second.data) {
            if (y) {
                ...
                out << std::endl;
                if (flags)
                    out << fmt::hex_data((u8 const*)base, get_page_size(), base);
            }
            base += get_page_size();
        }
    }
    ...
    int argc, char**argv, char**env)
{
    ...
    CHECK_STATUS(core.PrepareThread(sp, bp, tp, eip));
    core.SetIp(ip);
