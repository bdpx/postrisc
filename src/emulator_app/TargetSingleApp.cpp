...
TargetSingleApp::StartEmulation(const Configuration& configuration, int argc, char**argv, const std::map<std::string, std::string>& env)
{
    ...
    const auto stack_top = prepare_stack<ELF64>(stack, argc, argv, env, elf_info);
    if (!stack_top) {
    ...
    const u64 sp = reinterpret_cast<uintptr_t>(stack_top);
    const u64 bsp = reinterpret_cast<uintptr_t>(stack.data());
    const u64 bp = reinterpret_cast<uintptr_t>(backstore.data());
    const u64 tp = 0;
    ...
    const u64 ip = elf_info.entry;
    // FIXME: starting eip should be valid
    const u64 eip = elf_info.text_base + elf_info.text_size;
    ...
    const auto frames_num = doom_interface.getFramesNum();
    ...
    if (frames_num > 0) {
        ...
        if (core.GetPerfTime() > 0) {
            const double runtime = 1.0e-9 * core.GetPerfTime(); // from nanoseconds to seconds
