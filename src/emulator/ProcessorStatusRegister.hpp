...
ALWAYS_INLINE void ProcessorStatusRegister::NullifyFuture(u32 mask)
{
...
ALWAYS_INLINE unsigned ProcessorStatusRegister::GetShadowSize(void) const
{
    return ri;
}

ALWAYS_INLINE void ProcessorStatusRegister::ClearShadow(void)
{
    // on the new bundle start
    ri = 0;
}

ALWAYS_INLINE void ProcessorStatusRegister::ClearFuture(void)
{
    // on each branch/jump/call we clear future predication
    future = 0;
}

ALWAYS_INLINE void ProcessorStatusRegister::SetFuture(u32 f)
{
    // on each return we restore saved future predication
    future = f;
}

ALWAYS_INLINE u32 ProcessorStatusRegister::GetFuture(void) const
{
    return future;
}

ALWAYS_INLINE bool ProcessorStatusRegister::PopFuture(void)
{
