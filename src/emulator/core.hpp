...
public:
    ...
    void DisassembleDynamic(IAddressSpace& as, std::ostream& out, i64 start, i64 stop);
    CStatus PrepareThread(u64 bsp, u64 sp, u64 bp, u64 tp, u64 eip);
    ...
    TranslateAddress(const IAddressSpace &as, const CVirtAddress va,
...
private:
    ...
    template<class CMD> CStatus ExecuteInstruction(IAddressSpace& as, unsigned shadow_size, const CMD cmd);
    ...
    template<class T, class OP> ALWAYS_INLINE CStatus ConditionalMove(void);
    ...
    template<class INDEX> ALWAYS_INLINE CStatus compute_address_indexed(IAddressSpace& as);
    template<class T, class INDEX> ALWAYS_INLINE CStatus read_indexed(IAddressSpace& as);
    template<class T, class INDEX> ALWAYS_INLINE CStatus write_indexed(IAddressSpace& as);
    ...
    // atomic templates
    template<class T> CStatus atomic_load(IAddressSpace& as, std::memory_order);
    template<class T> CStatus atomic_store(IAddressSpace& as, std::memory_order);
    template<class T, class OP> CStatus fetch_and_op_atomic(IAddressSpace& as, std::memory_order order);
    template<class T, class OP> CStatus store_op_atomic(IAddressSpace& as, std::memory_order order);
    template<class T> CStatus compare_and_swap(IAddressSpace& as, std::memory_order order);
    CStatus atomic_fence(IAddressSpace& as, std::memory_order);
...
#define DEFINE_INSN_FMT_IMPLEMENTATION(name) \
    ...
    template<class T> u64 ComputeScaledIndex(void) const;
...
#define X(mnem, name, subset, format, descr) static CStatus insn_##name(Core& core, IAddressSpace& as);
...
ALWAYS_INLINE
std::tuple<CStatus, CPhysAddress>
Core::TranslateAddress(const IAddressSpace &as, const CVirtAddress va,
                       u64 requested_rights, u64 unexpected_rights)
{
