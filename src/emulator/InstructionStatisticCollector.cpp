...
public:
    ...
    bool operator>(const InstructionSorter& r) const {
        return freq > r.freq || (freq == r.freq && std::strcmp(to_string(id), to_string(r.id)) < 0);
    }
public:
    EInstructionID id;
    u64    freq;
...
void InstructionStatisticCollector::dump(u64 cmd1, u64 cmd2, const DumpFormatter& out, char const *title) const
{
    ...
    if (out.is_html()) {
        out << "<table>\n";
        out << "<caption>" << title << ": " << all << " (" << cmd1 << " short, " << cmd2 << " long)</caption>\n";
    } else {
        out << title << ": " << all << '\n';
    }
