...
template<typename T> ALWAYS_INLINE Result Core::crc32c(void)
{
    const u32 prev = rb().scalar<u32>();
    const T data = rc().scalar<T>();

    const u32 res = crc32c::hash_intermediate(prev, data);
    ra() = res;
    return FpuFinalizeInstruction();
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(crc32cb) { return core.crc32c<u8>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(crc32ch) { return core.crc32c<u16>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(crc32cw) { return core.crc32c<u32>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(crc32cd) { return core.crc32c<u64>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(crc32cq) { return core.crc32c<u128>(); }

DEFINE_INSN_IMPLEMENTATION_NOMEM(clmul)
{
    const u32 x = core.cmd.scale & 1;
    const u32 y = (core.cmd.scale >> 1) & 1;
    core.ra() = aes::clmul( core.rb().vector<u64>(x), core.rc().vector<u64>(y) );
