...
CStatus Core::read_u8   (const CDeviceOffset UNUSED(device_offset),   u8& UNUSED(value)) { return CStatus::device_error; }
CStatus Core::read_u16  (const CDeviceOffset UNUSED(device_offset),  u16& UNUSED(value)) { return CStatus::device_error; }
CStatus Core::read_u32  (const CDeviceOffset UNUSED(device_offset),  u32& UNUSED(value)) { return CStatus::device_error; }
CStatus Core::read_u64  (const CDeviceOffset UNUSED(device_offset),  u64& UNUSED(value)) { return CStatus::device_error; }
CStatus Core::read_u128 (const CDeviceOffset UNUSED(device_offset), u128& UNUSED(value)) { return CStatus::device_error; }
...
CStatus Core::read(const CDeviceOffset device_offset, size_t len, void* UNUSED(bytes))
{
...
CStatus Core::read_config(const CDeviceOffset device_offset, size_t len, void *bytes)
{
...
CStatus Core::write_config(const CDeviceOffset device_offset, size_t len, const void *bytes)
{
    ...
    if (offset == offsetof(space, m_InterruptVector)
    ...
    else if (offset == offsetof(space, m_TimeCmp)
...
Core::RaiseTrap(const CStatus status, const CBundle bundle)
{
    ...
    if (shadow_size == 0) {
        // trap after taken branch, use iipa-stored info from ExecuteJump
        SetupDebugPlace(RoundToBundle(iipa), iipa & 3);
        ...
        SetupDebugPlace(ip - sizeof(CBundle), shadow_size);
        psr.ClearShadow();
    } else {
        // restart current bundle
        ip -= sizeof(CBundle);
...
Core::CallSubroutine(IAddressSpace& as, const unsigned preserve, CGeneralRegister& retreg, u64 address)
{
    ...
    if (!psr.IsLastInstructionInBundle()) {
        // check if all bundle instructions are in the predication shadow
        unsigned shadow_size = psr.GetShadowSize();
        LOG_DEBUG(LOG_CALLSTACK | LOG_PREDICATION) << " shadow_size=" << shadow_size;
        // we make call, so we have at least 1 instruction (call itself)
        assert(shadow_size > 0);
        assert(shadow_size < 3);
        // prepare new future for nullification after return
        // by inserting shadow_size ones into future
        u32 const mask = util::makemask32(shadow_size);
        assert(((future >> (8 * sizeof(future) - shadow_size)) & mask) == 0);
        future = (future << shadow_size) | mask;
        // stay in current bundle after return
        ret_addr -= sizeof(CBundle);
    }

    // pack together return addresses & preserved frame size
    u64 const eip = exception_state.GetEIP();
    u64 const eip_offset = (eip - ret_addr) >> log_bytes_per_bundle;
...
Core::ReturnFromSubroutine(IAddressSpace& as, const CGeneralRegister link)
{
    ...
    u64 const eretaddr = retaddr + (eip_offset << log_bytes_per_bundle);
    ...
    // ExecuteJump clears future, so setup old future later
...
***************************************************************************/
CStatus
Core::GetSpecialRegister(unsigned int regnum, u64& out)
{
...
***************************************************************************/
CStatus
Core::SetSpecialRegister(unsigned int regnum, u64 value)
{
    if (unlikely(arch_spr_regtype[regnum] == ESpecialRegisterSubset::reserved)) {
    ...
    switch (regnum) {
        ...
        case reg_eip:
            if (RoundToBundle(value) != value) {
               LOG_ERROR(LOG_EXCEPTION) << "Invalid EIP write value: " << fmt::hex(value);
               return CStatus::invalid_parameter;
            }
            exception_state.SetEIP(value);
            break;

        case reg_reip:
            if (RoundToBundle(value) != value) {
               LOG_ERROR(LOG_EXCEPTION) << "Invalid REIP write value: " << fmt::hex(value);
               return CStatus::invalid_parameter;
            }
            exception_state.SetREIP(value);
            break;

        case reg_fpcr:     fpu.set_control_word(value);      break;

        case reg_eca:      exception_state.SetContext(value); break;

        case reg_kip:      kip  = value;   break;
        case reg_krsp:     krsp = value;   break;
        case reg_ksp:      ksp  = value;   break;

        case reg_peb:      peb = value;    break;
        case reg_teb:      teb = value;    break;
        case reg_itc:      itc = value;    break;
        case reg_itm:      itm = value;    break;

        case reg_rsc:
            if (unlikely((value & util::makemask(bits_per_register)) != value)) {
               LOG_ERROR(LOG_EXCEPTION) << "Invalid RSC write value: " << fmt::hex(value);
               return CStatus::invalid_parameter;
            }
            rsc.set(value);
            break;
        ...
        case reg_rsp:
            rsp = value & ~util::makemask(log_bytes_per_address);
            break;

        case reg_pta:
            return mmu.SetConfig(value);
            break;

        case reg_psr:
            if (unlikely((value & CProcessorStatusRegister::FLAGS_ALL) != value)) {
        ...
        *******************************************************************/
        ...
        case reg_iva:
            if (unlikely((value & ~util::makemask(bits_per_iva_alignment)) != value)) {
                LOG_ERROR(LOG_EXCEPTION) << "invalid iva write value: " << fmt::hex(value);
                return CStatus::invalid_parameter;
            }
            iva = value;
            break;
        ...
        case reg_iip:
            iip = RoundToBundle(value);
            break;

        case reg_iipa:
            iipa = RoundToBundle(value);
            break;
        ...
        *******************************************************************/
        ...
        case reg_lid:
            if (unlikely((value & util::makemask(bits_per_processor_id)) != value)) {
                LOG_ERROR(LOG_EXCEPTION) << "invalid pid write value: " << fmt::hex(value);
                return CStatus::invalid_parameter;
            }
            pic.lid = static_cast<unsigned>(value);
            break;

        case reg_tpr:
            if (unlikely((value & util::makemask(bits_per_int_vector)) != value)) {
                LOG_ERROR(LOG_EXCEPTION) << "invalid tpr write value: " << fmt::hex(value);
                return CStatus::invalid_parameter;
            }
            pic.tpr = static_cast<u8>(value);
            break;

        case reg_itcv:     pic.itcv.Set(value);      break;
        case reg_tsv:      pic.tsv.Set(value);       break;
        case reg_pmv:      pic.pmv.Set(value);       break;
        case reg_cmcv:     pic.cmcv.Set(value);      break;
...
Core::dump_state(const DumpFormatter& out) const
{
    ...
    for (size_t i = 0; i < arch_spr_regtype.size(); i++) {
        if (arch_spr_regtype[i] != ESpecialRegisterSubset::reserved) {
            out << std::left
                << std::setfill(' ')
                << std::setw(8)
                << Decoder::GetSprName(i);
            (void)OutputSpecialRegister(out, i);
            out << fmt::endl;
        }
    }

    unsigned const mask = 0x3;
    ...
    for (unsigned i = base_rotate_regs; i < base_rotate_regs + max_stacked_framesize; i++) {
        ...
        out << std::right << std::setfill(' ') << std::setw(name_width) << Decoder::GetGprName(i)
            << std::left << ' ' << gpr[n];
...
#define X(name, subset, format, descr) &Core::insn_##name,
#include "arch/insn_table.hpp"
#undef X
};
...
#define STATIC_ROTATE_REG(NAME)                                       \
    unsigned const r##NAME = cmd.r##NAME();                           \
...
DEFINE_INSN_FMT_IMPLEMENTATION(Rabcd)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
...
DEFINE_INSN_FMT_IMPLEMENTATION(sprsr)
{
    STATIC_ROTATE_REG(a)
    core.cmd.rb = cmd.rb();
    STATIC_ROTATE_REG(c)
...
DEFINE_INSN_FMT_IMPLEMENTATION(Rabc)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
...
DEFINE_INSN_FMT_IMPLEMENTATION(RabcMo)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
    core.cmd.memory_order = cmd.GetMemoryOrder();
    DO_INSN;
}

DEFINE_INSN_FMT_IMPLEMENTATION(RabMo)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    core.cmd.memory_order = cmd.GetMemoryOrder();
...
DEFINE_INSN_FMT_IMPLEMENTATION(RbcMo)
{
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
    core.cmd.memory_order = cmd.GetMemoryOrder();
...
DEFINE_INSN_FMT_IMPLEMENTATION(fence)
{
    core.cmd.memory_order = cmd.GetMemoryOrder();
    DO_INSN;
}
...
DEFINE_INSN_FMT_IMPLEMENTATION(Rab)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    DO_INSN;
...
ALWAYS_INLINE
CStatus
Core::ExecuteInstruction(IAddressSpace& as, const CMD insn_cmd)
{
...
#define INSTRUCTION_FORMAT_HANDLER_X(name, extname, descr, format, txt) insn_fmt_handler_##name<CMD>,
    ...
    CStatus const status = inst_fmt_table[insn_format[id]](*this, as, id, insn_cmd);
...
Core::ExecuteBundle(IAddressSpace& as, const CBundle bundle)
{
    ...
    switch (bundle_template) {
        case EBundleTemplate::bundle_sss:
            ...
            status = ExecuteInstruction(as, bundle.s0());
            if (likely(status.continuable())) {
               status = ExecuteInstruction(as, bundle.s1());
               if (likely(status.continuable())) {
                   psr.SetIsLastInstructionInBundle();
                   status = ExecuteInstruction(as, bundle.s2());
               }
            }
            break;
        ...
        case EBundleTemplate::bundle_sll:
            ...
            if (likely(status.continuable())) {
                psr.SetIsLastInstructionInBundle();
                status = ExecuteInstruction(as, bundle.l1());
            }
            break;

        case EBundleTemplate::bundle_lls:
            LOG_DEBUG(LOG_DECODER) << "lls: " << bundle;
            status = ExecuteInstruction(as, bundle.l0());
            if (likely(status.continuable())) {
                psr.SetIsLastInstructionInBundle();
                status = ExecuteInstruction(as, bundle.s2());
            }
        ...
        WITH_DEBUG(LOG_EXCEPTION) {
            while (!m_debug_link_info_stack.empty()) {
                ...
                dbgs() << "stack:"
                   << " sp=" << fmt::hex(item.sp)
                   << " fp=" << fmt::hex(item.fp)
                   << " ip=" << fmt::addr_info(as.get_decoder(), item.ip)
                   << std::endl;
            }
        }
#endif
...
Core::DumpProfile(const DumpFormatter& out) const
{
    ...
    for (const auto& x : m_profile) {
        out << std::setw(12) << x.second
            << ':' << fmt::addr_info(out.get_decoder(), x.first) << fmt::endl;
...
public:
    ...
    bool operator > (const InstructionSorter& r) const {
        return freq > r.freq || (freq == r.freq && strcmp(asm_name[id], asm_name[r.id]) < 0);
    }
public:
    size_t id;
    u64    freq;
...
CStatus
Core::DisassembleBundle(IAddressSpace& as, std::ostream& out, i64 offset, DecoderContext& decoderContext)
{
    ...
    CStatus const status = as.mmu_fetch_code(*this,
                            cacheLine,
                            sizeof(bundles), &bundles);
    if (!status.success()) {
    ...
    if (offset == 0) {
       ...
       decoderContext.future_ri = 1;
       decoderContext.future_predication = psr.GetFuture();
    } else {
       decoderContext.ri = 0;
       decoderContext.future_ri = 0;
       // but don't zero future
    }
    as.get_decoder().DisassembleBundle(bundle, address + sizeof(bundle), out, decoderContext);
    return CStatus::continue_execution;
}

CStatus
Core::PrepareThread(u64 sp, u64 bp, u64 tp, u64 eip)
{
