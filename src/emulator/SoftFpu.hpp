...
public:
    ...
    template<typename FLOAT> FLOAT add(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT sub(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT mul(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT div(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT min(FLOAT a, FLOAT b);
    template<typename FLOAT> FLOAT max(FLOAT a, FLOAT b);
    template<typename FLOAT> FLOAT muladd(FLOAT a, FLOAT b, FLOAT c, const RoundingModeEnum rounding_mode, muladd_negate_t negate_flags);
    template<typename FLOAT> FLOAT sqrt(FLOAT a, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT rsqrt(FLOAT a, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT log2(FLOAT a, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT exp(FLOAT a, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT scalbn(FLOAT a, int exp, const RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT round_to_int(FLOAT a, const RoundingModeEnum rounding_mode);

    template<typename FLOAT_DST, typename FLOAT_SRC> FLOAT_DST extend(FLOAT_SRC a);
    template<typename FLOAT_DST, typename FLOAT_SRC> FLOAT_DST cvt_f_f(FLOAT_SRC a, const RoundingModeEnum rounding_mode);
    template<typename INTEGER, typename FLOAT> INTEGER cvt_f_i(FLOAT a, const RoundingModeEnum rounding_mode);
    template<typename FLOAT, typename INTEGER> FLOAT cvt_i_f(INTEGER a, const RoundingModeEnum rounding_mode);
...
private:
    ...
    template<typename FLOAT> FLOAT round_pack_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT normalize_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rounding_mode);
    template<typename FLOAT> FLOAT normalize2_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant1, typename FLOAT::F_UINT a_mant0, RoundingModeEnum rounding_mode);
...
template<typename FLOAT>
FLOAT
SoftFPU::round_pack_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rounding_mode)
{
    LOG_DEBUG(LOG_EVALUATE) << " exp=" << a_exp << " mant=" << fmt::hex(a_mant);
    ...
    case RM_RDN:
    case RM_RUP:
        if (a_sign ^ (rounding_mode == RM_RUP)) // rounding_mode & 1
            addend = (1U << FLOAT::RND_SIZE) - 1;
        else
            addend = 0;
        break;
    ...
    if (rounding_mode == RM_RNE && rnd_bits == (1U << (FLOAT::RND_SIZE - 1U)))
...
template<typename FLOAT>
FLOAT
SoftFPU::normalize_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rounding_mode)
{
    LOG_DEBUG(LOG_EVALUATE) << " exp=" << a_exp << " mant=" << fmt::hex(a_mant);
    ...
    a_exp -= shift;
    a_mant <<= shift;
    return round_pack_sf<FLOAT>(a_sign, a_exp, a_mant, rounding_mode);
}
...
SoftFPU::normalize2_sf(u32 a_sign, int a_exp,
    typename FLOAT::F_UINT a_mant1, typename FLOAT::F_UINT a_mant0, RoundingModeEnum rounding_mode)
{
    ...
    return round_pack_sf<FLOAT>(a_sign, a_exp, a_mant1, rounding_mode);
}
...
template<typename FLOAT> FLOAT SoftFPU::add(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode)
{
    ...
    return normalize_sf<FLOAT>(a_sign, a_exp, a_mant, rounding_mode);
}

template<typename FLOAT> FLOAT SoftFPU::sub(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode)
{
    b.v ^= FLOAT::SIGN_MASK;
    return add(a, b, rounding_mode);
}
...
template<typename FLOAT> FLOAT SoftFPU::mul(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode)
{
    auto a_sign = a.get_sign();
    auto b_sign = b.get_sign();
    ...
    r_mant |= !!r_mant_low; // != 0
    return normalize_sf<FLOAT>(r_sign, r_exp, r_mant, rounding_mode);
}

template<typename FLOAT>
FLOAT
SoftFPU::muladd(FLOAT a, FLOAT b, FLOAT c, const RoundingModeEnum rounding_mode, muladd_negate_t negate_mask)
{
    ...
    if (c_exp == 0) {
        if (c_mant == 0) {
            // add zero
            r_mant1 |= (r_mant0 != 0);
            return normalize_sf<FLOAT>(r_sign, r_exp, r_mant1, rounding_mode);
        }
    ...
    return normalize_sf<FLOAT>(r_sign, r_exp, r_mant1, rounding_mode);
#else
    return normalize2_sf<FLOAT>(r_sign, r_exp, r_mant1, r_mant0, rounding_mode);
#endif
}
...
template<typename FLOAT> FLOAT SoftFPU::div(FLOAT a, FLOAT b, const RoundingModeEnum rounding_mode)
{
    auto a_sign = a.get_sign();
    auto b_sign = b.get_sign();
    auto r_sign = a_sign ^ b_sign;
...
template<typename FLOAT> FLOAT SoftFPU::sqrt(FLOAT a, const RoundingModeEnum rounding_mode)
{
    auto a_sign = a.get_sign();
    auto a_exp = a.get_exp();
    auto a_mant = a.get_fraction();
...
template<typename FLOAT> FLOAT SoftFPU::rsqrt(FLOAT a, const RoundingModeEnum rounding_mode)
{
    FLOAT one = FLOAT{ typename FLOAT::F_UINT{FLOAT::BIAS} << FLOAT::MANT_SIZE };
    return div(one, sqrt(a, rounding_mode), rounding_mode);
}
...
template<typename FLOAT_DST, typename FLOAT_SRC>
FLOAT_DST
SoftFPU::cvt_f_f(FLOAT_SRC a, const RoundingModeEnum rounding_mode)
{
    static_assert(sizeof(FLOAT_SRC) > sizeof(FLOAT_DST));
    ...
    return normalize_sf<FLOAT_DST>(a_sign, a_exp, (typename FLOAT_DST::F_UINT)a_mant, rounding_mode);
}
...
template<typename FLOAT>
FLOAT
SoftFPU::round_to_int(FLOAT a, const RoundingModeEnum rounding_mode)
{
    auto a_sign = a.get_sign();
    auto aExp   = a.get_exp();
    ...
    else if (rounding_mode != RM_RTZ) {
        if ( FLOAT{z}.get_sign() ^ (rounding_mode == RM_RUP) ) {
            z += roundBitsMask;
        }
    }
    z &= ~ roundBitsMask;
...
INTEGER
SoftFPU::cvt_f_i(FLOAT a, const RoundingModeEnum rounding_mode)
{
    ...
    if (a_exp >= 0) {
        ...
        case RM_RDN:
        case RM_RUP:
            if (a_sign ^ (rounding_mode == RM_RUP/*& 1*/))
                addend = (1 << FLOAT::RND_SIZE) - 1;
            else
                addend = 0;
            break;
        ...
        // half way: select even result
        if (rounding_mode == RM_RNE && rnd_bits == (1 << (FLOAT::RND_SIZE - 1)))
            a_mant &= ~(typename FLOAT::F_UINT)1;
        if (a_mant > r_max) {
            raise(FFLAG_INVALID);
            return r_max;
...
FLOAT
SoftFPU::cvt_i_f(INTEGER a, const RoundingModeEnum rounding_mode)
{
    static_assert(std::numeric_limits<INTEGER>::is_integer);
    static_assert(fp_traits<FLOAT>::is_fp());
...
*----------------------------------------------------------------------------*/
template<typename FLOAT> FLOAT SoftFPU::log2(FLOAT a, const RoundingModeEnum rounding_mode)
{
    using F_UINT = typename FLOAT::F_UINT;
    ...
    return normalize_sf<FLOAT>(zSign, FLOAT::BIAS + FLOAT::EXP_SIZE - 1, zSig, rounding_mode);
}

/* Multiply A by 2 raised to the power N.  */
template<typename FLOAT> FLOAT SoftFPU::scalbn(FLOAT a, int n, const RoundingModeEnum rounding_mode)
{
    auto a_sign = a.get_sign();
    auto aSig   = a.get_fraction();
    auto aExp   = a.get_exp();
    ...
    return normalize_sf<FLOAT>(a_sign, aExp, aSig, rounding_mode);
}
...
template<typename FLOAT> FLOAT SoftFPU::exp(FLOAT a, const RoundingModeEnum rounding_mode)
{
    auto aSig = a.get_fraction();
    auto aExp = a.get_exp();
    auto a_sign = a.get_sign();
    ...
    for (int i = 0 ; i < N; i++) {
        s[i] = xn;
        n = add(n, f_one, rounding_mode);
        xn = mul(xn, x, rounding_mode);
        xn = div(xn, n, rounding_mode);
    }

    FLOAT r = zero_sf<FLOAT>(0);
    for (int i = N - 1; i >= 0; i--) {
        r = add(r, s[i], rounding_mode);
    }
    r = add(r, f_one, rounding_mode);
