...
***************************************************************************/
CStatus
CTranslateBuffer::insert_translate_buffer(tlb_ppn_t gpr, uint64_t ifa)
{
    // check page size
    ...
    uint64_t insert_mask = ~util::makemask(insert_ps);

    uint64_t insert_vp = ifa;
    ...
    uint64_t insert_pp = gpr.address & ~util::makemask(bits_per_minpageoffset);
    ...
    for (size_t i=0; i<entry.size(); i++)
    {
        ...
        uint64_t tlb_vpa = entry[i].vpn.get_vpa();
        ...
        uint64_t tlb_pagemask = ~util::makemask(tlb_ps);
        assert((tlb_vpa & tlb_pagemask) == tlb_vpa);

        uint64_t cmp_mask = tlb_pagemask & insert_mask;
...
{
    LOG_TRACE(LOG_ITLB | LOG_DTLB) << "address=" << fmt::hex(address);

    const uint64_t vpn = address & util::makemask(bits_per_vpn);
    ...
    for (unsigned tlbnum=0; tlbnum < entry.size(); tlbnum++) {
        ...
        const uint64_t pagemask = ~util::makemask(pagesize);

        const uint64_t tlbvpn = entry[tlbnum].vpn.get_vpa();
        ...
        if ((rights & page_rights) != rights) {
            ...
            return CStatus(CStatus::data_access_right);
        }

        // all is right
        uint64_t paddr = entry[tlbnum].ppn.get_ppa();
    ...
    if (clevel > 0) {
        ...
        return CStatus(CStatus::data_tlb_miss);
    }
