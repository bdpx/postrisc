...
public:
    ...
    CStatus mmu_read_u8  (Core& core, const CVirtAddress va, u8& value) override;
    CStatus mmu_read_u16 (Core& core, const CVirtAddress va, u16& value) override;
    CStatus mmu_read_u32 (Core& core, const CVirtAddress va, u32& value) override;
    CStatus mmu_read_u64 (Core& core, const CVirtAddress va, u64& value) override;
    CStatus mmu_read_u128(Core& core, const CVirtAddress va, u128& value) override;
    CStatus mmu_read(Core& core, const CVirtAddress va, size_t len, void *data) override;
    ...
    CStatus mmu_write(Core& core, const CVirtAddress va, size_t len, const void *data) override;

    CStatus mmu_read_u32_finalized(Core& core, const CVirtAddress va, u32& value) override;
    CStatus mmu_read_u64_finalized(Core& core, const CVirtAddress va, u64& value) override;
    CStatus mmu_read_backing_store(Core& core, const CVirtAddress va, size_t len, void *data) override;
...
mmu_read_##TYPE(Core& core, const CVirtAddress va, TYPE& data)                           \
{                                                                                        \
    LOG_DEBUG(LOG_LOAD) << "read_" #TYPE ": "                                            \
        << fmt::addr_info(get_decoder(), va.get_address());                              \
    CHECK_ALIGNMENT(CDataHelper, va, sizeof(data));                                      \
    auto [status, pa] =                                                                  \
...
// universal read variant
template<class DerivedT>
CStatus
IManagedAddressSpace<DerivedT>::mmu_read(Core& core, const CVirtAddress va, size_t len, void *data)
{
    LOG_DEBUG(LOG_LOAD)
        << "read: len=" << len
        << " addr=" << fmt::addr_info(get_decoder(), va.get_address());
    ...
    CHECK_STATUS(status);
    status = derived()->DerivedT::bus_read(pa, len, data);
    LOG_DEBUG(LOG_LOAD) << "loaded value: " << fmt::data_logger(data, len);
    return status;
}
...
IManagedAddressSpace<DerivedT>::mmu_read_##TYPE##_finalized(Core& core,                  \
    const CVirtAddress va, TYPE& data)                                                   \
{                                                                                        \
...
// universal write variant
template<class DerivedT>
CStatus
IManagedAddressSpace<DerivedT>::mmu_write(Core& core,
    const CVirtAddress va, size_t len, void const *data)
{
    LOG_DEBUG(LOG_STORE)
        << "write: len=" << len
        << " addr=" << fmt::addr_info(get_decoder(), va.get_address());
    ...
    LOG_DEBUG(LOG_STORE) << "stored value: "<< fmt::data_logger(data, len);
    return status;
}
