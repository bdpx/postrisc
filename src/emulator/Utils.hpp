...
class subtract_carry { public: template<typename T> static T func(T x, T y)
{
    return (T)(x-y) > x;
}
...
/***************************************************************************
unsigned averages:
   floor: (a+b)/2 = avgu(a,b) = (a & b) + ((a ^ b) >> 1)
   ceil: (a+b+1)/2 = ~avgu(~a,~b)
***************************************************************************/
class average_unsigned_ceil { public: template<typename T> static T func(T x, T y)
{
    static_assert(std::numeric_limits<T>::is_integer && std::is_unsigned_v<T>);
...
/***************************************************************************
signed averages:
   ceil: (a+b+1)/1 = (a | b) - ((a ^ b) >>signed 1)
   ceil: (a+b+1)/2 = (a & b & 1) + (a >>signed 1) + (b >>signed 1)
   floor: (a+b)/2 = bias(avg(bias(a),bias(b))), bias(x) = sign ^ x
***************************************************************************/
class average_signed_ceil { public: template<typename T> static T func(T x, T y)
{
    static_assert(std::numeric_limits<T>::is_integer && std::is_signed_v<T>);
...
class fround   { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, fpu::RoundingModeEnum rm) { return fpu.round<T>(a, rm, false /*inexact*/); }};
class froundnx { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, fpu::RoundingModeEnum rm) { return fpu.round<T>(a, rm, true /*inexact*/); }};
