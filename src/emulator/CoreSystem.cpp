...
DEFINE_INSN_IMPLEMENTATION_NOMEM(getspr)
{
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(setspr)
{
    return core.SetSpecialRegister(core.cmd.rc, core.ra());
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(getmr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(setmr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(getibr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(setibr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(getdbr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(setdbr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(setitr)
{
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(setdtr)
{
    u64 const index = core.rb().base();
    LOG_DEBUG(LOG_EMULATOR)
        << "index=" << index << " a=" << core.ra() << " c=" << core.rc();
...
DEFINE_INSN_IMPLEMENTATION(syscall)
{
    ...
    // count systime separately for the fair performance of app-level syscall-emulator
    const std::chrono::steady_clock::time_point start = std::chrono::steady_clock::now();

    const int result = as.DoSyscall(rn, core.gpr[rin_result].scalar<u64>(), r2, r3, r4, r5, r6, r7);

    const std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    const auto diff = end - start;
    const u64 run_time = std::chrono::duration <u64, std::nano> (diff).count();
    core.performance_monitor.report_event(PMC_SYS_RUNTIME, run_time);
