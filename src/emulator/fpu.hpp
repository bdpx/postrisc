...
class f16 {
public:
    FLOAT_BASE(f16, 16, 5, u16, _Float16)
    using F_ULONG = u32;
};
...
class f64 {
public:
    FLOAT_BASE(f64, 64, 11, u64, double)
    using F_ULONG = u128;
};
...
class f128 {
public:
    FLOAT_BASE(f128, 128, 15, u128, __float128)
};
...
public:
    ...
    friend std::ostream& operator<<(std::ostream & out, const CSoftFPU& fpu);
...
template<class Archive> void CSoftFPU::serialize(Archive & ar, const unsigned int UNUSED(version))
{
...
enum {
  disp_rounding_mode       = 3,
  disp_flags               = 8,
};

inline u64
CSoftFPU::get_control_word(void) const
{
    u64 word = 0;
...
inline void CSoftFPU::set_control_word(u64 word)
{
...
inline void CSoftFPU::change_control_word(u64 and_mask, u64 or_mask)
{
...
inline std::ostream &
operator << (std::ostream & out, const CSoftFPU& fpu)
{
    return out
...
template<typename FLOAT> inline FLOAT CSoftFPU::pack_sf(u32 a_sign, u32 a_exp, typename FLOAT::F_UINT a_mant)
{
...
template<typename FLOAT> inline FLOAT CSoftFPU::zero_sf(u32 sign)
{
...
template<typename FLOAT> FLOAT CSoftFPU::abs(FLOAT a)
{
    const typename FLOAT::F_UINT tmp = a.v & ~(typename FLOAT::F_UINT{1} << (FLOAT::F_SIZE - 1));
    return FLOAT{tmp};
}

template<typename FLOAT> FLOAT CSoftFPU::chs(FLOAT a)
{
    const typename FLOAT::F_UINT tmp = a.v ^ (typename FLOAT::F_UINT{1} << (FLOAT::F_SIZE - 1));
    return FLOAT{tmp};
}

template<typename FLOAT> FLOAT CSoftFPU::nabs(FLOAT a)
{
    const typename FLOAT::F_UINT tmp = a.v | (typename FLOAT::F_UINT{1} << (FLOAT::F_SIZE - 1));
    return FLOAT{tmp};
}
...
template<typename FLOAT>
FLOAT
CSoftFPU::merge(FLOAT sign, FLOAT exponent, FLOAT mantissa)
{
...
template<typename FLOAT>
FLOAT
CSoftFPU::round_pack_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rm)
{
    LOG_DEBUG(LOG_EVALUATE) << " exp=" << a_exp << " mant=" << fmt::hex(a_mant);
...
template<typename FLOAT>
FLOAT
CSoftFPU::normalize_sf(u32 a_sign, int a_exp, typename FLOAT::F_UINT a_mant, RoundingModeEnum rm)
{
    LOG_DEBUG(LOG_EVALUATE) << " exp=" << a_exp << " mant=" << fmt::hex(a_mant);
...
template<typename FLOAT>
FLOAT
CSoftFPU::normalize2_sf(u32 a_sign, int a_exp,
...
template<typename FLOAT> FLOAT CSoftFPU::add(FLOAT a, FLOAT b)
{
...
template<typename FLOAT> FLOAT CSoftFPU::sub(FLOAT a, FLOAT b)
{
    b.v ^= FLOAT::SIGN_MASK;
    return add(a, b);
}
...
template<> inline u16 mul_u(u16 *plow, u16 a, u16 b)
{
    u32 r = (u32)a * (u32)b;
    *plow = r;
    return r >> 16;
...
template<> inline u32 mul_u(u32 *plow, u32 a, u32 b)
{
    u64 r = (u64)a * (u64)b;
    *plow = r;
    return r >> 32;
...
template<> inline u64 mul_u(u64 *plow, u64 a, u64 b)
{
...
template<> inline u128 mul_u(u128 *plow, u128 a, u128 b)
{
    using F_UINT = u128;
    using F_UHALF = u64;
...
template<typename FLOAT> FLOAT CSoftFPU::mul(FLOAT a, FLOAT b)
{
    auto a_sign = a.get_sign();
    auto b_sign = b.get_sign();
...
template<typename FLOAT>
FLOAT
CSoftFPU::muladd(FLOAT a, FLOAT b, FLOAT c, muladd_negate_t negate_mask)
{
...
template<> inline u16 divrem_u(u16 *pr, u16 ah, u16 al, u16 b)
{
    u32 a = ((u32)ah << 16) | al;
    *pr = a % b;
    return a / b;
...
template<> inline u32 divrem_u(u32 *pr, u32 ah, u32 al, u32 b)
{
    u64 a = ((u64)ah << 32) | al;
    *pr = a % b;
    return a / b;
...
template<> inline u64 divrem_u(u64 *pr, u64 ah, u64 al, u64 b)
{
...
template<typename FLOAT> FLOAT CSoftFPU::div(FLOAT a, FLOAT b)
{
    auto a_sign = a.get_sign();
    auto b_sign = b.get_sign();
    auto r_sign = a_sign ^ b_sign;
...
template<typename FLOAT> FLOAT CSoftFPU::sqrt(FLOAT a)
{
    auto a_sign = a.get_sign();
    auto a_exp = a.get_exp();
    auto a_mant = a.get_fraction();
...
template<typename FLOAT> FLOAT CSoftFPU::rsqrt(FLOAT a)
{
...
template<typename FLOAT> FLOAT CSoftFPU::min(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> FLOAT CSoftFPU::max(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> u32 CSoftFPU::classify(FLOAT a)
{
    auto a_sign = a.get_sign();
    auto a_exp = a.get_exp();
    auto a_mant = a.get_fraction();
...
template<typename FLOAT_DST, typename FLOAT_SRC>
FLOAT_DST
CSoftFPU::cvt_f_f(FLOAT_SRC a, RoundingModeEnum rm)
{
    static_assert(sizeof(FLOAT_SRC) > sizeof(FLOAT_DST));
...
template<typename FLOAT>
FLOAT
CSoftFPU::round_to_int(FLOAT a, RoundingModeEnum rm)
{
    auto a_sign = a.get_sign();
    auto aExp   = a.get_exp();
...
INTEGER
CSoftFPU::cvt_f_i(FLOAT a, RoundingModeEnum rm)
{
...
FLOAT
CSoftFPU::cvt_i_f(INTEGER a, RoundingModeEnum rm)
{
    static_assert(std::numeric_limits<INTEGER>::is_integer);
    static_assert(fp_traits<FLOAT>::is_fp());
...
template<typename FLOAT> bool CSoftFPU::compare_ordered(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        //if (is_signan(a) || is_signan(b)) {
        //    raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_unordered(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        //if (is_signan(a) || is_signan(b)) {
        //    raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_ordered_eq(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_unordered_eq(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_ordered_ne(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_unordered_ne(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_ordered_lt(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_unordered_lt(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_ordered_le(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
template<typename FLOAT> bool CSoftFPU::compare_unordered_le(FLOAT a, FLOAT b)
{
    if (is_nan(a) || is_nan(b)) {
        if (is_signan(a) || is_signan(b)) {
            raise(FFLAG_INVALID);
...
*----------------------------------------------------------------------------*/
template<typename FLOAT> FLOAT CSoftFPU::log2(FLOAT a)
{
    using F_UINT = typename FLOAT::F_UINT;
...
template<typename FLOAT> FLOAT CSoftFPU::scalbn(FLOAT a, int n)
{
    auto a_sign = a.get_sign();
    auto aSig   = a.get_fraction();
    auto aExp   = a.get_exp();
...
template<typename FLOAT> FLOAT CSoftFPU::exp(FLOAT a)
{
    auto aSig = a.get_fraction();
    auto aExp = a.get_exp();
    auto a_sign = a.get_sign();
