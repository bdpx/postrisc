...
{
    size_t const len = std::min<u64>(core.rd().base(), sizeof(CGeneralRegister));
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(aesenc)     { core.ra() = aes::encrypt       ( core.rb(), core.rc() );   return core.FinalizeInstruction(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(aesenclast) { core.ra() = aes::encrypt_last  ( core.rb(), core.rc() );   return core.FinalizeInstruction(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(aesdec)     { core.ra() = aes::decrypt       ( core.rb(), core.rc() );   return core.FinalizeInstruction(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(aesdeclast) { core.ra() = aes::decrypt_last  ( core.rb(), core.rc() );   return core.FinalizeInstruction(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(aesimc)     { core.ra() = aes::aesimc        ( core.rb() );              return core.FinalizeInstruction(); }

DEFINE_INSN_IMPLEMENTATION_NOMEM(aeskeygenassist)
{
