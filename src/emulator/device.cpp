...
CStatus
Device::read(const CDeviceOffset UNUSED(device_offset), size_t UNUSED(len), void *UNUSED(bytes))
{
    return CStatus::device_error;
}

CStatus
Device::write(const CDeviceOffset UNUSED(device_offset), size_t UNUSED(len), const void *UNUSED(bytes))
{
    return CStatus::device_error;
}

CStatus
Device::read_config_raw(const CDeviceOffset device_offset, void const *config, size_t config_size, size_t len, void *bytes)
{
...
CStatus
Device::write_config_raw(const CDeviceOffset device_offset, void *config, size_t config_size, size_t len, void const *bytes)
{
    u64 const offset = device_offset.get_offset();
...
void
Device::dump_config_raw(const CPhysAddress pa, u8 const *config, size_t config_size, const DumpFormatter& out)
{
...
void
Device::dump_page(u64 address, u8 const *base, size_t page_size, const DumpFormatter& out)
{
...
void
PhysMemoryMapping::dump(u8 const *base, size_t device_size, const DumpFormatter& out) const
{
    u64 address = get_address();
    u64 size    = get_size();
    u64 offset  = get_offset();
