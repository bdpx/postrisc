...
class fadd   { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.add(a, b, rm);                     }};
class fsub   { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.sub(a, b, rm);                     }};
class fnadd  { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.chs(fpu.add(a, b, rm));            }};
class fmul   { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.mul(a, b, rm);                     }};
class fdiv   { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.div(a, b, rm);                     }};
class fnmul  { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.chs(fpu.mul(a, b, rm));            }};
class fabs_diff  { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.abs(fpu.sub(a, b, rm));            }};
class fnabs_diff { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return fpu.nabs(fpu.sub(a, b, rm));           }};
...
class fsqrt  { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, fpu::RoundingModeEnum rm) { return fpu.sqrt(a, rm);  }};
class frsqrt { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, fpu::RoundingModeEnum rm) { return fpu.rsqrt(a, rm); }};

class fround   { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, fpu::RoundingModeEnum rm) { return fpu.round_to_int<T>(a, rm); }};
class froundnx { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, fpu::RoundingModeEnum rm) { return fpu.round_to_int<T>(a, rm); }};
...
class fmadd  { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, T c, fpu::RoundingModeEnum rm) { return fpu.muladd(a, b, c, rm, fpu::FLOAT_MULADD_NEGATE_NONE);    }};
class fmsub  { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, T c, fpu::RoundingModeEnum rm) { return fpu.muladd(a, b, c, rm, fpu::FLOAT_MULADD_NEGATE_ADDEND);  }};
class fnmadd { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, T c, fpu::RoundingModeEnum rm) { return fpu.muladd(a, b, c, rm, fpu::FLOAT_MULADD_NEGATE_PRODUCT); }};
class fnmsub { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, T c, fpu::RoundingModeEnum rm) { return fpu.muladd(a, b, c, rm, fpu::FLOAT_MULADD_NEGATE_RESULT);  }};

class fadd_alt { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return { fpu.add(a.re, b.re, rm), fpu.sub(a.im, b.im, rm) };  }};
class fsub_alt { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, fpu::RoundingModeEnum rm) { return { fpu.sub(a.re, b.re, rm), fpu.add(a.im, b.im, rm) };  }};

class fmadd_alt { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, T c, fpu::RoundingModeEnum rm) {
 return { fpu.muladd(a.re, b.re, c.re, rm, fpu::FLOAT_MULADD_NEGATE_NONE),
          fpu.muladd(a.im, b.im, c.im, rm, fpu::FLOAT_MULADD_NEGATE_ADDEND) };   }};

class fmsub_alt { public: template<typename T> static T func(fpu::SoftFPU& fpu, T a, T b, T c, fpu::RoundingModeEnum rm) {
 return { fpu.muladd(a.re, b.re, c.re, rm, fpu::FLOAT_MULADD_NEGATE_ADDEND),
          fpu.muladd(a.im, b.im, c.im, rm, fpu::FLOAT_MULADD_NEGATE_NONE) };   }};
...
#define FP_BASE_SCALAR(NAME, TYPE) \
    ...
    DEFINE_INSN_IMPLEMENTATION_NOMEM(sqrt_          ## NAME) { return core.ScalarOp2RoundingMode < TYPE, TYPE, fsqrt  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(rsqrt_         ## NAME) { return core.ScalarOp2RoundingMode < TYPE, TYPE, frsqrt > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(add_           ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fadd   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(sub_           ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fsub   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nadd_          ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fnadd  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(mul_           ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fmul   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nmul_          ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fnmul  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(div_           ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fdiv   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(abs_diff_      ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fabs_diff > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nabs_diff_     ## NAME) { return core.ScalarOp3RoundingMode < TYPE, TYPE, fnabs_diff> (); } \
    ...
    DEFINE_INSN_IMPLEMENTATION_NOMEM(madd_          ## NAME) { return core.ScalarOp4RoundingMode < TYPE, TYPE, fmadd  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(msub_          ## NAME) { return core.ScalarOp4RoundingMode < TYPE, TYPE, fmsub  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nmadd_         ## NAME) { return core.ScalarOp4RoundingMode < TYPE, TYPE, fnmadd > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nmsub_         ## NAME) { return core.ScalarOp4RoundingMode < TYPE, TYPE, fnmsub > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(round_         ## NAME) { return core.ScalarOp2RoundingMode < TYPE, TYPE, fround > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(roundnx_       ## NAME) { return core.ScalarOp2RoundingMode < TYPE, TYPE, froundnx > (); } \
    ...
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_i32_       ## NAME) { return core.ScalarOp2RoundingMode < TYPE,  i32, cvt_i_f> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_u32_       ## NAME) { return core.ScalarOp2RoundingMode < TYPE,  u32, cvt_i_f> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_i64_       ## NAME) { return core.ScalarOp2RoundingMode < TYPE,  i64, cvt_i_f> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_u64_       ## NAME) { return core.ScalarOp2RoundingMode < TYPE,  u64, cvt_i_f> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_i128_      ## NAME) { return core.ScalarOp2RoundingMode < TYPE, i128, cvt_i_f> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_u128_      ## NAME) { return core.ScalarOp2RoundingMode < TYPE, u128, cvt_i_f> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME ## _i32 ) { return core.ScalarOp2RoundingMode <  i32, TYPE, cvt_f_i> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME ## _u32 ) { return core.ScalarOp2RoundingMode <  u32, TYPE, cvt_f_i> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME ## _i64 ) { return core.ScalarOp2RoundingMode <  i64, TYPE, cvt_f_i> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME ## _u64 ) { return core.ScalarOp2RoundingMode <  u64, TYPE, cvt_f_i> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME ## _i128) { return core.ScalarOp2RoundingMode < i128, TYPE, cvt_f_i> (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME ## _u128) { return core.ScalarOp2RoundingMode < u128, TYPE, cvt_f_i> (); } \
...
#define FP_BASE_VECTOR(NAME, INAME, TYPE, INTTYPE, UINTTYPE) \
    ...
    DEFINE_INSN_IMPLEMENTATION_NOMEM(sqrt_          ## NAME) { return core.VectorOp2RoundingMode < TYPE, TYPE, fsqrt  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(rsqrt_         ## NAME) { return core.VectorOp2RoundingMode < TYPE, TYPE, frsqrt > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(add_           ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fadd   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(sub_           ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fsub   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nadd_          ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fnadd  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(mul_           ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fmul   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(div_           ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fdiv   > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nmul_          ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fnmul  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(abs_diff_      ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fabs_diff > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nabs_diff_     ## NAME) { return core.VectorOp3RoundingMode < TYPE, TYPE, fnabs_diff> (); } \
    ...
    DEFINE_INSN_IMPLEMENTATION_NOMEM(madd_          ## NAME) { return core.VectorOp4RoundingMode < TYPE, TYPE, fmadd  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(msub_          ## NAME) { return core.VectorOp4RoundingMode < TYPE, TYPE, fmsub  > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nmadd_         ## NAME) { return core.VectorOp4RoundingMode < TYPE, TYPE, fnmadd > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(nmsub_         ## NAME) { return core.VectorOp4RoundingMode < TYPE, TYPE, fnmsub > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(madd_alt_      ## NAME) { return core.VectorOp4RoundingMode < complex<TYPE>, complex<TYPE>, fmadd_alt > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(msub_alt_      ## NAME) { return core.VectorOp4RoundingMode < complex<TYPE>, complex<TYPE>, fmsub_alt > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(add_alt_       ## NAME) { return core.VectorOp3RoundingMode < complex<TYPE>, complex<TYPE>, fadd_alt > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(sub_alt_       ## NAME) { return core.VectorOp3RoundingMode < complex<TYPE>, complex<TYPE>, fsub_alt > (); } \
    ...
    DEFINE_INSN_IMPLEMENTATION_NOMEM(round_         ## NAME) { return core.VectorOp2RoundingMode < TYPE, TYPE, fround > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(roundnx_       ## NAME) { return core.VectorOp2RoundingMode < TYPE, TYPE, froundnx > (); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME  ## _vu ## INAME) { return core.VectorOp2RoundingMode<UINTTYPE, TYPE, cvt_f_i>(); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_  ## NAME  ## _vi ## INAME) { return core.VectorOp2RoundingMode< INTTYPE, TYPE, cvt_f_i>(); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_vi ## INAME ## _   ## NAME) { return core.VectorOp2RoundingMode<TYPE,  INTTYPE, cvt_i_f>(); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_vu ## INAME ## _   ## NAME) { return core.VectorOp2RoundingMode<TYPE, UINTTYPE, cvt_i_f>(); } \
...
* all-to-all
***************************************************************************/
DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_f32_f16)  { return core.ScalarOp2RoundingMode<fpu::f16, fpu::f32,  cvt_f_f>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_f64_f16)  { return core.ScalarOp2RoundingMode<fpu::f16, fpu::f64,  cvt_f_f>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_f128_f16) { return core.ScalarOp2RoundingMode<fpu::f16, fpu::f128, cvt_f_f>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_f64_f32)  { return core.ScalarOp2RoundingMode<fpu::f32, fpu::f64,  cvt_f_f>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_f128_f32) { return core.ScalarOp2RoundingMode<fpu::f32, fpu::f128, cvt_f_f>(); }
DEFINE_INSN_IMPLEMENTATION_NOMEM(cvt_f128_f64) { return core.ScalarOp2RoundingMode<fpu::f64, fpu::f128, cvt_f_f>(); }
