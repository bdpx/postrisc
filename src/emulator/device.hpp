...
public:
    ...
    explicit CDeviceOffset(u64 base, u64 offset) : m_offset(base + offset) {}
...
public:
    ...
    virtual CStatus read_u8(const CDeviceOffset address, u8& value) = 0;
    virtual CStatus read_u16(const CDeviceOffset address, u16& value) = 0;
    virtual CStatus read_u32(const CDeviceOffset address, u32& value) = 0;
    virtual CStatus read_u64(const CDeviceOffset address, u64& value) = 0;
    virtual CStatus read_u128(const CDeviceOffset address, u128& value) = 0;
    virtual CStatus read(const CDeviceOffset address, size_t len, void *bytes) = 0;
    // normal write
    ...
    virtual CStatus read_config(const CDeviceOffset address, size_t len, void *bytes) = 0;
...
public:
    ...
    static void dump_page(u64 address, u8 const *base, size_t page_size, const DumpFormatter& out);
...
protected:
    static void dump_config_raw(const CPhysAddress pa,
        u8 const *config, size_t config_size, const DumpFormatter& out);
    CStatus read_config_raw(const CDeviceOffset address,
        void const *config, size_t config_size, size_t len, void *bytes);
    CStatus write_config_raw(const CDeviceOffset address,
        void *config, size_t config_size, size_t len, void const *bytes);

private:
    u64                m_reads_count;
    u64                m_writes_count;
    u64                m_complex_ops_count;
    u64                m_config_reads_count;
    u64                m_config_writes_count;
    std::string        m_name;
};
...
#define STANDARD_DEVICE_INTERFACE                                                                     \
    CStatus read_u8(const CDeviceOffset address, u8& value) override;                                 \
    CStatus read_u16(const CDeviceOffset address, u16& value) override;                               \
    CStatus read_u32(const CDeviceOffset address, u32& value) override;                               \
    CStatus read_u64(const CDeviceOffset address, u64& value) override;                               \
    CStatus read_u128(const CDeviceOffset address, u128& value) override;                             \
    CStatus read(const CDeviceOffset address, size_t len, void *bytes) override;                      \
    ...
    CStatus read_config(const CDeviceOffset addr, size_t len, void *bytes) override;                  \
...
public:
    PhysMemoryMapping(u64 address, u64 size, u64 offset,
                      size_t page_size, boost::intrusive_ptr<Device> device)
        : m_address(address), m_size(size), m_offset(offset)
        , m_page_size(page_size), m_device(device) {}

    // simple variant with monolith one-to-one mapping
    PhysMemoryMapping(u64 address,
                      size_t page_size, boost::intrusive_ptr<Device> device)
        : m_address(address), m_size(device->size()), m_offset(0)
        , m_page_size(page_size), m_device(device) {}
    ...
    void dump(u8 const *base, size_t device_size, const DumpFormatter& out) const;
