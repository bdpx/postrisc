...
public:
    CAtomicCompareAndSwap(T expected, T desired, std::memory_order success_order)
        : m_expected(expected)
        , m_desired(desired)
...
template<class T>
CStatus
Core::compare_and_swap(IAddressSpace& as, std::memory_order order)
{
    static_assert(sizeof(std::atomic<T>) == sizeof(T));
    ...
    CAtomicCompareAndSwap op(expected, desired, order);
    CHECK_STATUS_ADDRESS(*this, address,
        as.mmu_complex_operation(*this, address, sizeof(T),
...
public:
    CAtomicFetchAndOp(T value, std::memory_order success_order)
        : m_value(value)
        , m_success_order(success_order)
    {}
...
template<class T, class OP>
CStatus
Core::fetch_and_op_atomic(IAddressSpace& as, std::memory_order order)
{
    static_assert(sizeof(std::atomic<T>) == sizeof(T));
    ...
    CAtomicFetchAndOp<T,OP> op(value, order);
    CHECK_STATUS_ADDRESS(*this, address,
        as.mmu_complex_operation(*this, address, sizeof(T),
            VM_PAGE_RIGHT_READ | VM_PAGE_RIGHT_WRITE,
            VM_PAGE_RIGHT_EXECUTE | VM_PAGE_RIGHT_FINAL, op));
