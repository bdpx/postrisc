...
typedef union _tlb_ppn_t {
    ...
    u64 get_ppa(void) const { return address & ~util::makemask(bits_per_minpageoffset); }
} tlb_ppn_t;

typedef union _tlb_vpn_t {
    u64    address;
    struct {
        u64    ps    : bits_per_shamt;    // pagesize
...
class CTranslateBuffer {
    public:
        ...
        CStatus translate_address(u64 address, unsigned int rights, unsigned int level) const;
        CStatus insert_translate_buffer(tlb_ppn_t gpr, u64 ifa);
};
