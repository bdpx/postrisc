...
public:
    CRegisterNameFormatter(unsigned regnum_)
        : regnum(regnum_) {}
...
template<class T>
void
OutputScalar(std::ostream& out, const T b)
{
    if constexpr (std::numeric_limits<T>::is_integer && std::numeric_limits<T>::is_signed)
       out << std::showpos;
...
// %%, %reg.fmt %r23.vi1 %ip
CStatus
Core::OutputStringFormatting(IAddressSpace& as, std::ostream& stream, std::istream& input)
{
    // %%
    char chr;
    if (!input.get(chr)) {
    ...
    if (flags & FORMAT_COMMAND) {
        if (reg_name == "dump") {
            DumpFormatterTxt formatter(as.get_decoder(), stream);
    ...
    LOG_DEBUG(LOG_EMULATOR) << CRegisterNameFormatter(regnum);
    ...
    if (flags & FORMAT_FLOAT) {
        if (flags & FORMAT_VECTOR) {
          ...
          switch (size) {
              case 32:   OutputScalar(stream, a.template scalar<float>());      break;
              case 64:   OutputScalar(stream, a.template scalar<double>());     break;
              case 128:  OutputScalar(stream, a.template scalar<quadruple>());  break;
              default:
...
***************************************************************************/
CStatus
Core::OutputString(IAddressSpace& as, u64 address)
{
    ...
    for (;;) {
        u8 chr;
        const CVirtAddress va(address);
        CHECK_STATUS_ADDRESS(*this, va, as.mmu_read(*this, va, &chr));
        ++address;
        if (chr == '\0')
            break;
    ...
    while (input.get(chr)) {
        if (chr == '%') {
            const CVirtAddress va(address);
            CHECK_STATUS_ADDRESS(*this, va, OutputStringFormatting(as, stream, input));
        } else {
            stream << chr << fmt::flush;
        }
    }
