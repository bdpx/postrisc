...
void EmulatorConfiguration::SetDefaults(void)
{
    ...
    base_address = UINT64_C(0xffffffffffa00000);

    core_devices.emplace_back();
    core_devices.back().name = "core:0";

    memory_devices.emplace_back();
    memory_devices.back().size = 0x400000;
    memory_devices.back().mappings.emplace_back();
    memory_devices.back().mappings.back().address = UINT64_C(0x0000065432100000);
    memory_devices.back().mappings.back().size    = 0x400000;
    memory_devices.back().mappings.back().offset  = 0x0;

    rom_devices.emplace_back();
    rom_devices.back().phys_address = UINT64_C(0x0000000fcc000000);
    rom_devices.back().virt_address = UINT64_C(0xffffffffcc000000);
    rom_devices.back().data = {{ 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18 }};

    video_device.phys_address = UINT64_C(0x0000000fb0000000);
    video_device.virt_address = UINT64_C(0xffffffffb0000000);
    video_device.width = 240;
    video_device.height = 160;

    device_array_config.phys_address = UINT64_C(0x0000000fee000000);
    device_array_config.virt_address = UINT64_C(0xffffffffee000000);
...
#undef DEFINE_PERFORMANCE_MONITOR_DEF
    ...
    assembler.RegisterDefine("IPI_VIRT_BASE", device_array_config.virt_address);
    assembler.RegisterDefine("DEVICE_CONFIG_SPACE_SIZE", device_array_config.max_config_space_size);

    if (!rom_devices.empty()) {
        assembler.RegisterDefine("ROM_VIRT_BASE", rom_devices.back().virt_address);
    }
    ...
    assembler.RegisterDefine("VIDEO_VIRT_BASE", video_device.virt_address);
    assembler.RegisterDefine("VIDEO_COMMAND_VIRT_BASE",
        device_array_config.virt_address + id * device_array_config.max_config_space_size);
...
#define CHECK_ADDRESS_ALIGNMENT(ADDRESS, ALIGNMENT)                          \
     if (util::align_down<uint64_t>((ADDRESS), ALIGNMENT) != (ADDRESS)) {    \
...
bool EmulatorConfiguration::parse(char const *filename)
{
    ...
    LOG_DEBUG(LOG_PLATFORM) << "bitsPerPageOffset=" << bitsPerPageOffset << std::endl;
    if (bitsPerPageOffset < 12 || bitsPerPageOffset > 16) {
        std::cerr << "page_offset_bits should be between 12 and 16 bits" << std::endl;
        return false;
    }
    uint64_t const page_size = UINT64_C(1) << bitsPerPageOffset;

    numberOfPagingLevels = system.get<unsigned>("page_table_levels");
    LOG_DEBUG(LOG_PLATFORM) << "numberOfPagingLevels=" << numberOfPagingLevels << std::endl;
    ...
    TranslationUnit mmu;
    mmu.SetPagingParameters(0, bitsPerPageOffset, numberOfPagingLevels);

    base_address = config.get<util::hexadecimal<uint64_t>>("loader.base_address");
    LOG_DEBUG(LOG_PLATFORM) << "base_address=0x" << fmt::hex(base_address) << std::endl;
    ...
    for (const auto& device : config.get_child("devices")) {
        LOG_DEBUG(LOG_PLATFORM) << device.first << std::endl;
        ...
        if (device.first == "core") {
            ...
            uint64_t const memory_size = device.second.get<util::hexadecimal<uint64_t>>("size");
            LOG_DEBUG(LOG_PLATFORM) << "size=" << memory_size << std::endl;
            memory_devices.emplace_back();
            auto& memory_device = memory_devices.back();
            memory_device.size = memory_size;
            for (const auto& param : device.second) {
                if (param.first == "mapping") {
                    auto const& mapping = param.second;
                    uint64_t const mapping_address = mapping.get<util::hexadecimal<uint64_t>>("address");
                    LOG_DEBUG(LOG_PLATFORM) << fmt::hex(mapping_address) << std::endl;
                    CHECK_ADDRESS_BITS(mapping_address, bits_per_physical_address);
                    CHECK_ADDRESS_ALIGNMENT(mapping_address, page_size);
                    uint64_t const mapping_size = mapping.get<util::hexadecimal<uint64_t>>("size");
                    LOG_DEBUG(LOG_PLATFORM) << fmt::hex(mapping_size) << std::endl;
                    uint64_t const mapping_offset = mapping.get<util::hexadecimal<uint64_t>>("offset");
                    LOG_DEBUG(LOG_PLATFORM) << fmt::hex(mapping_offset) << std::endl;
                    CHECK_ADDRESS_BITS(mapping_address + mapping_offset - 1, bits_per_physical_address);
                    memory_device.mappings.emplace_back();
                    auto& device_mapping = memory_device.mappings.back();
                    device_mapping.address = mapping_address;
                    device_mapping.size    = mapping_size;
                    device_mapping.offset  = mapping_offset;
                }
            }
            ...
            uint64_t const phys_address = device.second.get<util::hexadecimal<uint64_t>>("phys_address");
            rom_device.phys_address = phys_address;
            CHECK_ADDRESS_BITS(phys_address, bits_per_physical_address);
            CHECK_ADDRESS_ALIGNMENT(phys_address, page_size);

            uint64_t const virt_address = device.second.get<util::hexadecimal<uint64_t>>("virt_address");
            rom_device.virt_address = virt_address;
            ...
            uint64_t const phys_address = device.second.get<util::hexadecimal<uint64_t>>("phys_address");
            video_device.phys_address = phys_address;
            LOG_DEBUG(LOG_PLATFORM) << "video.phys_address " << fmt::hex(phys_address);
            CHECK_ADDRESS_BITS(phys_address, bits_per_physical_address);
            CHECK_ADDRESS_ALIGNMENT(phys_address, page_size);

            uint64_t const virt_address = device.second.get<util::hexadecimal<uint64_t>>("virt_address");
            video_device.virt_address = virt_address;
            ...
            uint64_t const cs_size = device.second.get<util::hexadecimal<uint64_t>>("max_config_space_size");
            ...
            uint64_t const phys_address = device.second.get<util::hexadecimal<uint64_t>>("phys_address");
            device_array_config.phys_address = phys_address;
            CHECK_ADDRESS_BITS(device_array_config.phys_address, bits_per_physical_address);
            CHECK_ADDRESS_ALIGNMENT(device_array_config.phys_address, std::max(page_size, cs_size));

            uint64_t const virt_address = device.second.get<util::hexadecimal<uint64_t>>("virt_address");
            device_array_config.virt_address = virt_address;
            CHECK_CANONICAL_ADDRESS(virt_address);
