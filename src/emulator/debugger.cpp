...
{
    PhysMemoryMapping mapping(0, 0, 0, core);
    DumpFormatterTxt formatter(std::cerr);
...
{
    LOG_TRACE(LOG_DEBUGGER) << "single step start" << fmt::endl;
    ...
    LOG_TRACE(LOG_DEBUGGER) << "single step end: " << status << fmt::endl; 
    return true;
}
