...
union alignas(16) CGeneralRegister {
    ...
    vector_array<float>      m_float;
    vector_array<double>     m_double;
    ...
    constexpr explicit CGeneralRegister(
        u32 v3, u32 v2, u32 v1, u32 v0)
        : m_u32{v0,v1,v2,v3} {}
    constexpr explicit CGeneralRegister(
        u16 v7, u16 v6, u16 v5, u16 v4,
        u16 v3, u16 v2, u16 v1, u16 v0)
        : m_u16{v0,v1,v2,v3,v4,v5,v6,v7} {}

    constexpr explicit CGeneralRegister(
        u8 v15, u8 v14, u8 v13, u8 v12,
        u8 v11, u8 v10, u8 v9, u8 v8,
        u8 v7, u8 v6, u8 v5, u8 v4,
        u8 v3, u8 v2, u8 v1, u8 v0)
...
template<> ALWAYS_INLINE      float & CGeneralRegister::vector<float>     (size_t index) { return m_float[index]; }
template<> ALWAYS_INLINE     double & CGeneralRegister::vector<double>    (size_t index) { return m_double[index]; }
...
template<> ALWAYS_INLINE  quadruple & CGeneralRegister::vector<quadruple> (size_t ) { return m_quadruple; }
...
template<> ALWAYS_INLINE      float const & CGeneralRegister::vector<float>     (size_t index) const { return m_float[index]; }
template<> ALWAYS_INLINE     double const & CGeneralRegister::vector<double>    (size_t index) const { return m_double[index]; }
...
template<> ALWAYS_INLINE  quadruple const & CGeneralRegister::vector<quadruple> (size_t ) const { return m_quadruple; }

inline CGeneralRegister
funnel_shift_right(CGeneralRegister hi, CGeneralRegister lo, unsigned int shamt)
{
    u64  a0, a1;
    ...
    if (shamt < 64) {
       rshamt = 64 - shamt;
       a1 = (lo.m_u64[0] >> shamt) | (lo.m_u64[1] << rshamt);
       a0 = (lo.m_u64[0] >> shamt) | (hi.m_u64[1] << rshamt);
    } else if (shamt > 64) {
       rshamt = 128 - shamt;
       lshamt = shamt - 64;
       a1 = (lo.m_u64[0] >> lshamt) | (hi.m_u64[1] << rshamt);
       a0 = (hi.m_u64[0] >> lshamt) | (hi.m_u64[1] << rshamt);
    } else {
       assert(shamt == 64);
       a1 = lo.m_u64[1];
       a0 = hi.m_u64[0];
