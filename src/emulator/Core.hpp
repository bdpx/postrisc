...
public:
    ...
    Result fetch_code(const AddressSpace &as, const PhysAddress pa, size_t len, void *data);
...
private:
    ...
    template<typename CMD> void report_fault(AddressSpace& as, const CMD instruction, Result result) const;
    ...
    Result GetRoundingMode(const RoundingMode rmc, fpu::RoundingModeEnum& rm) const;
    Result GetMemoryOrdering(const MemoryOrdering cmd_mo, std::memory_order& mo) const;
    ...
    // atomic templates
    template<typename T> Result atomic_load(AddressSpace& as);
    template<typename T> Result atomic_store(AddressSpace& as);
    template<typename T, typename STORE_T, typename OP> Result fetch_and_op_atomic(AddressSpace& as);
    template<typename T> Result compare_and_swap(AddressSpace& as);
    ...
    template<typename T, typename OP> Result VectorTernaryRegRegReg(void);
    template<typename T, typename OP> Result VectorBinaryRegShamt(void);
    template<typename T, typename OP> Result VectorTernaryRegShamt(void);
    ...
    template<typename D, typename T, typename OP> Result ScalarOp3RoundingMode(void);
    template<typename D, typename T, typename OP> Result ScalarOp4(void);
    template<typename D, typename T, typename OP> Result ScalarOp4RoundingMode(void);
    ...
    template<typename D, typename T, typename OP> Result VectorOp3RoundingMode(void);
    template<typename D, typename T, typename OP> Result VectorOp3Horizontal(void);
    template<typename D> Result VectorSplat(void);
    template<typename D> Result VectorSplatImmediate(void);
    ...
    template<typename D, typename T, typename OP> Result VectorOp4RoundingMode(void);

    template<typename T> Result crc32c(void);

    template<typename CMD> using instr_fmt_handler = Result (*)(Core& core, AddressSpace& as, EInstructionID insn, const CMD cmd);

#define INSTRUCTION_FORMAT_PROTO_X(name, extname, descr, format, txt) \
    template<typename CMD> static Result instr_fmt_handler_##name(Core& core, AddressSpace& as, EInstructionID insn, const CMD cmd);

    DECLARE_INSTRUCTION_FORMATS(INSTRUCTION_FORMAT_PROTO_X)

#define DEFINE_INSN_FMT_IMPLEMENTATION(name) \
    template<typename CMD> Result Core::instr_fmt_handler_##name(Core& core, AddressSpace& as, EInstructionID insn, const CMD cmd)
...
#define X(name, code, subset, format, descr) \
    static Result insn_##name(Core& core, AddressSpace& as);
...
#define ROTATE_REG(core, DST, TEMP, REGSOURCE)                      \
    ...
    if (likely(is_rotate_reg(TEMP))) {                              \
        if (unlikely((core).rsc.IsOutOfFrame(TEMP))) {              \
            return (core).trigger_register_fault(TEMP);             \
        }                                                           \
...
ALWAYS_INLINE
Result
Core::fetch_code(const AddressSpace &as, const PhysAddress pa, size_t len, void *data)
{
