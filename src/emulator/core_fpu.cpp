...
template<class T> ALWAYS_INLINE CStatus
Core::PackedMulAddAlternate(fpu::muladd_negate_t even_sig, fpu::muladd_negate_t odd_sig)
{
    ...
    for (size_t i = 0; i < CGeneralRegister::size<T>(); i += 2) {
        a.template vector<T>(i+0) =
        fpu.muladd(b.template vector<T>(i+0),
                   c.template vector<T>(i+0),
                   d.template vector<T>(i+0), even_sig);
        a.template vector<T>(i+1) =
        fpu.muladd(b.template vector<T>(i+1),
                   c.template vector<T>(i+1),
                   d.template vector<T>(i+1), odd_sig);
    }
    return FpuFinalizeInstruction();
}

template<class T> ALWAYS_INLINE CStatus Core::PackedAddSubAlternate(void)
{
    CGeneralRegister& a = ra();
    CGeneralRegister const & b = rb();
    CGeneralRegister const & c = rc();
    ...
    for (size_t i = 0; i < CGeneralRegister::size<T>(); i += 2) {
        a.template vector<T>(i+0) = fpu.add(b.template vector<T>(i+0), c.template vector<T>(i+0));
        a.template vector<T>(i+1) = fpu.sub(b.template vector<T>(i+1), c.template vector<T>(i+1));
    }
    return FpuFinalizeInstruction();
}

template<class T> ALWAYS_INLINE CStatus Core::PackedSubAddAlternate(void)
{
    CGeneralRegister& a = ra();
    CGeneralRegister const & b = rb();
    CGeneralRegister const & c = rc();
    ...
    for (size_t i = 0; i < CGeneralRegister::size<T>(); i += 2) {
        a.template vector<T>(i+0) = fpu.sub(b.template vector<T>(i+0), c.template vector<T>(i+0));
        a.template vector<T>(i+1) = fpu.add(b.template vector<T>(i+1), c.template vector<T>(i+1));
    }
    return FpuFinalizeInstruction();
}
...
#define FP_BASE_VECTOR(NAME, INAME, TYPE, INTTYPE, UINTTYPE) \
    ...
    DEFINE_INSN_IMPLEMENTATION_NOMEM(fmadda  ## NAME               ) { return core.PackedMulAddAlternate<TYPE>(fpu::FLOAT_MULADD_NEGATE_NONE, fpu::FLOAT_MULADD_NEGATE_ADDEND); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(fmsuba  ## NAME               ) { return core.PackedMulAddAlternate<TYPE>(fpu::FLOAT_MULADD_NEGATE_ADDEND, fpu::FLOAT_MULADD_NEGATE_NONE); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(faddc   ## NAME               ) { return core.PackedAddSubAlternate<TYPE>(); } \
    DEFINE_INSN_IMPLEMENTATION_NOMEM(fsubc   ## NAME               ) { return core.PackedSubAddAlternate<TYPE>(); } \
