...
#define INTERRUPTION_VECTORS_ENUM_X(name) name,
    ...
    CStatus(void) {}
    constexpr CStatus(CEventEnum code) : m_Code(code) {}

    // Allow conversion to enum
    constexpr operator CEventEnum() const { return m_Code; }
    // Prevent usage as: if(status)
    explicit operator bool() const = delete;

    static CStatus ConvertFromVector(unsigned vector) { return CStatus(static_cast<CEventEnum>(vector)); }
