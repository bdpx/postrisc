...
public:
    ...
    PhysAddress align_down(u64 size) const {
        return PhysAddress(util::align_down<u64>(m_address, size));
    }
...
public:
    ...
    u64 get_address(void) const { return m_address; }
    VirtAddress align_down(u64 size) const {
        return VirtAddress(util::align_down<u64>(m_address, size));
    }
private:
    u64 m_address;
};
