...
public:
    explicit CTranslationEntry(u64 _data) : data(_data) {}
    explicit CTranslationEntry(CPhysAddress pa) : data(pa.get_address()) {}
    ALWAYS_INLINE CPhysAddress GetAddress(void) const {
        const u64 address = (data & VM_PAGE_PHYS_ADDRESS_MASK);
        return CPhysAddress(address);
    }
...
public:
    ...
    CStatus SetConfig(u64 value);

    void SetPagingParameters(CPhysAddress pta,
    ...
    CPhysAddress GetPageTableRootAddress(void) const;
    ...
    CPhysAddress get_next_te_pa(u64 vaddress, CPhysAddress pa, int level) const;
    ...
    TranslateAddress(const IAddressSpace &as, CPerformanceMonitor &performance_monitor,
...
private:
    ...
    template<class T> CCoreTranslationCache& ChooseTranslationCache(void);

private:
    CPhysAddress              m_RootPageAddress = CPhysAddress(0);
...
template<> ALWAYS_INLINE CCoreTranslationCache&
TranslationUnit::ChooseTranslationCache<CInstructionHelper>(void) { return m_CodeTranslationCache; }
template<> ALWAYS_INLINE CCoreTranslationCache&
TranslationUnit::ChooseTranslationCache<CDataHelper>(void) { return m_DataTranslationCache; }
template<> ALWAYS_INLINE CCoreTranslationCache&
TranslationUnit::ChooseTranslationCache<CBackstoreHelper>(void) { return m_DataTranslationCache; }
...
ALWAYS_INLINE CPhysAddress TranslationUnit::GetPageTableRootAddress(void) const
{
...
ALWAYS_INLINE CPhysAddress
TranslationUnit::get_next_te_pa(u64 virt_address, CPhysAddress pa, int level) const
{
     ...
     return CPhysAddress(curpta);
}

template<class helper>
ALWAYS_INLINE
std::tuple<CStatus, CPhysAddress>
TranslationUnit::TranslateAddress(const IAddressSpace &as, CPerformanceMonitor &performance_monitor,
    const CVirtAddress va, u64 requested_rights, u64 unexpected_rights)
{
    if (unlikely(is_real_mode())) {
        LOG_DEBUG(LOG_PAGETABLE) << "va==pa: " << va;
        return { CStatus::continue_execution, CPhysAddress(va.get_address()) };
    }
    ...
    if (likely(tce.virt_address == vpn)) {
        ...
        for (int level = GetNumberOfPagingLevels() - 1; level >= 0; --level) {
            CPhysAddress const next_te_pa = get_next_te_pa(virt_address, CPhysAddress(phys_address), level);
            u64 next_pta;
            CStatus status = as.bus_read(next_te_pa, sizeof(next_pta), &next_pta);
            if (unlikely(!status.success())) return { status, CPhysAddress(0) };
            ...
            if (unlikely(!CTranslationEntry(next_pta).is_valid())) {
                ...
                return { helper::page_not_present(), CPhysAddress(0) };
            }
    ...
    return { CStatus::continue_execution, CPhysAddress(phys_address) };
}
