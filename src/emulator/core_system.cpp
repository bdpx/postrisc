...
DEFINE_INSN_IMPLEMENTATION(ehthrow)
{
    ...
    CHECK_STATUS(core.exception_state.RaiseException(address));
    return core.ExecuteJump(as, core.exception_state.GetEIP());
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(ehcatch)
{
...
DEFINE_INSN_IMPLEMENTATION(ehnext)
{
    ...
    if (context && old) {
        ...
        return CStatus::software_double_exception;
    }
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(mfspr)
{
    u64 value = 0;
    CHECK_STATUS(core.GetSpecialRegister(core.cmd.rb, value));
    core.ra() = value;
    return core.FinalizeInstruction();
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(mtspr)
{
    return core.SetSpecialRegister(core.cmd.rb, core.ra().base());
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(sprsr)
{
    CHECK_KERNEL_MODE(core);
    const unsigned sprnum = core.cmd.rb;
    u64 state;
    CHECK_STATUS(core.GetSpecialRegister(sprnum, state));
    u64 new_state = state;
    new_state |= core.rc().base();
    new_state &= ~core.rd().base();
    CHECK_STATUS(core.SetSpecialRegister(sprnum, new_state));
    core.ra() = state;
    return core.FinalizeInstruction();
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(mfmr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
    CHECK_STATUS(core.performance_monitor.Get(index, core.ra().scalar<u64>()));
    return core.FinalizeInstruction();
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(mtmr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
    CHECK_STATUS(core.performance_monitor.Set(core.ra().scalar<u64>(), index));
    return core.FinalizeInstruction();
}

DEFINE_INSN_IMPLEMENTATION_NOMEM(mfibr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(mtibr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(mfdbr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(mtdbr)
{
    u64 const index = core.rb().base() + core.cmd.imm;
    LOG_DEBUG(LOG_EMULATOR) << "index=" << index;
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(mtitr)
{
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(mtdtr)
{
    u64 const index = core.rb().base();
    LOG_DEBUG(LOG_EMULATOR)
        << "index=" << index << " a=" << core.ra() << " c=" << core.rc();
...
DEFINE_INSN_IMPLEMENTATION(rfi)
{
    ...
    CHECK_STATUS(core.ExecuteJump(as, RoundToBundle(core.iip)));
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(ptc)
{
    ...
    LOG_DEBUG(LOG_EMULATOR) << "address=" << fmt::hex(address) << " asid=" << asid;
    return core.unimplemented();
}

class CTimePeeker {
public:
    CTimePeeker(CPerformanceMonitor& pm_) : pm(pm_), start(std::chrono::steady_clock::now()) {}
    ~CTimePeeker(void) {
...
{
    // CTimePeeker time_keeper(core.performance_monitor);
...
DEFINE_INSN_IMPLEMENTATION(tpa)
{
    ...
    CHECK_KERNEL_MODE(core);
    const CVirtAddress va(core.rb().base());
    auto [status, pa] = as.mmu_translate(core, va);
    CHECK_STATUS_ADDRESS(core, va, status);
...
DEFINE_INSN_IMPLEMENTATION(int)
{
    ...
    u8 const int_vector = value & util::makemask(bits_per_vector_num);
    switch (int_vector) {
        case CStatus::instruction_debug:
            Debugger(as, &core).execute();
            break;
        default:
            CHECK_STATUS(core.post_interrupt(CStatus::ConvertFromVector(int_vector)));
            break;
    }
    return core.FinalizeInstruction();
}
...
DEFINE_INSN_IMPLEMENTATION_NOMEM(halt)
{
    ...
    core.SetupDebugPlace(core.ip - sizeof(CBundle), core.psr.GetShadowSize());
