...
static const std::array<EInstructionFormat, INSTRUCTION_LIST_SIZE> instr_format = {
#define X(name, code, subset, format, descr) inst_fmt_##format,
    DECLARE_INSN_TABLE(X)
#undef X
};

static const std::array<util::default_initializer<
    Result (*)(Core& core, AddressSpace& as), &Core::insn_illegal>, INSTRUCTION_LIST_SIZE> instr_table = {
...
#define DO_INSN return instr_table[insn](core, as)
...
DEFINE_INSN_FMT_IMPLEMENTATION(memrel)
{
    STATIC_ROTATE_REG(a)
...
DEFINE_INSN_FMT_IMPLEMENTATION(r3scale)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
...
DEFINE_INSN_FMT_IMPLEMENTATION(r2rm)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    CHECK_RESULT(core.GetRoundingMode(cmd.GetRoundingMode(), core.cmd.rm));
    DO_INSN;
}

DEFINE_INSN_FMT_IMPLEMENTATION(r3rm)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
...
DEFINE_INSN_FMT_IMPLEMENTATION(r4rm)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
...
{
    CHECK_RESULT(core.GetMemoryOrdering(cmd.GetMemoryOrdering(), core.cmd.memory_order));
    DO_INSN;
}

DEFINE_INSN_FMT_IMPLEMENTATION(r2mo)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    CHECK_RESULT(core.GetMemoryOrdering(cmd.GetMemoryOrdering(), core.cmd.memory_order));
...
DEFINE_INSN_FMT_IMPLEMENTATION(r3mo)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
...
DEFINE_INSN_FMT_IMPLEMENTATION(r4mo)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    STATIC_ROTATE_REG(c)
...
DEFINE_INSN_FMT_IMPLEMENTATION(fpclass)
{
    STATIC_ROTATE_REG(a)
    STATIC_ROTATE_REG(b)
    core.cmd.imm = cmd.GetFpClassifyMask();
    DO_INSN;
}
...
void Core::report_fault(AddressSpace& as, const CMD instruction, Result result) const
{
    ...
    LOG_ERROR(LOG_EMULATOR) << "exception " << result //LOG_ERROR(LOG_EMULATOR)
       ...
       << fmt::endl
       << " cmd=" << instruction
       << ": " << InstructionDisassembler<CMD>(as.get_decoder(), instruction, ip)
       << fmt::endl
       << " psr=" << psr
       << fmt::endl;
    ...
    LOG_OUTPUT(LOG_LOADER) << result
        ...
        << " psr=" << psr
        << " cmd=" << instruction
        << ": " << InstructionDisassembler<CMD>(as.get_decoder(), instruction, ip) << std::endl;
...
Core::ExecuteInstruction(AddressSpace& as, const CMD instruction)
{
    static const instr_fmt_handler<CMD> inst_fmt_table[inst_fmt_last] = {
#define INSTRUCTION_FORMAT_HANDLER_X(name, extname, descr, format, txt) instr_fmt_handler_##name<CMD>,
        DECLARE_INSTRUCTION_FORMATS(INSTRUCTION_FORMAT_HANDLER_X)
    };

    if (unlikely(0 == (itc--))) {
        LOG_CRIT(LOG_ALWAYS) << "instruction limit exceed (itc)";
    ...
    performance_monitor.count(instruction);

    Result result = Result::continue_execution;
    cmd.num_slots = CMD::SlotsNum;

    if (unlikely(is_nullified)) {
        LOG_DEBUG(LOG_PREDICATION) << "nullified " << instruction;
        performance_monitor.report_event(PMC_QUALIFIED_NOP_INSTRUCTION);
    } else {
        EInstructionID const id = Decoder::Decode(instruction);
        m_instructionStatisticCollector.count(id);
        result = inst_fmt_table[instr_format[id]](*this, as, id, instruction);
    }
    ...
    if (likely(result.success())) {  // continue or jump
        ...
        if (result != Result::halt) {
            report_fault<CMD>(as, instruction, result);
        }
...
Core::DumpProfile(const DumpFormatter& out) const
{
    ...
    out << "<table>\n"
           "<caption>program profile</caption>\n"
           "<thead>"
           "<tr>"
           "<th>position</th>"
           "<th>frequency</th>"
...
Core::dump_statistic(const DumpFormatter& out) const
{
    ...
    m_instructionStatisticCollector.dump(
           performance_monitor.get(PMC_SHORT_INSTRUCTION),
           performance_monitor.get(PMC_LONG_INSTRUCTION), out, "Non-nullified instruction frequencies");
