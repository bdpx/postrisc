...
# modify PATH, add MSYS paths
# Define the new path to add
# Append the new path to the existing PATH
# os.pathsep is used to ensure platform-independent path separation (e.g., ':' on Unix, ';' on Windows)
#current_path = os.environ.get('PATH', '')
#new_path_entry = f"C:{os.sep}msys64{os.sep}ucrt64{os.sep}bin" + os.pathsep + f"C:{os.sep}msys64{os.sep}usr{os.sep}bin"
...
CLANG_OPTIONS=[
    ...
    f"-triple {ARCHNAME}",                    # target
    f"-target-cpu {CPUNAME}",                 # cpu-model
    f"-mrelocation-model {RELOCATION_MODEL}", # pic/statis
