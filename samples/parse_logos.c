...
static int sanitize_print_string(char *string, FILE *fff) {
...
static void free_logo_memory2(struct logo_info *logo_ptr) {
...
int not_main(int argc, char **argv) {
	...
	if (fff==NULL) {
		printf("\nWarning!  No logo_config file exists!  "
			"No Logos will be compiled in!\n\n");
	...
	while (!feof(fff)) {
		if ( fgets(temp_st,BUFSIZ,fff) !=NULL) {
			...
			fprintf(ggg,"\tnew_logo=calloc("
				"1,sizeof(struct logo_info));\n");
      ...
      free_logo_memory2(logo_info_temp);
    }
  } 
  printf("\n");
  return 0;
