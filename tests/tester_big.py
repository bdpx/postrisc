...
print("assemble preprocessed file to program.bin")
...
print("disassemble object file 'program.bin'")
with open(f"program.bin", "r") as infile:
    with open(f"{OUTPUT_DIR}{os.sep}out_diz.txt", "w") as outfile:
        subprocess.run([EXEFILE, "--disasm"], stdin=infile, stdout=outfile, capture_output=False, text=True, check=True)

print("disassemble and binary dump object file 'program.bin'")
# (with instruction fields bit-to-bit description)
with open(f"program.bin", "r") as infile:
    with open(f"{OUTPUT_DIR}{os.sep}out_dump.txt", "w") as outfile:
        subprocess.run([EXEFILE, "--dumpbin"], stdin=infile, stdout=outfile, capture_output=False, text=True, check=True)

print("assemble from disassembled 'out_diz.txt' to 'out_diz.bin'")
with open(f"{OUTPUT_DIR}{os.sep}out_diz.txt", "r") as infile:
...
print("assemble from disassembled 'out_dump.s' to 'out_dump.bin'")
with open(f"{OUTPUT_DIR}{os.sep}out_dump.txt", "r") as infile:
    with open(f"{OUTPUT_DIR}{os.sep}out_dump.bin", "w") as outfile:
        subprocess.run([EXEFILE, "--assemble"], stdin=infile, stdout=outfile, capture_output=False, text=True, check=True)

print(f"compare with {SAMPLE_DIR} files")
compare_text_files(f"{OUTPUT_DIR}{os.sep}out_diz.txt", f"{SAMPLE_DIR}{os.sep}out_diz.txt")
compare_text_files(f"{OUTPUT_DIR}{os.sep}out_dump.txt", f"{SAMPLE_DIR}{os.sep}out_dump.txt")
