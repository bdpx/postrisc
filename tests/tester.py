...
with open(PREPROC_FILE, "r") as infile:
    with open(f"{OUTPUT_DIR}{os.sep}{TEST_NAME}.bin", "w") as outfile:
        subprocess.run([EXEFILE, "--assemble"] + ASSEMBLE_ARGS,
...
with open(f"{OUTPUT_DIR}{os.sep}{TEST_NAME}.bin", "r") as infile:
...
with open(TEST_OUT_FILE, "w") as outfile:
    result = subprocess.run([EXEFILE, "--execute", f"{OUTPUT_DIR}{os.sep}{TEST_NAME}.bin"] + EXE_ARGS, stdout=outfile, capture_output=False, text=True, check=True)

if os.path.exists(RESULT_FILE):
    print(f"diff: {RESULT_FILE} {TEST_OUT_FILE}")
    with open(f"{OUTPUT_DIR}{os.sep}{TEST_NAME}.diff", "w") as outfile:
        diff_result = subprocess.run(["diff", "--text", "-u", RESULT_FILE, TEST_OUT_FILE], stdout=outfile) #, check=True
        print(f"diff_result = {diff_result.returncode}")
        if diff_result.returncode != 0:
            print(f"comparison failed: {RESULT_FILE}")
            exit(1)
else:
...
os.remove(f"{OUTPUT_DIR}{os.sep}{TEST_NAME}.bin")
