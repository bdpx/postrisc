...
def cmakeGenerate(buildType, buildDir, CC, CPPC):
    ...
    if CC == "gcc":
        CMAKE_OPTIONS.extend(["-DUSE_QUADMATH=On"])  # use quadmath
    else:
        CMAKE_OPTIONS.extend(["-DUSE_QUADMATH=Off"])  # use mpfr

    # VCPKG_ROOT="E:\\vcpkg"
    subprocess.run(CMAKE_OPTIONS, capture_output=False, text=True, check=True)
